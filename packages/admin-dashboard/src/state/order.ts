...
export const useOrder = create<Order & Actions>()((set, get) => {
  return {
    ...
    modifyOrder: async (onSuccess) => {
      const { setOrder, setModifiedOrder, order, modifiedOrder, modifyOrderInput, setModifyOrderInput, orderHistory } =
        get();
      ...
      try {
          ...
          modifyOrder: [
            {
              input: {
                ...
                adjustOrderLines: modifiedOrder?.lines
                  ...
                  .map((ol) => {
                    if ('customFields' in ol && ol.customFields) {
                      return { orderLineId: ol.id, quantity: ol.quantity, customFields: ol.customFields };
                    }
                ...
                addItems: modifiedOrder?.lines
                  .filter((modifiedLine) => !order.lines.some((originalLine) => originalLine.id === modifiedLine.id))
                  .map((l) => {
                    ...
                    return { productVariantId: l.productVariant.id, quantity: l.quantity };
                  }),
        ...
        const { order: finalOrder } = await apiClient('query')({
