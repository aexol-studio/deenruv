...
import { AdminDetailsType } from '@/graphql/admins';
...
interface PageHeaderProps {
  admin: AdminDetailsType | undefined;
  editMode: boolean;
...
export const PageHeader: React.FC<PageHeaderProps> = ({ admin, editMode, buttonDisabled, onCreate, onEdit }) => {
    ...
    <>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => navigate(Routes.admins.list)}>
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <h1 className="flex-1 shrink-0 whitespace-nowrap text-xl font-semibold tracking-tight sm:grow-0">
            {editMode ? `${admin?.firstName} ${admin?.lastName}` : t('create')}
          </h1>
        </div>
