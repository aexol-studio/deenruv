...
async function parseComponentFile(filePath: string) {
    ...
    const customTypes = Array.from(content.matchAll(customTypeRegex)).map(match => match[0]);
    ...
    await Promise.all(
        items.map(async item => {
            ...
            if (stats.isDirectory()) {
                ...
                await Promise.all(
                    innerFiles
                        .filter(innerFile => innerFile.endsWith('.tsx') || innerFile.endsWith('.ts'))
                        .map(innerFile => parseComponentFile(path.join(itemPath, innerFile))),
                );
