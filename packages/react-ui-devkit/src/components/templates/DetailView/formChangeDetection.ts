...
const normalizeEntityValues = (entityValues: EntityType) => {
  ...
  if ("assets" in normalizedEntityValues) {
    normalizedEntityValues.assetIds = normalizedEntityValues["assets"]?.map(
      (a: { id: string }) => a.id,
    );
  ...
  if ("facetValues" in normalizedEntityValues) {
    normalizedEntityValues.facetValueIds = normalizedEntityValues[
      "facetValues"
  ...
  if ("featuredAsset" in normalizedEntityValues) {
    normalizedEntityValues.featuredAssetId =
      normalizedEntityValues["featuredAsset"]?.id;
    delete normalizedEntityValues.featuredAsset;
  }

  if ("members" in normalizedEntityValues) {
    normalizedEntityValues.memberIds = normalizedEntityValues["members"].map(
      (m: { id: string }) => m.id,
    );
  ...
  if ("customerGroup" in normalizedEntityValues) {
    normalizedEntityValues.customerGroupId =
      normalizedEntityValues["customerGroup"]?.id;
  ...
  if ("defaultShippingZone" in normalizedEntityValues) {
    normalizedEntityValues.defaultShippingZoneId =
      normalizedEntityValues["defaultShippingZone"]?.id;
    delete normalizedEntityValues.defaultShippingZone;
  }

  if ("defaultTaxZone" in normalizedEntityValues) {
    normalizedEntityValues.defaultTaxZoneId =
      normalizedEntityValues["defaultTaxZone"]?.id;
  ...
  if ("fulfillmentHandler" in normalizedEntityValues) {
    normalizedEntityValues.fulfillmentHandlerCode =
      normalizedEntityValues["fulfillmentHandler"].code;
  ...
  for (const key in formStateValues) {
    ...
    if (Array.isArray(formValue) && Array.isArray(entityValue)) {
      ...
      for (let i = 0; i < sortedFormValue.length; i++) {
        ...
        if (
          sortedFormString.includes('"arguments"') &&
          sortedEntityString.includes('"args"')
        ) {
          const normalizedFormString = sortedFormString.replace(
            /"args"/g,
            '"arguments"',
          );
          const normalizedEntityString = sortedEntityString.replace(
            /"args"/g,
            '"arguments"',
          );
