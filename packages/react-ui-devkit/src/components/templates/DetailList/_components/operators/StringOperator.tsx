...
export const StringOperator: React.FC<Props<StringOperator>> = ({
  currentValue,
  onSubmit,
}) => {
  const { t } = useTranslation("table");
  const defaultType = currentValue
    ? (Object.keys(currentValue)[0] as keyof StringOperator)
    : "eq";
  ...
  const [value, setValue] = useState<string | string[] | boolean | undefined>(
    () => {
      ...
      if (ARRAY_TYPES.includes(type))
        return (Object.values(currentValue)[0] as string[]).join(",");
