...
type FilterKey<T extends keyof ListType> =
  keyof ModelTypes[(typeof ListTypeKeys)[T]];

export function RenderOperators<
  T extends keyof ListType,
  F extends ModelTypes[(typeof ListTypeKeys)[T]],
>({
  filter,
  currentFilter,
  setCurrentFilter,
    ...
    return (
      ...
      >
        ...
        >
          <RenderOperator
            filter={{
              __typename: type as keyof FilterInputType,
              ...(filter && filter[name]),
            }}
            name={name as string}
            onSubmit={(value) => {
