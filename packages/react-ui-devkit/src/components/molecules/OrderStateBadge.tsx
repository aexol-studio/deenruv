...
export const OrderStateBadge: React.FC<{
  ...
  state?: string;
  className?: string;
}> = ({ fullWidth, state = "default", className }) => {
  ...
  const labelAndStyles = useMemo<{ className: string; label: string }>(() => {
    switch (state) {
      ...
      case ORDER_STATE.PAYMENT_AUTHORIZED:
        return {
          className: "border-amber-600 bg-amber-50 text-amber-600",
          label: t("paymentAuthorized"),
        };
      case ORDER_STATE.PAYMENT_SETTLED:
        return {
          className: "border-amber-700 bg-amber-700 text-amber-50",
          label: t("paymentSettled"),
        };
      case ORDER_STATE.PARTIALLY_SHIPPED:
        return {
          className: "border-violet-700 bg-violet-100 text-violet-700",
          label: t("partiallyShipped"),
        };
      case ORDER_STATE.SHIPPED:
        return {
          className: "border-violet-700 bg-violet-700 text-violet-100",
          label: t("shipped"),
        };
      ...
      case ORDER_STATE.DELIVERED:
        return {
          className: "border-green-800 bg-green-800 text-green-100",
          label: t("delivered"),
        };
      case ORDER_STATE.CANCELLED:
        return {
          className: "border-red-700 bg-red-700 text-red-100",
          label: t("cancelled"),
        };
      case ORDER_STATE.MODIFYING:
        return {
          className: "border-orange-500 bg-orange-500 text-orange-100",
          label: t("modifying"),
        };
      default:
        return {
          className: "border-primary bg-primary-foreground text-primary",
          label: state,
        };
    }
  }, [t, state]);
