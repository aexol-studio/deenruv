...
import { mergeSelectorWithCustomFields } from "@/utils/zeus-utils.js";
...
type Props<T extends ViableEntity> = {
  ...
  onChange?: (
    customFields: CF,
    translations?: Array<{ languageCode: LanguageCode }>,
  ) => void;
  hideButton?: boolean;
...
const entityDictionary: Partial<
  Record<
    ViableEntity,
    { inputName: keyof ModelTypes; mutationName: keyof ModelTypes["Mutation"] }
  >
> = {
  product: {
...
export function EntityCustomFields<T extends ViableEntity>({
  ...
  const entityCustomFields = useServer((p) =>
    p.serverConfig?.entityCustomFields?.find(
      (el) =>
        el.entityName.charAt(0).toLowerCase() + el.entityName.slice(1) ===
        entityName,
    ),
  )?.customFields;
  ...
  const relationFields = useMemo(
    () =>
      entityCustomFields
        ?.filter((el) => el.__typename === "RelationCustomFieldConfig")
        .map((el) => el.name),
    [entityCustomFields],
  );
  ...
  const capitalizedEntityName = useMemo(
    () =>
      (entityName.charAt(0).toUpperCase() +
        entityName.slice(1)) as Capitalize<T>,
    [entityName],
  );
  ...
  const updateEntity = async () => {
    ...
    try {
      ...
      if (mutation) {
        ...
        if (!mutationName)
          throw new Error(
            "no mutationName provided. Add it to entityDictionary or provide custom mutation prop",
          );
      ...
      toast.error(getGraphqlError(err) || t("toasts.error.mutation"));
    } finally {
      setIsUpdating(false);
    }
  };
      ...
      <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
        ...
        <div>
          <p className="font-medium">
            {t("custom-fields.noFields", "No custom fields available")}
          </p>
          <p className="text-muted-foreground mt-1 text-sm">
            {t(
              "custom-fields.noFieldsHint",
              "This entity type does not have any custom fields configured",
            )}
          </p>
        </div>
      </div>
  ...
  return withoutBorder ? (
    ...
    <CustomCard
      ...
      description={t(
        ...
        `Manage custom fields for this ${getEntityDisplayName()}`,
      )}
      color="rose"
      notCollapsible
