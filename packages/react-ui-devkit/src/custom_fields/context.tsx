...
export const CustomFieldsProvider: React.FC<
  PropsWithChildren<
    Omit<DynamicContext<Field, unknown>, "label" | "description">
  >
> = ({ children, ..._value }) => {
  const {
    i18n: { language },
  ...
  const translated = useMemo(
    () => ({
      label: _value?.field?.label?.find((el) => el.languageCode === language)
        ?.value,
      description: _value?.field?.description?.find(
