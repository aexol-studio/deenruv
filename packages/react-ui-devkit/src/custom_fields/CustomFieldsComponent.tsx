...
export function CustomFieldsComponent<
  K extends { customFields?: ModelTypes["JSON"] | undefined },
>({
  customFields,
  value,
  setValue,
  ...
  useEffect(() => {
    const result = generateInputComponents(
      customFields,
      getInputComponent,
    ).reduce(
      (acc, field) => {
      ...
      {Object.entries(rendered).map(([tab, fields]) => (
        <TabsContent key={tab} value={tab}>
          <div className="flex flex-wrap gap-4">
            {fields.map((_f) => {
              const _field = customFields?.find(
                (f) => "name" in f && f.name === _f.name,
              );
              ...
              return (
                <InputFieldComponent
                  ...
                  value={_value}
                  setValue={(data) =>
                    setValue(_field, data as string | number | boolean)
                  }
