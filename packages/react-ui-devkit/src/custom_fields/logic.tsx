...
export function generateInputComponents<
  T extends Field = CustomFieldConfigType,
>(
  fields: T[],
  ...
  for (const field of fields) {
    const ui =
      "ui" in field
        ? (field.ui as unknown as Record<string, unknown>)
        : undefined;
    const tab = ((ui && "tab" in ui && ui?.tab) || "General") as string;
    const hidden = ui && "hidden" in ui && ui.hidden;
    if (hidden) continue;
    const Registered =
      ui && "component" in ui && getInputComponent(ui.component as string);
    if (Registered) {
      result.push({
        ...field,
        tab,
...
function generateSingleFields<
  T extends { type: string; list?: boolean; ui?: Record<string, unknown> },
>({ field }: { field: T }) {
  ...
  switch (field.type) {
    ...
    default:
      ...
      return {
        name: "🏗️",
        component: (
          <span className="text-sm font-bold">Not implemented yet</span>
        ),
      };
  }
}
