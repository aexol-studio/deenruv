...
export const giveObjectsDiffrence = (
  ...
  const handleDifference = (
    ...
    if (parsedOrginalValue && parsedMabeyModifiedValue) {
      ...
      typeof orginalValue === "object" &&
      typeof mabeyModifiedValue === "object"
    ) {
...
export const giveModificationInfo = (
  ...
  const changes = giveObjectsDiffrence(
    orginalObject,
    mabeyModifiedObject,
    excludedProperties,
    path,
  ...
  const lineChanges = (changes.linesChanges as any[]).reduce((acc, change) => {
    ...
    const lineId = (
      orginalObject[line][index] ?? mabeyModifiedObject[line][index]
    ).id;
