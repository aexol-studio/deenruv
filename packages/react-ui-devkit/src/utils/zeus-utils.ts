...
const generateCustomFieldsSelector = (
        ...
        acc.customFields = {
          ...
          [field.name]:
            customFieldSelectors?.[
              field.entity as keyof typeof customFieldSelectors
            ],
        };
  ...
  if (!Object.keys(reduced.translations?.customFields || {}).length)
    delete reduced.translations;
  if (!Object.keys(reduced.customFields || {}).length)
    delete reduced.customFields;
  return reduced;
};

export function mergeSelectorWithCustomFields<
  T extends object,
  K extends keyof ValueTypes,
>(selectorA: T, key: K, customFields?: CustomFieldConfigType[]): T {
  const selectorB = Selector(key)(
    generateCustomFieldsSelector(customFields || []) as any,
  );
