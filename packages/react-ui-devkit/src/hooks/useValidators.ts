...
export const useValidators = () => {
  ...
  const stringValidator = (requiredMessage?: string) => ({
    validate: (v: string | null | undefined) => {
      if (!v || v === "")
        return [requiredMessage ?? t("validation.fieldRequired")];
    },
  });
  ...
  const emailValidator = {
    validate: (v: string | null | undefined) => {
      ...
      if (!v || v === "") errors.push(t("validation.emailRequired"));
        ...
        errors.push(t("validation.emailPattern"));
  ...
  const configurableOperationValidator = (
    requiredCodeMessage?: string,
    requiredArgsMessage?: string,
  ) => ({
    validate: (
      v?: ModelTypes["ConfigurableOperationInput"] | null | undefined,
    ) => {
      ...
      if (!hasCode)
        errors.push(
          requiredCodeMessage ?? t("validation.configurableCodeRequired"),
        );
      if (hasInvalidArguments)
        errors.push(
          requiredArgsMessage ?? t("validation.configurableArgsRequired"),
        );
      return errors;
    },
  });

  const configurableOperationArrayValidator = (
    requiredCodeMessage?: string,
    requiredArgsMessage?: string,
  ) => ({
    validate: (
      v?: Array<ModelTypes["ConfigurableOperationInput"]> | null | undefined,
    ) => {
      if (!v)
        return [
          requiredCodeMessage ?? t("validation.configurableCodeRequired"),
  ...
  const arrayValidator = (requiredMessage?: string) => ({
    validate: (v: any[] | null | undefined) => {
      if (!v || !v.length)
        return [requiredMessage ?? t("validation.nameRequired")];
    },
  });
