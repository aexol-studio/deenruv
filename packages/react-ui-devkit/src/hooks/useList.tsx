...
export const useList = <T extends PromisePaginated, K extends keyof ListType>({
  ...
  const fakeParams = useFakeSearchParams();
  ...
  const setFilter = (
    filter: ModelTypes[ListType[typeof listType]] | undefined,
  ) => {
  ...
  const setFilterLogicalOperator = (
    to: LogicalOperator | LogicalOperator | undefined,
  ) => {
    if (to === undefined) {
      searchParams.delete(SearchParamKey.FILTER_OPERATOR);
      setSearchParams(searchParams);
    } else {
      searchParams.set(SearchParamKey.FILTER_OPERATOR, to);
      setSearchParams(searchParams);
    }
  };
  ...
  const setFilterField = (
    ...
    try {
      ...
      if (filterURL) {
        const filterFromParamsJSON = JSON.parse(
          filterURL,
        ) as ModelTypes[ListType[typeof listType]];
        searchParams.set(
          SearchParamKey.FILTER,
          JSON.stringify({
            ...filterFromParamsJSON,
        ...
        searchParams.set(
          SearchParamKey.FILTER,
          JSON.stringify({ [filterField]: fieldValue }),
        );
        setSearchParams(searchParams);
      }
    } catch (err) {
      ...
      return {
        ...
        sort:
          sort && sortDir
            ...
            : undefined,
        filter: filter
          ? (JSON.parse(filter) as ModelTypes[ListType[typeof listType]])
          : undefined,
        filterOperator: filterOperator
          ? (filterOperator as LogicalOperator)
          : LogicalOperator.OR,
      };
    } catch (err) {
      throw new Error(`Parsing filter searchParams Key to JSON failed: ${err}`);
