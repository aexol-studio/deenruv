import { DeenruvSettingsWindowType } from "@/DeenruvAdminPanelSettingsTypes.js";
...
const buildSelectionSet = (lookup: string, depth: number): any => {
  if (depth) {
    const selections = Object.entries(
      window.__DEENRUV_SCHEMA__?.fieldLookup[lookup] || {},
    )
      .map(([field, tp]) => {
        const selectionSet = buildSelectionSet(tp, depth - 1);
        if (
          !window.__DEENRUV_SCHEMA__?.fieldLookup[tp] ||
          selectionSet?.selections.length
        ) {
          const selections = CUSTOM_MAP[tp as keyof typeof CUSTOM_MAP]?.map(
            (field) => ({
              kind: "Field",
...
const findType = (rootType: string, path: string[]) =>
  path.reduce(
    (pv, cv) => window.__DEENRUV_SCHEMA__?.fieldLookup[pv]?.[cv]!,
    rootType,
  );
  ...
  const result = visit(ast, {
    ...
    Field: {
      enter(node) {
        ...
        const type = findType(
          operation === "query" ? queryType : mutationType,
          path,
        );
        const fields = fieldLookup[type];
        if (
          fields &&
          "customFields" in fields &&
          !path.includes("customFields")
        ) {
...
export const deenruvAPICall = (options?: CallOptions) => {
  return async (
    ...
    const { translationsLanguage, selectedChannel, token, logIn } =
      useSettings.getState();
    const { authTokenName, channelTokenName, uri } =
      window.__DEENRUV_SETTINGS__.api;
    const { type } = options || {};
    ...
    const headers: Record<string, string> = token
      ? {
          ...
          ...(selectedChannel?.token && {
            [channelTokenName]: selectedChannel.token,
          }),
        }
      : {};
    ...
    if (type === "upload") {
      ...
      formData.append(
        "operations",
        JSON.stringify({ query: _query, variables }),
      );
      const mapData: Record<string, string[]> = {};
      const files = variables.input as ResolverInputTypes["CreateAssetInput"][];
      files.forEach((_, index) => {
        mapData[(index + 1).toString()] = [
          "variables.input." + index + ".file",
        ];
      });
...
export const apiClient = Thunder(deenruvAPICall({ type: "standard" }), {
  scalars,
});
export const apiUploadClient = Thunder(deenruvAPICall({ type: "upload" }), {
  scalars,
});
