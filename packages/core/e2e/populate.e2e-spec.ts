...
import { DefaultLogger, User } from "@deenruv/core";
...
describe("populate() function", () => {
  ...
  describe("populating default channel", () => {
    ...
    it("populates products", async () => {
      await adminClient.asSuperAdmin();
      const { products } =
        await adminClient.query<GetProductListQuery>(GET_PRODUCT_LIST);
    ...
    it("populates assets", async () => {
      const { assets } =
        await adminClient.query<GetAssetListQuery>(GET_ASSET_LIST);
    ...
    it("populates collections", async () => {
      const { collections } =
        await adminClient.query<GetCollectionsQuery>(GET_COLLECTIONS);
      expect(collections.items.map((i) => i.name).sort()).toEqual([
        "Collection 1",
      ]);
    });
  });
  ...
  describe("populating a non-default channel", () => {
    ...
    it("populates assets", async () => {
      const { assets } =
        await adminClient.query<GetAssetListQuery>(GET_ASSET_LIST);
      expect(assets.items.map((i) => i.name).sort()).toEqual([
        "vincent-botta-736919-unsplash.jpg",
      ]);
    });
    ...
    it("product also assigned to default channel", async () => {
      ...
      const { products } =
        await adminClient.query<GetProductListQuery>(GET_PRODUCT_LIST);
      expect(products.items.map((i) => i.name).includes("Model Hand")).toBe(
        true,
      );
    });
  });
  ...
  describe("clashing option names", () => {
    ...
    beforeAll(async () => {
      ...
      const csvFile = path.join(
        __dirname,
        "fixtures",
        "product-import-option-values.csv",
      );
      app = await populate(
        async () => {
          await server.bootstrap();
    ...
    it("populates variants & options", async () => {
      ...
      expect(product?.optionGroups.map((og) => og.name).sort()).toEqual([
        "Bar",
        "Foo",
      ]);
      expect(
