...
describe("Promotion resolver", () => {
  ...
  const promotionGuard: ErrorResultGuard<Codegen.PromotionFragment> =
    createErrorResultGuard((input) => !!input.couponCode);
  ...
  describe("channels", () => {
    ...
    it("does not list Promotions not in active channel", async () => {
      ...
      const { promotions } =
        await adminClient.query<Codegen.GetPromotionListQuery>(
          GET_PROMOTION_LIST,
        );
    ...
    it("assignPromotionsToChannel", async () => {
      ...
      expect(assignPromotionsToChannel).toEqual([
      ...
      expect(promotions.items.map(pick(["id"]))).toEqual([
        { id: promotion.id },
      ]);
    });
    ...
    it("removePromotionsFromChannel", async () => {
      ...
      expect(removePromotionsFromChannel).toEqual([
        { id: promotion.id, name: promotion.name },
      ]);
  ...
  describe("deletion", () => {
    ...
    it("deleted promotion omitted from list", async () => {
      ...
      expect(
        result.promotions.items.map((c) => c.id).includes(promotionToDelete.id),
      ).toBe(false);
    });
...
function generateTestCondition(code: string): PromotionCondition<any> {
  return new PromotionCondition({
    code,
    description: [
      { languageCode: LanguageCode.en, value: `description for ${code}` },
    ],
    args: { arg: { type: "int" } },
