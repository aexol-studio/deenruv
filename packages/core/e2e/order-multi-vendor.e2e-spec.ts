...
describe("Multi-vendor orders", () => {
  ...
  it("assign products to sellers", async () => {
    ...
    bobsPartsChannel.variantIds = assignProductsToChannel[0].variants.map(
      (v) => v.id,
    );
    ...
    expect(result2[0].channels.map((c) => c.code)).toEqual([
      "__default_channel__",
      "alices-wares",
    ]);
  ...
  it("adds items and sets shipping methods", async () => {
    ...
    const { setOrderShippingMethod } = await shopClient.query(
      ...
      {
        id: [
          eligibleShippingMethods.find((m) => m.code === "bobs-parts-shipping")!
            .id,
          eligibleShippingMethods.find(
            (m) => m.code === "alices-wares-shipping",
          )!.id,
        ],
      },
    );
    ...
    expect(
      setOrderShippingMethod.shippingLines
        .map((l) => l.shippingMethod.code)
        .sort(),
    ).toEqual(["alices-wares-shipping", "bobs-parts-shipping"]);
  });
  ...
  it("order lines get split", async () => {
    ...
    expect(
      order?.sellerOrders?.[0].lines.map((l) => l.productVariant.name),
    ).toEqual(["Laptop 13 inch 8GB"]);
    expect(
      order?.sellerOrders?.[1].lines.map((l) => l.productVariant.name),
    ).toEqual(["Road Bike"]);
  });
  ...
  it("shippingLines get split", async () => {
    ...
    expect(order?.sellerOrders?.[0]?.shippingLines[0].shippingMethod.code).toBe(
      "bobs-parts-shipping",
    );
    expect(order?.sellerOrders?.[1]?.shippingLines[0].shippingMethod.code).toBe(
      "alices-wares-shipping",
    );
  });
});
