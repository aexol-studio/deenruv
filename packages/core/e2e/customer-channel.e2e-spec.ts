...
type CustomerListItem =
  Codegen.GetCustomerListQuery["customers"]["items"][number];
...
describe("ChannelAware Customers", () => {
  ...
  describe("Customer manipulation", () => {
    ...
    it("creates customers on current and default channel", async () => {
      ...
      expect(customersDefaultChannel.customers.totalItems).toBe(
        numberOfCustomers + 1,
      );
    });
    ...
    it("brings customer to current channel when creating with existing emailAddress", async () => {
      ...
      >(GET_CUSTOMER_LIST);
      expect(
        customersDefaultChannel.customers.items.map(
          (customer) => customer.emailAddress,
        ),
      ).toContain(firstCustomer.emailAddress);
      expect(
        customersSecondChannel.customers.items.map(
          (customer) => customer.emailAddress,
        ),
      ...
      customersDefaultChannel = await adminClient.query<
        ...
        Codegen.GetCustomerListQueryVariables
      >(GET_CUSTOMER_LIST);
      const firstCustomerOnNewChannel =
        customersSecondChannel.customers.items.find(
          (customer) => customer.emailAddress === firstCustomer.emailAddress,
        );
      const firstCustomerOnDefaultChannel =
        customersDefaultChannel.customers.items.find(
          (customer) => customer.emailAddress === firstCustomer.emailAddress,
        );

      expect(firstCustomerOnNewChannel).not.toBeNull();
      expect(firstCustomerOnNewChannel?.emailAddress).toBe(
        firstCustomer.emailAddress,
      );
      expect(firstCustomerOnNewChannel?.firstName).toBe(
        firstCustomer.firstName + "_new",
      );
      expect(firstCustomerOnNewChannel?.lastName).toBe(
        firstCustomer.lastName + "_new",
      );

      expect(firstCustomerOnDefaultChannel).not.toBeNull();
      expect(firstCustomerOnDefaultChannel?.emailAddress).toBe(
        firstCustomer.emailAddress,
      );
      expect(firstCustomerOnDefaultChannel?.firstName).toBe(
        firstCustomer.firstName + "_new",
      );
      expect(firstCustomerOnDefaultChannel?.lastName).toBe(
        firstCustomer.lastName + "_new",
      );
    });
  });
  ...
  describe("Shop API", () => {
    it("assigns authenticated customers to the channels they visit", async () => {
      ...
      expect(
        customers.items.map((customer) => customer.emailAddress),
      ).toContain(secondCustomer.emailAddress);
    });

    it("assigns newly registered customers to channel", async () => {
      shopClient.setChannelToken(SECOND_CHANNEL_TOKEN);
      await shopClient.asAnonymousUser();
      await shopClient.query<RegisterMutation, RegisterMutationVariables>(
        REGISTER_ACCOUNT,
        {
          input: {
  ...
  describe("Customergroup manipulation", () => {
    ...
    it("only shows customers from current channel in customerGroup", async () => {
      ...
      expect(
        customerGroup!.customers.items.map((customer) => customer.id),
      ).toContain(secondCustomer.id);
    });
