  ...
  implements ShippingLineAssignmentStrategy
{
  assignShippingLineToOrderLines(
    ...
    return order.lines.filter(
      (l) => l.linePriceWithTax >= minPrice && l.linePriceWithTax <= maxPrice,
    );
  }
}
...
describe("Multiple shipping orders", () => {
  const { server, adminClient, shopClient } = createTestEnvironment(
    mergeConfig(testConfig(), {
      ...
      shippingOptions: {
        shippingLineAssignmentStrategy:
          new CustomShippingLineAssignmentStrategy(),
      },
    }),
  );
  ...
  it("assigns shipping methods to correct order lines", async () => {
    ...
    expect(
      eligibleShippingMethods.map((m) => m.id).includes(lessThan100MethodId),
    ).toBe(true);
    expect(
    ...
    expect(order?.lines[0].shippingLine?.shippingMethod.code).toBe(
      "greater-than-100",
    );
    expect(order?.lines[1].shippingLine?.shippingMethod.code).toBe(
      "less-than-100",
    );
  ...
  it("removes shipping methods that are no longer applicable", async () => {
    ...
    expect(activeOrder2?.shippingWithTax).toBe(
      summate(activeOrder2!.shippingLines, "priceWithTax"),
    );
  });
