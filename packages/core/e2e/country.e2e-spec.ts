...
describe("Country resolver", () => {
  ...
  it("country", async () => {
    const result = await adminClient.query<
      Codegen.GetCountryQuery,
      Codegen.GetCountryQueryVariables
    >(GET_COUNTRY, {
      id: GB.id,
    });
  ...
  describe("deletion", () => {
    it("deletes Country not used in any address", async () => {
      ...
      const result2 = await adminClient.query<Codegen.GetCountryListQuery>(
        GET_COUNTRY_LIST,
        {},
      );
      expect(
        result2.countries.items.find((c) => c.id === AT.id),
      ).toBeUndefined();
    });
    ...
    it("does not delete Country that is used in one or more addresses", async () => {
      ...
      expect(result1.deleteCountry).toEqual({
        ...
        message:
          "The selected Country cannot be deleted as it is used in 1 Address",
      });
      ...
      expect(
        result2.countries.items.find((c) => c.id === GB.id),
      ).not.toBeUndefined();
    });
  });
});
