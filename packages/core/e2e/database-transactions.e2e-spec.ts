...
describe("Transaction infrastructure", () => {
  ...
  it("non-failing mutation", async () => {
    ...
    expect(!!verify.admins.find((a: any) => a.emailAddress === "test1")).toBe(
      true,
    );
  ...
  it("failing mutation", async () => {
    ...
    expect(verify.admins.length).toBe(2);
    expect(verify.users.length).toBe(2);
    expect(!!verify.admins.find((a: any) => a.emailAddress === "test2")).toBe(
      false,
    );
  ...
  it("failing mutation with promise concurrent execution", async () => {
    ...
    expect(verify.users.length).toBe(2);
    expect(
      !!verify.admins.find((a: any) => a.emailAddress.includes("testN")),
    ).toBe(false);
    expect(
  ...
  it("failing manual mutation", async () => {
    ...
    expect(verify.admins.length).toBe(2);
    expect(verify.users.length).toBe(2);
    expect(!!verify.admins.find((a: any) => a.emailAddress === "test3")).toBe(
      false,
    );
  ...
  it("failing manual mutation without transaction", async () => {
    ...
    expect(!!verify.admins.find((a: any) => a.emailAddress === "test4")).toBe(
      false,
    );
  ...
  it("failing mutation inside connection.withTransaction() wrapper with request context", async () => {
    ...
    expect(verify.admins.length).toBe(2);
    expect(verify.users.length).toBe(3);
    expect(!!verify.admins.find((a: any) => a.emailAddress === "test5")).toBe(
      false,
    );
  ...
  it("non-failing mutation inside connection.withTransaction() wrapper with failing nested transactions and request context", async () => {
    ...
    expect(
      verify.admins.filter((a: any) =>
        a.emailAddress.includes("testNestedTransactionsN"),
      ),
    ).toHaveLength(1);
    expect(
      verify.users.filter((u: any) =>
        u.identifier.includes("testNestedTransactionsN"),
      ),
    ).toHaveLength(1);
  });
  ...
  // Testing https://github.com/deenruv-ecommerce/deenruv/issues/520
  it("passing transaction via EventBus", async () => {
    ...
    expect(createTestAdministrator.emailAddress).toBe(
      TRIGGER_ATTEMPTED_UPDATE_EMAIL,
    );
    expect(TransactionTestPlugin.errorHandler).not.toHaveBeenCalled();
  });

  // Testing https://github.com/deenruv-ecommerce/deenruv/issues/1107
  it("passing transaction via EventBus with delay in committing transaction", async () => {
    TransactionTestPlugin.reset();
    const { createTestAdministrator4 } = await adminClient.query(
      CREATE_ADMIN4,
      {
    ...
    await TransactionTestPlugin.eventHandlerComplete$.toPromise();
    expect(createTestAdministrator4.emailAddress).toBe(
      TRIGGER_ATTEMPTED_READ_EMAIL,
    );
    expect(TransactionTestPlugin.errorHandler).not.toHaveBeenCalled();
  });
...
const CREATE_N_ADMINS3 = gql`
  mutation CreateNTestAdmins3(
    $emailAddress: String!
    $failFactor: Float!
    $n: Int!
  ) {
    createNTestAdministrators3(
      emailAddress: $emailAddress
      failFactor: $failFactor
      n: $n
    )
