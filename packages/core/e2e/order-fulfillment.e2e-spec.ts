...
const testFulfillmentHandler = new FulfillmentHandler({
  ...
  description: [
    { languageCode: LanguageCode.en, value: "Test fulfillment handler" },
  ],
  args: {
    trackingCode: {
      type: "string",
...
describe("Order fulfillments", () => {
  ...
  it("fulfillmentHandlers query", async () => {
    const { fulfillmentHandlers } =
      await adminClient.query<Codegen.GetFulfillmentHandlersQuery>(
  ...
  it("onFulfillmentTransition can prevent state transition", async () => {
    ...
    expect(transitionFulfillmentToState.errorCode).toBe(
      ErrorCode.FULFILLMENT_STATE_TRANSITION_ERROR,
    );
    expect(transitionFulfillmentToState.transitionError).toBe(
  ...
  it("throwing from createFulfillment returns CreateFulfillmentError result", async () => {
    ...
    expect(addFulfillmentToOrder.errorCode).toBe(
      ErrorCode.CREATE_FULFILLMENT_ERROR,
    );
    expect(
      (addFulfillmentToOrder as CreateFulfillmentError).fulfillmentHandlerError,
    ).toBe("The code was bad!");
  });
});
