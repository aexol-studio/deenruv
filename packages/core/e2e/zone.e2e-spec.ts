...
describe("Zone resolver", () => {
  ...
  beforeAll(async () => {
    ...
    const result = await adminClient.query<Codegen.GetCountryListQuery>(
      GET_COUNTRY_LIST,
      {},
    );
  ...
  it("zones", async () => {
    const result =
      await adminClient.query<Codegen.GetZonesQuery>(GET_ZONE_LIST);
  ...
  it("zone", async () => {
    const result = await adminClient.query<
      Codegen.GetZoneQuery,
      Codegen.GetZoneQueryVariables
    >(GET_ZONE, {
      id: oceania.id,
    });
  ...
  it("zone.members field resolver", async () => {
    const { activeChannel } =
      await adminClient.query<Codegen.GetActiveChannelWithZoneMembersQuery>(
  ...
  it("createZone", async () => {
    ...
    expect(pangaea.members.map((m) => m.name)).toEqual([
      countries[0].name,
      countries[1].name,
    ]);
  });
  ...
  it("addMembersToZone", async () => {
    ...
    expect(
      !!result.addMembersToZone.members.find(
        (m) => m.name === countries[2].name,
      ),
    ).toBe(true);
    expect(
      !!result.addMembersToZone.members.find(
        (m) => m.name === countries[3].name,
      ),
    ).toBe(true);
  });
  ...
  it("removeMembersFromZone", async () => {
    ...
    expect(
      !!result.removeMembersFromZone.members.find(
        (m) => m.name === countries[0].name,
      ),
    ).toBe(false);
    expect(
      !!result.removeMembersFromZone.members.find(
  ...
  describe("deletion", () => {
    it("deletes Zone not used in any TaxRate", async () => {
      ...
      const result2 =
        await adminClient.query<Codegen.GetZonesQuery>(GET_ZONE_LIST);
      expect(
        result2.zones.items.find((c) => c.id === pangaea.id),
      ).toBeUndefined();
    });
    ...
    it("does not delete Zone that is used in one or more TaxRates", async () => {
      ...
      const result2 =
        await adminClient.query<Codegen.GetZonesQuery>(GET_ZONE_LIST);
      expect(
        result2.zones.items.find((c) => c.id === oceania.id),
      ).not.toBeUndefined();
    });
