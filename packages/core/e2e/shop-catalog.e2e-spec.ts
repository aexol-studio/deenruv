import { facetValueCollectionFilter, JobQueueService } from "@deenruv/core";
...
describe("Shop catalog", () => {
  ...
  describe("products", () => {
    beforeAll(async () => {
      ...
      await adminClient.query<
        Codegen.DisableProductMutation,
        Codegen.DisableProductMutationVariables
      >(DISABLE_PRODUCT, {
        id: "T_1",
      });
    ...
    it("products list omits disabled products", async () => {
      ...
      expect(result.products.items.map((item) => item.id).sort()).toEqual([
        "T_2",
        "T_3",
        "T_4",
      ]);
    ...
    it("omits disabled variants from product response", async () => {
      const result = await shopClient.query<Codegen.GetProduct2VariantsQuery>(
        gql`
  ...
  describe("facets", () => {
    ...
    it("omits private ProductVariant.facetValues", async () => {
      ...
      expect(
        result.product!.variants[0].facetValues.map((fv) => fv.name),
      ).toEqual([]);
    });
  });
  ...
  describe("collections", () => {
    ...
    beforeAll(async () => {
      ...
      const sportsEquipment = category.values.find(
        (v) => v.code === "sports-equipment",
      )!;
    ...
    it("collection list", async () => {
      const result =
        await shopClient.query<Codegen.GetCollectionListQuery>(
          GET_COLLECTION_LIST,
        );
    ...
    it("product.collections list omits private collections", async () => {
      const result = await shopClient.query<Codegen.GetProductCollectionQuery>(
        gql`
    ...
    it("private children not returned in Shop API", async () => {
      const { createCollection: parent } = await createNewCollection(
        "public-parent",
        false,
      );
      const { createCollection: child } = await createNewCollection(
        "private-child",
        true,
        parent.id,
      );
