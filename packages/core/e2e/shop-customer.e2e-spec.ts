...
describe("Shop customers", () => {
  ...
  beforeAll(async () => {
    ...
    const { customers } = await adminClient.query<Codegen.GetCustomerIdsQuery>(
      gql`
  ...
  describe("logged in Customer", () => {
    ...
    it("updatePassword return error result with incorrect current password", async () => {
      ...
      expect(updateCustomerPassword.message).toBe(
        "The provided credentials are invalid",
      );
      expect(updateCustomerPassword.errorCode).toBe(
