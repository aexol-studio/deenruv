...
describe("ShippingMethod resolver", () => {
  ...
  it("shippingEligibilityCheckers", async () => {
    const { shippingEligibilityCheckers } =
      await adminClient.query<Codegen.GetEligibilityCheckersQuery>(
  ...
  it("shippingCalculators", async () => {
    const { shippingCalculators } =
      await adminClient.query<Codegen.GetCalculatorsQuery>(GET_CALCULATORS);
  ...
  it("deleteShippingMethod", async () => {
    ...
    expect(listResult1.shippingMethods.items.map((i) => i.id)).toEqual([
      "T_1",
      "T_2",
      "T_3",
    ]);
    ...
    expect(listResult2.shippingMethods.items.map((i) => i.id)).toEqual([
      "T_1",
      "T_2",
    ]);
  });
