...
describe("Import resolver", () => {
  ...
  it("imports products", async () => {
    ...
    const easel = productResult.products.items.find(
      (p: any) => p.name === "Mabef M/02 Studio Easel",
    );
    const pencils = productResult.products.items.find(
      (p: any) => p.name === "Giotto Mega Pencils",
    );
    ...
    expect(
      omit(paperStretcher, ["facetValues", "options"], true),
    ).toMatchSnapshot();
    ...
    expect(paperStretcher.variants[1].facetValues.map(byName).sort()).toEqual([
      "Accessory",
      "KB",
    ]);
    expect(paperStretcher.variants[2].facetValues.map(byName).sort()).toEqual([
      "Accessory",
      "KB",
    ]);
    expect(paperStretcher.variants[0].options.map(byCode).sort()).toEqual([
      "half-imperial",
    ]);
    expect(paperStretcher.variants[1].options.map(byCode).sort()).toEqual([
      "quarter-imperial",
    ]);
    expect(paperStretcher.variants[2].options.map(byCode).sort()).toEqual([
      "full-imperial",
    ]);
    expect(easel.variants[0].facetValues.map(byName).sort()).toEqual([
      "Easel",
      "Mabef",
    ]);
    expect(pencils.variants[0].facetValues.map(byName).sort()).toEqual([
      "Xmas Sale",
    ]);
    expect(pencils.variants[1].facetValues.map(byName).sort()).toEqual([
      "Xmas Sale",
    ]);
    expect(pencils.variants[0].options.map(byCode).sort()).toEqual([
      "box-of-8",
    ]);
    expect(pencils.variants[1].options.map(byCode).sort()).toEqual([
      "box-of-12",
    ]);
    ...
    expect(smock.variants[0].options.map(byCode).sort()).toEqual([
      "beige",
      "small",
    ]);
    expect(smock.variants[1].options.map(byCode).sort()).toEqual([
      "beige",
      "large",
    ]);
    expect(smock.variants[2].options.map(byCode).sort()).toEqual([
      "navy",
      "small",
    ]);
    expect(smock.variants[3].options.map(byCode).sort()).toEqual([
      "large",
      "navy",
    ]);
    ...
    expect(paperStretcher.customFields.keywords).toEqual([
      "paper",
      "stretching",
      "watercolor",
    ]);
  ...
  it("imports products with multiple languages", async () => {
    ...
    const csvFile = path.join(
      __dirname,
      "fixtures",
      "e2e-product-import-multi-languages.csv",
    );
    const result = await adminClient.fileUploadMutation({
      mutation: gql`
        mutation ImportProducts($csvFile: Upload!) {
    ...
    expect(paperStretcher.facetValues.map(byName).sort()).toEqual([
      "KB",
      "饰品",
    ]);

    expect(paperStretcher.variants[0].options.map(byName).sort()).toEqual([
      "半英制",
    ]);
    expect(paperStretcher.variants[1].options.map(byName).sort()).toEqual([
      "四分之一英制",
    ]);
    expect(paperStretcher.variants[2].options.map(byName).sort()).toEqual([
      "全英制",
    ]);
  ...
  describe("asset urls", () => {
    ...
    it("imports assets with url paths", async () => {
      ...
      const csvFile = path.join(
        __dirname,
        "fixtures",
        "e2e-product-import-asset-urls.csv",
      );
      const result = await adminClient.fileUploadMutation({
        mutation: gql`
          mutation ImportProducts($csvFile: Upload!) {
