...
describe("Asset resolver", () => {
  ...
  it("assets", async () => {
    ...
    expect(assets.items.map((a) => omit(a, ["id"]))).toEqual([
      ...
      },
      {
        ...
        preview:
          "test-url/test-assets/florian-olivo-1166419-unsplash__preview.jpg",
      ...
      },
      {
        ...
        preview:
          "test-url/test-assets/vincent-botta-736919-unsplash__preview.jpg",
  ...
  it("asset", async () => {
    const { asset } = await adminClient.query<
      Codegen.GetAssetQuery,
      Codegen.GetAssetQueryVariables
    >(GET_ASSET, {
      id: firstAssetId,
    });
  ...
  /**
   * https://github.com/deenruv-ecommerce/deenruv/issues/459
   */
  ...
  describe("createAssets", () => {
    ...
    it("permitted types by mime type", async () => {
      ...
      const { createAssets }: Codegen.CreateAssetsMutation =
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/727
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/990
    it("errors if the filesize is too large", async () => {
      ...
      const filename = path.join(
        __dirname,
        "fixtures/assets/temp_large_file.pdf",
      );
      ...
      try {
        ...
        expect(e.message).toContain(
          "File truncated as it exceeds the 20971520 byte size limit",
        );
      } finally {
        fs.rmSync(filename);
      }
  ...
  describe("filter by tags", () => {
    ...
    it("or", async () => {
      ...
      expect(assets.items.map((i) => i.id).sort()).toEqual([
        "T_10",
        "T_11",
        "T_9",
      ]);
  ...
  describe("deleteAsset", () => {
    ...
    it("featured asset not deleted", async () => {
      ...
      expect(deleteAsset.message).toContain(
        "The selected Asset is featured by 1 Product",
      );
