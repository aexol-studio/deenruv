...
describe("Default search plugin", () => {
  ...
  beforeAll(async () => {
    await server.init({
      ...
      productsCsvPath: path.join(
        __dirname,
        "fixtures",
        "default-search-plugin-sort-by.csv",
      ),
      customerCount: 1,
    });
  ...
  function searchProductsShop(
    queryVariables: SearchProductsShopQueryVariables,
  ) {
    return shopClient.query<
      SearchProductsShopQuery,
      SearchProductsShopQueryVariables
    >(SEARCH_PRODUCTS_SHOP, queryVariables);
  }

  function searchProductsAdmin(
    queryVariables: SearchProductsAdminQueryVariables,
  ) {
    return adminClient.query<
      SearchProductsAdminQuery,
      SearchProductsAdminQueryVariables
    >(SEARCH_PRODUCTS_ADMIN, queryVariables);
  }
  ...
  async function testSortByPriceAsc(searchProducts: SearchProducts) {
    const resultPage1 = await testSearchProducts(
      searchProducts,
      false,
      "price",
      SortOrder.ASC,
    ...
    const resultPage2 = await testSearchProducts(
      searchProducts,
      false,
      "price",
      SortOrder.ASC,
    ...
    expect(resultPage1.search.items.map((i) => i.productVariantId)).toEqual([
      "T_4",
      "T_5",
      "T_6",
    ]);
    expect(resultPage2.search.items.map((i) => i.productVariantId)).toEqual([
      "T_7",
      "T_8",
      "T_9",
    ]);
  ...
  async function testSortByPriceAscGroupByProduct(
    searchProducts: SearchProducts,
  ) {
    const resultPage1 = await testSearchProducts(
      searchProducts,
    ...
    expect(new Set(pvIds3).size).equals(6);
    expect(resultPage1.search.items.map((i) => i.productId)).toEqual([
      "T_4",
      "T_5",
      "T_6",
    ]);
    expect(resultPage2.search.items.map((i) => i.productId)).toEqual([
      "T_1",
      "T_2",
      "T_3",
    ]);
  ...
  async function testSortByPriceDescGroupByProduct(
    searchProducts: SearchProducts,
  ) {
    const resultPage1 = await testSearchProducts(
      searchProducts,
  ...
  describe("Search products shop", () => {
    ...
    it("sort by price ASC group by product", () =>
      testSortByPriceAscGroupByProduct(searchProducts));
    it("sort by price DESC group by product", () =>
      testSortByPriceDescGroupByProduct(searchProducts));
  });
