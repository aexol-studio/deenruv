...
describe("Channels", () => {
  ...
  it("createChannel returns error result defaultLanguageCode not available", async () => {
    ...
    expect(createChannel.errorCode).toBe(
  ...
  it("superadmin has all permissions on new channel", async () => {
    ...
    const secondChannelData = me!.channels.find(
  ...
  it("createAdministrator with second-channel-admin role", async () => {
    ...
    expect(createAdministrator.user.roles.map((r) => r.description)).toEqual([
  ...
  describe("setting defaultLanguage", () => {
    it("returns error result if languageCode not in availableLanguages", async () => {
      ...
      expect(updateChannel.errorCode).toBe(
        ErrorCode.LANGUAGE_NOT_AVAILABLE_ERROR,
      );
  ...
  it("deleteChannel", async () => {
    ...
    expect(assignProductsToChannel[0].channels.map((c) => c.id).sort()).toEqual(
      ["T_1", "T_2"],
    );
    ...
    const { channels } =
      await adminClient.query<Codegen.GetChannelsQuery>(GET_CHANNELS);
