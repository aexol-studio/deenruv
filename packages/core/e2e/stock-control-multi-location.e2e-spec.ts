import { manualFulfillmentHandler, mergeConfig } from "@deenruv/core";
import {
...
describe("Stock control", () => {
  ...
  beforeAll(async () => {
    await server.init({
      ...
      productsCsvPath: path.join(
        __dirname,
        "fixtures/e2e-products-stock-control-multi.csv",
      ),
      customerCount: 3,
    });
  ...
  it("default StockLocation exists", async () => {
    const { stockLocations } =
      await adminClient.query<Codegen.GetStockLocationsQuery>(
        GET_STOCK_LOCATIONS,
      );
  ...
  it("variant stock is all at default StockLocation", async () => {
    ...
    expect(
      productVariants.items.every(
        (variant) => variant.stockLevels.length === 1,
      ),
    ).toBe(true);
    expect(
      productVariants.items.every(
  ...
  it("update ProductVariants with stock levels in second location", async () => {
    ...
    >(UPDATE_PRODUCT_VARIANTS, {
      input: productVariants.items.map((variant) => ({
        id: variant.id,
        stockLevels: [
          { stockLocationId: secondStockLocationId, stockOnHand: 120 },
        ],
      })),
    });
    ...
    const {
      ...
      Codegen.GetVariantStockLevelsQueryVariables
    >(GET_VARIANT_STOCK_LEVELS, {});
    expect(items.every((variant) => variant.stockLevels.length === 2)).toBe(
      true,
    );
    expect(
      items.every((variant) => {
  ...
  describe("stock movements", () => {
    ...
    it("creates Allocations according to StockLocationStrategy", async () => {
      ...
      expect(
        productVariants.items.find((v) => v.id === "T_1")?.stockLevels,
      ).toEqual([
        {
          stockLocationId: "T_1",
          stockOnHand: 100,
      ...
      expect(
        productVariants.items.find((v) => v.id === "T_2")?.stockLevels,
      ).toEqual([
        {
          stockLocationId: "T_1",
          stockOnHand: 100,
