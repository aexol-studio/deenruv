...
class DelegateMergeStrategy implements OrderMergeStrategy {
  ...
  merge(
    ...
    return DelegateMergeStrategy.activeStrategy.merge(
      ctx,
      guestOrder,
      existingOrder,
    );
...
describe("Order merging", () => {
  ...
  async function testMerge(options: {
    ...
    for (const line of existingOrderLines) {
      await shopClient.query<
        AddItemToOrderMutation,
        AddItemToOrderWithCustomFields
      >(ADD_ITEM_TO_ORDER_WITH_CUSTOM_FIELDS, line);
    }
    ...
    const { activeOrder } = await shopClient.query(
      GET_ACTIVE_ORDER_WITH_CUSTOM_FIELDS,
    );
    return activeOrder;
  }
  ...
  // https://github.com/deenruv-ecommerce/deenruv/issues/1454
  it("does not throw FK error when merging with a cart with an existing session", async () => {
    ...
    const { nextOrderStates } =
      await shopClient.query<GetNextOrderStatesQuery>(GET_NEXT_STATES);
    ...
    const { login } = await shopClient.query<
      AttemptLoginMutation,
      AttemptLoginMutationVariables
    >(ATTEMPT_LOGIN, { username: customers[7].emailAddress, password: "test" });
