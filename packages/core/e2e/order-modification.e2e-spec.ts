...
describe("Order modification", () => {
  const { server, adminClient, shopClient } = createTestEnvironment(
    mergeConfig(testConfig(), {
      ...
      orderOptions: {
        orderItemPriceCalculationStrategy:
          new TestOrderItemPriceCalculationStrategy(),
      },
      shippingOptions: {
  ...
  it("modifyOrder returns error result when not in Modifying state", async () => {
    ...
    >(MODIFY_ORDER, {
      input: {
        dryRun: false,
        orderId,
        adjustOrderLines: order!.lines.map((l) => ({
          orderLineId: l.id,
          quantity: 3,
        })),
      },
    ...
    expect(modifyOrder.errorCode).toBe(
      ErrorCode.ORDER_MODIFICATION_STATE_ERROR,
    );
  });
    ...
    it("no refund paymentId specified", async () => {
      ...
      >(MODIFY_ORDER, {
        input: {
          ...
          orderId,
          surcharges: [
            { price: -500, priceIncludesTax: true, description: "Discount" },
          ],
        },
      });
      orderGuard.assertErrorResult(modifyOrder);
    ...
    it("adjustOrderLines insufficient stock", async () => {
      ...
      >(MODIFY_ORDER, {
        input: {
          ...
          orderId,
          adjustOrderLines: [
            { orderLineId: order!.lines[0].id, quantity: 500 },
          ],
        },
      });
      orderGuard.assertErrorResult(modifyOrder);
    ...
    it("adjustOrderLines order limit", async () => {
      ...
      >(MODIFY_ORDER, {
        input: {
          ...
          orderId,
          adjustOrderLines: [
            { orderLineId: order!.lines[1].id, quantity: 9999 },
          ],
        },
      });
      orderGuard.assertErrorResult(modifyOrder);
  ...
  describe("dry run", () => {
    ...
    it("addItems with existing variant id increments existing OrderLine", async () => {
      ...
      const lineT1 = modifyOrder.lines.find(
        (l) => l.productVariant.id === "T_1",
      );
    ...
    it("adjustOrderLines up", async () => {
      ...
      const expectedTotal =
        order!.totalWithTax + order!.lines[0].unitPriceWithTax * 2;
    ...
    it("adjustOrderLines down", async () => {
      ...
      const expectedTotal =
        order!.totalWithTax - order!.lines[1].unitPriceWithTax;
    ...
    it("adjustOrderLines to zero", async () => {
      ...
      const expectedTotal =
        order!.totalWithTax -
        order!.lines[0].unitPriceWithTax * order!.lines[0].quantity;
      expect(modifyOrder.totalWithTax).toBe(expectedTotal);
  ...
  describe("wet run", () => {
    async function assertModifiedOrderIsPersisted(
      order: OrderWithModificationsFragment,
    ) {
      ...
      expect(
        order2!.payments!.map((p) => pick(p, ["id", "amount", "method"])),
      ).toEqual(
    ...
    it("adjustOrderLines up", async () => {
      ...
      expect(modifyOrder.modifications[0].priceChange).toBe(priceDelta);
      expect(modifyOrder.modifications[0].lines.length).toBe(1);
      expect(modifyOrder.lines[0].id).toEqual(
        modifyOrder.modifications[0].lines[0].orderLineId,
      );
      await assertModifiedOrderIsPersisted(modifyOrder);
    });
    ...
    it("adjustOrderLines down", async () => {
      ...
      expect(modifyOrder.modifications.length).toBe(1);
      expect(modifyOrder.modifications[0].priceChange).toBe(priceDelta);
      expect(modifyOrder.modifications[0].surcharges).toEqual(
        modifyOrder.surcharges.map(pick(["id"])),
      );
      expect(modifyOrder.modifications[0].lines.length).toBe(1);
      expect(modifyOrder.lines[0].id).toEqual(
        modifyOrder.modifications[0].lines[0].orderLineId,
    ...
    it("adjustOrderLines with changed customField value", async () => {
      ...
      const { order: orderWithLines } = await adminClient.query(
  ...
  describe("refunds for multiple payments", () => {
    ...
    beforeAll(async () => {
      ...
      const transitionOrderToState2 = await adminTransitionOrderToState(
        orderId2,
        "PaymentSettled",
      );
    ...
    it("apply couponCode to create first refund", async () => {
      ...
      expect(
        modifyOrder?.payments?.find((p) => p.id === additionalPaymentId)
          ?.refunds,
      ).toEqual([
        {
          id: "T_4",
      ...
      expect(modifyOrder.totalWithTax).toBe(
        getOrderPaymentsTotalWithRefunds(modifyOrder),
      );
    });
    ...
    it("reduce quantity to create second refund", async () => {
      ...
      expect(
        modifyOrder?.payments
          ?.find((p) => p.id === additionalPaymentId)
          ?.refunds.sort(sortById),
      ).toEqual([
        {
          id: "T_4",
          paymentId: additionalPaymentId,
  ...
  describe("correct price calculations when prices include tax", () => {
    ...
    it("without promotion", async () => {
      ...
      expect(modifyOrder.lines[0].linePriceWithTax).toBe(
        order.lines[0].linePriceWithTax * 2,
      );
    });
  ...
  describe("refund handling when promotions are active on order", () => {
    ...
    it("refunds correct amount when order-level promotion applied", async () => {
      ...
      expect(modifyOrder.payments![0].refunds[0].total).toBe(
        order.lines[0].proratedUnitPriceWithTax,
      );
      expect(modifyOrder.totalWithTax).toBe(
        getOrderPaymentsTotalWithRefunds(modifyOrder),
      );
    ...
    describe("issue 1865", () => {
      ...
      beforeAll(async () => {
        ...
        >(CREATE_PROMOTION, {
          input: {
            ...
            translations: [
              { languageCode: LanguageCode.en, name: "50 off orders over 100" },
            ],
          },
        });
      ...
      it("refund handling when order-level promotion becomes invalid on modification", async () => {
        ...
        const expectedTotal =
          football.priceWithTax * 2 + shippingPrice - promoDiscount;
        ...
        const expectedNewTotal =
          order.lines[0].unitPriceWithTax + shippingPrice;
        expect(modifyOrder.totalWithTax).toBe(expectedNewTotal);
        expect(modifyOrder.payments![0].refunds[0].total).toBe(
          expectedTotal - expectedNewTotal,
        );
        expect(modifyOrder.totalWithTax).toBe(
          getOrderPaymentsTotalWithRefunds(modifyOrder),
        );
  ...
  describe("refund on shipping when change made to shippingAddress", () => {
    ...
    it("creates a Refund with the correct amount", async () => {
      expect(order.payments?.[0].refunds[0].total).toBe(
        SHIPPING_OTHER - SHIPPING_GB,
      );
    });
  ...
  describe("updating stock levels", () => {
    ...
    it("updates stock when increasing quantity after fulfillment", async () => {
      const result = await adminTransitionOrderToState(
        orderId4,
        "ArrangingAdditionalPayment",
      );
      orderGuard.assertSuccess(result);
    ...
    it("updates stock when adding item before fulfillment", async () => {
      ...
      orderGuard.assertSuccess(addManualPaymentToOrder);
      const result2 = await adminTransitionOrderToState(
        orderId5,
        "PaymentSettled",
      );
  ...
  describe("couponCode handling", () => {
    ...
    beforeAll(async () => {
      ...
      >(CREATE_PROMOTION, {
        input: {
          ...
          translations: [
            { languageCode: LanguageCode.en, name: "Free shipping" },
          ],
        },
      });
    ...
    it("creates history entry for applying couponCode", async () => {
      ...
      >(GET_ORDER_HISTORY, {
        id: order.id,
        options: {
          filter: { type: { eq: HistoryEntryType.ORDER_COUPON_APPLIED } },
        },
      });
      orderGuard.assertSuccess(history);
    ...
    it("creates history entry for removing couponCode", async () => {
      ...
      >(GET_ORDER_HISTORY, {
        id: order.id,
        options: {
          filter: { type: { eq: HistoryEntryType.ORDER_COUPON_REMOVED } },
        },
      });
      orderGuard.assertSuccess(history);
  ...
  function getOrderPaymentsTotalWithRefunds(
    _order: OrderWithModificationsFragment,
  ) {
    return (
      _order.payments?.reduce(
        (sum, p) => sum + p.amount - summate(p?.refunds, "total"),
        0,
      ) ?? 0
    );
  }
