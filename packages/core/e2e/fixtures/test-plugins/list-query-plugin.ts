...
export class CustomFieldOtherRelationTestEntity extends DeenruvEntity {
  ...
  @ManyToOne(
    () => TestEntity,
    (testEntity) => testEntity.customFields.otherRelation,
  )
  parent: Relation<TestEntity>;
}
...
@Entity()
export class TestEntityTranslation
  extends DeenruvEntity
  implements Translation<TestEntity>
{
      ...
      .then(([items, totalItems]) => {
        for (const item of items) {
          if (item.prices && item.prices.length) {
            item.activePrice = item.prices.find(
              (p) => p.channelId === 1,
            )!.price;
          }
        }
        return {
          items: items.map((i) =>
            translateDeep(i, ctx.languageCode, ["parent"]),
          ),
          totalItems,
        };
      });
...
export class ListQueryPlugin implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    const count = await this.connection.rawConnection
      ...
      .count();
    if (count === 0) {
      const testEntities = await this.connection.rawConnection
        .getRepository(TestEntity)
        .save([
          new TestEntity({
            label: "A",
      ...
      for (const testEntity of testEntities) {
        ...
        for (const code of [LanguageCode.en, LanguageCode.de]) {
          ...
          if (translation) {
            await this.connection.rawConnection
              .getRepository(TestEntityTranslation)
              .save(
        ...
        if (nestedData[testEntity.label]) {
          for (const nestedContent of nestedData[testEntity.label]) {
            await this.connection.rawConnection
              .getRepository(CustomFieldRelationTestEntity)
              .save(
