...
class TestResolver {
  ...
  @Mutation()
  @Transaction()
  async createNTestAdministrators3(
    @Ctx() ctx: RequestContext,
    @Args() args: any,
  ) {
    const result: any[] = [];
  ...
  adminApiExtensions: {
    schema: gql`
      extend type Mutation {
        ...
        createTestAdministrator2(
          emailAddress: String!
          fail: Boolean!
        ): Administrator
        createTestAdministrator3(
          emailAddress: String!
          fail: Boolean!
        ): Administrator
        createTestAdministrator4(
          emailAddress: String!
          fail: Boolean!
        ): Administrator
  ...
  onApplicationBootstrap(): any {
    ...
    this.subscription = this.eventBus
      ...
      .subscribe(async (event) => {
        ...
        if (
          administrator.emailAddress?.includes(TRIGGER_ATTEMPTED_UPDATE_EMAIL)
        ) {
          TransactionTestPlugin.callHandler();
          const adminRepository = this.connection.getRepository(
            ctx,
            Administrator,
          );
          await new Promise((resolve) => setTimeout(resolve, 50));
        ...
        if (
          administrator.emailAddress?.includes(TRIGGER_ATTEMPTED_READ_EMAIL)
        ) {
          TransactionTestPlugin.callHandler();
          ...
          try {
            await adminRepository.findOneOrFail({
