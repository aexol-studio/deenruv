...
describe("Orders resolver", () => {
  ...
  const paymentGuard: ErrorResultGuard<PaymentFragment> =
    createErrorResultGuard((input) => !!input.state);
  const fulfillmentGuard: ErrorResultGuard<FulfillmentFragment> =
    createErrorResultGuard((input) => !!input.method);
  const refundGuard: ErrorResultGuard<RefundFragment> = createErrorResultGuard(
    (input) => !!input.total,
  );
  ...
  describe("querying", () => {
    it("orders", async () => {
      const result =
        await adminClient.query<Codegen.GetOrderListQuery>(GET_ORDERS_LIST);
      expect(result.orders.items.map((o) => o.id).sort()).toEqual([
        "T_1",
        "T_2",
      ]);
    });
    ...
    it("sort by totalWithTax", async () => {
      ...
      expect(
        result.orders.items.map((o) => pick(o, ["id", "totalWithTax"])),
      ).toEqual([
    ...
    it("sort by totalQuantity", async () => {
      ...
      expect(
        result.orders.items.map((o) => pick(o, ["id", "totalQuantity"])),
      ).toEqual([
    ...
    it("sort by customerLastName", async () => {
      ...
      expect(result1.items.map((order) => order.customer?.lastName)).toEqual([
        "Donnelly",
        "Zieme",
      ]);
      ...
      expect(result2.items.map((order) => order.customer?.lastName)).toEqual([
        "Zieme",
        "Donnelly",
      ]);
    });
    ...
    it("filter by totalWithTax", async () => {
      ...
      expect(
        result.orders.items.map((o) => pick(o, ["id", "totalWithTax"])),
      ).toEqual([{ id: "T_2", totalWithTax: 959520 }]);
    });
    ...
    it("filter by totalQuantity", async () => {
      ...
      expect(
        result.orders.items.map((o) => pick(o, ["id", "totalQuantity"])),
      ).toEqual([{ id: "T_2", totalQuantity: 4 }]);
    });
    ...
    it("filter by customerLastName", async () => {
      ...
      expect(result.orders.items[0].customer?.lastName).toEqual(
  ...
  describe("payments", () => {
    ...
    it("settlePayment fails", async () => {
      ...
      expect((settlePayment as any).paymentErrorMessage).toBe(
  ...
  describe("fulfillment", () => {
    ...
    it("return error result if lines is empty", async () => {
      ...
      expect(addFulfillmentToOrder.message).toBe(
        "At least one OrderLine must be specified",
      );
      expect(addFulfillmentToOrder.errorCode).toBe(
    ...
    it("creates the first fulfillment", async () => {
      ...
      expect(
        result.order!.fulfillments![0]!.lines.filter(
          (l) => l.orderLineId === lines[1].id,
        ).length,
      ).toBe(0);
    });
    ...
    it("order.fulfillments still lists second (cancelled) fulfillment", async () => {
      ...
      expect(
        order?.fulfillments?.sort(sortById).map(pick(["id", "state"])),
      ).toEqual([
    ...
    it("order.fulfillments.summary", async () => {
      ...
      expect(
        order?.fulfillments
          ?.sort(sortById)
          .map(pick(["id", "state", "summary"])),
      ).toEqual([
        {
          id: f1Id,
          state: "Pending",
          summary: [{ orderLine: { id: "T_3" }, quantity: 1 }],
        },
        {
          id: f2Id,
          state: "Cancelled",
          summary: [{ orderLine: { id: "T_4" }, quantity: 3 }],
        },
      ]);
    });
    ...
    it("lines.fulfillments", async () => {
      ...
      expect(
        order?.lines.find((l) => l.id === "T_3")!.fulfillmentLines,
      ).toEqual([
        {
          fulfillment: { id: f1Id, state: "Pending" },
          orderLineId: "T_3",
          quantity: 1,
        },
      ]);
      ...
      expect(
        order?.lines.find((l) => l.id === "T_4")!.fulfillmentLines,
      ).toEqual([]);
    });
    ...
    it("returns error result if an OrderItem already part of a Fulfillment", async () => {
      ...
      expect(addFulfillmentToOrder.errorCode).toBe(
        ErrorCode.ITEMS_ALREADY_FULFILLED_ERROR,
      );
    });
    ...
    it("order.fulfillments resolver for single order", async () => {
      ...
      expect(
        order!.fulfillments
          ?.sort(sortById)
          .map(pick(["id", "method", "state", "nextStates"])),
      ).toEqual([
        {
          id: f1Id,
          method: "Test1",
    ...
    it("order.fulfillments resolver for order list", async () => {
      const { orders } =
        await adminClient.query<Codegen.GetOrderListFulfillmentsQuery>(
  ...
  describe("cancellation by orderId", () => {
    ...
    it("cancel from PaymentAuthorized state with cancelShipping: true", async () => {
      ...
      expect(
        variant1.stockMovements.items.map(pick(["type", "quantity"])),
      ).toEqual([
    ...
    async function assertNoStockMovementsCreated(productId: string) {
      ...
      expect(
        variant2.stockMovements.items.map(pick(["type", "quantity"])),
      ).toEqual([{ type: StockMovementType.ADJUSTMENT, quantity: 100 }]);
    }
  });
  ...
  describe("cancellation by OrderLine", () => {
    ...
    it("returns error result if lines are empty", async () => {
      ...
      expect(cancelOrder.errorCode).toBe(
        ErrorCode.EMPTY_ORDER_LINE_SELECTION_ERROR,
      );
    });
  ...
  describe("refunds", () => {
    ...
    it("returns error result if payment and order lines do not belong to the same Order", async () => {
      ...
      expect(refundOrder.errorCode).toBe(
        ErrorCode.PAYMENT_ORDER_MISMATCH_ERROR,
      );
    });
    ...
    it("order history contains expected entries", async () => {
      ...
      expect(
        order!.history.items.sort(sortById).map(pick(["type", "data"])),
      ).toEqual([
        {
          type: HistoryEntryType.ORDER_STATE_TRANSITION,
          data: {
  ...
  describe("multiple payments", () => {
    ...
    it("partial refunding of order with multiple payments", async () => {
      ...
      expect(
        orderWithPayments?.payments!.sort(sortById)[0].refunds.length,
      ).toBe(1);
      expect(
        orderWithPayments?.payments!.sort(sortById)[0].refunds[0].total,
      ).toBe(PARTIAL_PAYMENT_AMOUNT);

      expect(
        orderWithPayments?.payments!.sort(sortById)[1].refunds.length,
      ).toBe(1);
      expect(
        orderWithPayments?.payments!.sort(sortById)[1].refunds[0].total,
      ).toBe(productInOrder!.variants[0].priceWithTax - PARTIAL_PAYMENT_AMOUNT);
    });
    ...
    it("refunding remaining amount of order with multiple payments", async () => {
      ...
      expect(
        orderWithPayments?.payments!.sort(sortById)[1].refunds[1].total,
    ...
    it("manual call to settlePayment works with multiple payments", async () => {
      ...
      const { settlePayment } = await adminClient.query<
        ...
        Codegen.SettlePaymentMutationVariables
      >(SETTLE_PAYMENT, {
        id: order.payments!.find((p) => p.method === partialPaymentMethod.code)!
          .id,
      });
  ...
  describe("issues", () => {
    ...
    it("get an Order with a deleted ShippingMethod", async () => {
      ...
      expect(order2?.shippingLines[0]).toEqual({
        priceWithTax: 500,
        shippingMethod: pick(shippingMethod, [
          "id",
          "name",
          "code",
          "description",
    ...
    it("resolves deleted Product of OrderLine ProductVariants", async () => {
      ...
      expect(
        activeCustomer!.orders.items[activeCustomer!.orders.items.length - 1]
          .lines[0].productVariant.product.slug,
      ).toBe("gaming-pc");
    });
    ...
    it("creates correct history entries and results in correct state when manually adding payment to order", async () => {
      ...
      const { order: orderWithHistory } = await adminClient.query(
        GetOrderHistoryDocument,
        {
...
async function getUnfulfilledOrderLineInput(
  ...
  const allFulfillmentLines =
    order?.fulfillments
      ...
      .reduce(
        (all, f) => [...all, ...f.lines],
        [] as Codegen.FulfillmentFragment["lines"],
      ) || [];
  ...
  const unfulfilledItems =
    order?.lines
      .map((l) => {
        ...
        return {
          orderLineId: l.id,
          unfulfilled: l.quantity - fulfilledQuantity,
        };
      })
