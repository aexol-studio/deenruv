import { ErrorCode } from "@deenruv/common/generated-shop-types";
...
const currentUserGuard: ErrorResultGuard<CurrentUserFragment> =
  createErrorResultGuard((input) => input.identifier != null);
const customerGuard: ErrorResultGuard<CustomerFragment> =
  createErrorResultGuard((input) => input.emailAddress != null);
...
describe("AuthenticationStrategy", () => {
  ...
  describe("external auth", () => {
    ...
    it("user authenticationMethod populated", async () => {
      ...
      expect(customer?.user?.authenticationMethods[0].strategy).toBe(
        "test_strategy",
      );
    });
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/695
    ...
    it("registerCustomerAccount with external email", async () => {
      ...
      expect(
        customer?.user?.authenticationMethods.map((m) => m.strategy),
      ).toEqual(["test_strategy", "test_strategy2", "native"]);
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/926
    it("Customer and Admin external auth does not reuse same User for different strategies", async () => {
      ...
      const { authenticate: adminAuth } =
        await adminClient.query<Codegen.AuthenticateMutation>(AUTHENTICATE, {
          input: {
  ...
  describe("native auth", () => {
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/486#issuecomment-704991768
...
describe("No NativeAuthStrategy on Shop API", () => {
  ...
  // https://github.com/deenruv-ecommerce/deenruv/issues/2282
