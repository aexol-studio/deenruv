import { DefaultJobQueuePlugin, mergeConfig } from "@deenruv/core";
...
describe("JobQueue", () => {
  ...
  function getJobsInTestQueue(state?: JobState) {
    return adminClient
      .query<GetRunningJobsQuery, GetRunningJobsQueryVariables>(
        GET_RUNNING_JOBS,
        {
          options: {
            filter: {
  ...
  it("cancels a running job", async () => {
    ...
    const { cancelJob } = await adminClient.query<
      CancelJobMutation,
      CancelJobMutationVariables
    >(CANCEL_JOB, {
      id: jobId,
