...
describe("EntityIdStrategy", () => {
  ...
  it("Does not doubly-encode ids from resolved properties", async () => {
    ...
    expect(
      products.items[0].variants[0].options.map((o) => o.id).sort(),
    ).toEqual(["T_1", "T_3"]);
  });
    ...
    const { updateProduct } = await adminClient.query<IdTest5.Mutation>(gql`
      mutation IdTest5 {
        updateProduct(
          input: {
            id: "T_1"
            translations: [{ id: "T_1", languageCode: en, name: "changed" }]
          }
        ) {
          id
          name
    ...
    >(
      ...
      `,
      {
        input: {
          id: "T_1",
          translations: [
            { id: "T_1", languageCode: LanguageCode.en, name: "changed again" },
          ],
        },
      },
    );
