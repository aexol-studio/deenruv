...
const orderResultGuard: ErrorResultGuard<UpdatedOrderFragment> =
...
describe("Entity hydration", () => {
  ...
  it("includes existing relations", async () => {
    const { hydrateProduct } = await adminClient.query<HydrateProductQuery>(
      GET_HYDRATED_PRODUCT,
      {
        id: "T_1",
      },
    );
  ...
  it("hydrates top-level single relation", async () => {
    ...
    expect(hydrateProduct.featuredAsset.name).toBe(
      "derick-david-409858-unsplash.jpg",
    );
  });
  ...
  it("hydrates top-level array relation", async () => {
    ...
    expect(hydrateProduct.assets[0].asset.name).toBe(
      "derick-david-409858-unsplash.jpg",
    );
  });
  ...
  it("populates ProductVariant price data", async () => {
    ...
    expect(getVariantWithName(hydrateProduct, "Laptop 13 inch 8GB").price).toBe(
      129900,
    );
    expect(
      getVariantWithName(hydrateProduct, "Laptop 13 inch 8GB").priceWithTax,
    ).toBe(155880);
    expect(
      getVariantWithName(hydrateProduct, "Laptop 13 inch 16GB").price,
    ).toBe(219900);
    expect(
      getVariantWithName(hydrateProduct, "Laptop 13 inch 16GB").priceWithTax,
    ).toBe(263880);
    expect(getVariantWithName(hydrateProduct, "Laptop 15 inch 8GB").price).toBe(
      139900,
    );
    expect(
      getVariantWithName(hydrateProduct, "Laptop 15 inch 8GB").priceWithTax,
    ).toBe(167880);
    expect(
      getVariantWithName(hydrateProduct, "Laptop 15 inch 16GB").price,
    ).toBe(229900);
    expect(
      getVariantWithName(hydrateProduct, "Laptop 15 inch 16GB").priceWithTax,
    ).toBe(275880);
  });
  ...
  it("correctly expands missing relations", async () => {
    ...
    }>(GET_HYDRATED_VARIANT, { id: "T_1" });

    expect(hydrateProductVariant.product.id).toBe("T_1");
    expect(
      hydrateProductVariant.product.facetValues.map((fv) => fv.id).sort(),
    ).toEqual(["T_1", "T_2"]);
  });
  ...
  it("can hydrate entity with getters (Order)", async () => {
    ...
    const { hydrateOrder } = await adminClient.query<{ hydrateOrder: Order }>(
      GET_HYDRATED_ORDER,
      {
  ...
  it("hydrates a nested custom field", async () => {
    ...
    expect(
      hydrateChannelWithNestedRelation.customFields.additionalConfig,
    ).toBeDefined();
  });
