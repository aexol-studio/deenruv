  ...
  async setCustomerForOrder(
    ...
    if (TestGuestCheckoutStrategy.allowGuestCheckout === false) {
      return new GuestCheckoutError({
        errorDetail: "Guest orders are disabled",
      });
    }
    if (ctx.activeUserId) {
    ...
    const errorOnExistingUser =
      !TestGuestCheckoutStrategy.allowGuestCheckoutForRegisteredCustomers;
...
describe("Order taxes", () => {
  ...
  it("with guest checkout disabled", async () => {
    ...
    expect((setCustomerForOrder as any).errorDetail).toBe(
      "Guest orders are disabled",
    );
  });
