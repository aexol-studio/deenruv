...
describe("Payment process", () => {
  ...
  class TestOrderPlacedStrategy implements OrderPlacedStrategy {
    shouldSetAsPlaced(
      ...
      return (
        ...
        toState === ("ValidatingPayment" as any)
      );
    }
  }

  const orderGuard: ErrorResultGuard<
    CodegenShop.TestOrderFragmentFragment | Codegen.OrderFragment
  > = createErrorResultGuard((input) => !!input.total);

  const paymentGuard: ErrorResultGuard<Codegen.PaymentFragment> =
  ...
  it("calls transition hooks", async () => {
    ...
    expect(transitionEndSpy.mock.calls[0].slice(0, 2)).toEqual([
      "Created",
      "Validating",
    ]);
  ...
  it("Payment next states", async () => {
    ...
    expect(order?.payments?.[0].nextStates).toEqual([
      "Settled",
      "Declined",
      "Cancelled",
    ]);
  ...
  describe("failing, cancelling, and manually adding a Payment", () => {
    ...
    it("attempting to transition payment to settled fails", async () => {
      ...
      expect(transitionPaymentToState.errorCode).toBe(
        ErrorCode.PAYMENT_STATE_TRANSITION_ERROR,
      );
      expect((transitionPaymentToState as any).transitionError).toBe(
    ...
    it("manually adds payment", async () => {
      ...
      expect(addManualPaymentToOrder.payments![1].amount).toBe(
        addManualPaymentToOrder.totalWithTax,
      );
    });
