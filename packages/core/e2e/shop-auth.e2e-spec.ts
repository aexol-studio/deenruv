...
const successErrorGuard: ErrorResultGuard<{ success: boolean }> =
  createErrorResultGuard((input) => input.success != null);

const currentUserErrorGuard: ErrorResultGuard<CurrentUserShopFragment> =
  createErrorResultGuard((input) => input.identifier != null);
...
describe("Shop auth & accounts", () => {
  ...
  describe("customer account creation with deferred password", () => {
    ...
    it("verification fails with wrong token", async () => {
      ...
      expect(verifyCustomerAccount.errorCode).toBe(
    ...
    it("verification fails with invalid password", async () => {
      ...
      expect(
        (verifyCustomerAccount as PasswordValidationError)
          .validationErrorMessage,
      ).toBe("Password must be more than 8 characters");
      expect(verifyCustomerAccount.errorCode).toBe(
    ...
    it("verification succeeds with password and correct token", async () => {
      ...
      const { activeCustomer } =
        await shopClient.query<CodegenShop.GetActiveCustomerQuery>(
          GET_ACTIVE_CUSTOMER,
        );
  ...
  describe("customer account creation with up-front password", () => {
    ...
    it("registerCustomerAccount fails with invalid password", async () => {
      ...
      expect(registerCustomerAccount.errorCode).toBe(
        ErrorCode.PASSWORD_VALIDATION_ERROR,
      );
      expect(registerCustomerAccount.message).toBe("Password is invalid");
      expect(
        (registerCustomerAccount as PasswordValidationError)
          .validationErrorMessage,
      ).toBe("Don't use 12345678!");
    });
    ...
    it("register a new account with password", async () => {
      ...
      expect(
        ...
        pick(input, ["firstName", "lastName", "emailAddress", "phoneNumber"]),
      );
    });

    it("login fails before verification", async () => {
      const result = await shopClient.asUserWithCredentials(
        emailAddress,
        password,
      );
      expect(result.errorCode).toBe(ErrorCode.NOT_VERIFIED_ERROR);
      expect(result.message).toBe(
        "Please verify this email address before logging in",
      );
    });
    ...
    it("verification fails with password", async () => {
      ...
      expect(verifyCustomerAccount.errorCode).toBe(
        ErrorCode.PASSWORD_ALREADY_SET_ERROR,
      );
    });
  ...
  describe("password reset", () => {
    ...
    it("resetPassword returns error result with wrong token", async () => {
      ...
      expect(resetPassword.errorCode).toBe(
    ...
    it("resetPassword fails with invalid password", async () => {
      ...
      expect(
        (resetPassword as PasswordValidationError).validationErrorMessage,
      ).toBe("Password must be more than 8 characters");
    ...
    it("customer history for password reset", async () => {
      ...
      expect(
        result.customer?.history.items.map(pick(["type", "data"])),
      ).toEqual([
        {
  ...
  describe("updating emailAddress", () => {
    ...
    it("return error result if password is incorrect", async () => {
      ...
      expect(requestUpdateCustomerEmailAddress.message).toBe(
        "The provided credentials are invalid",
      );
      expect(requestUpdateCustomerEmailAddress.errorCode).toBe(
    ...
    it("triggers event with token", async () => {
      ...
      const { identifierChangeToken, pendingIdentifier } =
        await emailUpdateTokenPromise;
    ...
    it("return error result for bad token", async () => {
      ...
      expect(updateCustomerEmailAddress.message).toBe(
    ...
    it("verify the new email address", async () => {
      ...
      expect(sendEmailFn).toHaveBeenCalled();
      expect(
        sendEmailFn.mock.calls[0][0] instanceof IdentifierChangeEvent,
      ).toBe(true);
    });
  ...
  async function assertRequestAllowed<V>(
  ...
  async function assertRequestForbidden<V>(
    operation: DocumentNode,
    variables: V,
  ) {
    try {
...
describe("Registration without email verification", () => {
  ...
  it("Returns error result if no password is provided", async () => {
    ...
    expect(registerCustomerAccount.message).toBe(
      "A password must be provided.",
    );
    expect(registerCustomerAccount.errorCode).toBe(
      ErrorCode.MISSING_PASSWORD_ERROR,
...
describe("Updating email address without email verification", () => {
  ...
  it("updates email address", async () => {
    ...
    expect(sendEmailFn).toHaveBeenCalledTimes(1);
    expect(sendEmailFn.mock.calls[0][0] instanceof IdentifierChangeEvent).toBe(
      true,
    );
