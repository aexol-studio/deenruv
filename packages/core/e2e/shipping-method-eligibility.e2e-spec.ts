...
describe("ShippingMethod eligibility", () => {
    ...
    >(CREATE_SHIPPING_METHOD, {
      input: {
        ...
        translations: [
          {
            languageCode: LanguageCode.en,
            name: "Optimized with shouldRunCheck",
            description: "",
          },
        ],
      },
  ...
  describe("default behavior", () => {
    ...
    it("Runs checks when assigned method becomes ineligible", async () => {
      ...
      expect(activeOrder?.shippingLines?.[0]?.shippingMethod?.id).toBe(
        multiLineShippingMethod.id,
      );
      ...
      expect(removeOrderLine.shippingLines[0].shippingMethod?.id).toBe(
        singleLineShippingMethod.id,
      );
    });
  });
