...
describe("custom OrderItemPriceCalculationStrategy", () => {
  ...
  beforeAll(async () => {
    ...
    const { search } = await shopClient.query<
      SearchProductsShopQuery,
      SearchProductsShopQueryVariables
    >(SEARCH_PRODUCTS_SHOP, {
  ...
  it("does not add surcharge", async () => {
    ...
    const { addItemToOrder } = await shopClient.query(
      ADD_ITEM_TO_ORDER_CUSTOM_FIELDS,
      {
    ...
    expect(addItemToOrder.lines[0].unitPrice).toEqual(
      (variant0.price as SinglePrice).value,
    );
  });
  ...
  it("re-calculates when customFields changes", async () => {
    const { adjustOrderLine } = await shopClient.query(
      ADJUST_ORDER_LINE_CUSTOM_FIELDS,
      {
  ...
  it("applies discount for quantity greater than 3", async () => {
    ...
    expect(adjustOrderLine.subTotal).toEqual(
      variantPrice + (variantPrice / 2) * 4,
    );
  });
});
