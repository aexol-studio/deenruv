...
describe("Promotions applied to Orders", () => {
  ...
  beforeAll(async () => {
    await server.init({
      ...
      productsCsvPath: path.join(
        __dirname,
        "fixtures/e2e-products-promotions.csv",
      ),
      customerCount: 2,
    });
  ...
  describe("coupon codes", () => {
    ...
    it("applyCouponCode returns error result when code is nonexistant", async () => {
      ...
      expect(applyCouponCode.errorCode).toBe(
    ...
    it("applyCouponCode returns error when code is expired", async () => {
      ...
      expect(applyCouponCode.errorCode).toBe(
        ErrorCode.COUPON_CODE_EXPIRED_ERROR,
      );
    });
    ...
    it("applies a valid coupon code", async () => {
      ...
      expect(applyCouponCode.discounts[0].description).toBe(
        "Free with test coupon",
      );
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/649
    ...
    it("does not record removal of coupon code that was not added", async () => {
      ...
      expect(
        removeCouponCode!.history.items.map((i) => omit(i, ["id"])),
      ).toEqual([
        {
          type: HistoryEntryType.ORDER_STATE_TRANSITION,
          data: {
    ...
    describe("coupon codes in other channels", () => {
      ...
      // https://github.com/deenruv-ecommerce/deenruv/issues/1692
  ...
  describe("default PromotionConditions", () => {
    ...
    it("minimumOrderAmount", async () => {
      ...
      expect(adjustOrderLine.discounts[0].description).toBe(
    ...
    it("containsProducts", async () => {
      ...
      const { adjustOrderLine } = await shopClient.query<
        ...
        CodegenShop.AdjustItemQuantityMutationVariables
      >(ADJUST_ITEM_QUANTITY, {
        orderLineId: addItemToOrder.lines.find(
          (l) => l.productVariant.id === item5000.id,
        )!.id,
        quantity: 2,
      });
      orderResultGuard.assertSuccess(adjustOrderLine);
    ...
    it("customerGroup", async () => {
        ...
        conditions: [
          {
            ...
            arguments: [
              { name: "customerGroupId", value: createCustomerGroup.id },
            ],
          },
        ],
        actions: [freeOrderAction],
      ...
      expect(addItemToOrder.discounts[0].description).toBe(
  ...
  describe("default PromotionActions", () => {
    ...
    async function assignPromotionToTaxIncludedChannel(
      ...
      >(ASSIGN_PROMOTIONS_TO_CHANNEL, {
        input: {
          promotionIds: Array.isArray(promotionId)
            ? promotionId
            : [promotionId],
          channelId: taxIncludedChannel.id,
        },
    ...
    describe("orderPercentageDiscount", () => {
      ...
      // https://github.com/deenruv-ecommerce/deenruv/issues/1773
    ...
    describe("discountOnItemWithFacets", () => {
      ...
      >(lines: T): T[number] {
        return lines.find(
          (l) => l.productVariant.id === getVariantBySlug("item-sale-100").id,
        )!;
      }
      ...
      it("prices exclude tax", async () => {
        ...
        expect(getItemSale1Line(applyCouponCode.lines).discounts.length).toBe(
          1,
        ); // 1x promotion
        ...
        expect(getItemSale1Line(removeCouponCode!.lines).discounts.length).toBe(
          0,
        );
    ...
    describe("freeShipping", () => {
      ...
      it("prices exclude tax", async () => {
        ...
        const method = await createTestShippingMethod(
          E2E_DEFAULT_CHANNEL_TOKEN,
        );
        const { setOrderShippingMethod } = await shopClient.query<
          CodegenShop.SetShippingMethodMutation,
      ...
      // https://github.com/deenruv-ecommerce/deenruv/pull/1150
  ...
  describe("per-customer usage limit", () => {
    ...
    describe("guest customer", () => {
      ...
      it("does not remove valid couponCode when setting guest customer", async () => {
        ...
        const { activeOrder } =
          await shopClient.query<GetActiveOrder.Query>(GET_ACTIVE_ORDER);
    ...
    describe("signed-in customer", () => {
      ...
      // https://github.com/deenruv-ecommerce/deenruv/issues/1466
  ...
  // https://github.com/deenruv-ecommerce/deenruv/issues/710
  ...
  // https://github.com/deenruv-ecommerce/deenruv/issues/1492
  ...
  // https://github.com/deenruv-ecommerce/deenruv/issues/2385
  ...
  async function getProducts() {
    const result =
      await adminClient.query<Codegen.GetProductsWithVariantPricesQuery>(
        GET_PRODUCTS_WITH_VARIANT_PRICES,
        {
          options: {
            take: 10,
  ...
  async function createPromotion(
    input: Omit<Codegen.CreatePromotionInput, "translations"> & {
      name: string;
    },
