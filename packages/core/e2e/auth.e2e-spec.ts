...
import {
  ProtectedFieldsPlugin,
  transactions,
} from "./fixtures/test-plugins/with-protected-field-resolver";
...
describe("Authorization & permissions", () => {
  ...
  describe("admin permissions", () => {
    ...
    describe("Customer user", () => {
      ...
      it("cannot login", async () => {
        const result = await adminClient.asUserWithCredentials(
    ...
    describe("ReadCatalog permission", () => {
      beforeAll(async () => {
        ...
        const { identifier, password } =
          await createAdministratorWithPermissions("ReadCatalog", [
      ...
      it("cannot update", async () => {
        await assertRequestForbidden<Codegen.MutationUpdateProductArgs>(
          UPDATE_PRODUCT,
          {
            input: {
              id: "1",
      ...
      it("cannot create", async () => {
        await assertRequestForbidden<Codegen.MutationCreateProductArgs>(
          CREATE_PRODUCT,
          {
            input: {
              translations: [],
    ...
    describe("CRUD on Customers permissions", () => {
      beforeAll(async () => {
        await adminClient.asSuperAdmin();
        const { identifier, password } =
          await createAdministratorWithPermissions("CRUDCustomer", [
  ...
  describe("administrator and customer users with the same email address", () => {
    ...
    const loginErrorGuard: ErrorResultGuard<Codegen.CurrentUserFragment> =
    ...
    it("cannot log in as an administrator using a customer password", async () => {
      ...
      expect(loginResult.login.errorCode).toEqual(
  ...
  describe("protected field resolvers", () => {
    ...
    beforeAll(async () => {
      await adminClient.asSuperAdmin();
      transactionsAdmin = await createAdministratorWithPermissions(
        "Transactions",
        [Permission.ReadCatalog, transactions.Permission],
      );
      readCatalogAdmin = await createAdministratorWithPermissions(
        "ReadCatalog",
        [Permission.ReadCatalog],
      );
    });
    ...
    it("protected field not resolved without permissions", async () => {
      await adminClient.asUserWithCredentials(
        readCatalogAdmin.identifier,
        readCatalogAdmin.password,
      );

      try {
        const status = await adminClient.query(
          gql(GET_PRODUCT_WITH_TRANSACTIONS),
          { id: "T_1" },
        );
    ...
    it("protected field is resolved with permissions", async () => {
      await adminClient.asUserWithCredentials(
        transactionsAdmin.identifier,
        transactionsAdmin.password,
      );
