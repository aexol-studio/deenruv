import { SUPER_ADMIN_USER_IDENTIFIER } from "@deenruv/common/shared-constants";
import {
...
describe("Administrator resolver", () => {
  ...
  it("activeAdministrator", async () => {
    ...
    const { activeAdministrator: result1 } =
      await adminClient.query<Codegen.ActiveAdministratorQuery>(
  ...
  it("supports case-sensitive admin identifiers", async () => {
    const loginResultGuard: ErrorResultGuard<CurrentUser> =
