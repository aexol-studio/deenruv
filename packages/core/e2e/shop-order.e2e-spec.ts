...
describe("Shop orders", () => {
  ...
  it("availableCountries returns enabled countries", async () => {
    ...
    const result =
      await shopClient.query<CodegenShop.GetAvailableCountriesQuery>(
        GET_AVAILABLE_COUNTRIES,
      );
    expect(result.availableCountries.length).toBe(countries.items.length - 1);
    expect(
      result.availableCountries.find((c) => c.id === AT.id),
    ).toBeUndefined();
  });
  ...
  describe("ordering as anonymous user", () => {
    ...
    it("activeOrder returns null before any items have been added", async () => {
      const result =
        await shopClient.query<CodegenShop.GetActiveOrderQuery>(
          GET_ACTIVE_ORDER,
        );
    ...
    it("addItemToOrder errors with a negative quantity", async () => {
      ...
      expect(addItemToOrder.message).toEqual(
        "The quantity for an OrderItem cannot be negative",
      );
      expect(addItemToOrder.errorCode).toEqual(
    ...
    describe("OrderLine customFields", () => {
      ...
      // https://github.com/deenruv-ecommerce/deenruv/issues/1670
      it("adding a second item after adjusting custom field adds new OrderLine", async () => {
        const { addItemToOrder: add1 } =
          await shopClient.query<AddItemToOrder.Mutation>(
            ADD_ITEM_TO_ORDER_WITH_CUSTOM_FIELDS,
            {
              productVariantId: "T_3",
              quantity: 1,
        ...
        const { activeOrder: ao1 } = await shopClient.query(
          GET_ORDER_WITH_ORDER_LINE_CUSTOM_FIELDS,
        );
        ...
        expect(
          activeOrder.lines.find((l: any) => l.id === updatedNotesLineId)
            ?.customFields,
        ).toEqual({
          lineImage: null,
          lineImages: [],
      ...
      it("addItemToOrder with relation customField", async () => {
        ...
        expect(activeOrder.lines[1].customFields.lineImage).toEqual({
          id: "T_1",
        });
      });
      ...
      it("addItemToOrder with different relation customField adds new line", async () => {
        ...
        expect(activeOrder.lines[2].customFields.lineImage).toEqual({
          id: "T_2",
        });
      });
      ...
      it("adjustOrderLine updates relation reference", async () => {
        ...
        expect(adjustOrderLine.lines[2].customFields.lineImage).toEqual({
          id: "T_1",
        });
      ...
      it("addItemToOrder with list relation customField", async () => {
        ...
        expect(activeOrder.lines[1].customFields.lineImages).toContainEqual({
          id: "T_1",
        });
        expect(activeOrder.lines[1].customFields.lineImages).toContainEqual({
          id: "T_2",
      ...
      it("addItemToOrder with different list relation customField adds new line", async () => {
        ...
        expect(activeOrder.lines[2].customFields.lineImages).toEqual([
          { id: "T_1" },
        ]);
    ...
    it("adjustOrderLine with quantity 0 removes the line", async () => {
      ...
      orderResultGuard.assertSuccess(addItemToOrder);
      expect(addItemToOrder.lines.length).toBe(2);
      expect(addItemToOrder.lines.map((i) => i.productVariant.id)).toEqual([
        "T_1",
        "T_3",
      ]);
      ...
      orderResultGuard.assertSuccess(adjustOrderLine);
      expect(adjustOrderLine.lines.length).toBe(1);
      expect(adjustOrderLine.lines.map((i) => i.productVariant.id)).toEqual([
        "T_1",
      ]);
    });
    ...
    it("adjustOrderLine with quantity > stockOnHand only allows user to have stock on hand", async () => {
      ...
      expect(adjustLine2.lines.map((i) => i.productVariant.id)).toEqual([
        "T_1",
      ]);
    });

    // https://github.com/deenruv-ecommerce/deenruv/issues/2702
    ...
    it("adjustOrderLine handles stockOnHand correctly with multiple order lines with different custom fields when out of stock", async () => {
      ...
      expect(adjustOrderLine.errorCode).toBe(
    ...
    it("removeItemFromOrder removes the correct item", async () => {
      ...
      orderResultGuard.assertSuccess(removeOrderLine);
      expect(removeOrderLine.lines.length).toBe(1);
      expect(removeOrderLine.lines.map((i) => i.productVariant.id)).toEqual([
        "T_3",
      ]);
    });
    ...
    it("nextOrderStates returns next valid states", async () => {
      const result =
        await shopClient.query<CodegenShop.GetNextOrderStatesQuery>(
          GET_NEXT_STATES,
        );
    ...
    it("transitionOrderToState returns error result for invalid state", async () => {
      ...
      expect(transitionOrderToState!.errorCode).toBe(
    ...
    it("setCustomerForOrder returns error result on email address conflict", async () => {
      const { customers } =
        await adminClient.query<Codegen.GetCustomerListQuery>(
          GET_CUSTOMER_LIST,
        );
      ...
      expect(setCustomerForOrder.errorCode).toBe(
    ...
    it("customer default Addresses are updated after payment", async () => {
      ...
      const shippingAddress = result.customer!.addresses!.find(
      ...
      const billingAddress = result.customer!.addresses!.find(
  ...
  describe("ordering as authenticated user", () => {
    ...
    describe("shipping", () => {
      ...
      it("setOrderShippingMethod sets the shipping method", async () => {
        ...
        expect(order.shippingLines[0].shippingMethod.id).toBe(
      ...
      it("shipping method is preserved after adjustOrderLine", async () => {
        ...
        expect(adjustOrderLine.shippingLines[0].shippingMethod.id).toBe(
          shippingMethods[1].id,
        );
        expect(
          adjustOrderLine.shippingLines[0].shippingMethod.description,
        ).toBe(shippingMethods[1].description);
      });
    });
    ...
    describe("payment", () => {
      it("attempting add a Payment returns error result when in AddingItems state", async () => {
        ...
        expect(addPaymentToOrder.errorCode).toBe(
      ...
      it("attempting to add an item returns error result when in ArrangingPayment state", async () => {
        ...
        expect(addItemToOrder.errorCode).toBe(
      ...
      it("attempting to remove an item returns error result when in ArrangingPayment state", async () => {
        ...
        expect(removeOrderLine.errorCode).toBe(
          ErrorCode.ORDER_MODIFICATION_ERROR,
        );
      });
      ...
      it("attempting to remove all items returns error result when in ArrangingPayment state", async () => {
        const { removeAllOrderLines } =
          await shopClient.query<CodegenShop.RemoveAllOrderLinesMutation>(
            REMOVE_ALL_ORDER_LINES,
          );
        ...
        expect(removeAllOrderLines.errorCode).toBe(
          ErrorCode.ORDER_MODIFICATION_ERROR,
        );
      });
      ...
      it("attempting to setOrderShippingMethod returns error result when in ArrangingPayment state", async () => {
        ...
        expect(setOrderShippingMethod.errorCode).toBe(
          ErrorCode.ORDER_MODIFICATION_ERROR,
        );
      });
      ...
      it("adds a declined payment", async () => {
        ...
        expect((addPaymentToOrder as any).paymentErrorMessage).toBe(
          "Insufficient funds",
        );
      ...
      it("adds a successful payment and transitions Order state", async () => {
        ...
        const payment = addPaymentToOrder.payments!.find(
          (p) => p.transactionId === "12345",
        )!;
        expect(addPaymentToOrder.state).toBe("PaymentSettled");
        expect(addPaymentToOrder.active).toBe(false);
  ...
  describe("order merging", () => {
    ...
    /**
     * See https://github.com/deenruv-ecommerce/deenruv/issues/263
     */
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/754
  ...
  describe("security of customer data", () => {
    ...
    it("guest cannot access Addresses of guest customer", async () => {
      ...
      const { activeOrder } =
        await shopClient.query<CodegenShop.GetCustomerAddressesQuery>(
          GET_ACTIVE_ORDER_ADDRESSES,
        );
    ...
    it("guest cannot access Orders of guest customer", async () => {
      ...
      const { activeOrder } =
        await shopClient.query<CodegenShop.GetCustomerOrdersQuery>(
          GET_ACTIVE_ORDER_ORDERS,
        );
  ...
  describe("order custom fields", () => {
    ...
    it("setting order custom fields", async () => {
      const { setOrderCustomFields } = await shopClient.query(
  ...
  describe("validation of product variant availability", () => {
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/1567
  ...
  // https://github.com/deenruv-ecommerce/deenruv/issues/1195
  describe("shipping method invalidation", () => {
    ...
    beforeAll(async () => {
      ...
      const { shippingMethods } =
        await adminClient.query<Codegen.GetShippingMethodListQuery>(
          GET_SHIPPING_METHOD_LIST,
        );
      ...
      function createCountryCodeShippingMethodInput(
        countryCode: string,
      ): CreateShippingMethodInput {
        return {
    ...
    it("if selected method no longer eligible, next best is set automatically", async () => {
      ...
      expect(result1.activeOrder?.shippingLines[0].shippingMethod.id).toBe(
        GBShippingMethodId,
      );
      ...
      expect(result2.activeOrder?.shippingLines[0].shippingMethod.id).toBe(
        ATShippingMethodId,
      );
    });
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/1441
  ...
  describe("edge cases", () => {
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/2548
    it("hydrating Order in the ShippingEligibilityChecker does not break order modification", async () => {
      ...
      expect(
        result1.activeOrder?.lines.map((l) => l.linePriceWithTax).sort(),
      ).toEqual([155880, 503640]);
      ...
      const { setOrderShippingMethod } = await shopClient.query<
        ...
        CodegenShop.SetShippingMethodMutationVariables
      >(SET_SHIPPING_METHOD, {
        id: eligibleShippingMethods.find((m) => m.code === "hydrating-checker")!
          .id,
      });
      orderResultGuard.assertSuccess(setOrderShippingMethod);
      ...
      expect(
        result2.activeOrder?.lines.map((l) => l.linePriceWithTax).sort(),
      ).toEqual([503640]);
