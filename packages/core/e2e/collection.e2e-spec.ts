...
describe("Collection resolver", () => {
  ...
  beforeAll(async () => {
    await server.init({
      ...
      productsCsvPath: path.join(
        __dirname,
        "fixtures/e2e-products-collections.csv",
      ),
      customerCount: 1,
    });
    ...
    const facetValuesResult =
      await adminClient.query<Codegen.GetFacetValuesQuery>(GET_FACET_VALUES);
  ...
  it("collection breadcrumbs works after bootstrap", async () => {
    const result =
      await adminClient.query<Codegen.GetCollectionBreadcrumbsQuery>(
  ...
  describe("createCollection", () => {
    ...
    it("slug is normalized to be url-safe", async () => {
      ...
      expect(
        createCollection.translations.find(
    ...
    it("creates a root collection to become a 1st level collection later #779", async () => {
      ...
      expect(computersBreadcrumbsCollection.parent!.name).toBe(
    ...
    it("creates a root collection to be a parent collection for 1st level collection with id greater than child collection #779", async () => {
      ...
      expect(electronicsBreadcrumbsCollection.parent!.name).toBe(
        ROOT_COLLECTION_NAME,
      );
    });
    it("creates a 2nd level nested collection #779", async () => {
      ...
      expect(pearBreadcrumbsCollection.parent!.name).toBe(
        computersBreadcrumbsCollection.name,
      );
    });
  });
  ...
  describe("updateCollection", () => {
    ...
    it("updating existing assets", async () => {
      ...
      await awaitRunningJobs(adminClient, 5000);
      expect(updateCollection.assets.map((a) => a.id)).toEqual([
        assets[3].id,
        assets[0].id,
      ]);
    });
  ...
  describe("querying", () => {
    ...
    it("collections.assets", async () => {
      const { collections } =
        await adminClient.query<Codegen.GetCollectionsWithAssetsQuery>(gql`
    ...
    it("sorting on Collection.productVariants.price", async () => {
      ...
      expect(collection!.productVariants.items.map((i) => i.price)).toEqual([
        3799, 5374, 6900, 7489, 7896, 9299, 13435, 14374, 16994, 93120, 94920,
        108720, 109995, 129900, 139900, 219900, 229900,
      ]);
    });
  });
  ...
  describe("moveCollection", () => {
    it("moves a collection to a new parent", async () => {
      ...
      expect(positions.map((i) => i.id)).toEqual([
        pearCollection.id,
        computersCollection.id,
      ]);
    });
    ...
    it("re-evaluates Collection contents on move", async () => {
      ...
      expect(
        result.collection!.productVariants.items.map((i) => i.name),
      ).toEqual([
        "Laptop 13 inch 8GB",
        "Laptop 15 inch 8GB",
        "Laptop 13 inch 16GB",
    ...
    it("moves a 1st level collection to a new parent to check breadcrumbs", async () => {
      ...
      expect(result.moveCollection.parent!.id).toBe(
        electronicsBreadcrumbsCollection.id,
      );

      const positions = await getChildrenOf(
        electronicsBreadcrumbsCollection.id,
      );
      expect(positions.map((i) => i.id)).toEqual([
        computersBreadcrumbsCollection.id,
      ]);
    });
    ...
    it("alters the position in the current parent 1", async () => {
      ...
      expect(afterResult.map((i) => i.id)).toEqual([
        computersCollection.id,
        pearCollection.id,
      ]);
    });
    ...
    it("children correctly ordered", async () => {
      ...
      expect(
        result.collection.children?.map((c) => (c as any).position),
    ...
    async function getChildrenOf(
      ...
      const result =
        await adminClient.query<Codegen.GetCollectionsQuery>(GET_COLLECTIONS);
  ...
  describe("filters", () => {
    ...
    describe("variantName filter", () => {
      ...
      it("contains operator", async () => {
        const collection = await createVariantNameFilteredCollection(
          "contains",
          "camera",
        );
        ...
        expect(
          result.collection!.productVariants.items.map((i) => i.name),
        ).toEqual(["Instant Camera", "Camera Lens", "SLR Camera"]);
      });
      ...
      it("endsWith operator", async () => {
        ...
        expect(
          result.collection!.productVariants.items.map((i) => i.name),
        ).toEqual(["Instant Camera", "SLR Camera"]);
      });
      ...
      it("nested variantName filter", async () => {
        ...
        expect(
          parentResult.collection?.productVariants.items.map((i) => i.name),
        ).toEqual([
          "Laptop 13 inch 8GB",
          "Laptop 15 inch 8GB",
          "Laptop 13 inch 16GB",
        ...
        const child = await createVariantNameFilteredCollection(
          "contains",
          "GB",
          parent.id,
        );
        ...
        expect(
          childResult.collection?.productVariants.items.map((i) => i.name),
        ).toEqual([
          "Laptop 13 inch 8GB",
          "Laptop 15 inch 8GB",
          "Laptop 13 inch 16GB",
    ...
    describe("variantId filter", () => {
      it("contains expects variants", async () => {
        ...
        expect(
          result.collection!.productVariants.items.map((i) => i.id).sort(),
        ).toEqual(["T_1", "T_4"]);
      });
    });
    ...
    describe("re-evaluation of contents on changes", () => {
      ...
      beforeAll(async () => {
        const result =
          await adminClient.query<Codegen.GetProductsWithVariantIdsQuery>(gql`
    ...
    describe("filter inheritance", () => {
      ...
      it("grandchild collection with inheritance (root -> no inherit -> inherit", async () => {
        const { createCollection: footwearCollection } =
          await adminClient.query<
            Codegen.CreateCollectionSelectVariantsMutation,
            Codegen.CreateCollectionSelectVariantsMutationVariables
          >(CREATE_COLLECTION_SELECT_VARIANTS, {
    ...
    describe("previewCollectionVariants", () => {
      it("returns correct contents", async () => {
        ...
        expect(
          previewCollectionVariants.items.map((i) => i.name).sort(),
        ).toEqual([
          "Curvy Monitor 24 inch",
          "Curvy Monitor 27 inch",
          "Gaming PC i7-8700 240GB SSD",
  ...
  describe("productVariants list", () => {
    ...
    it("does not list disabled variants in Shop API", async () => {
      ...
      expect(
        collection!.productVariants.items.map((i) => i.id).includes("T_1"),
      ).toBe(false);
    });
    ...
    it("does not list variants of disabled products in Shop API", async () => {
      ...
      expect(
        collection!.productVariants.items.map((i) => i.id).includes("T_2"),
      ).toBe(false);
      expect(
        collection!.productVariants.items.map((i) => i.id).includes("T_3"),
      ).toBe(false);
      expect(
        collection!.productVariants.items.map((i) => i.id).includes("T_4"),
      ).toBe(false);
    });
        ...
        input: [
          {
            id: "T_2",
            translations: [
              { languageCode: LanguageCode.de, name: "Taschenrechner 15 Zoll" },
            ],
          },
        ],
      });
  ...
  describe("channel assignment & removal", () => {
    ...
    it("assign to channel", async () => {
      ...
      expect(before.items.map((i) => i.id).includes(testCollection.id)).toBe(
        false,
      );
      ...
      expect(assignCollectionsToChannel.map((c) => c.id)).toEqual([
        testCollection.id,
      ]);
      ...
      expect(after.items.map((i) => i.id).includes(testCollection.id)).toBe(
        true,
      );
    });
    ...
    it("remove from channel", async () => {
      ...
      expect(removeCollectionsFromChannel.map((c) => c.id)).toEqual([
        testCollection.id,
      ]);
...
const REMOVE_COLLECTIONS_FROM_CHANNEL = gql`
  mutation RemoveCollectionsFromChannel(
