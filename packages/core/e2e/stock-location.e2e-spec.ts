...
describe("Stock location", () => {
  ...
  it("updateStockLocation", async () => {
    ...
    expect(updateStockLocation.description).toBe(
      "Second location description updated",
    );
  });
  ...
  it("assign stock to second location", async () => {
    ...
    expect(
      updateProductVariants[0]?.stockLevels.find(
    ...
    expect(
      updateProductVariants[0]?.stockLevels.find(
        (sl) => sl.stockLocationId === secondStockLocationId,
      ),
    ).toEqual({
      stockOnHand: 50,
      stockAllocated: 0,
  ...
  it("delete second stock location and assign stock to default location", async () => {
    const { deleteStockLocation } = await adminClient.query(
      TestDeleteStockLocationDocument,
      {
        input: {
          id: secondStockLocationId,
  ...
  it("cannot delete last remaining stock location", async () => {
    ...
    expect(deleteStockLocation.message).toBe(
      "The last remaining StockLocation cannot be deleted",
    );
  ...
  describe("multi channel", () => {
    ...
    const channelGuard: ErrorResultGuard<Codegen.ChannelFragment> =
      createErrorResultGuard<Codegen.ChannelFragment>(
...
const ASSIGN_STOCK_LOCATION_TO_CHANNEL = gql`
  mutation TestAssignStockLocationToChannel(
...
const REMOVE_STOCK_LOCATIONS_FROM_CHANNEL = gql`
  mutation TestRemoveStockLocationsFromChannel(
