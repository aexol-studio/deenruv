...
describe("Draft Orders resolver", () => {
  ...
  it("create draft order", async () => {
    const { createDraftOrder } =
      await adminClient.query<Codegen.CreateDraftOrderMutation>(
        CREATE_DRAFT_ORDER,
      );
  ...
  it("custom does not see draft orders in history", async () => {
    ...
    const { activeCustomer } =
      await shopClient.query<GetActiveCustomerOrdersQuery>(
  ...
  it("applyCouponCodeToDraftOrder", async () => {
    ...
    expect(applyCouponCodeToDraftOrder.couponCodes).toEqual([
      freeOrderCouponCode,
    ]);
  ...
  it("setDraftOrderShippingMethod", async () => {
    ...
    expect(setDraftOrderShippingMethod.shippingLines[0].shippingMethod.id).toBe(
      "T_2",
    );
  });
  ...
  it("sets order as placed when payment is settled", async () => {
    ...
    expect(TestOrderPlacedStrategy.spy.mock.calls[0][0].code).toBe(
      draftOrder.code,
    );
  });
});
