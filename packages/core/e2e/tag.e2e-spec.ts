...
describe("Tag resolver", () => {
  ...
  it("tag", async () => {
    const { tag } = await adminClient.query<
      Codegen.GetTagQuery,
      Codegen.GetTagQueryVariables
    >(GET_TAG, {
      id: "T_1",
    });
  ...
  it("tags", async () => {
    const { tags } = await adminClient.query<
      Codegen.GetTagListQuery,
      Codegen.GetTagListQueryVariables
    >(GET_TAG_LIST);
