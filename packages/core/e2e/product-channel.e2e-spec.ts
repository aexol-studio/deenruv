...
describe("ChannelAware Products and ProductVariants", () => {
  ...
  const orderResultGuard: ErrorResultGuard<{ lines: Array<{ id: string }> }> =
  ...
  describe("assigning Product to Channels", () => {
    ...
    it("assigns Product to Channel and applies price factor", async () => {
      ...
      expect(
        assignProductsToChannel[0].channels.map((c) => c.id).sort(),
      ).toEqual(["T_1", "T_2"]);
      ...
      expect(product!.variants.map((v) => v.currencyCode)).toEqual([
        "GBP",
        "GBP",
        "GBP",
        "GBP",
    ...
    it("ProductVariant.channels includes all Channels from default Channel", async () => {
      ...
      expect(product?.variants[0].channels.map((c) => c.id)).toEqual([
        "T_1",
        "T_2",
      ]);
    });
    ...
    it("removes Product from Channel", async () => {
      ...
      expect(removeProductsFromChannel[0].channels.map((c) => c.id)).toEqual([
        "T_1",
      ]);
    });
    ...
    it("querying an Order with a variant that was since removed from the channel", async () => {
      ...
      const { order } = await adminClient.query(
        gql(GET_ORDER_WITHOUT_VARIANT_PRICE),
        {
  ...
  describe("assigning ProductVariant to Channels", () => {
    ...
    it("assigns ProductVariant to Channel and applies price factor", async () => {
      ...
      expect(
        assignProductVariantsToChannel[0].channels.map((c) => c.id).sort(),
      ).toEqual(["T_1", "T_3"]);
      ...
      expect(check?.variants[0].channels.map((c) => c.id).sort()).toEqual([
        "T_1",
        "T_3",
      ]);
      expect(check?.variants[1].channels.map((c) => c.id).sort()).toEqual([
        "T_1",
      ]);
    });
  ...
  describe("creating Product in sub-channel", () => {
    ...
    it("creates a Product in sub-channel", async () => {
      ...
      expect(product?.variants[0].channels.map((c) => c.id).sort()).toEqual([
        "T_1",
        "T_2",
      ]);
    });
  ...
  describe("updating channel defaultCurrencyCode", () => {
    ...
    const channelGuard: ErrorResultGuard<ChannelFragment> =
      createErrorResultGuard((input) => !!input.id);

    beforeAll(async () => {
      const { channels } = await adminClient.query(GetChannelsDocument);
      secondChannelId = channels.items.find(
        (c) => c.token === SECOND_CHANNEL_TOKEN,
      )!.id;
    });
    ...
    it("does not change prices in other currencies", async () => {
      ...
      expect(
        updateProductVariants[0]?.prices.sort((a, b) => a.price - b.price),
      ).toEqual([
      ...
      const { productVariants: after } = await adminClient.query(
        GetProductVariantListDocument,
        {},
      );
