...
describe("Default search plugin", () => {
  ...
  beforeAll(async () => {
    await server.init({
      ...
      productsCsvPath: path.join(
        __dirname,
        "fixtures/e2e-products-default-search.csv",
      ),
      customerCount: 1,
    });
  ...
  function testProductsShop(
    input: SearchProductsShopQueryVariablesExt["input"],
  ) {
    return shopClient.query<
      SearchProductsShopQuery,
      SearchProductsShopQueryVariablesExt
    >(SEARCH_PRODUCTS_SHOP, { input });
  }
  ...
  async function testSortingNoGrouping(
    ...
    expect(result.search.items.map((i) => i.productVariantName)).toEqual(
      expected,
    );
  }
  ...
  async function testMatchFacetValueFiltersOrWithAnd(
    testProducts: TestProducts,
  ) {
    const result = await testProducts({
      groupByProduct: true,
  ...
  async function testMatchFacetValueFiltersWithFacetIdsOr(
    testProducts: TestProducts,
  ) {
    const result = await testProducts({
  ...
  async function testMatchFacetValueFiltersWithFacetIdsAnd(
    testProducts: TestProducts,
  ) {
    const result = await testProducts({
      facetValueIds: ["T_1"],
  ...
  describe("shop api", () => {
    ...
    it("matches by facetId with OR operator", () =>
      testMatchFacetIdsOr(testProductsShop));

    it("matches by FacetValueFilters AND", () =>
      testMatchFacetValueFiltersAnd(testProductsShop));

    it("matches by FacetValueFilters OR", () =>
      testMatchFacetValueFiltersOr(testProductsShop));
    ...
    it("matches by collectionId", () =>
      testMatchCollectionId(testProductsShop));

    it("matches by collectionSlug", () =>
      testMatchCollectionSlug(testProductsShop));
    ...
    // https://github.com/aexol-studio/deenruv/issues/1236
          ...
          values: [
            {
              code: "massive",
              translations: [
                { languageCode: LanguageCode.en, name: "massive" },
              ],
            },
          ],
        },
    ...
    it("encodes the productId and productVariantId", async () => {
      ...
      expect(
        pick(result.search.items[0], ["productId", "productVariantId"]),
      ).toEqual({
        productId: "T_1",
    ...
    it("sort name with grouping", () =>
      testSortingWithGrouping(testProductsShop, "name"));

    it("sort price with grouping", () =>
      testSortingWithGrouping(testProductsShop, "price"));

    it("sort name without grouping", () =>
      testSortingNoGrouping(testProductsShop, "name"));

    it("sort price without grouping", () =>
      testSortingNoGrouping(testProductsShop, "price"));
    ...
    it("omits results for disabled ProductVariants", async () => {
      ...
      expect(result.search.items.map((i) => i.productVariantId)).toEqual([
        "T_1",
        "T_2",
        "T_4",
      ]);
  ...
  describe("admin api", () => {
    ...
    it("matches by facetId with OR operator", () =>
      testMatchFacetIdsOr(testProductsAdmin));

    it("matches by FacetValueFilters AND", () =>
      testMatchFacetValueFiltersAnd(testProductsAdmin));

    it("matches by FacetValueFilters OR", () =>
      testMatchFacetValueFiltersOr(testProductsAdmin));
    ...
    it("matches by collectionId", () =>
      testMatchCollectionId(testProductsAdmin));

    it("matches by collectionSlug", () =>
      testMatchCollectionSlug(testProductsAdmin));
    ...
    it("sort name with grouping", () =>
      testSortingWithGrouping(testProductsAdmin, "name"));

    it("sort price with grouping", () =>
      testSortingWithGrouping(testProductsAdmin, "price"));

    it("sort name without grouping", () =>
      testSortingNoGrouping(testProductsAdmin, "name"));

    it("sort price without grouping", () =>
      testSortingNoGrouping(testProductsAdmin, "price"));

    describe("updating the index", () => {
      it("updates index when ProductVariants are changed", async () => {
        await awaitRunningJobs(adminClient);
        const { search } = await testProductsAdmin({
          term: "drive",
          groupByProduct: false,
        });
      ...
      it("updates index when a Product is deleted", async () => {
        ...
        expect(search.items.map((i) => i.productId)).toEqual([
          "T_2",
          "T_3",
          "T_4",
          "T_5",
        ...
        expect(search2.items.map((i) => i.productId)).toEqual([
          "T_2",
          "T_3",
          "T_4",
          "T_6",
      ...
      it("updates index when a Collection is changed", async () => {
        ...
        await awaitRunningJobs(adminClient);
        const result1 = await testProductsAdmin({
          collectionId: "T_2",
          groupByProduct: true,
        });
        ...
        const result2 = await testProductsAdmin({
          collectionSlug: "plants",
          groupByProduct: true,
        });
      ...
      describe("asset changes", () => {
        ...
        it("updates index when asset focalPoint is changed", async () => {
          ...
          expect(search2.items[0].productAsset!.focalPoint).toEqual({
            x: 0.42,
            y: 0.42,
          });
        });
      ...
      it("returns enabled field when not grouped", async () => {
        ...
        expect(
          result.search.items.map(pick(["productVariantId", "enabled"])),
        ).toEqual([
      ...
      // https://github.com/aexol-studio/deenruv/issues/295
      ...
      // https://github.com/aexol-studio/deenruv/issues/1482
          ...
          input: [
            {
              ...
              translations: [
                {
                  languageCode: LanguageCode.en,
                  name: "Very long description variant",
                },
              ],
            },
          ],
    ...
    // https://github.com/aexol-studio/deenruv/issues/609
    ...
    describe("channel handling", () => {
      ...
      it("assign product variant to channel", async () => {
        ...
        expect(searchGrouped.items.map((i) => i.productId)).toEqual([
          "T_1",
          "T_3",
          "T_4",
        ]);

        const { search: searchUngrouped } = await testProductsAdmin({
          groupByProduct: false,
        });
      ...
      // https://github.com/aexol-studio/deenruv/issues/896
    ...
    describe("multiple language handling", () => {
      ...
      describe("search products", () => {
        ...
        it("fallbacks to default language en", async () => {
          ...
          const laptopVariants = search.items.filter(
            (i) => i.productId === "T_1",
          );
          expect(laptopVariants.length).toEqual(4);

          const laptopVariantT1 = laptopVariants.find(
            (i) => i.productVariantId === "T_1",
          );
          expect(laptopVariantT1?.productVariantName).toEqual(
            "Laptop variant T_1 en",
          );
          ...
          const laptopVariantT2 = laptopVariants.find(
            (i) => i.productVariantId === "T_2",
          );
          expect(laptopVariantT2?.productVariantName).toEqual(
            "Laptop variant T_2 en",
          );
          ...
          const laptopVariantT3 = laptopVariants.find(
            (i) => i.productVariantId === "T_3",
          );
          expect(laptopVariantT3?.productVariantName).toEqual(
            "Laptop variant T_3 en",
          );
          ...
          const laptopVariantT4 = laptopVariants.find(
            (i) => i.productVariantId === "T_4",
          );
          expect(laptopVariantT4?.productVariantName).toEqual(
            "Laptop variant T_4 en",
          );
      ...
      describe("search products grouped by product and sorted by name ASC", () => {
        ...
        // https://github.com/aexol-studio/deenruv/issues/1752
        // https://github.com/aexol-studio/deenruv/issues/1746
        it("fallbacks to default language en", async () => {
          const { search } = await searchInLanguage(LanguageCode.af);
    ...
    // https://github.com/aexol-studio/deenruv/issues/1789
