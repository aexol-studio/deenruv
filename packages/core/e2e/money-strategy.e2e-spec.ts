...
const orderGuard: ErrorResultGuard<CodegenShop.UpdatedOrderFragment> =
...
describe("Custom MoneyStrategy", () => {
  ...
  beforeAll(async () => {
    await server.init({
      ...
      productsCsvPath: path.join(
        __dirname,
        "fixtures/e2e-products-money-handling.csv",
      ),
      customerCount: 1,
    });
  ...
  // https://github.com/deenruv-ecommerce/deenruv/issues/838
  ...
  // https://github.com/deenruv-ecommerce/deenruv/issues/1835
  // 31 * 1.2 = 37.2
