...
describe("Default search plugin with UUIDs", () => {
  ...
  beforeAll(async () => {
    ...
    const { facets } = await adminClient.query<
      GetFacetListQuery,
      GetFacetListQueryVariables
    >(GET_FACET_LIST, {
      options: {
    ...
    furnitureFacetValue = facets.items[0].values.find(
      (v) => v.code === "furniture",
    )!;
    photoFacetValue = facets.items[0].values.find((v) => v.code === "photo")!;
    electronicsFacetValue = facets.items[0].values.find(
      (v) => v.code === "electronics",
    )!;
  }, TEST_SETUP_TIMEOUT_MS);
