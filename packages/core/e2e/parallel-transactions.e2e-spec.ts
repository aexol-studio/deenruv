...
describe("Parallel transactions", () => {
  ...
  it("does not fail on many concurrent, slow transactions", async () => {
    ...
    expect(result).toEqual(
      Array.from({ length: CONCURRENCY_LIMIT }).map(() => ({
        slowMutation: true,
      })),
    );
  }, 100000);
  ...
  it("does not deadlock on concurrent creating ProductVariants", async () => {
    ...
    const sizes = Array.from({ length: CONCURRENCY_LIMIT }).map(
      (i) => `size-${i as string}`,
    );
          ...
          input: [
            {
              ...
              optionIds: [option.id],
              translations: [
                { languageCode: LanguageCode.en, name: `Variant ${i}` },
              ],
              price: 1000,
              taxCategoryId: "T_1",
