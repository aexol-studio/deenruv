...
describe("ErrorHandlerStrategy", () => {
  ...
  it("invokes the server handler", async () => {
    ...
    expect(
      TestErrorHandlerStrategy.serverErrorSpy.mock.calls[0][0],
    ).toBeInstanceOf(Error);
    expect(
      TestErrorHandlerStrategy.serverErrorSpy.mock.calls[0][0].message,
    ).toBe("server error");
    expect(
      TestErrorHandlerStrategy.serverErrorSpy.mock.calls[0][1].host,
    ).toBeDefined();
  });
  ...
  it("invokes the worker handler", async () => {
    ...
    expect(
      TestErrorHandlerStrategy.workerErrorSpy.mock.calls[0][0],
    ).toBeInstanceOf(Error);
    expect(
      TestErrorHandlerStrategy.workerErrorSpy.mock.calls[0][0].message,
    ).toBe("worker error");
    expect(
      TestErrorHandlerStrategy.workerErrorSpy.mock.calls[0][1].job,
    ).toHaveProperty("queueName", "test-queue");
  });
});
