...
describe("Customer resolver", () => {
  ...
  const customerErrorGuard: ErrorResultGuard<Codegen.CustomerFragment> =
  ...
  describe("addresses", () => {
    ...
    it("customer query returns addresses", async () => {
      ...
      firstCustomerAddressIds = result
        .customer!.addresses!.map((a) => a.id)
        .sort();
    });
  ...
  describe("creation", () => {
    it("triggers verification event if no password supplied", async () => {
      ...
      expect(
        sendEmailFn.mock.calls[0][0] instanceof AccountRegistrationEvent,
      ).toBe(true);
      expect(sendEmailFn.mock.calls[0][0].user.identifier).toBe(
        "test1@test.com",
      );
    });
    ...
    it("return error result when using an existing, non-deleted emailAddress", async () => {
      ...
      expect(createCustomer.message).toBe(
        "The email address is not available.",
      );
      expect(createCustomer.errorCode).toBe(
  ...
  describe("update", () => {
    ...
    // https://github.com/aexol-studio/deenruv/issues/1071
    ...
    // https://github.com/aexol-studio/deenruv/issues/2449
  ...
  describe("deletion", () => {
    ...
    // https://github.com/aexol-studio/deenruv/issues/1960
    it("delete a guest Customer", async () => {
      const orderErrorGuard: ErrorResultGuard<ActiveOrderCustomerFragment> =
        createErrorResultGuard((input) => !!input.lines);
