...
const customCollectionDuplicator = new EntityDuplicator({
  ...
  description: [
    { languageCode: LanguageCode.en, value: "Custom Collection Duplicator" },
  ],
  args: {
    throwError: {
      type: "boolean",
...
describe("Duplicating entities", () => {
  ...
  const duplicateEntityGuard: ErrorResultGuard<{ newEntityId: string }> =
    createErrorResultGuard((result) => !!result.newEntityId);
  ...
  it("get entity duplicators", async () => {
    const { entityDuplicators } =
      await adminClient.query<Codegen.GetEntityDuplicatorsQuery>(
        GET_ENTITY_DUPLICATORS,
      );

    expect(
      entityDuplicators.find((d) => d.code === "custom-collection-duplicator"),
    ).toEqual({
      args: [
        {
          defaultValue: false,
  ...
  describe("default entity duplicators", () => {
    describe("Product duplicator", () => {
      let originalProduct: NonNullable<
      ...
      it("facet values are duplicated", async () => {
        ...
        expect(product?.facetValues.map((fv) => fv.name).sort()).toEqual([
          "computers",
          "electronics",
        ]);
      });
      ...
      it("variant assets are preserved", async () => {
        ...
        expect(
          product?.variants.find((v) => v.name === originalFirstVariant.name)
            ?.assets,
        ).toEqual(originalFirstVariant.assets);
      ...
      it("variant prices are duplicated", async () => {
        ...
        const variant = product?.variants.find((v) =>
          v.sku.startsWith(originalFirstVariant.sku),
        );
      ...
      it("variant prices are duplicated on a channel specific basis", async () => {
        ...
        const { product: productWithVariantChannelNull } =
          await adminClient.query<
            Codegen.GetProductWithVariantsQuery,
            Codegen.GetProductWithVariantsQueryVariables
          >(GET_PRODUCT_WITH_VARIANTS, {
        ...
        const productVariantChannelNull =
          productWithVariantChannelNull!.variants.find((v) =>
        ...
        const { duplicateEntity: duplicateEntitySecondChannel } =
          await adminClient.query<
            Codegen.DuplicateEntityMutation,
            Codegen.DuplicateEntityMutationVariables
          >(DUPLICATE_ENTITY, {
        ...
        const productVariantChannel2 =
          productWithVariantChannel2!.variants.find((v) =>
            v.sku.startsWith(productVariant.sku),
          );

        expect(productVariantChannel2!.price).toEqual(
          productVariant.price + 150,
        );
      });
    ...
    describe("Promotion duplicator", () => {
      ...
      const promotionGuard: ErrorResultGuard<{ id: string }> =
        createErrorResultGuard((result) => !!result.id);
      ...
      it("properties are duplicated", async () => {
        ...
        expect(promotion?.perCustomerUsageLimit).toBe(
          testPromotion.perCustomerUsageLimit,
        );
