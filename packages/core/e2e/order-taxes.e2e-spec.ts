...
class TestTaxZoneStrategy implements TaxZoneStrategy {
  ...
  async determineTaxZone(
    ...
    const zoneForCountryCode = await this.getZoneForCountryCode(
      ctx,
      countryCode,
    );
...
describe("Order taxes", () => {
  ...
  beforeAll(async () => {
    await server.init({
      ...
      productsCsvPath: path.join(
        __dirname,
        "fixtures/e2e-products-order-taxes.csv",
      ),
      customerCount: 2,
    });
  ...
  describe("Channel.pricesIncludeTax = false", () => {
    ...
    it("prices are correct", async () => {
      ...
      const { activeOrder } =
        await shopClient.query<CodegenShop.GetActiveOrderWithPriceDataQuery>(
          GET_ACTIVE_ORDER_WITH_PRICE_DATA,
        );
  ...
  describe("Channel.pricesIncludeTax = true", () => {
    ...
    it("re-calculates OrderLine prices when shippingAddress causes activeTaxZone change", async () => {
      ...
      const taxRatesAsia = taxRates.items.filter((tr) =>
        tr.name.includes("Asia"),
      );
  ...
  it("taxSummary works", async () => {
    ...
    expect(taxSummaryBaseTotal + taxSummaryTaxTotal).toBe(
