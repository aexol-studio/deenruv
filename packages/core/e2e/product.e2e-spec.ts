...
describe("Product resolver", () => {
  ...
  const removeOptionGuard: ErrorResultGuard<Codegen.ProductWithOptionsFragment> =
    createErrorResultGuard((input) => !!input.optionGroups);

  const updateChannelGuard: ErrorResultGuard<Codegen.ChannelFragment> =
  ...
  describe("product query", () => {
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/820
  ...
  describe("productVariants list query", () => {
    it("returns list", async () => {
      ...
      expect(
        productVariants.items.map((i) =>
          pick(i, ["id", "name", "price", "priceWithTax", "sku"]),
        ),
      ).toEqual([
        {
          id: "T_34",
  ...
  describe("product mutation", () => {
    ...
    it("createProduct creates a new Product", async () => {
      ...
      expect(
        result.createProduct.translations.map((t) => t.description).sort(),
      ).toEqual(["A baked potato", "Eine baked Erdapfel"]);
    ...
    it("updateProduct updates a Product", async () => {
      ...
      expect(
        result.updateProduct.translations.map((t) => t.description).sort(),
      ).toEqual(["A blob of mashed potato", "Eine blob von gemashed Erdapfel"]);
    });
    ...
    it("updateProduct accepts partial input", async () => {
      ...
      expect(
        result.updateProduct.translations.find(
          (t) => t.languageCode === LanguageCode.de,
        )!.name,
    ...
    it("updateProduct updates assets", async () => {
      ...
      expect(result.updateProduct.assets.map((a) => a.id)).toEqual([
        "T_1",
        "T_2",
      ]);
    });
    ...
    it("addOptionGroupToProduct adds an option group", async () => {
      const optionGroup = await createOptionGroup("Quark-type", [
        "Charm",
        "Strange",
      ]);
      const result = await adminClient.query<
      ...
      expect(result.addOptionGroupToProduct.optionGroups[0].id).toBe(
        optionGroup.id,
      );
    ...
    it("removeOptionGroupFromProduct return error result if the optionGroup is being used by variants", async () => {
      ...
      expect(removeOptionGroupFromProduct.errorCode).toBe(
        ErrorCode.PRODUCT_OPTION_IN_USE_ERROR,
      );
      expect(removeOptionGroupFromProduct.optionGroupCode).toBe(
        "curvy-monitor-monitor-size",
      );
    ...
    describe("variants", () => {
      ...
      let optionGroup2: NonNullable<
        Codegen.GetOptionGroupQuery["productOptionGroup"]
      >;
      let optionGroup3: NonNullable<
        Codegen.GetOptionGroupQuery["productOptionGroup"]
      >;

      beforeAll(async () => {
        optionGroup2 = await createOptionGroup("group-2", [
          "group2-option-1",
          "group2-option-2",
        ]);
        optionGroup3 = await createOptionGroup("group-3", [
          "group3-option-1",
          "group3-option-2",
        ]);
        await adminClient.query<
            ...
            input: [
              {
                ...
                optionIds: [],
                translations: [
                  { languageCode: LanguageCode.en, name: "Variant 1" },
                ],
              },
            ],
          });
            ...
            input: [
              {
                ...
                sku: "PV1",
                optionIds: [
                  optionGroup2.options[0].id,
                  optionGroup2.options[1].id,
                ],
                translations: [
                  { languageCode: LanguageCode.en, name: "Variant 1" },
                ],
      ...
      it("createProductVariants works", async () => {
        ...
        expect(
          createProductVariants[0]!.options.map(pick(["id"])),
        ).toContainEqual({
          ...
          input: [
            {
              ...
              translations: [
                { languageCode: LanguageCode.en, name: "Variant 2" },
              ],
            },
            {
              productId: newProduct.id,
              ...
              optionIds: [
                optionGroup2.options[1].id,
                optionGroup3.options[1].id,
              ],
              translations: [
                { languageCode: LanguageCode.en, name: "Variant 3" },
              ],
            },
          ],
        });
        const variant2 = createProductVariants.find(
          (v) => v!.name === "Variant 2",
        )!;
        const variant3 = createProductVariants.find(
          (v) => v!.name === "Variant 3",
        )!;
        expect(variant2.options.map(pick(["id"]))).toContainEqual({
          id: optionGroup2.options[1].id,
        });
        expect(variant2.options.map(pick(["id"]))).toContainEqual({
          id: optionGroup3.options[0].id,
        });
        expect(variant3.options.map(pick(["id"]))).toContainEqual({
          id: optionGroup2.options[1].id,
        });
        expect(variant3.options.map(pick(["id"]))).toContainEqual({
      ...
      // https://github.com/deenruv-ecommerce/deenruv/issues/1101
      it("after update, the updatedAt should be modified", async () => {
        ...
        const updatedVariant = updateProductVariants.find(
      ...
      describe("adding options to existing variants", () => {
        ...
        it("addOptionGroupToProduct and then update existing ProductVariant with a new option", async () => {
          ...
          expect(result.addOptionGroupToProduct.optionGroups[2].id).toBe(
            optionGroup4.id,
          );
      ...
      it("deleteProductVariant", async () => {
        ...
        const sortedVariantIds = result1
        ...
        expect(result2.product!.variants.map((v) => v.id).sort()).toEqual([
          "T_36",
          "T_37",
        ]);

        deletedVariant = result1.product!.variants.find(
          (v) => v.id === "T_35",
        )!;
      });

      /** Testing https://github.com/deenruv-ecommerce/deenruv/issues/412 **/
      it("createProductVariants ignores deleted variants when checking for existing combinations", async () => {
          ...
          input: [
            {
              ...
              sku: "RE1",
              optionIds: [
                deletedVariant.options[0].id,
                deletedVariant.options[1].id,
              ],
              translations: [
                { languageCode: LanguageCode.en, name: "Re-created Variant" },
              ],
            },
          ],
        });
        ...
        expect(
          createProductVariants[0]!.options.map((o) => o.code).sort(),
        ).toEqual(deletedVariant.options.map((o) => o.code).sort());
      });

      // https://github.com/deenruv-ecommerce/deenruv/issues/980
          ...
          input: [
            {
              ...
              optionIds: [],
              translations: [
                { languageCode: LanguageCode.de, name: "Ananas Klein" },
              ],
            },
          ],
        });
      ...
      // https://github.com/deenruv-ecommerce/deenruv/issues/1631
            ...
            input: [
              {
                ...
                optionIds: [],
                translations: [
                  { languageCode: LanguageCode.en, name: "Bottle" },
                ],
              },
            ],
          });
  ...
  describe("deletion", () => {
    ...
    let productToDelete: NonNullable<
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/1096
    ...
    it("deleted product omitted from list", async () => {
      const result =
        await adminClient.query<Codegen.GetProductListQuery>(GET_PRODUCT_LIST);

      expect(result.products.items.length).toBe(allProducts.length - 1);
      expect(
        result.products.items.map((c) => c.id).includes(productToDelete.id),
      ).toBe(false);
    });
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/558
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/1505
    ...
    // https://github.com/deenruv-ecommerce/deenruv/issues/800
...
export const GET_PRODUCT_WITH_VARIANT_LIST = gql`
  query GetProductWithVariantList(
    $id: ID
