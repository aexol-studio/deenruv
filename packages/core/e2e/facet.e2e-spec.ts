...
describe("Facet resolver", () => {
  const { server, adminClient, shopClient } =
    createTestEnvironment(testConfig());
  ...
  it("updateFacet", async () => {
    ...
    >(UPDATE_FACET, {
      input: {
        ...
        translations: [
          { languageCode: LanguageCode.en, name: "Speaker Category" },
        ],
        isPrivate: true,
      },
    });
  ...
  it("createFacetValues", async () => {
      ...
      input: [
        {
          ...
          code: "pc",
          translations: [
            { languageCode: LanguageCode.en, name: "PC Speakers" },
          ],
        },
        {
          facetId: speakerTypeFacet.id,
          code: "hi-fi",
          translations: [
            { languageCode: LanguageCode.en, name: "Hi Fi Speakers" },
          ],
        },
      ],
    });
    ...
    expect(
      pick(createFacetValues.find((fv) => fv.code === "pc")!, [
        "code",
        "facet",
        "name",
      ]),
    ...
    expect(
      pick(createFacetValues.find((fv) => fv.code === "hi-fi")!, [
        "code",
        "facet",
        "name",
      ]),
  ...
  it("updateFacetValues", async () => {
    const portableFacetValue = speakerTypeFacet.values.find(
      (v) => v.code === "portable",
    )!;
    const result = await adminClient.query<
  ...
  it("facets", async () => {
    const result =
      await adminClient.query<Codegen.GetFacetListQuery>(GET_FACET_LIST);
  ...
  it("facets by shop-api", async () => {
    const result = await shopClient.query<Codegen.GetFacetListQuery>(
      GET_FACET_LIST_SIMPLE,
    );
  ...
  it("product.facetValues resolver omits private facets in shop-api", async () => {
    ...
    expect(
      product?.facetValues.map((v) => v.id).includes(publicFacetValue.id),
    ).toBe(true);
    expect(
      product?.facetValues.map((v) => v.id).includes(privateFacetValue.id),
    ).toBe(false);
  });
  ...
  it("productVariant.facetValues resolver omits private facets in shop-api", async () => {
    ...
    expect(
      productVariant1?.facetValues
        .map((v) => v.id)
        .includes(publicFacetValue.id),
    ).toBe(true);
    expect(
      productVariant1?.facetValues
        .map((v) => v.id)
        .includes(privateFacetValue.id),
    ).toBe(false);
  });
  ...
  describe("deletion", () => {
    ...
    beforeAll(async () => {
      ...
      const result1 =
        await adminClient.query<Codegen.GetProductListWithVariantsQuery>(
          GET_PRODUCTS_LIST_WITH_VARIANTS,
        );
      products = result1.products.items;
      const pcFacetValue = speakerTypeFacet.values.find(
        (v) => v.code === "pc",
      )!;
      const hifiFacetValue = speakerTypeFacet.values.find(
        (v) => v.code === "hi-fi",
      )!;
    ...
    it("deleteFacetValues deletes unused facetValue", async () => {
      const facetValueToDelete = speakerTypeFacet.values.find(
        (v) => v.code === "compact",
      )!;
      ...
      expect(result1.deleteFacetValues).toEqual([
        {
          ...
          message:
            'The FacetValue "pc" is assigned to 1 Product, 1 ProductVariant',
        },
      ]);

      expect(
        result2.facet!.values.find((v) => v.id === facetValueToDelete.id),
      ).toBeDefined();
    });
    ...
    it("deleteFacet that is in use returns NOT_DELETED", async () => {
      ...
      expect(result1.deleteFacet).toEqual({
        result: DeletionResult.NOT_DELETED,
        message:
          'The Facet "speaker-type" includes FacetValues which are assigned to 1 Product',
      });
    ...
    it("deleteFacet that is in use can be force deleted", async () => {
      ...
      expect(result1.deleteFacet).toEqual({
        result: DeletionResult.DELETED,
        message:
          "The Facet was deleted and its FacetValues were removed from 1 Product",
      });
  ...
  describe("channels", () => {
    ...
    it("create Facet in channel", async () => {
      ...
      >(CREATE_FACET, {
        input: {
          ...
          code: "channel-facet",
          translations: [
            { languageCode: LanguageCode.en, name: "Channel Facet" },
          ],
          values: [
            {
              code: "channel-value-1",
              translations: [
                { languageCode: LanguageCode.en, name: "Channel Value 1" },
              ],
            },
            {
              code: "channel-value-2",
              translations: [
                { languageCode: LanguageCode.en, name: "Channel Value 2" },
              ],
            },
          ],
        },
    ...
    it("Product.facetValues in channel", async () => {
      ...
      >(UPDATE_PRODUCT, {
        input: {
          id: "T_1",
          facetValueIds: [
            brandFacet.values[0].id,
            ...createdFacet.values.map((v) => v.id),
          ],
        },
      });
      await adminClient.query<
    ...
    it("ProductVariant.facetValues in channel", async () => {
      ...
      expect(
        product?.variants[0].facetValues.map((fv) => fv.code).sort(),
      ).toEqual(["channel-value-1", "channel-value-2"]);
    });
    ...
    it("updating Product facetValuesIds in channel only affects that channel", async () => {
      ...
      expect(productC2?.facetValues.map((fv) => fv.code)).toEqual([
    ...
    it("updating ProductVariant facetValuesIds in channel only affects that channel", async () => {
      ...
      expect(
        productC2?.variants
          .find((v) => v.id === "T_1")
          ?.facetValues.map((fv) => fv.code),
      ).toEqual([createdFacet.values[0].code]);
      ...
      expect(
        productCD?.variants
          .find((v) => v.id === "T_1")
          ?.facetValues.map((fv) => fv.code),
      ).toEqual([brandFacet.values[0].code, createdFacet.values[0].code]);
    });
          ...
          input: [
            {
              ...
              code: "channel-brand",
              translations: [
                { languageCode: LanguageCode.en, name: "Channel Brand" },
              ],
            },
          ],
        });
    ...
    it("removing from channel with error", async () => {
      adminClient.setChannelToken(SECOND_CHANNEL_TOKEN);
      const { facets: before } =
        await adminClient.query<Codegen.GetFacetListSimpleQuery>(
          GET_FACET_LIST_SIMPLE,
        );
    ...
    function createFacetWithCode(code: string) {
      ...
      >(CREATE_FACET, {
        input: {
          ...
          code,
          translations: [
            { languageCode: LanguageCode.en, name: `Test Facet (${code})` },
          ],
          values: [],
        },
      });
