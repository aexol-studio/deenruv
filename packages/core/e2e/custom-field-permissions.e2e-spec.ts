...
import { createTestEnvironment, SimpleGraphQLClient } from "@deenruv/testing";
...
describe("Custom field permissions", () => {
  ...
  it("readProductUpdateProductAdmin can read public and updateProduct custom fields", async () => {
    await adminClient.asUserWithCredentials(
      readProductUpdateProductAdmin.emailAddress,
      "test",
    );

    const { product } = await adminClient.query(
      GET_PRODUCT_WITH_CUSTOM_FIELDS,
      {
        id: "T_1",
      },
    );
  ...
  it("readProductUpdateCustomerAdmin can read public and updateCustomer custom fields", async () => {
    await adminClient.asUserWithCredentials(
      readProductUpdateCustomerAdmin.emailAddress,
      "test",
    );
    ...
    >(UPDATE_PRODUCT, {
      input: {
        ...
        customFields: {
          ...
          updateProductOrCustomerField:
            "new updateProductOrCustomerField Value",
  ...
  it("readProductUpdateProductAdmin can update updateProduct custom field", async () => {
    ...
    expect(product.customFields.updateProductField).toBe(
      "new updateProductField Value 2",
    );
  });
  ...
  describe("Shop API", () => {
    ...
    it("all public fields are accessible in Shop API regardless of permissions", async () => {
      ...
      const { product } = await shopClient.query(
        GET_PRODUCT_WITH_PUBLIC_CUSTOM_FIELDS,
        {
          id: "T_1",
        },
      );
