...
describe("Product prices", () => {
  const { server, adminClient, shopClient } = createTestEnvironment(
    mergeConfig(
      { ...testConfig() },
      {
        catalogOptions: {
          productVariantPriceUpdateStrategy:
            new TestProductVariantPriceUpdateStrategy(),
        },
      },
    ),
  );
  ...
  const createChannelResultGuard: ErrorResultGuard<{ id: string }> =
  ...
  it("updates ProductVariant with multiple prices", async () => {
    ...
    expect(
      product?.variants[0]?.prices.sort((a, b) => a.price - b.price),
    ).toEqual([
      { currencyCode: CurrencyCode.GBP, price: 900 },
      { currencyCode: CurrencyCode.EUR, price: 1100 },
      { currencyCode: CurrencyCode.USD, price: 1200 },
  ...
  describe("ProductVariantPriceUpdateStrategy", () => {
    ...
    beforeAll(async () => {
      const { createChannel: channel2Result } = await adminClient.query(
        CreateChannelDocument,
        {
          input: {
            code: "second-channel",
      ...
      const { createChannel: channel3Result } = await adminClient.query(
        CreateChannelDocument,
        {
          input: {
            code: "third-channel",
    ...
    it("syncing prices in other channels", async () => {
      const { product: productChannel3 } = await adminClient.query(
        GetProductWithVariantsDocument,
        {
          id: multiPriceProduct.id,
        },
      ...
      const { product: productChannel2 } = await adminClient.query(
        GetProductWithVariantsDocument,
        {
          id: multiPriceProduct.id,
        },
