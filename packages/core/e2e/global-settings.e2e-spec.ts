...
describe("GlobalSettings resolver", () => {
  ...
  const globalSettingsGuard: ErrorResultGuard<GlobalSettingsFragment> =
  ...
  beforeAll(async () => {
    ...
    const result =
      await adminClient.query<GetGlobalSettingsQuery>(GET_GLOBAL_SETTINGS);
  ...
  describe("globalSettings query", () => {
    ...
    it("includes customFieldConfig", () => {
      expect(globalSettings.serverConfig.customFieldConfig.Customer).toEqual([
        { name: "age" },
      ]);
    });

    it("includes non-internal permission definitions", () => {
      const permissionNames = globalSettings.serverConfig.permissions.map(
        (p) => p.name,
      );
  ...
  describe("update", () => {
    ...
    it("successful update", async () => {
      ...
      expect(updateGlobalSettings.availableLanguages).toEqual([
        LanguageCode.en,
        LanguageCode.zh,
      ]);
