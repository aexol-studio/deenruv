...
class TestOrderPlacedStrategy extends DefaultOrderPlacedStrategy {
  shouldSetAsPlaced(
    ...
    if ((order.customFields as any).test1557) {
      // This branch is used in testing https://github.com/aexol-studio/deenruv/issues/1557
...
describe("Stock control", () => {
  ...
  beforeAll(async () => {
    await server.init({
      ...
      productsCsvPath: path.join(
        __dirname,
        "fixtures/e2e-products-stock-control.csv",
      ),
      customerCount: 3,
    });
  ...
  describe("stock adjustments", () => {
    ...
    it("increasing stockOnHand creates a StockMovement with correct quantity", async () => {
      ...
      expect(updateProductVariants[0]!.stockMovements.items[0].quantity).toBe(
        5,
      );
    });
    ...
    it("decreasing stockOnHand creates a StockMovement with correct quantity", async () => {
      ...
      expect(updateProductVariants[0]!.stockMovements.items[1].quantity).toBe(
        -2,
      );
    });
  ...
  describe("sales", () => {
    ...
    it("creates an Allocation when order completed", async () => {
      ...
      expect(variant1.stockMovements.items[1].type).toBe(
    ...
    it("creates a Release on cancelling an allocated order item and updates stockAllocated", async () => {
      ...
      expect(variant2.stockMovements.items[2].type).toBe(
    ...
    it("creates a Sale on Fulfillment creation", async () => {
      ...
      expect(variant1.stockMovements.items[2].type).toBe(
    ...
    it("creates Cancellations when cancelling items which are part of a Fulfillment", async () => {
      ...
      expect(variant1.stockMovements.items[3].type).toBe(
    ...
    it("creates Cancellations & adjusts stock when cancelling a Fulfillment", async () => {
      ...
      expect(
        trackedVariant4.stockMovements.items.map(pick(["quantity", "type"])),
      ).toEqual([
  ...
  describe("saleable stock level", () => {
    ...
    it("returns InsufficientStockError when tracking inventory & adding too many at once", async () => {
      ...
      expect((addItemToOrder as any).order.lines[0].productVariant.id).toBe(
        variantId,
      );
    ...
    it("returns InsufficientStockError for positive threshold", async () => {
      ...
      expect((addItemToOrder as any).order.lines[2].productVariant.id).toBe(
        variantId,
      );
    ...
    it("addFulfillmentToOrder returns ErrorResult when insufficient stock on hand", async () => {
      ...
      expect(addFulfillmentToOrder.errorCode).toBe(
        AdminErrorCode.INSUFFICIENT_STOCK_ON_HAND_ERROR,
      );
    ...
    describe("edge cases", () => {
      ...
      // https://github.com/aexol-studio/deenruv/issues/691
      ...
      // https://github.com/aexol-studio/deenruv/issues/1273
      ...
      // https://github.com/aexol-studio/deenruv/issues/1557
  ...
  // https://github.com/aexol-studio/deenruv/issues/1028
  ...
  // https://github.com/aexol-studio/deenruv/issues/1738
  describe("going out of stock after being added to order", () => {
    ...
    it("prevents checkout if no saleable stock", async () => {
      ...
      await proceedToArrangingPayment(shopClient);
      const result1 = await addPaymentToOrder(
        shopClient,
        testSuccessfulPaymentMethod,
      );
