  ...
  implements InspectableJobQueueStrategy
{
  ...
  async add<Data extends JobData<Data> = object>(
    ...
    const newRecord = this.toRecord(
      job,
      constrainedData,
      this.setRetries(job.queueName, job),
    );
  ...
  async next(queueName: string): Promise<Job | undefined> {
    ...
    const isSQLite =
      ...
      connectionType === "sqljs" ||
      connectionType === "better-sqlite3";
  ...
  private async getNextAndSetAsRunning(
    ...
    const qb = manager
      ...
      .andWhere(
        new Brackets((qb1) => {
          qb1
            .where("record.state = :pending", {
              pending: JobState.PENDING,
            })
            .orWhere("record.state = :retrying", {
    ...
    if (record) {
      ...
      if (
        ...
        if (msSinceLastFailure < backOffDelayMs) {
          return await this.getNextAndSetAsRunning(
            manager,
            queueName,
            setLock,
            [...waitingJobIds, record.id],
          );
        }
      }
      job.start();
  ...
  async findOne(id: ID): Promise<Job | undefined> {
    ...
    const record = await this.rawConnection
      .getRepository(JobRecord)
      .findOne({ where: { id } });
    if (record) {
      return this.fromRecord(record);
  ...
  async removeSettledJobs(queueNames: string[] = [], olderThan?: Date) {
    ...
    const toDelete = await this.rawConnection
      .getRepository(JobRecord)
      .find({ where: findOptions });
    const deleteCount = await this.rawConnection
      .getRepository(JobRecord)
