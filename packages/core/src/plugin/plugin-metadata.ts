...
export function getPluginAPIExtensions(
  ...
  const extensions =
    apiType === "shop"
      ? plugins.map((p) =>
          reflectMetadata(p, PLUGIN_METADATA.SHOP_API_EXTENSIONS),
        )
      : plugins.map((p) =>
          reflectMetadata(p, PLUGIN_METADATA.ADMIN_API_EXTENSIONS),
        );
...
export function isDynamicModule(
  input: Type<any> | DynamicModule,
): input is DynamicModule {
