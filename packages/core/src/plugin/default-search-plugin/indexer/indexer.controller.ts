...
export class IndexerController {
  ...
  reindex(
    ...
    return asyncObservable(async (observer) => {
      ...
      const channel =
        ctx.channel ?? (await this.loadChannel(ctx, ctx.channelId));
      const { count } = await this.getSearchIndexQueryBuilder(ctx, {
        channels: [channel],
      });
      Logger.verbose(
      ...
      await this.connection
        ...
        .delete({ channelId: ctx.channelId });
  ...
  updateVariantsById(
    job: Job<UpdateVariantsByIdJobData>,
  ): Observable<ReindexMessageResponse> {
    ...
    return asyncObservable(async (observer) => {
      ...
      if (ids.length) {
        ...
        for (let i = 0; i < batches; i++) {
          ...
          const { variants: batch } = await this.getSearchIndexQueryBuilder(
            ctx,
            {
  ...
  async assignVariantToChannel(
    data: VariantChannelMessageData,
  ): Promise<boolean> {
    const ctx = MutableRequestContext.deserialize(data.ctx);
    return this.updateVariantsInChannel(
      ctx,
  ...
  async updateAsset(data: UpdateAssetMessageData): Promise<boolean> {
    ...
    await this.connection
      .getRepository(ctx, SearchIndexItem)
      .update(
        { productVariantAssetId: id },
        { productVariantPreviewFocalPoint: focalPoint },
      );
    return true;
  }
  ...
  private async updateProductInChannel(
    ...
    const product = await this.getProductInChannelQueryBuilder(
      ctx,
      productId,
      channel,
    );
    ...
    if (product) {
      ...
      const { variants: updatedVariants } =
        await this.getSearchIndexQueryBuilder(ctx, {
      ...
      if (updatedVariants.length === 0) {
        ...
        await this.entityHydrator.hydrate(ctx, clone, {
          relations: ["translations" as never],
        });
        ...
        Logger.verbose(
          `Updating ${variantsInCurrentChannel.length} variants`,
          workerLoggerCtx,
        );
  ...
  private async deleteProductInChannel(
    ...
    const channels = await Promise.all(
      channelIds.map((channelId) => this.loadChannel(ctx, channelId)),
    );
    const product = await this.getProductInChannelQueryBuilder(
      ctx,
      productId,
  ...
  private async getProductInChannelQueryBuilder(
    ...
    const channelLanguages = unique(
      channels
        .flatMap((c) => c.availableLanguageCodes)
        .concat(this.configService.defaultLanguageCode),
    );
    ...
    const product = await this.connection.getRepository(ctx, Product).findOne({
      ...
      where: {
        id: Equal(productId),
        channels: { id: In(channels.map((x) => x.id)) },
      },
    });
      ...
      for (const languageCode of availableLanguageCodes) {
        ...
        for (const channel of variant.channels) {
          ...
          const item = new SearchIndexItem({
            ...
            description: this.constrainDescription(
              productTranslation?.description ?? "",
            ),
            productVariantName: variantTranslation?.name ?? "",
            productAssetId: product.featuredAsset
            ...
            productVariantPreviewFocalPoint: variant.featuredAsset
              ...
              : null,
            productVariantAssetId: variant.featuredAsset
              ? variant.featuredAsset.id
              : null,
            productPreview: product.featuredAsset
              ? product.featuredAsset.preview
              : "",
            productVariantPreview: variant.featuredAsset
          ...
          if (this.options.indexStockStatus) {
            item.inStock =
              0 <
              (await this.productVariantService.getSaleableStockLevel(
                ctx,
                variant,
              ));
              ...
              () =>
                this.connection
                  ...
                  .then((_variants) =>
                    Promise.all(
                      _variants.map((v) =>
                        this.productVariantService.getSaleableStockLevel(
                          ctx,
                          v,
                        ),
                      ),
                  ...
                  .then((stockLevels) =>
                    stockLevels.some((stockLevel) => 0 < stockLevel),
                  ),
            );
  ...
  private async saveSyntheticVariant(ctx: RequestContext, product: Product) {
    ...
    await this.queue.push(() =>
      this.connection.getRepository(ctx, SearchIndexItem).save(item),
    );
  }
   ...
   */
  private async removeSyntheticVariants(
    ctx: RequestContext,
    variants: ProductVariant[],
  ) {
  ...
  private async removeSearchIndexItems(
    ...
    await this.queue.push(() =>
      this.connection.getRepository(ctx, SearchIndexItem).delete(keys as any),
    );
  }
