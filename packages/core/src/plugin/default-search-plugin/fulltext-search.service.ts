...
export class FulltextSearchService {
  ...
  async search(
    ...
    const items = await this._searchStrategy.getSearchResults(
      ctx,
      input,
      enabledOnly,
    );
    const totalItems = await this._searchStrategy.getTotalCount(
      ctx,
      input,
      enabledOnly,
    );
  ...
  async facetValues(
    ...
    const facetValueIdsMap = await this._searchStrategy.getFacetValueIds(
      ctx,
      input,
      enabledOnly,
    );
  ...
  async collections(
    ...
    const collectionIdsMap = await this._searchStrategy.getCollectionIds(
      ctx,
      input,
      enabledOnly,
    );
    const collections = await this.collectionService.findByIds(
      ctx,
      Array.from(collectionIdsMap.keys()),
    );
