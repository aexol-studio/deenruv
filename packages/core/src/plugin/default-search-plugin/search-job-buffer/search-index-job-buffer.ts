  ...
  implements JobBuffer<UpdateIndexQueueJobData>
{
  ...
  collect(job: Job<UpdateIndexQueueJobData>): boolean | Promise<boolean> {
    return (
      ...
      ["update-product", "update-variants", "update-variants-by-id"].includes(
        job.data.type,
      )
    );
  }
  ...
  reduce(collectedJobs: Array<Job<UpdateIndexQueueJobData>>): Array<Job<any>> {
    const variantsJobs = this.removeBy<
      Job<UpdateVariantsByIdJobData | UpdateVariantsJobData>
    >(
      collectedJobs,
      (item) =>
        item.data.type === "update-variants-by-id" ||
        item.data.type === "update-variants",
    );
    ...
    if (variantsJobs.length) {
      ...
      for (const job of variantsJobs) {
        const ids =
          job.data.type === "update-variants-by-id"
            ? job.data.ids
            : job.data.variantIds;
   ...
   */
  private removeBy<R extends T, T = any>(
    input: T[],
