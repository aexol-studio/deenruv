...
export class SearchJobBufferService implements OnApplicationBootstrap {
  ...
  async getPendingSearchUpdates(): Promise<number> {
    ...
    return (
      ...
      (bufferSizes[this.collectionJobBuffer.id] ?? 0)
    );
  }
  ...
  async runPendingSearchUpdates(): Promise<void> {
    ...
    const collectionFilterJobs = await this.jobQueueService.flush(
      this.collectionJobBuffer,
    );
    if (
      collectionFilterJobs.length &&
      isInspectableJobQueueStrategy(jobQueueStrategy)
    ) {
