...
export function mapToSearchResult(
  ...
  const enabled =
    raw.productEnabled != null ? !!Number(raw.productEnabled) : raw.si_enabled;
  return {
    sku: raw.si_sku,
    slug: raw.si_slug,
    price,
 ...
 */
export function createFacetIdCountMap(
  facetValuesResult: Array<{ facetValues: string }>,
) {
  const result = new Map<ID, number>();
  for (const res of facetValuesResult) {
    const facetValueIds: ID[] = unique(
      res.facetValues.split(",").filter((x) => x !== ""),
    );
 ...
 */
export function createCollectionIdCountMap(
  collectionsResult: Array<{ collections: string }>,
) {
  const result = new Map<ID, number>();
  for (const res of collectionsResult) {
    const collectionIds: ID[] = unique(
      res.collections.split(",").filter((x) => x !== ""),
    );
...
export function applyLanguageConstraints(
  ...
  if (languageCode === defaultLanguageCode) {
    ...
    qb.andWhere(
      new Brackets((qb1) => {
        qb1
          .where(`si.${lcEscaped} = :languageCode1`, {
