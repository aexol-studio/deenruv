...
export class MysqlSearchStrategy implements SearchStrategy {
  ...
  private applyTermAndFilters(
    ...
    if (term && term.length > this.minTermLength) {
      ...
      const termScoreQuery = this.connection
        ...
        .where(
          new Brackets((qb1) => {
            qb1
              .where("si_inner.sku LIKE :like_term")
              .orWhere(
                "MATCH (si_inner.productName) AGAINST (:term IN BOOLEAN MODE)",
              )
              .orWhere(
                "MATCH (si_inner.productVariantName) AGAINST (:term IN BOOLEAN MODE)",
              )
              .orWhere(
                "MATCH (si_inner.description) AGAINST (:term IN BOOLEAN MODE)",
              );
          }),
        )
      ...
      qb.innerJoin(
        `(${termScoreQuery.getQuery()})`,
        "term_result",
        "inner_productId = si.productId",
      )
        .addSelect(
          input.groupByProduct ? "MAX(term_result.score)" : "term_result.score",
          "score",
        )
    ...
    if (collectionId) {
      qb.andWhere("FIND_IN_SET (:collectionId, si.collectionIds)", {
        collectionId,
      });
    }
    if (collectionSlug) {
      qb.andWhere("FIND_IN_SET (:collectionSlug, si.collectionSlugs)", {
        collectionSlug,
      });
    }
