...
} from "./api/fulltext-search.resolver";
 ...
 * underlying database.
 *
 * The DefaultSearchPlugin is bundled with the `\@deenruv/core` package. If you are not using an alternative search
...
@DeenruvPlugin({
  ...
  adminApiExtensions: {
    schema: () =>
      DefaultSearchPlugin.options.indexStockStatus === true
...
export class DefaultSearchPlugin
  implements OnApplicationBootstrap, OnApplicationShutdown
{
  ...
  async onApplicationBootstrap() {
    ...
    collectionModification$
      ...
      .subscribe((events) => {
        return this.searchIndexService.updateVariantsById(
          events.ctx,
          events.ids,
        );
      });
