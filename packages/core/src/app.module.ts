...
export class AppModule implements NestModule, OnApplicationShutdown {
  ...
  configure(consumer: MiddlewareConsumer) {
    const { adminApiPath, shopApiPath, middleware } =
    ...
    const consumableMiddlewares = allMiddleware.filter(
      (mid) => !mid.beforeListen,
    );
    const middlewareByRoute = this.groupMiddlewareByRoute(
      consumableMiddlewares,
    );
   ...
   */
  private groupMiddlewareByRoute(middlewareArray: Middleware[]): {
    [route: string]: MiddlewareHandler[];
  } {
    const result = {} as { [route: string]: MiddlewareHandler[] };
    for (const middleware of middlewareArray) {
      const route =
        middleware.route || this.configService.apiOptions.adminApiPath;
      if (!result[route]) {
        result[route] = [];
      }
