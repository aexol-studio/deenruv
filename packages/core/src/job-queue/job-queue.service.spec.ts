...
import {
  ProcessContext,
  setProcessContext,
} from "../process-context/process-context";
...
describe("JobQueueService", () => {
  ...
  it("retries", async () => {
    ...
    expect(backoffStrategySpy.mock.calls[0]).toEqual([
      "test",
      1,
      await getJob(testJob),
    ]);
    ...
    expect(backoffStrategySpy.mock.calls[1]).toEqual([
      "test",
      2,
      await getJob(testJob),
    ]);
  ...
  it("sets long-running jobs to pending on destroy", async () => {
    ...
    expect((await testingJobQueueStrategy.findOne(testJob.id!))?.state).toBe(
      JobState.RUNNING,
    );
  ...
  describe("buffering", () => {
    ...
    it("buffers the specified jobs", async () => {
      ...
      const bufferedJobs = testJobBufferStorageStrategy.getBufferedJobs(
        testJobBuffer.id,
      );
