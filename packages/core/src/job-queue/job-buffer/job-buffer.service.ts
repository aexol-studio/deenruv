...
export class JobBufferService {
  ...
  constructor(private configService: ConfigService) {
    this.storageStrategy =
      configService.jobQueueOptions.jobBufferStorageStrategy;
  }

  addBuffer(buffer: JobBuffer<any>) {
    const idAlreadyExists = Array.from(this.buffers).find(
      (p) => p.id === buffer.id,
    );
    if (idAlreadyExists) {
  ...
  bufferSize(
    forBuffers?: Array<JobBuffer | string>,
  ): Promise<{ [bufferId: string]: number }> {
    const buffer = forBuffers ?? Array.from(this.buffers);
    return this.storageStrategy.bufferSize(
      buffer.map((p) => (typeof p === "string" ? p : p.id)),
    );
  }
