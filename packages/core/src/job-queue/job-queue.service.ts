...
import { JobQueue as GraphQlJobQueue } from "@deenruv/common/src/generated-types";
...
export class JobQueueService implements OnModuleDestroy {
   ...
   */
  bufferSize(
    ...forBuffers: Array<JobBuffer<any> | string>
  ...
  private shouldStartQueue(queueName: string): boolean {
    if (this.configService.jobQueueOptions.activeQueues.length > 0) {
      if (
        !this.configService.jobQueueOptions.activeQueues.includes(queueName)
      ) {
        return false;
      }
    }
