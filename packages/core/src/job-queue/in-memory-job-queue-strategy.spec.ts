import { JobListOptions, SortOrder } from "@deenruv/common/lib/generated-types";
...
describe("InMemoryJobQueueStrategy", () => {
  ...
  describe("findMany options", () => {
    ...
    it("take & skip", async () => {
      ...
      expect(await getIdResultsFor({ take: 10, skip: 2 })).toEqual([
        "email-1",
        "video-3",
        "email-2",
      ]);
    ...
    it("sort createdAt", async () => {
      expect(
        await getIdResultsFor({ sort: { createdAt: SortOrder.DESC } }),
      ).toEqual(["email-2", "video-3", "email-1", "video-2", "video-1"]);
      expect(
        await getIdResultsFor({ sort: { createdAt: SortOrder.ASC } }),
      ).toEqual(["video-1", "video-2", "email-1", "video-3", "email-2"]);
    });
    ...
    it("filter queueName", async () => {
      expect(
        await getIdResultsFor({ filter: { queueName: { eq: "video" } } }),
      ).toEqual(["video-1", "video-2", "video-3"]);

      expect(
        await getIdResultsFor({ filter: { queueName: { contains: "vid" } } }),
      ).toEqual(["video-1", "video-2", "video-3"]);
    });
    ...
    it("filter isSettled", async () => {
      ...
      expect(
        await getIdResultsFor({ filter: { isSettled: { eq: true } } }),
      ).toEqual(["video-1"]);
    });
  });
});
