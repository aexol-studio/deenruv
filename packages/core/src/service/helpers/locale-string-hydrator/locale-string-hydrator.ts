...
export class LocaleStringHydrator {
  ...
  async hydrateLocaleStringField<
    T extends DeenruvEntity & Translatable & { languageCode?: LanguageCode },
  >(
    ctx: RequestContext,
    entity: T,
  ...
  private async hydrateLocaleStrings<T extends DeenruvEntity & Translatable>(
    ...
    if (!entity.translations?.length) {
      ...
      let dbCallPromise = this.requestCache.get<Promise<T | null>>(
        ctx,
        cacheKey,
      );
    ...
    if (entity.translations.length) {
      ...
      for (const localeStringProp of Object.keys(entity.translations[0])) {
        ...
        if (localeStringProp === "customFields") {
          ...
          (entity as any)[localeStringProp] = (translated as any)[
            localeStringProp
          ];
        }
      }
