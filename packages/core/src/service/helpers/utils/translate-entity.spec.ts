...
describe("translateEntity()", () => {
  ...
  it("throw if there are no translations available", () => {
    ...
    expect(() =>
      translateEntity(product, [LanguageCode.en, LanguageCode.en]),
    ).toThrow("error.entity-has-no-translation-in-language");
  });

  it("falls back to default language", () => {
    expect(
      translateEntity(product, [LanguageCode.zu, LanguageCode.en]).name,
    ).toEqual(PRODUCT_NAME_EN);
  });

  it("falls back to default language", () => {
    expect(
      translateEntity(product, [LanguageCode.zu, LanguageCode.de]).name,
    ).toEqual(PRODUCT_NAME_DE);
  });
});
...
describe("translateDeep()", () => {
  ...
  it("should not throw if root entity has no translations", () => {
    expect(() =>
      translateDeep(testProduct, [LanguageCode.en, LanguageCode.en]),
    ).not.toThrow();
  });
  ...
  it("should translate a second-level nested non-array entity", () => {
    ...
    expect(result.singleTestVariant.singleOption).toHaveProperty(
      "name",
      OPTION_NAME_EN,
    );
  });
  ...
  it("should translate a second-level nested entity array (first-level is not array)", () => {
    ...
    expect(result.singleRealVariant.options[0]).toHaveProperty(
      "name",
      OPTION_NAME_EN,
    );
  });
  ...
  it("should translate a second-level nested entity array", () => {
    ...
    expect(result).toHaveProperty("name", PRODUCT_NAME_EN);
    expect(result.variants[0]).toHaveProperty("name", VARIANT_NAME_EN);
    expect(result.variants[0].options[0]).toHaveProperty(
      "name",
      OPTION_NAME_EN,
    );
  });
