...
export function orderItemsAreDelivered(order: Order) {
  return (
    getOrderLinesFulfillmentStates(order).every(
...
export function orderItemsArePartiallyDelivered(order: Order) {
  ...
  return (
    ...
    (!states.every((state) => state === "Delivered") ||
      isOrderPartiallyFulfilled(order))
  );
}

function getOrderLinesFulfillmentStates(
  order: Order,
): Array<FulfillmentState | undefined> {
...
export function orderItemsArePartiallyShipped(order: Order) {
  ...
  return (
    ...
    (!states.every((state) => state === "Shipped") ||
      isOrderPartiallyFulfilled(order))
  );
}
...
export async function getOrdersFromLines(
  ...
  for (const line of lines) {
    const inputLine = orderLinesInput.find((l) =>
