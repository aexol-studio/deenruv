...
export type OrderWithEmptyLines = Order & {
  lines: ArrayLike<OrderLine> & { length: 0 };
};
...
export class OrderMerger {
  ...
  merge(
    ...
    if (
      ...
      existingOrder &&
      !this.orderEmpty(existingOrder)
    ) {
      const { mergeStrategy } = this.configService.orderOptions;
      const mergedLines = mergeStrategy.merge(ctx, guestOrder, existingOrder);
      return {
        order: existingOrder,
        linesToInsert: this.getLinesToInsert(
          guestOrder,
          existingOrder,
          mergedLines,
        ),
        linesToModify: this.getLinesToModify(
          guestOrder,
          existingOrder,
          mergedLines,
        ),
        linesToDelete: this.getLinesToDelete(
          guestOrder,
          existingOrder,
          mergedLines,
        ),
  ...
  private getLinesToInsert(
    ...
    return guestOrder.lines
      .map((guestLine) => {
        const mergedLine = mergedLines.find((ml) =>
          idsAreEqual(ml.orderLineId, guestLine.id),
        );
        if (!mergedLine) {
          return;
        }
  ...
  private getLinesToModify(
    ...
    return existingOrder.lines
      .map((existingLine) => {
        const mergedLine = mergedLines.find((ml) =>
          idsAreEqual(ml.orderLineId, existingLine.id),
        );
        if (!mergedLine) {
          return;
        }
        const lineIsModified =
          ...
          JSON.stringify(mergedLine.customFields) !==
            JSON.stringify(existingLine.customFields);
        if (!lineIsModified) {
          return;
        }
        return {
