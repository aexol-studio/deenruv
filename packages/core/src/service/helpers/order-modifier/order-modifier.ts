...
import {
  ...
  JustErrorResults,
} from "../../../common/error/error-result";
import {
  EntityNotFoundError,
  InternalServerError,
  UserInputError,
...
export class OrderModifier {
   ...
   * OrderLines into account when constraining the quantity. See https://github.com/aexol-studio/deenruv/issues/2702
   * for more on this.
   */
  ...
  async getExistingOrderLine(
    ...
    for (const line of order.lines) {
      const match =
        ...
        (await this.customFieldsAreEqual(
          ctx,
          line,
          customFields,
          line.customFields,
  ...
  async getOrCreateOrderLine(
    ...
    const existingOrderLine = await this.getExistingOrderLine(
      ctx,
      order,
      productVariantId,
      customFields,
    ...
    const productVariant = await this.getProductVariantOrThrow(
      ctx,
      productVariantId,
    );
    const orderLine = await this.connection.getRepository(ctx, OrderLine).save(
      new OrderLine({
        ...
        featuredAsset:
          productVariant.featuredAsset ?? productVariant.product.featuredAsset,
    ...
    if (typeof orderSellerStrategy.setOrderLineSellerChannel === "function") {
      orderLine.sellerChannel =
        await orderSellerStrategy.setOrderLineSellerChannel(ctx, orderLine);
    ...
    const lineWithRelations = await this.connection.getEntityOrThrow(
      ctx,
      OrderLine,
      orderLine.id,
      {
  ...
  async cancelOrderByOrderLines(
    ...
    const fullOrder = await this.connection.getEntityOrThrow(
      ctx,
      Order,
      order.id,
      {
    ...
    for (const lineInput of lineInputs) {
      const orderLine = fullOrder.lines.find((l) =>
        idsAreEqual(l.id, lineInput.orderLineId),
      );
      ...
      const totalFulfilled =
        summate(fulfillmentsForLine, "quantity") -
        summate(cancellationsForLine, "quantity");
      if (0 < totalFulfilled) {
        fulfilledLines.push({
          orderLineId: lineInput.orderLineId,
    ...
    await this.stockMovementService.createCancellationsForOrderLines(
      ctx,
      fulfilledLines,
    );
    await this.stockMovementService.createReleasesForOrderLines(
      ctx,
      allocatedLines,
    );
    for (const line of lineInputs) {
      const orderLine = fullOrder.lines.find((l) =>
        idsAreEqual(l.id, line.orderLineId),
      );
      if (orderLine) {
        await this.connection
          .getRepository(ctx, OrderLine)
          .update(line.orderLineId, {
    ...
    const orderWithLines = await this.connection.getEntityOrThrow(
      ctx,
      Order,
      order.id,
      {
    ...
    if (input.cancelShipping === true) {
      for (const shippingLine of orderWithLines.shippingLines) {
        ...
        await this.connection
          ...
          .save(shippingLine, { reload: false });
      }
    }
    ...
    await this.connection
      .getRepository(ctx, Order)
      .save(orderWithLines, { reload: false });
  ...
  async modifyOrder(
    ...
    | { order: Order; modification: OrderModification }
  > {
    const { dryRun } = input;
    ...
    for (const row of input.addItems ?? []) {
      ...
      await this.updateOrderLineQuantity(
        ctx,
        orderLine,
    ...
    for (const row of input.adjustOrderLines ?? []) {
      ...
      const resultingOrderTotalQuantity =
        currentItemsCount + correctedQuantity - orderLine.quantity;
      ...
      if (correctedQuantity < quantity) {
        ...
        if (quantity < initialLineQuantity) {
          ...
          await this.cancelOrderByOrderLines(
            ctx,
            { orderId: order.id },
            cancelLinesInput,
          );
        ...
        const orderModificationLine = await this.connection
          ...
          .save(
            new OrderModificationLine({
              orderLine,
              quantity: quantity - initialLineQuantity,
            }),
          );
        modification.lines.push(orderModificationLine);
    ...
    for (const surchargeInput of input.surcharges ?? []) {
      const taxLines =
        surchargeInput.taxRate != null
          ...
          : [];
      const surcharge = await this.connection
        .getRepository(ctx, Surcharge)
        .save(
          new Surcharge({
      ...
      if (surcharge.priceWithTax < 0) {
        refundInputs.forEach(
          (ri) => (ri.adjustment += Math.abs(surcharge.priceWithTax)),
        );
      }
    }
    ...
    const updatedOrderLines = order.lines.filter((l) =>
      updatedOrderLineIds.includes(l.id),
    );
    const promotions =
      await this.promotionService.getActivePromotionsInChannel(ctx);
    const activePromotionsPre =
      await this.promotionService.getActivePromotionsOnOrder(ctx, order.id);
    if (input.shippingMethodIds) {
      const result = await this.setShippingMethods(
        ctx,
        order,
    ...
    const { orderItemPriceCalculationStrategy } =
    ...
    if (delta < 0) {
      ...
      const primaryRefund = refundInputs
        .slice()
      ...
      primaryRefund.adjustment += await this.calculateRefundAdjustment(
        ctx,
        delta,
        primaryRefund,
      );
      ...
      for (const refundInput of refundInputs) {
        ...
        const payment = existingPayments.find((p) =>
          idsAreEqual(p.id, refundInput.paymentId),
        );
        if (payment) {
  ...
  async setShippingMethods(
    ...
    if (shippingMethodIds.length < order.shippingLines.length) {
      const shippingLinesToDelete = order.shippingLines.splice(
        shippingMethodIds.length - 1,
      );
      await this.connection
        .getRepository(ctx, ShippingLine)
        .remove(shippingLinesToDelete);
    }
    ...
    await this.connection
      ...
      .execute();
    const { shippingLineAssignmentStrategy } =
      this.configService.shippingOptions;
    for (const shippingLine of order.shippingLines) {
      const orderLinesForShippingLine =
        await shippingLineAssignmentStrategy.assignShippingLineToOrderLines(
          ctx,
          shippingLine,
          order,
        );
  ...
  private async calculateRefundAdjustment(
    ...
    for (const lineInput of refundInput.lines) {
      const orderLine = await this.connection.getEntityOrThrow(
        ctx,
        OrderLine,
        lineInput.orderLineId,
      );
      itemAmount += orderLine.proratedUnitPriceWithTax * lineInput.quantity;
    }
    const calculatedDelta =
      itemAmount + refundInput.shipping + existingAdjustment;
  ...
  private async customFieldsAreEqual(
    ...
    if (inputCustomFields == null && typeof existingCustomFields === "object") {
      ...
      for (const def of customFieldDefs) {
        ...
        if (
          existingValue != null &&
          (!def.list || existingValue?.length !== 0)
        ) {
    ...
    const customFieldRelations = customFieldDefs.filter(
      (d) => d.type === "relation",
    );
    ...
    for (const def of customFieldDefs) {
      ...
      if (def.type !== "relation" && existingValue !== undefined) {
        const valuesMatch =
          JSON.stringify(inputCustomFields?.[key]) ===
          JSON.stringify(existingValue);
        const undefinedMatchesNull =
          existingValue === null && inputCustomFields?.[key] === undefined;
        const defaultValueMatch =
          inputCustomFields?.[key] === undefined &&
          def.defaultValue === existingValue;
        ...
        const existingRelation = (
          lineWithCustomFieldRelations!.customFields as any
        )[key];
        if (inputValue) {
          const customFieldNotEqual = def.list
            ? JSON.stringify((inputValue as ID[]).sort()) !==
              JSON.stringify(
                existingRelation
                  ?.map((relation: DeenruvEntity) => relation.id)
                  .sort(),
              )
   ...
   */
  private coerceValue(
    def: CustomFieldConfig,
    existingCustomFields: { [p: string]: any } | undefined,
  ) {
    const key = def.name;
    return def.type === "boolean" &&
      typeof existingCustomFields?.[key] === "number"
