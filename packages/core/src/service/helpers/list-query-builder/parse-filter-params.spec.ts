...
describe("parseFilterParams()", () => {
  ...
  it("works with localized fields", () => {
    ...
    connection.setRelations(Product, [
      { propertyName: "translations", type: ProductTranslation },
    ]);
  ...
  describe("number operators", () => {
    ...
    it("between", () => {
      ...
      expect(result[0].clause).toBe(
        "product.price BETWEEN :arg1_a AND :arg1_b",
      );
  ...
  describe("boolean operators", () => {
    it("eq", () => {
      const connection = new MockConnection();
      connection.setColumns(Product, [
        { propertyName: "available", type: "tinyint" },
      ]);
