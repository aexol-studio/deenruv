...
export class OrderSplitter {
  ...
  async createSellerOrders(
    ...
    for (const partialOrder of partialOrders) {
      ...
      for (const shippingLine of partialOrder.shippingLines) {
        const newShippingLine = await this.duplicateShippingLine(
          ctx,
          shippingLine,
        );
      ...
      const sellerOrder = await this.connection.getRepository(ctx, Order).save(
        new Order({
          ...
          channels: [
            new Channel({ id: partialOrder.channelId }),
            defaultChannel,
          ],
    ...
    await orderSellerStrategy.afterSellerOrdersCreated?.(
      ctx,
      order,
      sellerOrders,
    );
  ...
  private async duplicateOrderLine(
    ctx: RequestContext,
    line: OrderLine,
  ): Promise<OrderLine> {
