...
export class EntityHydrator {
  ...
  async hydrate<Entity extends DeenruvEntity>(
    ...
    if (options.relations) {
      ...
      if (options.applyProductVariantPrices === true) {
        const productVariantPriceRelations =
          this.getRequiredProductVariantRelations(target, missingRelations);
        missingRelations = unique([
          ...missingRelations,
          ...productVariantPriceRelations,
        ]);
      }
      ...
      if (missingRelations.length) {
        ...
        hydratedQb.setFindOptions({
          ...
          where: { id: target.id },
          relations: missingRelations.filter(
            (relationPath) => !joinedRelations.has(relationPath),
          ),
        });
        const hydrated = await hydratedQb.getOne();
        const propertiesToAdd = unique(
          missingRelations.map((relation) => relation.split(".")[0]),
        );
        for (const prop of propertiesToAdd) {
          (target as any)[prop] = mergeDeep(
            (target as any)[prop],
            hydrated[prop],
          );
        }
  ...
  private assignSettableProperties<Entity extends DeenruvEntity>(
    target: Entity,
    source: Entity,
  ) {
  ...
  private getMissingRelations<Entity extends DeenruvEntity>(
    ...
    for (const relation of options.relations.slice().sort()) {
      if (typeof relation === "string") {
        const parts = !relation.startsWith("customFields")
          ? relation.split(".")
          : [relation];
        ...
        for (const part of parts) {
          ...
          if (entity && entity[part]) {
            entity = Array.isArray(entity[part])
              ? entity[part][0]
              : entity[part];
          } else {
    ...
    return unique(
      missingRelations.filter(
        (relation) => !relation.endsWith(".customFields"),
      ),
    );
  }
  ...
  private getRelationEntityTypeAtPath(
    ...
    const targetMetadata = entityMetadatas.find(
    ...
    for (const pathPart of path.split(".")) {
      const relationMetadata =
        currentMetadata.findRelationWithPropertyPath(pathPart);
      if (relationMetadata) {
  ...
  private isTranslatable<T extends DeenruvEntity>(
    input: T | T[] | undefined,
  ): boolean {
