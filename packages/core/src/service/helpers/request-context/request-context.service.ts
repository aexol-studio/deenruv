...
export class RequestContextService {
  ...
  async fromRequest(
    ...
    const isAuthorized = this.userHasRequiredPermissionsOnChannel(
      requiredPermissions,
      channel,
      user,
    );
  ...
  private getChannelToken(
    req: Request<any, any, any, { [key: string]: any }>,
  ): string {
  ...
  private getCurrencyCode(
    ...
    const queryCurrencyCode =
      req.query && (req.query.currencyCode as CurrencyCode);
    if (
      queryCurrencyCode &&
      !channel.availableCurrencyCodes.includes(queryCurrencyCode)
    ) {
