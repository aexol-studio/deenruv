...
export class OrderCalculator {
  ...
  private createTaxRateGetter(
    ...
    return async (taxCategory: TaxCategory): Promise<TaxRate> => {
      ...
      const rate = await this.taxRateService.getApplicableTaxRate(
        ctx,
        activeZone,
        taxCategory,
      );
    ...
    for (const line of order.lines) {
      ...
      for (const promotion of applicablePromotions) {
        ...
        if (applicableOrState) {
          const state =
            typeof applicableOrState === "object"
              ? applicableOrState
              : undefined;
  ...
  private async applyOrderPromotions(
    ...
    const orderHasDistributedPromotions = !!order.discounts.find(
      (adjustment) =>
        adjustment.type === AdjustmentType.DISTRIBUTED_ORDER_PROMOTION,
    );
  ...
  private async applyShippingPromotions(
    ctx: RequestContext,
    order: Order,
    promotions: Promotion[],
  ) {
  ...
  private async applyShipping(ctx: RequestContext, order: Order) {
    ...
    // ShippingMethod.
    const orderLineShippingLineIds = order.lines.map(
      (line) => line.shippingLineId,
    );
    ...
    for (const shippingLine of order.shippingLines) {
      ...
      const results = await this.shippingCalculator.getEligibleShippingMethods(
        ctx,
        order,
  ...
  private addPromotion(order: Order, promotion: Promotion) {
    if (
      order.promotions &&
      !order.promotions.find((p) => idsAreEqual(p.id, promotion.id))
    ) {
