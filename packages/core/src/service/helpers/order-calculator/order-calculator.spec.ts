...
describe("OrderCalculator", () => {
  ...
  beforeAll(async () => {
    ...
    const mockConfigService = module.get<ConfigService, MockConfigService>(
      ConfigService,
    );
  ...
  describe("promotions", () => {
    ...
    const orderTotalCondition = new PromotionCondition({
      ...
      check(ctx, order, args) {
        const threshold = ctx.channel.pricesIncludeTax
    ...
    const percentageOrderAction = new PromotionOrderAction({
      ...
      execute(ctx, order, args) {
        const orderTotal = ctx.channel.pricesIncludeTax
          ? order.subTotalWithTax
          : order.subTotal;
    ...
    describe("OrderItem-level discounts", () => {
      describe("percentage items discount", () => {
        ...
        it("prices exclude tax", async () => {
          ...
          expect(order.lines[0].adjustments[0].description).toBe(
            "50% off each item",
          );
...
function createTestModule() {
  return Test.createTestingModule({
    providers: [
      ...
      {
        provide: ShippingCalculator,
        useValue: { getEligibleShippingMethods: () => [] },
      },
      {
...
function assertOrderTotalsAddUp(order: Order) {
  for (const line of order.lines) {
    ...
    if (pricesIncludeTax) {
      const lineDiscountsAmountWithTaxSum = summate(
        line.discounts,
        "amountWithTax",
      );
      expect(line.linePriceWithTax + lineDiscountsAmountWithTaxSum).toBe(
        line.proratedLinePriceWithTax,
      );
    } else {
      const lineDiscountsAmountSum = summate(line.discounts, "amount");
      expect(line.linePrice + lineDiscountsAmountSum).toBe(
  ...
  const displayPriceWithTaxSum = summate(
    order.lines,
  ...
  const differenceBetweenSumAndActual = Math.abs(
    displayPriceWithTaxSum +
      orderDiscountsWithTaxSum +
      surchargeWithTaxSum -
      order.subTotalWithTax,
  );
