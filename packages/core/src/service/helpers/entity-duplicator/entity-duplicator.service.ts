...
export class EntityDuplicatorService {
  ...
  getEntityDuplicators(ctx: RequestContext): EntityDuplicatorDefinition[] {
    return this.configArgService
      .getDefinitions("EntityDuplicator")
      .map((x) => ({
        ...x.toGraphQlType(ctx),
  ...
  async duplicateEntity(
    ...
    const duplicator = this.configService.entityOptions.entityDuplicators.find(
      (s) =>
        s.forEntities.includes(input.entityName) &&
        s.code === input.duplicatorInput.code,
    );
    if (!duplicator) {
      return new DuplicateEntityError({
        duplicationError: ctx.translate(
          `message.entity-duplication-no-strategy-found`,
          {
    ...
    if (
      ...
      return new DuplicateEntityError({
        duplicationError: ctx.translate(
          `message.entity-duplication-no-permission`,
        ),
      });
    }

    const parsedInput = this.configArgService.parseInput(
      "EntityDuplicator",
      input.duplicatorInput,
    );
