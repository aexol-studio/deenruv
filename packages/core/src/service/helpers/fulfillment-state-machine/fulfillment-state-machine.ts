...
export class FulfillmentStateMachine {
  ...
  > {
    ...
    const customProcesses =
      this.configService.shippingOptions.customFulfillmentProcess ?? [];
    const processes = [
      ...customProcesses,
      ...(this.configService.shippingOptions.process ?? []),
    ];
    const allTransitions = processes.reduce(
      (transitions, process) =>
        mergeTransitionDefinitions(
          transitions,
          process.transitions as Transitions<any>,
        ),
      {} as Transitions<FulfillmentState>,
    );

    const validationResult = validateTransitionDefinition(
      allTransitions,
      this.initialState,
    );
    ...
    return {
      ...
      onError: async (fromState, toState, message) => {
        ...
        throw new IllegalOperationError(
          message || "error.cannot-transition-fulfillment-from-to",
          {
            fromState,
            toState,
          },
