...
export class TaxRateService {
  ...
  async create(
    ...
    taxRate.category = await this.connection.getEntityOrThrow(
      ctx,
      TaxCategory,
      input.categoryId,
    );
    taxRate.zone = await this.connection.getEntityOrThrow(
      ctx,
      Zone,
      input.zoneId,
    );
    ...
    const newTaxRate = await this.connection
      .getRepository(ctx, TaxRate)
      .save(taxRate);
  ...
  async update(
    ...
    if (input.zoneId) {
      updatedTaxRate.zone = await this.connection.getEntityOrThrow(
        ctx,
        Zone,
        input.zoneId,
      );
    ...
    await this.connection
      .getRepository(ctx, TaxRate)
      .save(updatedTaxRate, { reload: false });
    await this.customFieldRelationService.updateRelations(
      ctx,
      TaxRate,
      input,
    ...
    await this.eventBus.publish(
      new TaxRateModificationEvent(ctx, updatedTaxRate),
    );
    await this.eventBus.publish(
  ...
  async getApplicableTaxRate(
    ...
    const rate = (await this.getActiveTaxRates(ctx)).find((r) =>
      r.test(zone, taxCategory),
    );
  ...
  private async ensureCacheExists() {
    ...
    this.activeTaxRates = await createSelfRefreshingCache({
      ...
      refresh: {
        fn: (ctx) => this.findActiveTaxRates(ctx),
