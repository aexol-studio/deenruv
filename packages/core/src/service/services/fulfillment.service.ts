...
export class FulfillmentService {
  ...
  async create(
    ...
    Fulfillment | InvalidFulfillmentHandlerError | CreateFulfillmentError
  > {
    const fulfillmentHandler =
      this.configService.shippingOptions.fulfillmentHandlers.find(
    ...
    const newFulfillment = await this.connection
    ...
    for (const { orderLineId, quantity } of lines) {
      const fulfillmentLine = await this.connection
    ...
    const fulfillmentWithRelations =
  ...
  > {
    ...
    const orderLinesIds = unique(
      fulfillment.lines.map((lines) => lines.orderLineId),
    );
    ...
    try {
      ...
      return new FulfillmentStateTransitionError({
        transitionError,
        fromState,
        toState: state,
      });
