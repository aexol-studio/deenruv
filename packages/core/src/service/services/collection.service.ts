...
export class CollectionService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.applyFiltersQueue = await this.jobQueueService.createQueue({
      ...
      process: async (job) => {
        ...
        Logger.verbose(
          `Processing ${job.data.collectionIds.length} Collections`,
        );
        let completed = 0;
        for (const collectionId of job.data.collectionIds) {
          ...
          try {
            ...
            Logger.warn(
              `Could not find Collection with id ${collectionId}, skipping`,
            );
          }
          completed++;
          if (collection) {
            ...
            try {
              ...
              const translatedCollection = this.translator.translate(
                collection,
                ctx,
              );
              Logger.error(
            ...
            if (affectedVariantIds.length) {
              await this.eventBus.publish(
                new CollectionModificationEvent(
                  ctx,
                  collection,
                  affectedVariantIds,
                ),
  ...
  async findByIds(
    ...
    const collections = this.connection.findByIdsInChannel(
      ctx,
      Collection,
      ids,
      ctx.channelId,
  ...
  async findOneBySlug(
    ...
    const translations = await this.connection
      .getRepository(ctx, CollectionTranslation)
      .find({
        relations: ["base"],
        where: {
          slug,
  ...
  getAvailableFilters(ctx: RequestContext): ConfigurableOperationDefinition[] {
    return this.configService.catalogOptions.collectionFilters.map((f) =>
      f.toGraphQlType(ctx),
    );
  }
  ...
  async getBreadcrumbs(
    ...
    return [
      ...
      ...ancestors.map(pickProps).reverse(),
        ...
        ancestors.forEach((a) => {
          ...
          if (category) {
            resultCategories.push(
              ctx ? this.translator.translate(category, ctx) : category,
            );
          }
        });
  ...
  async previewCollectionVariants(
    ...
    if (input.parentId && input.inheritFilters) {
      const parentFilters =
        (await this.findOne(ctx, input.parentId, []))?.filters ?? [];
      const ancestorFilters = await this.getAncestors(input.parentId).then(
        (ancestors) =>
          ancestors.reduce(
    ...
    for (const filterType of collectionFilters) {
      const filtersOfType = applicableFilters.filter(
        (f) => f.code === filterType.code,
      );
  ...
  async create(
    ...
    const collection = await this.translatableSaver.create({
      ...
      beforeSave: async (coll) => {
        ...
        coll.position = await this.getNextPositionInParent(
          ctx,
  ...
  async update(
    ...
    if (input.filters) {
      ...
      const affectedVariantIds =
        await this.getCollectionProductVariantIds(collection);
      await this.eventBus.publish(
  ...
  async delete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
    const collection = await this.connection.getEntityOrThrow(
      ctx,
      Collection,
      id,
      {
    ...
    for (const coll of [...descendants.reverse(), collection]) {
      const affectedVariantIds =
        await this.getCollectionProductVariantIds(coll);
  ...
  async move(
    ...
    const target = await this.connection.getEntityOrThrow(
      ctx,
      Collection,
      input.collectionId,
      {
  ...
  private getCollectionFiltersFromInput(
    ...
    if (input.filters) {
      for (const filter of input.filters) {
        filters.push(
          this.configArgService.parseInput("CollectionFilter", filter),
        );
      }
    }
    return filters;
   ...
   */
  private async getAncestorFilters(
    collection: Collection,
   ...
   */
  private async getFilteredProductVariantIds(
   ...
   */
  async getCollectionProductVariantIds(
    collection: Collection,
    ctx?: RequestContext,
  ): Promise<ID[]> {
   ...
   */
  private async getNextPositionInParent(
    ctx: RequestContext,
    maybeParentId?: ID,
  ): Promise<number> {
  ...
  private async getRootCollection(ctx: RequestContext): Promise<Collection> {
    ...
    // get persisted.
    const rootTranslation = await this.connection.rawConnection
  ...
  async assignCollectionsToChannel(
    ...
    await Promise.all(
      collectionsToAssign.map((collection) =>
        this.channelService.assignToChannels(ctx, Collection, collection.id, [
          input.channelId,
        ]),
      ),
    );
    ...
    const assetIds: ID[] = unique(
      ([] as ID[]).concat(
        ...collectionsToAssign.map((c) => c.assets.map((a) => a.assetId)),
      ),
    );
  ...
  async removeCollectionsFromChannel(
    ...
    const hasPermission = await this.roleService.userHasAnyPermissionsOnChannel(
      ctx,
      input.channelId,
      [Permission.DeleteCollection, Permission.DeleteCatalog],
    );
    if (!hasPermission) {
      throw new ForbiddenError();
    }
