...
export class SessionService implements EntitySubscriberInterface {
  ...
  constructor(
    ...
    this.sessionCacheStrategy =
      this.configService.authOptions.sessionCacheStrategy;
    this.sessionDurationInMs = ms(
      this.configService.authOptions.sessionDuration as string,
    );
  ...
  async createNewAuthenticatedSession(
    ...
    const authenticatedSession = await this.connection
  ...
  async createAnonymousSession(): Promise<CachedSession> {
    ...
    const newSession = await this.connection.rawConnection
  ...
  async getSessionFromToken(
    ...
    let serializedSession = await this.withTimeout(
      this.sessionCacheStrategy.get(sessionToken),
    );
    const stale = !!(
      serializedSession &&
      serializedSession.cacheExpiry < new Date().getTime() / 1000
    );
    const expired = !!(
  ...
  serializeSession(
    ...
    const expiry =
      ...
      this.configService.authOptions.sessionCacheTTL;
  ...
  async setActiveOrder(
    ...
    if (session) {
      ...
      await this.withTimeout(
        this.sessionCacheStrategy.set(updatedSerializedSession),
      );
   ...
   */
  async deleteSessionsByActiveOrderId(
    ctx: RequestContext,
    activeOrderId: ID,
  ): Promise<void> {
    const sessions = await this.connection
      .getRepository(ctx, Session)
      .find({ where: { activeOrderId } });
