...
export class FacetValueService {
    ...
    return repository
      ...
      .then((facetValues) =>
        facetValues.map((facetValue) =>
          translateDeep(facetValue, languageCode, ["facet"]),
        ),
      );
  }
      ...
      .then(([items, totalItems]) => {
        return {
          items: items.map((item) =>
            this.translator.translate(item, ctx, ["facet"]),
          ),
          totalItems,
        };
      });
  ...
  findOne(
    ...
    return this.connection
      ...
      .then(
        (facetValue) =>
          (facetValue &&
            this.translator.translate(facetValue, ctx, ["facet"])) ??
          undefined,
      );
  }
  ...
  findByIds(
    ...
    const facetValues = this.connection.findByIdsInChannel(
      ctx,
      FacetValue,
      ids,
      ctx.channelId,
  ...
  findByFacetId(
    ...
    return this.connection
      ...
      .then((values) =>
        values.map((facetValue) => this.translator.translate(facetValue, ctx)),
      );
  }
  ...
  async create(
    ...
    await this.eventBus.publish(
      new FacetValueEvent(ctx, facetValueWithRelations, "created", input),
    );
  ...
  async update(
    ...
    await this.eventBus.publish(
      new FacetValueEvent(ctx, facetValue, "updated", input),
    );
    return assertFound(this.findOne(ctx, facetValue.id));
  }
  ...
  async delete(
    ...
    const { productCount, variantCount } = await this.checkFacetValueUsage(
      ctx,
      [id],
    );
    ...
    const facetValue = await this.connection.getEntityOrThrow(
      ctx,
      FacetValue,
      id,
    );
    ...
    if (!isInUse) {
      ...
      await this.eventBus.publish(
        new FacetValueEvent(ctx, deletedFacetValue, "deleted", id),
      );
