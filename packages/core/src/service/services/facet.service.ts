...
export class FacetService {
  ...
  findByCode(
    ...
    return repository
      ...
      .then(
        (facet) =>
          (facet &&
            translateDeep(facet, languageCode, [
              "values",
              ["values", "facet"],
            ])) ??
          undefined
  ...
  private async ensureUniqueCode(ctx: RequestContext, code: string, id?: ID) {
    ...
    do {
      ...
      conflict =
        !!match && ((id != null && !idsAreEqual(match.id, id)) || id == null);
      if (conflict) {
        suffix++;
  ...
  async assignFacetsToChannel(
    ...
    await Promise.all<any>([
      ...facetsToAssign.map(async (facet) => {
        return this.channelService.assignToChannels(ctx, Facet, facet.id, [
          input.channelId,
        ]);
      }),
  ...
  async removeFacetsFromChannel(
    ...
    const results: Array<
      ErrorResultUnion<RemoveFacetFromChannelResult, Facet>
    > = [];
    ...
    for (const facet of facetsToRemove) {
      ...
      if (facet.values.length) {
        ...
        if (!isInUse || input.force) {
          await this.channelService.removeFromChannels(ctx, Facet, facet.id, [
            input.channelId,
          ]);
          await Promise.all(
            facet.values.map((fv) =>
