...
export interface CreateCustomerHistoryEntryArgs<
  T extends keyof CustomerHistoryEntryData,
> {
  customerId: ID;
  ctx: RequestContext;
  type: T;
...
export interface UpdateOrderHistoryEntryArgs<
  T extends keyof OrderHistoryEntryData,
> {
  entryId: ID;
  ctx: RequestContext;
...
export interface UpdateCustomerHistoryEntryArgs<
  T extends keyof CustomerHistoryEntryData,
> {
  entryId: ID;
  ctx: RequestContext;
...
export class HistoryService {
  ...
  async createHistoryEntryForOrder<T extends keyof OrderHistoryEntryData>(
    ...
    const history = await this.connection
      .getRepository(ctx, OrderHistoryEntry)
      .save(entry);
  ...
  async createHistoryEntryForCustomer<T extends keyof CustomerHistoryEntryData>(
    ...
    const history = await this.connection
      .getRepository(ctx, CustomerHistoryEntry)
      .save(entry);
    await this.eventBus.publish(
      new HistoryEntryEvent(ctx, history, "created", "customer", {
        type,
        data,
      }),
    );
  ...
  async updateOrderHistoryEntry<T extends keyof OrderHistoryEntryData>(
    ...
    const entry = await this.connection.getEntityOrThrow(
      ctx,
      OrderHistoryEntry,
      args.entryId,
      {
    ...
    const newEntry = await this.connection
      .getRepository(ctx, OrderHistoryEntry)
      .save(entry);
    await this.eventBus.publish(
  ...
  private async getAdministratorFromContext(
    ctx: RequestContext,
