...
export class TaxCategoryService {
  ...
  async create(
    ...
    const newTaxCategory = await this.connection
      .getRepository(ctx, TaxCategory)
      .save(taxCategory);
  ...
  async update(
    ...
    await this.connection
      .getRepository(ctx, TaxCategory)
      .save(updatedTaxCategory, { reload: false });
    await this.eventBus.publish(
      new TaxCategoryEvent(ctx, taxCategory, "updated", input),
    );
    return assertFound(this.findOne(ctx, taxCategory.id));
  }

  async delete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
    const taxCategory = await this.connection.getEntityOrThrow(
      ctx,
      TaxCategory,
      id,
    );
    ...
    if (0 < dependentRates) {
      const message = ctx.translate(
        "message.cannot-remove-tax-category-due-to-tax-rates",
        {
