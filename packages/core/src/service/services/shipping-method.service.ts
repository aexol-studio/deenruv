...
export class ShippingMethodService {
  ...
  async findOne(
    ...
    return (
      (shippingMethod && this.translator.translate(shippingMethod, ctx)) ??
      undefined
    );
  }
  ...
  async create(
    ...
    const shippingMethod = await this.translatableSaver.create({
      ...
      beforeSave: (method) => {
        ...
        method.calculator = this.configArgService.parseInput(
          "ShippingCalculator",
          input.calculator,
        );
      },
    ...
    const newShippingMethod = await this.connection
      ...
      .save(shippingMethod);
    const shippingMethodWithRelations =
    ...
    await this.eventBus.publish(
      new ShippingMethodEvent(
        ctx,
        shippingMethodWithRelations,
        "created",
        input,
      ),
    );
  ...
  async update(
    ...
    if (input.fulfillmentHandler) {
      updatedShippingMethod.fulfillmentHandlerCode =
        this.ensureValidFulfillmentHandlerCode(
    ...
    await this.eventBus.publish(
      new ShippingMethodEvent(ctx, shippingMethod, "updated", input),
    );
  ...
  async softDelete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
    ...
    await this.connection
      .getRepository(ctx, ShippingMethod)
      .save(shippingMethod, { reload: false });
    await this.eventBus.publish(
  ...
  async assignShippingMethodsToChannel(
    ...
    const hasPermission = await this.roleService.userHasAnyPermissionsOnChannel(
      ctx,
      input.channelId,
      [Permission.UpdateShippingMethod, Permission.UpdateSettings],
    );
    if (!hasPermission) {
    ...
    return this.connection
      ...
      .then((methods) =>
        methods.map((method) => this.translator.translate(method, ctx)),
      );
  }
  ...
  async removeShippingMethodsFromChannel(
    ...
    const hasPermission = await this.roleService.userHasAnyPermissionsOnChannel(
      ctx,
      input.channelId,
      [Permission.DeleteShippingMethod, Permission.DeleteSettings],
    );
    if (!hasPermission) {
      throw new ForbiddenError();
    }
  ...
  private async verifyShippingMethods() {
    const activeShippingMethods = await this.connection.rawConnection
    ...
    for (const method of activeShippingMethods) {
      ...
      const verifiedHandlerCode = this.ensureValidFulfillmentHandlerCode(
        method.code,
        handlerCode,
      );
  ...
  private ensureValidFulfillmentHandlerCode(
    ...
    let handler = fulfillmentHandlers.find(
      (h) => h.code === fulfillmentHandlerCode,
    );
    if (!handler) {
