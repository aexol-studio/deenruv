...
export class ProductOptionService {
  ...
  findAll(ctx: RequestContext): Promise<Array<Translated<ProductOption>>> {
    return this.connection
      ...
      .then((options) =>
        options.map((option) => this.translator.translate(option, ctx)),
      );
  }
  ...
  findOne(
    ...
    return this.connection
      ...
      .then(
        (option) =>
          (option && this.translator.translate(option, ctx)) ?? undefined,
      );
  }
  ...
  async delete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
    const productOption = await this.connection.getEntityOrThrow(
      ctx,
      ProductOption,
      id,
    );
    const deletedProductOption = new ProductOption(productOption);
    const inUseByActiveVariants = await this.isInUse(
      ctx,
      productOption,
      "active",
    );
    ...
    const isInUseBySoftDeletedVariants = await this.isInUse(
      ctx,
      productOption,
      "soft-deleted",
    );
    if (0 < isInUseBySoftDeletedVariants) {
      ...
      await this.connection
        .getRepository(ctx, ProductOption)
        .save(productOption, { reload: false });
    } else {
      // hard delete
      try {
        await this.connection
