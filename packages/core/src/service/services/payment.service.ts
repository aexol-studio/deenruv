...
export class PaymentService {
  ...
  async createPayment(
    ...
    const { paymentMethod, handler, checker } =
  ...
  async createRefund(
    ...
    do {
      ...
      for (const { orderLineId, quantity } of input.lines) {
        const refundLine = await this.connection
  ...
  private async getRefundAmount(
    ...
    for (const line of input.lines) {
      const orderLine = orderLines.find((l) =>
        idsAreEqual(l.id, line.orderLineId),
      );
      if (orderLine && 0 < orderLine.orderPlacedQuantity) {
        refundOrderLinesTotal +=
          line.quantity * orderLine.proratedUnitPriceWithTax;
      }
    }
