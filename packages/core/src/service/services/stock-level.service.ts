...
export class StockLevelService {
  ...
  async getStockLevelsForVariant(
    ctx: RequestContext,
    productVariantId: ID,
  ): Promise<StockLevel[]> {
  ...
  async getAvailableStock(
    ...
    return stockLocationStrategy.getAvailableStock(
      ctx,
      productVariantId,
      stockLevels,
    );
  ...
  async updateStockOnHandForLocation(
    ...
    if (stockLevel) {
      await this.connection
        .getRepository(ctx, StockLevel)
        .update(stockLevel.id, {
          stockOnHand: stockLevel.stockOnHand + change,
        });
    }
  }
  ...
  async updateStockAllocatedForLocation(
    ...
    if (stockLevel) {
      await this.connection
        .getRepository(ctx, StockLevel)
        .update(stockLevel.id, {
          stockAllocated: stockLevel.stockAllocated + change,
        });
    }
  }
}
