...
export class ProvinceService {
    ...
    return this.listQueryBuilder
      ...
      .then(([provinces, totalItems]) => {
        const items = provinces.map((province) =>
          this.translator.translate(province, ctx),
        );
        return {
          items,
          totalItems,
  ...
  findOne(
    ...
    return this.connection
      ...
      .then(
        (province) =>
          (province && this.translator.translate(province, ctx)) ?? undefined,
      );
  }
  ...
  async delete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
    const region = await this.connection.getEntityOrThrow(
      ctx,
      Province as Type<Province>,
      id,
    );
