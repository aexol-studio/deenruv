...
export class ProductService {
  ...
  async findAll(
    ...
    const hasFacetValueIdFilter =
      this.listQueryBuilder.filterObjectHasProperty<ProductFilterParameter>(
        options?.filter,
        "facetValueId",
      );
    const hasSkuFilter =
  ...
  async findOne(
    ...
    const product = await this.connection.findOneInChannel(
      ctx,
      Product,
      productId,
      ctx.channelId,
  ...
  async create(
    ...
    await this.eventBus.publish(
      new ProductEvent(ctx, product, "created", input),
    );
  ...
  async softDelete(
    ...
    for (const optionGroup of product.optionGroups) {
      if (!optionGroup.deletedAt) {
        const groupResult =
          await this.productOptionGroupService.deleteGroupAndOptionsFromProduct(
            ctx,
            optionGroup.id,
            productId,
          );
  ...
  async assignProductsToChannel(
    ...
    const assetIds: ID[] = unique(
      ([] as ID[]).concat(
        ...productsWithVariants.map((p) => p.assets.map((a) => a.assetId)),
      ),
    );
  ...
  async addOptionGroupToProduct(
    ...
    await this.connection
      ...
      .save(product, { reload: false });
    await this.eventBus.publish(
      new ProductOptionGroupChangeEvent(
        ctx,
        product,
        optionGroupId,
        "assigned",
  ...
  async removeOptionGroupFromProduct(
    ...
    ErrorResultUnion<RemoveOptionGroupFromProductResult, Translated<Product>>
  > {
    const product = await this.getProductWithOptionGroups(ctx, productId);
    const optionGroup = product.optionGroups.find((g) =>
      idsAreEqual(g.id, optionGroupId),
    );
    if (!optionGroup) {
      throw new EntityNotFoundError("ProductOptionGroup", optionGroupId);
    }
    const optionIsInUse = product.variants.some(
      (variant) =>
        ...
        variant.options.some((option) =>
          idsAreEqual(option.groupId, optionGroupId),
        ),
    );
    if (optionIsInUse) {
      if (!force) {
  ...
  private async getProductWithOptionGroups(
    ctx: RequestContext,
    productId: ID,
  ): Promise<Product> {
