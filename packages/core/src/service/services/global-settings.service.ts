...
import { UpdateGlobalSettingsInput } from "@deenruv/common/src/generated-types";
...
export class GlobalSettingsService {
  ...
  async initGlobalSettings() {
    try {
      ...
      if (1 < result.length) {
        // Strange edge case, see https://github.com/deenruv-ecommerce/deenruv/issues/987
  ...
  async getSettings(ctx: RequestContext): Promise<GlobalSettings> {
    const settings = await this.requestCache.get(
      ctx,
      CacheKey.GlobalSettings,
      () =>
        this.connection
