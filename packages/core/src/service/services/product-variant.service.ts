...
export class ProductVariantService {
  ...
  private findByProductIdsDataloader(
    ...
    return ctx.dataloader(
      this.dataloaderKey(
        `find_by_ids_product_${JSON.stringify(relations)}_${JSON.stringify(options)}`,
      ),
      async (keys: readonly ID[]) => {
        ...
        if (ctx.apiType === "shop") {
          ...
          countQb.andWhere("productvariant.enabled = :enabled", {
            enabled: true,
          });
        }
        ...
        const [variants, count] = await Promise.all([
          qb
            .getMany()
            .then((variants) =>
              this.applyPricesAndTranslateVariants(ctx, variants),
            ),
          countQb.getRawMany(),
        ]);
        return keys.map((k) => ({
      ...
      .then(async (result) => {
        if (result) {
          return this.translator.translate(
            await this.applyChannelPriceAndTax(result, ctx),
            ctx,
            ["product"],
          );
        }
  ...
  getVariantsByCollectionId(
    ...
    if (
      ...
      options.filter.enabled.eq === true
    ) {
   ...
   */
  async getProductVariantChannels(
    ctx: RequestContext,
    productVariantId: ID,
  ): Promise<Channel[]> {
  ...
  async getVariantByOrderLineId(
    ...
    return this.translator.translate(
      await this.applyChannelPriceAndTax(productVariant, ctx),
      ctx,
    );
  }
  ...
  getOptionsForVariant(
    ...
    return this.connection
      ...
      .then((variant) =>
        !variant
          ? []
          : variant.options.map((o) => this.translator.translate(o, ctx)),
      );
  }
  ...
  getFacetValuesForVariant(
    ...
    return this.connection
      ...
      .then((variant) =>
        !variant
          ? []
          : variant.facetValues.map((o) =>
              this.translator.translate(o, ctx, ["facet"]),
            ),
      );
  }
  ...
  async getProductForVariant(
    ...
    if (!variant.product) {
      product = await this.connection.getEntityOrThrow(
        ctx,
        Product,
        variant.productId,
        {
  ...
  async getSaleableStockLevel(
    ...
    const inventoryNotTracked =
      ...
      (variant.trackInventory === GlobalFlag.INHERIT &&
  ...
  private async getOutOfStockThreshold(
    ...
    if (inventoryNotTracked) {
      ...
      return variant.useGlobalOutOfStockThreshold
        ? outOfStockThreshold
        : variant.outOfStockThreshold;
    }
  }
   ...
   */
  async getFulfillableStockLevel(
    ctx: RequestContext,
    variant: ProductVariant,
  ): Promise<number> {
    const { outOfStockThreshold, trackInventory } =
  ...
  private async createSingle(
    ...
    input.taxCategoryId = (
      await this.getTaxCategoryForNewVariant(ctx, input.taxCategoryId)
    ).id;
    ...
    const createdVariant = await this.translatableSaver.create({
      ...
      beforeSave: async (variant) => {
        ...
        if (input.facetValueIds) {
          variant.facetValues = await this.facetValueService.findByIds(
            ctx,
            input.facetValueIds,
          );
        }
  ...
  private async updateSingle(
    ...
    const existingVariant = await this.connection.getEntityOrThrow(
      ctx,
      ProductVariant,
      input.id,
      {
    ...
    const outOfStockThreshold = await this.getOutOfStockThreshold(
      ctx,
      existingVariant,
    );
    ...
    if (input.optionIds) {
      await this.validateVariantOptionIds(
        ctx,
    ...
    const updatedVariant = await this.translatableSaver.update({
      ...
      beforeSave: async (v) => {
        if (input.taxCategoryId) {
          const taxCategory = await this.taxCategoryService.findOne(
            ctx,
            input.taxCategoryId,
          );
          if (taxCategory) {
        ...
        if (input.facetValueIds) {
          const facetValuesInOtherChannels = existingVariant.facetValues.filter(
            (fv) =>
              fv.channels.every(
                (channel) => !idsAreEqual(channel.id, ctx.channelId),
              ),
          );
          v.facetValues = [
  ...
  async createOrUpdateProductVariantPrice(
    ...
    const { productVariantPriceUpdateStrategy } =
    ...
    if (!targetPrice) {
      const createdPrice = await this.connection
        .getRepository(ctx, ProductVariantPrice)
        .save(
      ...
      additionalPricesToUpdate =
        await productVariantPriceUpdateStrategy.onPriceCreated(
          ctx,
          createdPrice,
          allPrices,
        );
      ...
      additionalPricesToUpdate =
        await productVariantPriceUpdateStrategy.onPriceUpdated(
          ctx,
          updatedPrice,
          allPrices,
        );
    }
    const uniqueAdditionalPricesToUpdate = unique(
      additionalPricesToUpdate,
      "id",
    ).filter(
      (p) =>
  ...
  async deleteProductVariantPrice(
    ...
    if (variantPrice) {
      ...
      const additionalPricesToUpdate =
        await productVariantPriceUpdateStrategy.onPriceDeleted(
          ctx,
          variantPrice,
          allPrices,
        );
   ...
   */
  async hydratePriceFields<
    F extends "currencyCode" | "price" | "priceWithTax" | "taxRateApplied",
  >(
    ctx: RequestContext,
    variant: ProductVariant,
    priceField: F,
    ...
    let populatePricesPromise = this.requestCache.get<Promise<ProductVariant>>(
      ctx,
      cacheKey,
    );
        ...
        try {
          if (!variant.productVariantPrices?.length) {
            ...
            variant.productVariantPrices =
              variantWithPrices.productVariantPrices;
          }
          if (!variant.taxCategory) {
            const variantWithTaxCategory =
          ...
          resolve(
            await this.applyChannelPriceAndTax(variant, ctx, undefined, true),
          );
        } catch (e: any) {
          reject(e);
        }
  ...
  private async applyPricesAndTranslateVariants(
    ...
    return await Promise.all(
      variants.map(async (variant) => {
        const variantWithPrices = await this.applyChannelPriceAndTax(
          variant,
          ctx,
        );
  ...
  async assignProductVariantsToChannel(
    ...
    const targetChannel = await this.connection.getEntityOrThrow(
      ctx,
      Channel,
      input.channelId,
    );
    for (const variant of variants) {
      ...
      const price = targetChannel.pricesIncludeTax
    ...
    for (const variant of variants) {
      await this.eventBus.publish(
        new ProductVariantChannelEvent(
          ctx,
          variant,
          input.channelId,
          "assigned",
  ...
  private async validateVariantOptionIds(
    ...
    const activeOptions =
      optionGroups && optionGroups.filter((group) => !group.deletedAt);
  ...
  private throwIncompatibleOptionsError(optionGroups: ProductOptionGroup[]) {
    throw new UserInputError(
      "error.product-variant-option-ids-not-compatible",
      {
