...
export class AdministratorService {
  ...
  async update(
    ...
    if (input.password) {
      ...
      if (user) {
        ...
        nativeAuthMethod.passwordHash = await this.passwordCipher.hash(
          input.password,
        );
        await this.connection
    ...
    if (input.roleIds) {
      ...
      if (isSoleSuperAdmin) {
        ...
        if (!input.roleIds.find((id) => idsAreEqual(id, superAdminRole.id))) {
          throw new InternalServerError(
            "error.superadmin-must-have-superadmin-role",
          );
        }
      }
      ...
      const addIds = (input.roleIds as ID[]).filter(
        (roleId) =>
          !administrator.user.roles.some((role) => role.id === roleId),
      );
      ...
      for (const roleId of input.roleIds) {
        updatedAdministrator = await this.assignRole(
          ctx,
          administrator.id,
          roleId,
        );
    ...
    for (const channelPermissions of permissionsRequired) {
      ...
      if (!activeUserHasRequiredPermissions) {
        throw new UserInputError(
          "error.active-user-does-not-have-sufficient-permissions",
        );
      }
    }
  }
  ...
  private async isSoleSuperadmin(ctx: RequestContext, id: ID) {
    const superAdminRole = await this.roleService.getSuperAdminRole(ctx);
    const allAdmins = await this.connection
      .getRepository(ctx, Administrator)
      .find({
