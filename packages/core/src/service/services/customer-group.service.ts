...
export class CustomerGroupService {
  ...
  async create(
    ...
    const newCustomerGroup = await this.connection
      .getRepository(ctx, CustomerGroup)
      .save(customerGroup);
    ...
    const savedCustomerGroup = await assertFound(
      this.findOne(ctx, newCustomerGroup.id),
    );
  ...
  async update(
    ...
    const customerGroup = await this.connection.getEntityOrThrow(
      ctx,
      CustomerGroup,
      input.id,
    );
    ...
    await this.connection
      .getRepository(ctx, CustomerGroup)
      .save(updatedCustomerGroup, { reload: false });
    await this.customFieldRelationService.updateRelations(
      ctx,
      CustomerGroup,
      input,
  ...
  async delete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
    const group = await this.connection.getEntityOrThrow(
      ctx,
      CustomerGroup,
      id,
    );
    ...
    for (const customer of customers) {
      if (!customer.groups.map((g) => g.id).includes(input.customerGroupId)) {
        throw new UserInputError(
          "error.customer-does-not-belong-to-customer-group",
        );
      }
      customer.groups = customer.groups.filter(
        (g) => !idsAreEqual(g.id, group.id),
      );
      await this.historyService.createHistoryEntryForCustomer({
        ctx,
        customerId: customer.id,
