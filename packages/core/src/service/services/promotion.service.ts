...
export class PromotionService {
  ...
  constructor(
    ...
    this.availableConditions =
      this.configService.promotionOptions.promotionConditions || [];
    this.availableActions =
      this.configService.promotionOptions.promotionActions || [];
  }
    ...
    return this.listQueryBuilder
      ...
      .then(([promotions, totalItems]) => {
        const items = promotions.map((promotion) =>
          this.translator.translate(promotion, ctx),
        );
        return {
          items,
          totalItems,
  ...
  async findOne(
    ...
    return this.connection
      ...
      .then(
        (promotion) =>
          (promotion && this.translator.translate(promotion, ctx)) ?? undefined,
      );
  }
  ...
  async createPromotion(
    ...
    await this.eventBus.publish(
      new PromotionEvent(ctx, promotionWithRelations, "created", input),
    );
  ...
  async updatePromotion(
    ...
    const promotion = await this.connection.getEntityOrThrow(
      ctx,
      Promotion,
      input.id,
      {
    ...
    const hasCouponCode =
      input.couponCode != null ? !!input.couponCode : !!promotion.couponCode;
    ...
    await this.eventBus.publish(
      new PromotionEvent(ctx, promotion, "updated", input),
    );
    return assertFound(this.findOne(ctx, updatedPromotion.id));
  }

  async softDeletePromotion(
    ctx: RequestContext,
    promotionId: ID,
  ...
  async assignPromotionsToChannel(
    ...
    for (const promotion of promotions) {
      await this.channelService.assignToChannels(ctx, Promotion, promotion.id, [
        input.channelId,
      ]);
    }
  ...
  async removePromotionsFromChannel(
    ctx: RequestContext,
  ...
  async validateCouponCode(
    ...
    if (customerId && promotion.perCustomerUsageLimit != null) {
      ...
      if (promotion.perCustomerUsageLimit <= usageCount) {
        return new CouponCodeLimitError({
          couponCode,
          limit: promotion.perCustomerUsageLimit,
        });
      }
    }
  ...
  getActivePromotionsInChannel(ctx: RequestContext) {
    return this.connection
      ...
      .getMany()
      .then((promotions) =>
        promotions.map((p) => this.translator.translate(p, ctx)),
      );
  }
  ...
  private calculatePriorityScore(
    ...
    const conditions = input.conditions
      ? input.conditions.map((c) =>
          this.configArgService.getByCode("PromotionCondition", c.code),
        )
      : [];
    const actions = input.actions
      ? input.actions.map((c) =>
          this.configArgService.getByCode("PromotionAction", c.code),
        )
      : [];
    return [...conditions, ...actions].reduce(
      (score, op) => score + op.priorityValue,
      0,
    );
  }
  ...
  private validateRequiredConditions(
    ...
    for (const { code: actionCode } of actions) {
      const actionDef = this.configArgService.getByCode(
        "PromotionAction",
        actionCode,
      );
      const actionDependencies: PromotionCondition[] =
        actionDef.conditions || [];
      if (!actionDependencies || actionDependencies.length === 0) {
        continue;
      }
      const missingConditions = actionDependencies.filter(
        (condition) => !conditionCodes[condition.code],
      );
