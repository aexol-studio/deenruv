...
export class RoleService {
  ...
  getChannelsForRole(ctx: RequestContext, roleId: ID): Promise<Channel[]> {
    return this.findOne(ctx, roleId).then((role) =>
      role ? role.channels : [],
    );
  }
  ...
  async userHasAnyPermissionsOnChannel(
    ...
    const permissionsOnChannel = await this.getActiveUserPermissionsOnChannel(
      ctx,
      channelId,
    );
  ...
  private async activeUserCanReadRole(
    ctx: RequestContext,
    role: Role,
  ): Promise<boolean> {
  ...
  private async getActiveUserPermissionsOnChannel(
    ...
    const userChannels = await this.requestContextCache.get(
      ...
      async () => {
        const user = await this.connection.getEntityOrThrow(
          ctx,
          User,
          activeUserId,
          {
  ...
  private checkPermissionsAreValid(permissions?: Permission[] | null) {
    ...
    for (const permission of permissions) {
      if (
        !allAssignablePermissions.includes(permission) ||
  ...
  private async ensureSuperAdminRoleExists() {
    ...
    try {
      ...
      await this.connection.rawConnection
        .getRepository(Role)
        .save(superAdminRole, { reload: false });
    } catch (err: any) {
