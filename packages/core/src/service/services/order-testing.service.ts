...
export class OrderTestingService {
  ...
  async testShippingMethod(
    ...
    const shippingMethod = new ShippingMethod({
      checker: this.configArgService.parseInput(
        "ShippingEligibilityChecker",
        input.checker,
      ),
      calculator: this.configArgService.parseInput(
        "ShippingCalculator",
        input.calculator,
      ),
    });
    const mockOrder = await this.buildMockOrder(
      ctx,
    ...
    const result = eligible
      ? await shippingMethod.apply(ctx, mockOrder)
      : undefined;
  ...
  async testEligibleShippingMethods(
    ...
    const eligibleMethods =
      await this.shippingCalculator.getEligibleShippingMethods(ctx, mockOrder);
