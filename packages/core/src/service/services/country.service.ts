...
export class CountryService {
    ...
    return this.listQueryBuilder
      ...
      .then(([countries, totalItems]) => {
        const items = countries.map((country) =>
          this.translator.translate(country, ctx),
        );
        return {
          items,
          totalItems,
  ...
  findOne(
    ...
    return this.connection
      ...
      .then(
        (country) =>
          (country && this.translator.translate(country, ctx)) ?? undefined,
      );
  }
  ...
  findAllAvailable(ctx: RequestContext): Promise<Array<Translated<Country>>> {
    return this.connection
      ...
      .then((items) =>
        items.map((country) => this.translator.translate(country, ctx)),
      );
  }
  ...
  async create(
    ...
    await this.eventBus.publish(
      new CountryEvent(ctx, country, "created", input),
    );
  ...
  async update(
    ...
    await this.eventBus.publish(
      new CountryEvent(ctx, country, "updated", input),
    );
    return assertFound(this.findOne(ctx, country.id));
  }
  ...
  async delete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
    ...
    if (0 < addressesUsingCountry) {
      return {
        ...
        message: ctx.translate("message.country-used-in-addresses", {
          count: addressesUsingCountry,
        }),
      };
    } else {
