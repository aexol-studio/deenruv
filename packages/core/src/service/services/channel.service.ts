...
import {
  ...
  SelfRefreshingCache,
} from "../../common/self-refreshing-cache";
import {
  ChannelAware,
  ListQueryOptions,
...
export class ChannelService {
   ...
   * @internal
   */
  async createCache(): Promise<
    SelfRefreshingCache<Channel[], [RequestContext]>
  > {
   ...
   * @private
   */
  private async getAssignedEntityChannels<
  ...
  >(
    ...
    if (!channelsRelation) {
      throw new InternalServerError(
        `Could not find the channels relation for entity ${metadata.name}`,
      );
    }

    const junctionTableName =
      channelsRelation.junctionEntityMetadata?.tableName;
    const junctionColumnName =
      channelsRelation.junctionEntityMetadata?.columns[0].databaseName;
  ...
  async getChannelFromToken(
    ...
    const [ctx, channelToken] =
      ctxOrToken instanceof RequestContext
        ? [ctxOrToken, token!]
  ...
  async create(
    ...
    const defaultLanguageValidationResult =
      await this.validateDefaultLanguageCode(ctx, input);
    ...
    const newChannel = await this.connection
      .getRepository(ctx, Channel)
      .save(channel);
    if (input.sellerId) {
      const seller = await this.connection.getEntityOrThrow(
        ctx,
        Seller,
        input.sellerId,
      );
    ...
    await this.eventBus.publish(
      new ChannelEvent(ctx, newChannel, "created", input),
    );
    return newChannel;
  }
  ...
  async update(
    ...
    if (input.defaultShippingZoneId) {
      updatedChannel.defaultShippingZone =
    ...
    if (
      input.availableCurrencyCodes &&
      !updatedChannel.availableCurrencyCodes.includes(
        updatedChannel.defaultCurrencyCode,
      )
    ) {
      throw new UserInputError(
        `error.available-currency-codes-must-include-default`,
        {
    ...
    await this.connection
      .getRepository(ctx, Channel)
      .save(updatedChannel, { reload: false });
    await this.customFieldRelationService.updateRelations(
      ctx,
      Channel,
      input,
    ...
    await this.allChannels.refresh(ctx);
    await this.eventBus.publish(
      new ChannelEvent(ctx, channel, "updated", input),
    );
  ...
  async delete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
    ...
    await this.eventBus.publish(
      new ChannelEvent(ctx, deletedChannel, "deleted", id),
    );
  ...
  private async ensureDefaultChannelExists() {
    ...
    let defaultChannel = await this.connection.rawConnection
    ...
    if (!defaultChannel.seller) {
      ...
      if (seller.length === 0) {
        throw new InternalServerError(
          "No Sellers were found. Could not initialize default Channel.",
        );
      }
  ...
  private async validateDefaultLanguageCode(
    ...
    if (input.defaultLanguageCode) {
      ...
      if (!availableLanguageCodes.includes(input.defaultLanguageCode)) {
        return new LanguageNotAvailableError({
          languageCode: input.defaultLanguageCode,
        });
      }
    }
  }
