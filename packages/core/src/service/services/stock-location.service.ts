...
export class StockLocationService {
  ...
  async create(
    ...
    const stockLocation = await this.connection
  ...
  async delete(
    ...
    const allStockLocations = await this.connection
      .getRepository(ctx, StockLocation)
      .find();
    ...
    if (input.transferToLocationId) {
      ...
      for (const stockLevel of stockLevelsToTransfer) {
        const existingStockLevel = await this.connection
          .getRepository(ctx, StockLevel)
          .findOne({
            where: {
  ...
  getAllStockLocations(ctx: RequestContext) {
    return this.requestContextCache.get(
      ctx,
      "StockLocationService.getAllStockLocations",
  ...
  async getSaleLocations(
    ...
    const { stockLocationStrategy } = this.configService.catalogOptions;
    const stockLocations = await this.getAllStockLocations(ctx);
    const saleLocations = await stockLocationStrategy.forSale(
      ctx,
      stockLocations,
      orderLine,
      quantity,
  ...
  async getCancellationLocations(
    ctx: RequestContext,
    orderLine: OrderLine,
    quantity: number,
  ) {
  ...
  private async ensureDefaultStockLocationExists() {
    ...
    if (stockLocations.length === 0) {
      const defaultStockLocation = await this.connection
        .getRepository(ctx, StockLocation)
        .save(
          new StockLocation({
