...
export class ZoneService {
  ...
  async findAll(
    ctx: RequestContext,
    options?: ListQueryOptions<Zone>,
  ): Promise<PaginatedList<Zone>> {
    return this.listQueryBuilder
      ...
      .then(([items, totalItems]) => {
        const translated = items.map((zone, i) => {
          ...
          cloneZone.members = zone.members.map((country) =>
  ...
  async delete(ctx: RequestContext, id: ID): Promise<DeletionResponse> {
    ...
    if (0 < taxRatesUsingZone.length) {
      ...
      await this.eventBus.publish(
        new ZoneEvent(ctx, deletedZone, "deleted", id),
      );
      return {
  ...
  async removeMembersFromZone(
    ...
    zone.members = zone.members.filter(
      (country) => !memberIds.includes(country.id),
    );
    await this.connection
      .getRepository(ctx, Zone)
      .save(zone, { reload: false });
