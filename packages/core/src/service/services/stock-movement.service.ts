...
export class StockMovementService {
  ...
  async adjustProductVariantStock(
      ...
      stockOnHandInputs = [
        {
          ...
          stockOnHand: stockOnHandNumberOrInput,
        },
      ];
    } else {
    ...
    for (const input of stockOnHandInputs) {
      ...
      const adjustment = await this.connection
  ...
  async createAllocationsForOrderLines(
    ...
    for (const { orderLineId, quantity } of lines) {
      ...
      const allocationLocations =
        await this.stockLocationService.getAllocationLocations(
          ctx,
          orderLine,
          quantity,
        );
      for (const allocationLocation of allocationLocations) {
        ...
        if (
          this.trackInventoryForVariant(productVariant, globalTrackInventory)
        ) {
    ...
    const savedAllocations = await this.connection
  ...
  async createSalesForOrder(
    ...
    const savedSales = await this.connection
  ...
  async createCancellationsForOrderLines(
    ...
    for (const orderLine of orderLines) {
      const lineInput = lineInputs.find((l) =>
        idsAreEqual(l.orderLineId, orderLine.id),
      );
      if (!lineInput) {
        continue;
      }
      const cancellationLocations =
        await this.stockLocationService.getCancellationLocations(
          ctx,
          orderLine,
          lineInput.quantity,
        );
    ...
    const savedCancellations = await this.connection
   ...
   */
  async createReleasesForOrderLines(
    ctx: RequestContext,
    lineInputs: OrderLineInput[],
  ): Promise<Release[]> {
    ...
    for (const orderLine of orderLines) {
      ...
      const releaseLocations =
        await this.stockLocationService.getReleaseLocations(
          ctx,
          orderLine,
          lineInput.quantity,
        );
    ...
    const savedReleases = await this.connection
