...
export class UserService {
  ...
  async createCustomerUser(
    ...
    const addNativeAuthResult = await this.addNativeAuthenticationMethod(
      ctx,
      user,
      identifier,
      password,
  ...
  async softDelete(ctx: RequestContext, userId: ID) {
    ...
    await this.connection
      .getRepository(ctx, User)
      .update({ id: userId }, { deletedAt: new Date() });
  }
  ...
  async verifyUserByToken(
    ...
    const user = await this.connection
      ...
      .where("authenticationMethod.verificationToken = :verificationToken", {
        verificationToken,
      })
      .getOne();
    if (user) {
  ...
  > {
    const user = await this.connection
      ...
      .leftJoinAndSelect("user.authenticationMethods", "aums")
      .leftJoin("user.authenticationMethods", "authenticationMethod")
      .where("authenticationMethod.passwordResetToken = :passwordResetToken", {
        passwordResetToken,
      })
      .getOne();
    if (!user) {
   ...
   */
  async changeUserAndNativeIdentifier(
    ctx: RequestContext,
    userId: ID,
   ...
   * flow.
   */
  async setIdentifierChangeToken(
    ctx: RequestContext,
    user: User,
  ): Promise<User> {
    const nativeAuthMethod = user.getNativeAuthenticationMethod();
  ...
  > {
    const user = await this.connection
      ...
      .leftJoin("user.authenticationMethods", "authenticationMethod")
      .where(
        "authenticationMethod.identifierChangeToken = :identifierChangeToken",
        {
  ...
  async updatePassword(
    ...
    const nativeAuthMethod = user.getNativeAuthenticationMethod();
    const matches = await this.passwordCipher.check(
      currentPassword,
      nativeAuthMethod.passwordHash,
    );
    if (!matches) {
  ...
  private async validatePassword(
    ...
    const passwordValidationResult =
      await this.configService.authOptions.passwordValidationStrategy.validate(
        ctx,
        password,
      );
    if (passwordValidationResult !== true) {
