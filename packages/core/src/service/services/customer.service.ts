...
export class CustomerService {
  ...
  findAll(
    ...
    const hasPostalCodeFilter =
      this.listQueryBuilder.filterObjectHasProperty<CustomerFilterParameter>(
        options?.filter,
        "postalCode",
      );
  ...
  findOneByUserId(
    ...
    if (filterOnChannel) {
      query = query.andWhere("channel.id = :channelId", {
        channelId: ctx.channelId,
      });
    }
  ...
  async create(
    ...
    if (password && password !== "") {
      const verificationToken =
        customer.user.getNativeAuthenticationMethod().verificationToken;
  ...
  async update(
    ctx: RequestContext,
    input: UpdateCustomerShopInput & { id: ID },
  ): Promise<Customer>;
  async update(
    ctx: RequestContext,
      ...
      if (input.emailAddress !== customer.emailAddress) {
        ...
        if (customer.user) {
          const existingUserWithEmailAddress =
            await this.userService.getUserByEmailAddress(
              ctx,
              input.emailAddress,
              "customer",
            );
  ...
  > {
    ...
    let user = await this.userService.getUserByEmailAddress(
      ctx,
      input.emailAddress,
    );
  ...
  async requestUpdateEmailAddress(
    ...
    const userWithConflictingIdentifier =
      await this.userService.getUserByEmailAddress(ctx, newEmailAddress);
    ...
    if (this.configService.authOptions.requireVerification) {
      user.getNativeAuthenticationMethod().pendingIdentifier =
  ...
  async createOrUpdate(
    ...
    if (existing) {
      ...
      customer.channels.push(
        await this.connection.getEntityOrThrow(ctx, Channel, ctx.channelId),
      );
    } else {
  ...
  async createAddressesForNewCustomer(ctx: RequestContext, order: Order) {
    ...
    const addresses = await this.findAddressesByCustomerId(
      ctx,
      order.customer.id,
    );
    ...
    if (addresses.length === 0 && order.shippingAddress?.country) {
      ...
      const hasSeparateBillingAddress =
        ...
        !this.addressesAreEqual(shippingAddress, billingAddress);
  ...
  private async enforceSingleDefaultAddress(
    ...
    const result = await this.connection.getRepository(ctx, Address).findOne({
      where: { id: addressId },
   ...
   * any.
   */
  private async reassignDefaultsForDeletedAddress(
    ctx: RequestContext,
    addressToDelete: Address,
  ) {
    if (
      !addressToDelete.defaultBillingAddress &&
      !addressToDelete.defaultShippingAddress
    ) {
      return;
    }
    const result = await this.connection.getRepository(ctx, Address).findOne({
