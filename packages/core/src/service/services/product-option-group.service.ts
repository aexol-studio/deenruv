...
export class ProductOptionGroupService {
  ...
  findAll(
    ...
    return this.connection
      ...
      .then((groups) =>
        groups.map((group) =>
          this.translator.translate(group, ctx, ["options"]),
        ),
      );
  }
  ...
  findOne(
    ...
    return this.connection
      ...
      .then(
        (group) =>
          (group && this.translator.translate(group, ctx, ["options"])) ??
          undefined,
      );
  }
   ...
   */
  async deleteGroupAndOptionsFromProduct(
    ctx: RequestContext,
    id: ID,
    productId: ID,
  ) {
    ...
    const optionsToDelete =
      optionGroup.options &&
      optionGroup.options.filter((group) => !group.deletedAt);

    for (const option of optionsToDelete) {
      const { result, message } = await this.productOptionService.delete(
        ctx,
        option.id,
      );
    ...
    const hasOptionsWhichAreInUse = await this.groupOptionsAreInUse(
      ctx,
      optionGroup,
    );
  ...
  private async groupOptionsAreInUse(
    ctx: RequestContext,
    productOptionGroup: ProductOptionGroup,
  ) {
    return this.connection
