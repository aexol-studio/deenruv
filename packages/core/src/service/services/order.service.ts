...
export class OrderService {
  ...
  getOrderProcessStates(): OrderProcessState[] {
    return Object.entries(this.orderStateMachine.config.transitions).map(
      ([name, { to }]) => ({
        name,
        to,
      }),
    ...
    qb.expressionMap.joinAttributes.forEach((join) => {
      if (!join.condition) {
        ...
        if (inverseTarget === FulfillmentLine) {
          const lines = qb.expressionMap.joinAttributes.find(
            (join) =>
              join.relationCache?.inverseEntityMetadata.target === OrderLine,
          );
          if (lines) {
  ...
  async findOne(
    ...
    if (effectiveRelations.includes("lines")) {
      qb.addOrderBy(
        `order__order_lines.${qb.escape("createdAt")}`,
        "ASC",
      ).addOrderBy(
  ...
  async findByCustomerId(
    ...
    const effectiveRelations = (
      relations ?? ["lines", "customer", "channels", "shippingLines"]
    ).filter(
      (r) =>
  ...
  async create(ctx: RequestContext, userId?: ID): Promise<Order> {
    ...
    const transitionResult = await this.transitionToState(
      ctx,
      order.id,
      "AddingItems",
    );
  ...
  private async createEmptyOrderEntity(ctx: RequestContext) {
    return new Order({
      ...
      code: await this.configService.orderOptions.orderCodeStrategy.generate(
        ctx,
      ),
  ...
  async updateOrderCustomer(
    ...
    const missingChannelIds = channelIds.filter(
      (id) => !customerChannelIds.includes(id),
    );
    ...
    const updatedOrder = await this.addCustomerToOrder(
      ctx,
      order.id,
      targetCustomer,
    );
  ...
  async addItemsToOrder(
    ...
    for (const item of items) {
      ...
      const correctedQuantity =
        await this.orderModifier.constrainQuantityToSaleable(
          ctx,
          variant,
          quantity,
      ...
      for (const middleware of orderMiddlewares) {
        if (middleware.shouldAddItemToOrder) {
          ...
          if (error) {
            errorResults.push(
              new OrderMiddlewareError({ middlewareError: error }),
            );
            shouldContinueOuterLoopNextInteration = true;
          }
        }
      }
      if (shouldContinueOuterLoopNextInteration) continue;
      ...
      if (correctedQuantity < quantity) {
        const newQuantity =
          (existingOrderLine ? existingOrderLine?.quantity : 0) +
          correctedQuantity;
    ...
    const updatedOrder = await this.applyPriceAdjustments(
      ctx,
      order,
      updatedOrderLines,
      relations,
  ...
  async adjustOrderLines(
    ...
    for (const line of lines) {
      ...
      const validationError =
        this.assertAddingItemsState(order) ||
        this.assertQuantityIsPositive(quantity) ||
        this.assertNotOverOrderItemsLimit(
          order,
      ...
      if (correctedQuantity === 0) {
        order.lines = order.lines.filter(
          (l) => !idsAreEqual(l.id, orderLine.id),
        );
  ...
  async removeItemsFromOrder(
    ...
    for (const orderLineId of orderLineIds) {
      ...
      for (const middleware of orderMiddlewares) {
        if (middleware.shouldRemoveItemFromOrder) {
          const error = await middleware.shouldRemoveItemFromOrder(
            ctx,
            order,
            orderLine,
          );
          if (error) {
            return new OrderMiddlewareError({ middlewareError: error });
          }
        }
      }
    ...
    order.lines = order.lines.filter(
      (line) => !orderLineIds.find((olId) => idsAreEqual(line.id, olId)),
    );
  ...
  async addSurchargeToOrder(
    ctx: RequestContext,
    orderId: ID,
    surchargeInput: Partial<
      Omit<Surcharge, "id" | "createdAt" | "updatedAt" | "order">
    >,
  ): Promise<Order> {
    const order = await this.getOrderOrThrow(ctx, orderId);
   ...
   */
  async removeSurchargeFromOrder(
    ctx: RequestContext,
    orderId: ID,
    surchargeId: ID,
  ): Promise<Order> {
    const order = await this.getOrderOrThrow(ctx, orderId);
    const surcharge = await this.connection.getEntityOrThrow(
      ctx,
      Surcharge,
      surchargeId,
    );
    if (order.surcharges.find((s) => idsAreEqual(s.id, surcharge.id))) {
      order.surcharges = order.surcharges.filter(
        (s) => !idsAreEqual(s.id, surchargeId),
      );
      const updatedOrder = await this.applyPriceAdjustments(ctx, order);
  ...
  async applyCouponCode(
    ...
    for (const middleware of orderMiddlewares) {
      if (middleware.shouldAddCouponCode) {
        const error = await middleware.shouldAddCouponCode(
          ctx,
          order,
          couponCode,
        );
  ...
  async setShippingMethod(
    ...
    const result = await this.orderModifier.setShippingMethods(
      ctx,
      order,
      shippingMethodIds,
    );
  ...
  async transitionRefundToState(
    ...
    const refund = await this.connection.getEntityOrThrow(
      ctx,
      Refund,
      refundId,
      {
  ...
  async createFulfillment(
    ...
    if (
      await this.requestedFulfillmentQuantityExceedsLineQuantity(ctx, input)
    ) {
      return new ItemsAlreadyFulfilledError();
    }

    const stockCheckResult = await this.ensureSufficientStockForFulfillment(
      ctx,
      input,
    );
    ...
    for (const inputLine of input.lines) {
      ...
      if (existingFulfillmentLine) {
        const unfulfilledQuantity =
          existingFulfillmentLine.orderLine.quantity -
  ...
  private async ensureSufficientStockForFulfillment(
    ...
    for (const line of lines) {
      const lineInput = input.lines.find((l) =>
        idsAreEqual(l.orderLineId, line.id),
      )!;

      const fulfillableStockLevel =
        await this.productVariantService.getFulfillableStockLevel(
          ctx,
          line.productVariant,
        );
      if (fulfillableStockLevel < lineInput.quantity) {
        ...
        const productVariant = this.translator.translate(
          line.productVariant,
          ctx,
        );
  ...
  async getOrderFulfillments(
    ...
    const { fulfillments } = await this.connection.getEntityOrThrow(
      ctx,
      Order,
      order.id,
      {
  ...
  async cancelOrder(
    ...
    const cancelResult =
      input.lines != null
        ? await this.orderModifier.cancelOrderByOrderLines(
            ctx,
            input,
            input.lines,
          )
  ...
  async refundOrder(
    ...
    const orders = await getOrdersFromLines(
      ctx,
      this.connection,
      input.lines ?? [],
    );
    ...
    if (
      orders &&
      orders.length &&
      !idsAreEqual(payment.order.id, orders[0].id)
    ) {
    ...
    const createdRefund = await this.paymentService.createRefund(
      ctx,
      input,
      order,
      payment,
  ...
  async deleteOrder(ctx: RequestContext, orderOrId: ID | Order) {
    ...
    // reference in order to avoid a foreign key error. See https://github.com/aexol-studio/deenruv/issues/1454
  ...
  async mergeOrders(
    ...
    if (guestOrder && guestOrder.customer) {
      ...
      // so we do not want to merge at all. See https://github.com/aexol-studio/deenruv/issues/263
      return existingOrder;
    }
    const mergeResult = this.orderMerger.merge(ctx, guestOrder, existingOrder);
    const { orderToDelete, linesToInsert, linesToDelete, linesToModify } =
      mergeResult;
    ...
    if (updatedOrderLines?.length) {
      ...
      for (const updatedOrderLine of updatedOrderLines) {
        ...
        const initialListPrice =
          updatedOrderLine.initialListPrice ?? priceResult.price;
