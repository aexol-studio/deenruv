...
export class PaymentMethodService {
  ...
  async update(
    ...
    await this.connection
      ...
      .save(updatedPaymentMethod, { reload: false });
  ...
  async delete(
    ...
    const paymentMethod = await this.connection.getEntityOrThrow(
      ctx,
      PaymentMethod,
      paymentMethodId,
      {
    ...
    if (ctx.channel.code === DEFAULT_CHANNEL_CODE) {
      ...
      if (0 < nonDefaultChannels.length && !force) {
        const message = ctx.translate(
          "message.payment-method-used-in-channels",
          {
      ...
      paymentMethod.channels = paymentMethod.channels.filter(
        (c) => !idsAreEqual(c.id, ctx.channelId),
      );
      await this.connection
        .getRepository(ctx, PaymentMethod)
        .save(paymentMethod);
  ...
  async assignPaymentMethodsToChannel(
    ...
    const hasPermission = await this.roleService.userHasAnyPermissionsOnChannel(
      ctx,
      input.channelId,
      [Permission.UpdatePaymentMethod, Permission.UpdateSettings],
    );
    if (!hasPermission) {
  ...
  async removePaymentMethodsFromChannel(
    ...
    const hasPermission = await this.roleService.userHasAnyPermissionsOnChannel(
      ctx,
      input.channelId,
      [Permission.DeletePaymentMethod, Permission.DeleteSettings],
    );
    if (!hasPermission) {
      throw new ForbiddenError();
    }
  ...
  getPaymentMethodHandlers(
    ...
    return this.configArgService
      .getDefinitions("PaymentMethodHandler")
      .map((x) => x.toGraphQlType(ctx));
  }
  ...
  async getMethodAndOperations(
    ...
    const handler = this.configArgService.getByCode(
      "PaymentMethodHandler",
      paymentMethod.handler.code,
    );
    const checker =
      paymentMethod.checker &&
      this.configArgService.getByCode(
        "PaymentMethodEligibilityChecker",
        paymentMethod.checker.code,
      );
