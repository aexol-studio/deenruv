...
export class EventBus implements OnModuleDestroy {
   ...
   * data related to the event.
   */
  filter<T extends DeenruvEvent>(
    predicate: (event: DeenruvEvent) => boolean,
  ): Observable<T> {
    return this.eventStream.asObservable().pipe(
      takeUntil(this.destroy$),
   ...
   * @since 2.2.0
   */
  registerBlockingEventHandler<T extends DeenruvEvent>(
  ...
  private async executeBlockingEventHandlers<T extends DeenruvEvent>(
    event: T,
  ): Promise<void> {
  ...
  private async awaitActiveTransactions<T extends DeenruvEvent>(
    ...
    const transactionManager: EntityManager | undefined = (ctx as any)[
