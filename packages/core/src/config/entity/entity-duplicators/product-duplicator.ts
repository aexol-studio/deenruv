...
export const productDuplicator = new EntityDuplicator({
  ...
  async duplicate({ ctx, id, args }) {
    ...
    const translations: ProductTranslationInput[] = product.translations.map(
      (translation) => {
        return {
    ...
    if (args.includeVariants) {
      const productVariants = await connection
      ...
      if (product.optionGroups && product.optionGroups.length) {
        for (const optionGroup of product.optionGroups) {
          ...
          const options: CreateProductOptionInput[] = optionGroup.options.map(
            (option) => {
              return {
                code: option.code,
          ...
          if (options && options.length) {
            for (const option of options) {
              const newOption = await productOptionService.create(
                ctx,
                newOptionGroup,
                option,
              );
      ...
      const variantInput: CreateProductVariantInput[] = productVariants.map(
        (variant, i) => {
          ...
          const price =
            variant.productVariantPrices.find((p) =>
              idsAreEqual(p.channelId, ctx.channelId),
            )?.price ?? variant.productVariantPrices[0]?.price;
          return {
      ...
      const duplicatedProductVariants = await productVariantService.create(
        ctx,
        variantInput,
      );
