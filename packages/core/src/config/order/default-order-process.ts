...
export function configureDefaultOrderProcess(
  ...
  const orderProcess: OrderProcess<OrderState> = {
    ...
    async init(injector) {
      ...
      const EventBus = await import("../../event-bus/index.js").then(
        (m) => m.EventBus,
      );
      const StockMovementService = await import("../../service/index.js").then(
        (m) => m.StockMovementService,
      );
      const StockLevelService = await import("../../service/index.js").then(
        (m) => m.StockLevelService,
      );
      const HistoryService = await import("../../service/index.js").then(
        (m) => m.HistoryService,
      );
      const OrderSplitter = await import("../../service/index.js").then(
        (m) => m.OrderSplitter,
      );
    ...
    async onTransitionStart(fromState, toState, { ctx, order }) {
      if (
        options.checkModificationPayments !== false &&
        fromState === "Modifying"
      ) {
      ...
      if (toState === "ArrangingPayment") {
        if (
          options.arrangingPaymentRequiresContents !== false &&
          order.lines.length === 0
        ) {
          return "message.cannot-transition-to-payment-when-order-is-empty";
        }
        if (
          options.arrangingPaymentRequiresCustomer !== false &&
          !order.customer
        ) {
        ...
        if (options.arrangingPaymentRequiresStock !== false) {
          ...
          for (const line of order.lines) {
            const availableStock =
              await productVariantService.getSaleableStockLevel(
                ctx,
                line.productVariant,
              );
      ...
      if (options.checkPaymentsCoverTotal !== false) {
        if (toState === "PaymentAuthorized") {
          const hasAnAuthorizedPayment = !!order.payments.find(
            (p) => p.state === "Authorized",
          );
          if (
            !orderTotalIsCovered(order, ["Authorized", "Settled"]) ||
            !hasAnAuthorizedPayment
          ) {
    ...
    async onTransitionEnd(fromState, toState, data) {
      ...
      const { stockAllocationStrategy, orderPlacedStrategy } =
      ...
      const shouldAllocateStock =
        await stockAllocationStrategy.shouldAllocateStock(
          ctx,
          fromState,
          toState,
          order,
