...
export interface OrderMiddleware extends InjectableStrategy {
    ...
    shouldAddCouponCode?(ctx: RequestContext, order: Order, couponCode: string): Promise<void | string>;
    shouldRemoveCouponCode?(ctx: RequestContext, order: Order, couponCode: string): Promise<void | string>;
}