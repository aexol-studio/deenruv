  ...
  implements OnApplicationBootstrap, OnApplicationShutdown
{
  ...
  private getInjectableStrategies(): InjectableStrategy[] {
    ...
    const { taxZoneStrategy, taxLineCalculationStrategy } =
      this.configService.taxOptions;
    const { jobQueueStrategy, jobBufferStorageStrategy } =
      this.configService.jobQueueOptions;
    const {
      mergeStrategy,
    ...
    const { customPaymentProcess, process: paymentProcess } =
    ...
    return [
      ...
      assetImportStrategy,
      changedPriceHandlingStrategy,
      ...(Array.isArray(activeOrderStrategy)
        ? activeOrderStrategy
        : [activeOrderStrategy]),
      orderSellerStrategy,
      shippingLineAssignmentStrategy,
  ...
  private getConfigurableOperations(): Array<ConfigurableOperationDef<any>> {
    const { paymentMethodHandlers, paymentMethodEligibilityCheckers } =
      this.configService.paymentOptions;
    const { collectionFilters } = this.configService.catalogOptions;
    const { entityDuplicators } = this.configService.entityOptions;
    const { promotionActions, promotionConditions } =
      this.configService.promotionOptions;
    const {
      shippingCalculators,
