...
describe("normalizeEmailAddress()", () => {
  ...
  it("should lowercase email addresses", async () => {
    expect(normalizeEmailAddress("JoeSmith@test.com")).toBe(
      "joesmith@test.com",
    );
    expect(normalizeEmailAddress("TEST@TEST.COM")).toBe("test@test.com");
    expect(normalizeEmailAddress("test.person@TEST.COM")).toBe(
      "test.person@test.com",
    );
    expect(normalizeEmailAddress("test.person+Extra@TEST.COM")).toBe(
      "test.person+extra@test.com",
    );
    expect(normalizeEmailAddress("TEST-person+Extra@TEST.COM")).toBe(
      "test-person+extra@test.com",
    );
...
describe("isEmailAddressLike()", () => {
  it("returns true for valid email addresses", () => {
    ...
    expect(
      isEmailAddressLike("disposable.style.email.with+symbol@example.com"),
    ).toBe(true);
    expect(isEmailAddressLike("other.email-with-hyphen@example.com")).toBe(
      true,
    );
    ...
    expect(isEmailAddressLike("example-indeed@strange-example.inininini")).toBe(
      true,
    );
  });
