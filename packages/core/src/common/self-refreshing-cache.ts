import { type Json } from "@deenruv/common/shared-types";
...
export async function createSelfRefreshingCache<V, RefreshArgs extends any[]>(
  ...
  const initialValue: V = await refresh.fn(
    ...(refreshArgs ?? refresh.defaultArgs),
  );
  ...
  const refreshValue = (
    ...
    return refresh
      ...
      .catch((err: any) => {
        ...
        const message =
          typeof _message === "string" ? _message : JSON.stringify(err.message);
        Logger.error(
  ...
  const memoize = async <Args extends any[], R>(
    ...
    const result = getValue(_refreshArgs, false).then((val) =>
      fn(val, ...args),
    );
    memoCache.set(key, {
      expires: now + ttl,
