...
export class RequestContext {
  ...
  private readonly _dataloaders: Record<
    string,
    DataLoader<unknown, unknown, unknown>
  >;
  ...
  constructor(options: {
    ...
    this._languageCode =
      languageCode || (channel && channel.defaultLanguageCode);
    this._currencyCode =
      currencyCode || (channel && channel.defaultCurrencyCode);
  ...
  static deserialize(ctxObject: SerializedRequestContext): RequestContext {
    return new RequestContext({
      ...
      session: {
        ...ctxObject._session,
        expires:
          ctxObject._session?.expires && new Date(ctxObject._session.expires),
      },
  ...
  userHasPermissions(permissions: Permission[]): boolean {
    ...
    const permissionsOnChannel = user.channelPermissions.find((c) =>
      idsAreEqual(c.id, this.channelId),
    );
  ...
  dataloader<K, V, C = K>(
    ...
    const loader =
      (this._dataloaders[key] as undefined | DataLoader<K, V, C>) ||
      new DataLoader(fetchFunc, opts);
