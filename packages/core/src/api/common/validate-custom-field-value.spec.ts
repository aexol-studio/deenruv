...
describe("validateCustomFieldValue()", () => {
  ...
  async function assertThrowsError(
    validateFn: (() => Promise<void>) | (() => void),
    message: string,
  ) {
    try {
      await validateFn();
  ...
  describe("string & localeString", () => {
    ...
    it("throws on invalid pattern", async () => {
      await assertThrowsError(
        validate(""),
        "error.field-invalid-string-pattern",
      );
      await assertThrowsError(
        validate("foo"),
        "error.field-invalid-string-pattern",
  ...
  describe("string options", () => {
    ...
    it("throws on invalid option", async () => {
      await assertThrowsError(
        validate("SMALL"),
        "error.field-invalid-string-option",
      );
      await assertThrowsError(
        validate(""),
        "error.field-invalid-string-option",
  ...
  describe("int & float", () => {
    ...
    it("throws on invalid range", async () => {
      await assertThrowsError(
        validate(4),
        "error.field-invalid-number-range-min",
      );
      await assertThrowsError(
        validate(11),
        "error.field-invalid-number-range-max",
      );
      await assertThrowsError(
        validate(-7),
        "error.field-invalid-number-range-min",
