...
export class CustomFieldRelationResolverService {
  ...
  ): Promise<DeenruvEntity | DeenruvEntity[]> {
  ...
  async translateEntity(
    ...
    if (fieldDef.entity === ProductVariant) {
      if (Array.isArray(result)) {
        await Promise.all(
          result.map((r) => this.applyVariantPrices(ctx, r as any)),
        );
      } else {
    ...
    const translated: any = Array.isArray(result)
      ? result.map((r) =>
          this.isTranslatable(r) ? this.translator.translate(r, ctx) : r,
        )
  ...
  private async applyVariantPrices(
    ctx: RequestContext,
