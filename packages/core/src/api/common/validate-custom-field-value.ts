...
export async function validateCustomFieldValue(
  ...
  if (config.readonly) {
    throw new UserInputError("error.field-invalid-readonly", {
      name: config.name,
    });
  }
  ...
  if (config.requiresPermission) {
    if (!userHasPermissionsOnCustomField(ctx, config)) {
      throw new UserInputError("error.field-invalid-no-permission", {
        name: config.name,
      });
    }
  }
  ...
  await validateCustomFunction(
    config as TypedCustomFieldConfig<any, any>,
    value,
    injector,
    ctx,
  );
...
async function validateCustomFunction<
  T extends TypedCustomFieldConfig<any, any>,
  ...
  if (typeof config.validate === "function") {
    ...
    if (Array.isArray(error)) {
      const localizedError =
        error.find((e) => e.languageCode === ctx.languageCode) || error[0];
...
function validateNumberField(
  config: IntCustomFieldConfig | FloatCustomFieldConfig,
  value: number,
): void {
  const { min, max } = config;
  if (min != null && value < min) {
    throw new UserInputError("error.field-invalid-number-range-min", {
      name: config.name,
      value,
      min,
    });
  ...
  if (max != null && max < value) {
    throw new UserInputError("error.field-invalid-number-range-max", {
      name: config.name,
      value,
      max,
    });
...
function validateDateTimeField(
  config: DateTimeCustomFieldConfig,
  value: string,
): void {
  const { min, max } = config;
