...
import { AuthOptions } from "../../config/deenruv-config";
...
export function setSessionToken(options: {
  ...
  const usingCookie =
    ...
    (Array.isArray(authOptions.tokenMethod) &&
      authOptions.tokenMethod.includes("cookie"));
  const usingBearer =
    authOptions.tokenMethod === "bearer" ||
    (Array.isArray(authOptions.tokenMethod) &&
      authOptions.tokenMethod.includes("bearer"));
