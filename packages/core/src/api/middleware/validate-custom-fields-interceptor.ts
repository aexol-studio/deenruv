...
import { getGraphQlInputName } from "@deenruv/common/shared-utils";
import {
  GraphQLInputType,
  GraphQLList,
  GraphQLNonNull,
...
export class ValidateCustomFieldsInterceptor implements NestInterceptor {
  ...
  constructor(
    ...
    this.inputsWithCustomFields = Object.keys(
      configService.customFields,
    ).reduce((inputs, entityName) => {
  ...
  async intercept(context: ExecutionContext, next: CallHandler<any>) {
    ...
    if (parsedContext.isGraphQL) {
      ...
      const ctx: RequestContext = (parsedContext.req as any)[
        REQUEST_CONTEXT_KEY
      ];
  ...
  private async validateInput(
    ...
    if (variableValues) {
      ...
      const customFieldConfig =
        this.configService.customFields[entityName as keyof CustomFields];
  ...
  private async validateCustomFieldsObject(
    ...
    for (const [key, value] of Object.entries(customFieldsObject)) {
      const config = customFieldConfig.find(
        (c) => getGraphQlInputName(c) === key,
      );
      if (config) {
