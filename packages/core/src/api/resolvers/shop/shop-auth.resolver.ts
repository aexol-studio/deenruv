...
export class ShopAuthResolver extends BaseAuthResolver {
  ...
  async registerCustomerAccount(
    ...
    const result = await this.customerService.registerCustomerAccount(
      ctx,
      args.input,
    );
  ...
  async resetPassword(
    ...
    const { token, password } = args;
    const resetResult = await this.customerService.resetPassword(
      ctx,
      token,
      password,
    );
    ...
    if (
      isGraphQlErrorResult(authResult) &&
      authResult.__typename === "NotVerifiedError"
    ) {
      return authResult;
    }
  ...
  async updateCustomerPassword(
    ...
    const result = await super.updatePassword(
      ctx,
  ...
  async updateCustomerEmailAddress(
    ...
    const result = await this.customerService.updateEmailAddress(
      ctx,
      args.token,
    );
    if (isGraphQlErrorResult(result)) {
  ...
  protected requireNativeAuthStrategy() {
    ...
    if (!nativeAuthStrategyIsConfigured) {
      const authStrategyNames = shopAuthenticationStrategy
        .map((s) => s.name)
        .join(", ");
      const errorMessage =
