...
export class ShopOrderResolver {
  ...
  async setOrderBillingAddress(
    ...
    if (ctx.authorizedAsOwnerOnly) {
      ...
      if (sessionOrder) {
        return this.orderService.setBillingAddress(
          ctx,
          sessionOrder.id,
          args.input,
        );
  ...
  async eligibleShippingMethods(
    ...
    if (ctx.authorizedAsOwnerOnly) {
      ...
      if (sessionOrder) {
        return this.orderService.getEligibleShippingMethods(
          ctx,
          sessionOrder.id,
        );
      }
  ...
  async transitionOrderToState(
    ...
    ErrorResultUnion<TransitionOrderToStateResult, Order> | undefined
  > {
    if (ctx.authorizedAsOwnerOnly) {
      const sessionOrder = await this.activeOrderService.getActiveOrder(
        ctx,
