...
export class ShopCustomerResolver {
  ...
  async updateCustomerAddress(
    ...
    const customerAddresses =
      await this.customerService.findAddressesByCustomerId(ctx, customer.id);
    if (
      !customerAddresses.find((address) =>
        idsAreEqual(address.id, args.input.id),
      )
    ) {
  ...
  async deleteCustomerAddress(
    ...
    const customerAddresses =
      await this.customerService.findAddressesByCustomerId(ctx, customer.id);
    if (
      !customerAddresses.find((address) => idsAreEqual(address.id, args.id))
    ) {
      throw new ForbiddenError();
    }
