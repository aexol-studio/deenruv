...
export class OrderAdminEntityResolver {
  ...
  async channels(@Ctx() ctx: RequestContext, @Parent() order: Order) {
    const channels =
      order.channels ?? (await this.orderService.getOrderChannels(ctx, order));
    return channels.filter((channel) =>
      ctx.session?.user?.channelPermissions.find((cp) =>
        idsAreEqual(cp.id, channel.id),
      ),
    );
  }
  ...
  async sellerOrders(@Ctx() ctx: RequestContext, @Parent() order: Order) {
    ...
    const userChannelIds =
      ctx.session?.user?.channelPermissions.map((cp) => cp.id) ?? [];
  ...
  async aggregateOrder(@Ctx() ctx: RequestContext, @Parent() order: Order) {
    const aggregateOrder = await this.orderService.getAggregateOrder(
      ctx,
      order,
    );
    const userChannelIds =
    ...
    return aggregateOrder &&
      userChannelIds.find((id) =>
        aggregateOrder.channels.find((channel) => idsAreEqual(channel.id, id)),
      )
      ? aggregateOrder
      : undefined;
  }
