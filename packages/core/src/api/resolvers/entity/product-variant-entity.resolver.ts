...
export class ProductVariantEntityResolver {
  ...
  async price(
    ...
    return this.productVariantService.hydratePriceFields(
      ctx,
      productVariant,
      "price",
    );
  ...
  async options(
    ...
    return this.productVariantService.getOptionsForVariant(
      ctx,
      productVariant.id,
    );
  }
  ...
  async facetValues(
    ...
    if (productVariant.facetValues?.[0]?.channels) {
      ...
      facetValues = await this.productVariantService.getFacetValuesForVariant(
        ctx,
        productVariant.id,
      );
    }
...
export class ProductVariantAdminEntityResolver {
  ...
  async stockAllocated(
    ...
    const { stockAllocated } = await this.stockLevelService.getAvailableStock(
      ctx,
      productVariant.id,
    );
  ...
  async stockLevels(
    ...
    return this.stockLevelService.getStockLevelsForVariant(
      ctx,
      productVariant.id,
    );
  }
  ...
  async prices(
    ...
    return this.productVariantService.getProductVariantPrices(
      ctx,
      productVariant.id,
    );
  }
