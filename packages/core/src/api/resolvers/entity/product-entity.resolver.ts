...
export class ProductEntityResolver {
  ...
  async collections(
    ...
    return this.collectionService.getCollectionsByProductId(
      ctx,
      product.id,
      apiType === "shop",
    );
  ...
  async optionGroups(
    ...
    return this.productOptionGroupService.getOptionGroupsByProductId(
      ctx,
      product.id,
    );
  }
  ...
  async facetValues(
    ...
    if (product.facetValues?.[0]?.channels) {
      ...
      facetValues = await this.productService.getFacetValuesForProduct(
        ctx,
        product.id,
      );
    }
...
export class ProductAdminEntityResolver {
  ...
  async channels(
    ...
    const channels =
      product.channels ||
      (await this.productService.getProductChannels(ctx, product.id));
