...
export class ShippingMethodResolver {
  ...
  deleteShippingMethods(
    ...
    return Promise.all(
      args.ids?.map((id) => this.shippingMethodService.softDelete(ctx, id)) ??
        [],
    );
  }
  ...
  testShippingMethod(
    @Ctx() ctx: RequestContext,
    @Args() args: QueryTestShippingMethodArgs,
  ) {
    const { input } = args;
  ...
  async assignShippingMethodsToChannel(
    ...
    return await this.shippingMethodService.assignShippingMethodsToChannel(
      ctx,
      args.input,
    );
  }
  ...
  async removeShippingMethodsFromChannel(
    ...
    return await this.shippingMethodService.removeShippingMethodsFromChannel(
      ctx,
      args.input,
    );
  }
