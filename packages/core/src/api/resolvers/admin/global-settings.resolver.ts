...
export class GlobalSettingsResolver {
  ...
  serverConfig(@Info() info: GraphQLResolveInfo): ServerConfig {
    ...
    return {
      ...
      moneyStrategyPrecision:
        this.configService.entityOptions.moneyStrategy.precision ?? 2,
    };
  }
    ...
    if (availableLanguages) {
      ...
      if (unavailableDefaults.length) {
        return new ChannelDefaultLanguageError({
          language: unavailableDefaults
            .map((c) => c.defaultLanguageCode)
            .join(", "),
  ...
  private generateCustomFieldConfig(
    ...
    for (const [entityType, customFields] of Object.entries(
      ...
      exposedCustomFieldConfig[entityType as keyof CustomFields] = customFields
        ...
        .map((c: any) => {
          ...
          if (c.type === "relation") {
            ...
            c.scalarFields = this.getScalarFieldsOfType(
              info,
              c.graphQLType || c.entity,
            );
          }
          return c;
        });
      ...
      .map(([entityType, customFields]) => {
        ...
        const customFieldsConfig = customFields
          ...
          .map((c) => {
            ...
            if (
              this.isRelationGraphQLType(customFieldConfig) &&
              this.isRelationConfigType(c)
            ) {
