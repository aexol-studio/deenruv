...
export class FacetResolver {
  ...
  facetValues(
    ...
    return this.facetValueService.findAllList(
      ctx,
  ...
  async deleteFacets(
    ...
    return Promise.all(
      args.ids.map((id) =>
        this.facetService.delete(ctx, id, args.force || false),
      ),
    );
  }
  ...
  async updateFacetValues(
    ...
    const { input } = args;
    return Promise.all(
      input.map((facetValue) => this.facetValueService.update(ctx, facetValue)),
    );
  }
  ...
  async deleteFacetValues(
    ...
    for (const id of args.ids) {
      results.push(
        await this.facetValueService.delete(ctx, id, args.force || false),
      );
    }
    return results;
  }
