...
async function createGraphQLOptions(
  ...
  return {
    ...
    // manually configure the graphql-upload package. See https://github.com/aexol-studio/deenruv/issues/396
    uploads: false,
  ...
  async function buildSchemaForApi(
    ...
    schema = addOrderLineCustomFieldsInput(
      schema,
      customFields.OrderLine || [],
    );
    schema = addModifyOrderCustomFields(schema, customFields.Order || []);
    schema = addShippingMethodQuoteCustomFields(
      schema,
      customFields.ShippingMethod || [],
    );
    schema = addPaymentMethodQuoteCustomFields(
      schema,
    ...
    if (apiType === "admin") {
      ...
      schema = addActiveAdministratorCustomFields(
        schema,
        customFields.Administrator,
      );
    }
    if (apiType === "shop") {
      schema = addRegisterCustomerCustomFieldsInput(
        schema,
        customFields.Customer || [],
      );
      schema = generateActiveOrderTypes(
        schema,
        configService.orderOptions.activeOrderStrategy,
      );
    }
    schema = generatePermissionEnum(
      schema,
      configService.authOptions.customPermissions,
    );
