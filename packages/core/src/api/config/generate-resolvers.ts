      ...
      switch (value.type) {
        ...
        default: {
          throw new InternalServerError(
            `No __resolveType defined for Region type "${value.type}"`,
          );
        }
      }
    },
  ...
  const resolvers =
    apiType === "admin"
      ? {
          ...commonResolvers,
          ...adminResolvers,
          ...getCustomScalars(configService, "admin"),
        }
      : {
          ...commonResolvers,
          ...shopResolvers,
          ...getCustomScalars(configService, "shop"),
        };
  return resolvers;
}
...
function generateCustomFieldRelationResolvers(
  ...
  for (const [entityName, customFields] of customFieldsConfig) {
    ...
    const allCustomFieldsAreNonPublic =
      customFields.length &&
      customFields.every((f) => f.public === false || f.internal === true);
...
function getCustomScalars(
  configService: ConfigService,
  apiType: "admin" | "shop",
) {
  return getPluginAPIExtensions(configService.plugins, apiType)
    .map((e) =>
      typeof e.scalars === "function" ? e.scalars() : (e.scalars ?? {}),
    )
    .reduce(
      (all, scalarMap) => ({
        ...all,
...
function isRelationalType(
  input: CustomFieldConfig,
): input is RelationCustomFieldConfig {
