...
export const GraphQLMoney = new GraphQLScalarType<number>({
  ...
  serialize(outputValue) {
    ...
    if (typeof num !== "number" || !Number.isFinite(num)) {
      throw new GraphQLError(
        `Money cannot represent non numeric value: ${inspect(coercedValue)}`,
      );
    }
    return num;
  },
  ...
  parseValue(inputValue) {
    if (typeof inputValue !== "number" || !Number.isFinite(inputValue)) {
      throw new GraphQLError(
        `Money cannot represent non numeric value: ${inspect(inputValue)}`,
      );
    }
    return inputValue;
  },
  ...
  parseLiteral(valueNode) {
    if (valueNode.kind !== Kind.FLOAT && valueNode.kind !== Kind.INT) {
      throw new GraphQLError(
        `Money cannot represent non numeric value: ${print(valueNode)}`,
        {
          nodes: valueNode,
        },
      );
