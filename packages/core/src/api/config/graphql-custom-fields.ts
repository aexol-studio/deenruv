import {
  assertNever,
  getGraphQlInputName,
} from "@deenruv/common/lib/shared-utils";
import {
  buildSchema,
  extendSchema,
...
export function addGraphQLCustomFields(
  ...
  const customFieldsConfig = getCustomFieldsConfigWithoutInterfaces(
    customFieldConfig,
    schema,
  );
  for (const [entityName, customFields] of customFieldsConfig) {
    ...
    const writeableLocalizedFields = localizedFields.filter(
      (field) => !field.readonly,
    );
    const writeableNonLocalizedFields = nonLocalizedFields.filter(
      (field) => !field.readonly,
    );
    ...
    const customEntityNonListFields = customEntityFields.filter(
      (f) => f.list !== true,
    );
    if (
      customEntityNonListFields.length &&
      schema.getType(`${entityName}SortParameter`)
    ) {
...
export function addRegisterCustomerCustomFieldsInput(
  ...
  const publicWritableCustomFields = customerCustomFields.filter((fieldDef) => {
    return (
      fieldDef.public !== false && !fieldDef.readonly && !fieldDef.internal
    );
  });
...
export function addModifyOrderCustomFields(
  ...
  if (
    schema.getType("ModifyOrderInput") &&
    schema.getType("UpdateOrderCustomFieldsInput")
  ) {
    const customFieldTypeDefs = `
...
export function addOrderLineCustomFieldsInput(
  ...
  const schema =
    typeof typeDefsOrSchema === "string"
      ? buildSchema(typeDefsOrSchema)
      : typeDefsOrSchema;
  const publicCustomFields = orderLineCustomFields.filter(
    (f) => f.public !== false,
  );
  ...
  const input = new GraphQLInputObjectType({
    ...
    fields: publicCustomFields.reduce((fields, field) => {
      ...
      const type =
        field.list === true ? new GraphQLList(primitiveType) : primitiveType;
...
export function addShippingMethodQuoteCustomFields(
  ...
  const schema =
    typeof typeDefsOrSchema === "string"
      ...
      : typeDefsOrSchema;
  let customFieldTypeDefs = "";
  const publicCustomFields = shippingMethodCustomFields.filter(
    (f) => f.public !== false,
  );
...
export function addPaymentMethodQuoteCustomFields(
  ...
  const schema =
    typeof typeDefsOrSchema === "string"
      ...
      : typeDefsOrSchema;
  let customFieldTypeDefs = "";
  const publicCustomFields = paymentMethodCustomFields.filter(
    (f) => f.public !== false,
  );
  if (0 < publicCustomFields.length) {
    customFieldTypeDefs = `
