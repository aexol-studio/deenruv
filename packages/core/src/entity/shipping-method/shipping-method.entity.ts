import type { ConfigurableOperation } from "@deenruv/common/generated-types";
  ...
  implements ChannelAware, SoftDeletable, HasCustomFields, Translatable
{
  ...
  constructor(input?: DeepPartial<ShippingMethod>) {
    super(input);
    const checkers =
      getConfig().shippingOptions.shippingEligibilityCheckers || [];
    const calculators = getConfig().shippingOptions.shippingCalculators || [];
    this.allCheckers = checkers.reduce(
