...
import { AdjustmentType } from "@deenruv/common/generated-shop-types";
...
import {
  PromotionCondition,
  PromotionConditionState,
} from "../../config/promotion/promotion-condition";
  ...
  implements ChannelAware, SoftDeletable, HasCustomFields, Translatable
{
  ...
  constructor(
    ...
    super(input);
    const conditions =
      (input && input.promotionConditions) ||
      getConfig().promotionOptions.promotionConditions ||
      [];
    const actions =
      (input && input.promotionActions) ||
      getConfig().promotionOptions.promotionActions ||
      [];
    this.allConditions = conditions.reduce(
      (hash, o) => ({ ...hash, [o.code]: o }),
      {},
    );
    this.allActions = actions.reduce(
      (hash, o) => ({ ...hash, [o.code]: o }),
      {},
    );
  }
    ...
    for (const action of this.actions) {
      ...
      if (promotionAction instanceof PromotionItemAction) {
        ...
        if (this.isOrderArg(args)) {
          const { order } = args;
          amount += roundMoney(
            await promotionAction.execute(ctx, order, action.args, state, this),
          );
        }
