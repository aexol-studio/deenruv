...
function validateCustomFieldsForEntity(
  ...
  return [
    ...
    ...(isTranslatable(entity)
      ? []
      : assertNoLocaleStringFields(entity.name, customFields)),
  ];
}
 ...
 */
function assertValidFieldNames(
  entityName: string,
  customFields: CustomFieldConfig[],
): string[] {
  const errors: string[] = [];
  ...
  for (const field of customFields) {
    if (!validNameRe.test(field.name)) {
      errors.push(
        `${entityName} entity has an invalid custom field name: "${field.name}"`,
      );
    }
  }
  return errors;
...
function assertNoNameConflictsWithEntity(
  entity: Type<any>,
  customFields: CustomFieldConfig[],
): string[] {
  const errors: string[] = [];
  for (const field of customFields) {
    ...
    if (conflicts(entity) || (translation && conflicts(translation))) {
      errors.push(
        `${entity.name} entity already has a field named "${field.name}"`,
      );
    }
  }
  return errors;
...
function assertNoDuplicatedCustomFieldNames(
  ...
  return Object.entries(nameCounts)
    ...
    .map(
      ([name, count]) =>
        `${entityName} entity has duplicated custom field name: "${name}"`,
    );
}
 ...
 */
function assertNoLocaleStringFields(
  entityName: string,
  customFields: CustomFieldConfig[],
): string[] {
  if (customFields.find((f) => f.type === "localeString")) {
    return [
      `${entityName} entity does not support custom fields of type "localeString"`,
    ];
  }
  return [];
}
 ...
 */
function assetNonNullablesHaveDefaults(
  entityName: string,
  customFields: CustomFieldConfig[],
): string[] {
  const errors: string[] = [];
...
function getEntityTranslation(entity: Type<any>): Type<any> | undefined {
  ...
  const translation = metadata
    .filterRelations(entity)
    .find((r) => r.propertyName === "translations");
  if (translation) {
  ...
  for (const key of Object.keys(customFieldConfig)) {
    ...
    if (entity && customEntityFields.length) {
      errors = errors.concat(
        validateCustomFieldsForEntity(entity, customEntityFields),
      );
    }
  }
  return {
