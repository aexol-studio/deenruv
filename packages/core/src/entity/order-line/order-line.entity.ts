...
export class OrderLine extends DeenruvEntity implements HasCustomFields {
  ...
  @ManyToOne(
    (type) => ShippingLine,
    (shippingLine) => shippingLine.orderLines,
    {
      nullable: true,
      onDelete: "SET NULL",
    },
  ...
  get discounts(): Discount[] {
    ...
    for (const adjustment of this.adjustments) {
      ...
      const unitAdjustmentAmount =
        (adjustment.amount /
          Math.max(this.orderPlacedQuantity, this.quantity)) *
        this.quantity;
  ...
  clearAdjustments(type?: AdjustmentType) {
    if (!type) {
      ...
      this.adjustments = this.adjustments
        ? this.adjustments.filter((a) => a.type !== type)
        : [];
    }
  }
  ...
  private _unitPrice(): number {
    return this.listPriceIncludesTax
      ? netPriceOf(this.listPrice, this.taxRate)
      : this.listPrice;
  }
  ...
  private _unitPriceWithTax(): number {
    return this.listPriceIncludesTax
      ? this.listPrice
      : grossPriceOf(this.listPrice, this.taxRate);
  }

  private _discountedUnitPrice(): number {
    const result =
      this.listPrice + this.getUnitAdjustmentsTotal(AdjustmentType.PROMOTION);
    return this.listPriceIncludesTax
      ? netPriceOf(result, this.taxRate)
      : result;
  }
  ...
  private _discountedUnitPriceWithTax(): number {
    ...
    return this.listPriceIncludesTax
      ? result
      : grossPriceOf(result, this.taxRate);
  }
