  ...
  implements ChannelAware, HasCustomFields
{
  ...
  get taxSummary(): OrderTaxSummary[] {
    ...
    const taxId = (taxLine: TaxLine): string =>
      `${taxLine.description}:${taxLine.taxRate}`;
    const taxableLines = [
      ...(this.lines ?? []),
      ...(this.shippingLines ?? []),
      ...(this.surcharges ?? []),
    ...
    for (const line of taxableLines) {
      ...
      for (const taxLine of line.taxLines) {
        ...
        const proportionOfTotalRate =
          0 < taxLine.taxRate ? taxLine.taxRate / taxRateTotal : 0;
        ...
        const amount = Math.round(
          (lineWithTax - lineBase) * proportionOfTotalRate,
        );
        if (row) {
          row.tax += amount;
          row.base += lineBase;
