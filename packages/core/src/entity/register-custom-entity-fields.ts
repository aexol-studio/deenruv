...
function registerCustomFieldsForEntity(
  ...
  if (customFields) {
    for (const customField of customFields) {
      ...
      const registerColumn = () => {
        if (customField.type === "relation") {
          ...
          const options: ColumnOptions = {
            type: list
              ? "simple-json"
              : getColumnType(dbEngine, customField.type),
          ...
          if (
            ...
            const defaultValueDecimalPlaces = customField.defaultValue
              .toString()
              .split(".")[1];
      ...
      const relationFieldsCount = customFields.filter(
        (f) => f.type === "relation",
      ).length;
...
function assertLocaleFieldsNotSpecified(
  config: DeenruvConfig,
  entityName: keyof CustomFields,
) {
  const customFields = config.customFields && config.customFields[entityName];
...
export function registerCustomEntityFields(config: DeenruvConfig) {
  ...
  for (const [entityName, customFieldsConfig] of Object.entries(
    config.customFields ?? {},
  )) {
    if (customFieldsConfig && customFieldsConfig.length) {
      ...
      if (translationsMetadata) {
        ...
        const translationType: Function = (
          translationsMetadata.type as Function
        )();
        const customFieldsTranslationsMetadata =
          getCustomFieldsMetadata(translationType);
        const customFieldsTranslationClass =
          customFieldsTranslationsMetadata.type();
        if (
          customFieldsTranslationClass &&
          typeof customFieldsTranslationClass !== "string"
        ) {
          registerCustomFieldsForEntity(
            config,
            entityName,
  ...
  function getCustomFieldsMetadata(
    ...
    const metadataArgs = metadataArgsStorage.embeddeds.find((item) => {
      if (item.propertyName === "customFields") {
        const targetName =
          typeof item.target === "string" ? item.target : item.target.name;
    ...
    if (!metadataArgs) {
      throw new Error(
        `Could not find embedded CustomFields property on entity "${entityName}"`,
      );
    }
    return metadataArgs;
  }
