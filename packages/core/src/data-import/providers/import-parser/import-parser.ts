...
import { GlobalFlag, LanguageCode } from "@deenruv/common/generated-types";
...
export class ImportParser {
   ...
   * returns a data structure which can then be used to populate Deenruv using the {@link FastImporterService}.
   */
  async parseProducts(
    input: string | Stream,
    ...
    return new Promise<ParseResult<ParsedProductWithVariants>>(
      (resolve, reject) => {
        let errors: string[] = [];
    ...
    for (const record of rest) {
      ...
      if (getRawMainTranslation(r, "name", mainLanguage)) {
        ...
        currentRow = {
          ...
          variants: [
            this.parseVariantFromRecord(r, usedLanguages, mainLanguage),
          ],
        };
      } else {
        if (currentRow) {
  ...
  private validateCustomFields(rowKeys: string[]): string[] {
    ...
    for (const rowKey of rowKeys) {
      ...
      if (parts.length === 2) {
        ...
        const customFieldConfig = customFieldConfigs.find(
          (config) => config.name === parts[1],
        );
  ...
  private validateHeaderTranslations(rowKeys: string[]): string | undefined {
    ...
    for (const baseKey of baseKeys) {
      const translatedKeys = languageCodes.map((code) =>
        [baseKey, code].join(":"),
      );
      if (rowKeys.includes(baseKey)) {
        ...
        if (
          !this.isTranslatable(baseKey) &&
          translatedKeys.some((key) => rowKeys.includes(key))
        ) {
  ...
  private parseProductFromRecord(
    ...
    const translationCodes =
      usedLanguages.length === 0 ? [mainLanguage] : usedLanguages;

    const optionGroups: ParsedOptionGroup[] = [];
    for (const languageCode of translationCodes) {
      const rawTranslOptionGroups = r.hasOwnProperty(
        `optionGroups:${languageCode}`,
      )
    ...
    const translations = translationCodes.map((languageCode) => {
      ...
      const parsedTranslatedCustomFields = parseCustomFields(
        "product",
        translatedFields,
      );
  ...
  private parseVariantFromRecord(
    ...
    const facets: ParsedFacet[] = [];
    for (const languageCode of translationCodes) {
      const rawTranslatedFacets = r.hasOwnProperty(
        `variantFacets:${languageCode}`,
      )
    ...
    const translations = translationCodes.map((languageCode) => {
      const rawTranslOptionValues = r.hasOwnProperty(
        `optionValues:${languageCode}`,
      )
...
function populateOptionGroupValues(currentRow: ParsedProductWithVariants) {
  for (const translation of currentRow.product.translations) {
    ...
    currentRow.product.optionGroups.forEach((og, i) => {
      const ogTranslation = og.translations.find(
        (t) => t.languageCode === translation.languageCode,
      );
      if (!ogTranslation) {
        throw new InternalServerError(
...
function getLanguageCode(rowKey: string): LanguageCode | undefined {
  ...
  if (parts.length === 3) {
    if (
      ["product", "productVariant"].includes(parts[0]) &&
      parts[2] in LanguageCode
    ) {
  ...
  for (const col of requiredColumns) {
    if (!rowKeys.includes(col)) {
      if (
        ...
        rowKeys.includes(`${col}:${languageCodes[0]}`)
      ) {
...
function validateOptionValueCount(
  ...
  const optionValueKeys = Object.keys(r).filter((key) =>
...
function getRawUntranslatedFields(r: { [key: string]: string }): {
  [key: string]: string;
} {
  return Object.entries(r)
    .filter(([key, value]) => {
