...
import { ConfigurableOperationInput } from "@deenruv/common/lib/generated-types";
...
export class Populator {
  ...
  async populateCollections(data: InitialData, channel?: Channel) {
    ...
    const allFacetValues = await this.facetValueService.findAll(
      ctx,
      ctx.languageCode,
    );
    ...
    for (const collectionDef of data.collections) {
      const parent =
        collectionDef.parentName && collectionMap.get(collectionDef.parentName);
      const parentId = parent ? parent.id.toString() : undefined;
      const { assets } = await this.assetImporter.getAssets(
        collectionDef.assetPaths || [],
        ctx,
      );
  ...
  private processFilterDefinition(
    ...
    switch (filter.code) {
      ...
      default:
        throw new Error(
          `Filter with code "${filter.code as string}" is not recognized.`,
        );
    }
  }
  ...
  private async setChannelDefaults(
    zoneMap: ZoneMap,
    data: InitialData,
    channel: Channel,
  ) {
  ...
  private async populateCountries(
    ...
    for (const zone of existingZones) {
      zoneMap.set(zone.name, {
        entity: zone,
        members: zone.members.map((m) => m.id),
      });
    }
  ...
  private async populateTaxRates(
    ...
    for (const taxRate of taxRates) {
      const category = await this.taxCategoryService.create(ctx, {
        name: taxRate.name,
      });
  ...
  private async populatePaymentMethods(
    ctx: RequestContext,
