...
import { ImportInfo, LanguageCode } from "@deenruv/common/lib/generated-types";
...
export class Importer {
    ...
    return new Observable((subscriber) => {
      const p = this.doParseAndImport(input, ctxOrLanguageCode, (progress) => {
        if (reportProgress) {
          if (!bar) {
            bar = new ProgressBar(
              "  importing [:bar] :percent :etas  Importing: :prodName",
              {
                complete: "=",
                incomplete: " ",
  ...
  async importProducts(
    ...
    for (const { product, variants } of rows) {
      ...
      const createProductAssets = await this.assetImporter.getAssets(
        product.assetPaths,
        ctx,
      );
      ...
      const createdProductId = await this.fastImporter.createProduct({
        ...
        facetValueIds: await this.getFacetValueIds(
          ctx,
          product.facets,
          ctx.languageCode,
        ),
      ...
      for (const [optionGroup, optionGroupIndex] of product.optionGroups.map(
        ...
        for (const [
          optionIndex,
          value,
        ] of optionGroupMainTranslation.values.map(
      ...
      for (const variant of variants) {
        ...
        const createVariantAssets = await this.assetImporter.getAssets(
          variant.assetPaths,
        );
        ...
        const createdVariant = await this.fastImporter.createProductVariant({
          ...
          facetValueIds,
          featuredAssetId: variantAssets.length
            ? variantAssets[0].id
            : undefined,
  ...
  private async getFacetValueIds(
    ...
    for (const item of facets) {
      const itemMainTranslation = this.getTranslationByCodeOrFirst(
        item.translations,
        languageCode,
      );
  ...
  private getTranslationByCodeOrFirst<
    Type extends { languageCode: LanguageCode },
  >(translations: Type[], languageCode: LanguageCode): Type {
