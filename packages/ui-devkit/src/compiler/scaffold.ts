...
export async function setupScaffold(
  outputPath: string,
  extensions: Extension[],
) {
  deleteExistingExtensionModules(outputPath);

  const adminUiExtensions = extensions.filter((e): e is AdminUiExtension =>
    isAdminUiExtension(e),
  );
  ...
  const sassVariableOverridesExtension = extensions.find(
    isSassVariableOverridesExtension,
  );
  await addGlobalStyles(
    outputPath,
    globalStyleExtensions,
    sassVariableOverridesExtension,
  );

  const allTranslationFiles = getAllTranslationFiles(
    extensions.filter(isTranslationExtension),
  );
...
async function copyExtensionModules(
  outputPath: string,
  extensions: AdminUiExtensionWithId[],
) {
  ...
  fs.writeFileSync(
    path.join(outputPath, EXTENSION_ROUTES_FILE),
    extensionRoutesSource,
    "utf8",
  );
  const sharedExtensionModulesSource =
    generateSharedExtensionModule(extensions);
  fs.writeFileSync(
    path.join(outputPath, SHARED_EXTENSIONS_FILE),
    sharedExtensionModulesSource,
    "utf8",
  );
  ...
  for (const extension of adminUiExtensions) {
    ...
    fs.copySync(extension.extensionPath, dest, {
      filter: (name) =>
        name === extension.extensionPath || exclude.every((e) => e !== name),
    });
  }
}
...
async function copyStaticAssets(
  outputPath: string,
  extensions: Array<Partial<StaticAssetExtension>>,
) {
...
async function addGlobalStyles(
  ...
  const globalStylesDir = path.join(
    outputPath,
    "src",
    GLOBAL_STYLES_OUTPUT_DIR,
  );
  ...
  const globalStylesSource =
    ...
    imports
      .map((file) => `@import "./${GLOBAL_STYLES_OUTPUT_DIR}/${file}";`)
      .join("\n");
...
export async function copyGlobalStyleFile(
  outputPath: string,
  stylePath: string,
) {
  const globalStylesDir = path.join(
...
function generateLazyExtensionRoutes(
  extensions: AdminUiExtensionWithId[],
): string {
  const routes: string[] = [];
  for (const extension of extensions) {
    ...
    for (const route of extension.routes ?? []) {
      const prefix =
        route.prefix === "" ? "" : `${route.prefix ?? "extensions"}/`;
      routes.push(`  {
  ...
  const moduleImports = adminUiExtensions
    .map((e) =>
      e.ngModules
        ...
        .map(
          (m) =>
            `import { ${m.ngModuleName} } from '${getModuleFilePath(e.id, m)}';\n`,
        )
        .join(""),
    )
    .filter((val) => !!val)
...
function copyAdminUiSource(
  ...
  const adminUiSrc = path.join(
    require.resolve("@deenruv/admin-ui"),
    "../../static",
  );

  if (!fs.existsSync(scaffoldDir)) {
    throw new Error(
      `Could not find the admin ui scaffold files at ${scaffoldDir}`,
    );
  }
  if (!fs.existsSync(adminUiSrc)) {
    throw new Error(
      `Could not find the @deenruv/admin-ui sources. Looked in ${adminUiSrc}`,
    );
  }
