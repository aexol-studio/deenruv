 ...
 */
export function getStaticAssetPath(
  staticAssetDef: StaticAssetDefinition,
): string {
...
export function copyUiDevkit(outputPath: string) {
  ...
  fs.copySync(
    require.resolve("@deenruv/ui-devkit"),
    path.join(devkitDir, "ui-devkit.js"),
  );
}
...
export async function copyStaticAsset(
  ...
  if (typeof staticAssetDef !== "string") {
    ...
    const newName = path.join(
      path.dirname(assetOutputPath),
      staticAssetDef.rename,
    );
    try {
...
export function isAdminUiExtension(
  input: Extension,
): input is AdminUiExtension {
  return input.hasOwnProperty("extensionPath");
}

export function isTranslationExtension(
  input: Extension,
): input is TranslationExtension {
  return input.hasOwnProperty("translations");
}

export function isStaticAssetExtension(
  input: Extension,
): input is StaticAssetExtension {
  return input.hasOwnProperty("staticAssets");
}

export function isGlobalStylesExtension(
  input: Extension,
): input is GlobalStylesExtension {
  return input.hasOwnProperty("globalStyles");
}

export function isSassVariableOverridesExtension(
  input: Extension,
): input is SassVariableOverridesExtension {
