...
export async function createVendureApp(
    ...
    try {
        const { populate } = await import(path.join(root, 'node_modules/@deenruv/core/cli/populate'));
        const { bootstrap, DefaultLogger, LogLevel, JobQueueService } = await import(
            path.join(root, 'node_modules/@deenruv/core/dist/index')
        );
...
async function copyEmailTemplates(root: string) {
    const templateDir = path.join(root, 'node_modules/@deenruv/email-plugin/templates');
    try {
