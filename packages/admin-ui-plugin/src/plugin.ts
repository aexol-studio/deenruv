...
export class AdminUiPlugin implements NestModule {
  ...
  async configure(consumer: MiddlewareConsumer) {
    ...
    const adminUiConfigPath = path.join(
      adminUiAppPath,
      "deenruv-ui-config.json",
    );
    ...
    if (AdminUiPlugin.isDevModeApp(app)) {
      ...
      app.compile().then(
        () => {
          Logger.info(
            "Admin UI compiling and watching for changes...",
            loggerCtx,
          );
        },
        (err: any) => {
          Logger.error(
            `Failed to compile: ${JSON.stringify(err)}`,
            loggerCtx,
            err.stack,
          );
        },
  ...
  private getAdminUiConfig(
    partialConfig?: Partial<AdminUiConfig>,
  ): AdminUiConfig {
    ...
    const propOrDefault = <Prop extends keyof AdminUiConfig>(
      ...
      if (isArray) {
        ...
        return partialConfig
          ? (partialConfig as AdminUiConfig)[prop] || defaultVal
          : defaultVal;
      }
    };
    return {
      ...
      availableLanguages: propOrDefault(
        "availableLanguages",
        defaultAvailableLanguages,
        true,
      ),
      availableLocales: propOrDefault(
        "availableLocales",
        defaultAvailableLocales,
        true,
      ),
      ...
      hideVersion: propOrDefault(
        "hideVersion",
        options.adminUiConfig?.hideVersion || false,
      ),
  ...
  private async overwriteAdminUiConfig(
    ...
    Logger.verbose(
      "Applied configuration to deenruv-ui-config.json file",
      loggerCtx,
    );
  }
  ...
  private async overwriteBaseHref(indexHtmlPath: string, baseHref: string) {
    ...
    try {
      ...
      throw new Error(
        "[AdminUiPlugin] Could not write index.html file:\n" +
          JSON.stringify(e.message),
      );
    }
    Logger.verbose(
      `Applied baseHref "/${baseHref}/" to index.html file`,
      loggerCtx,
    );
  }
