...
export class SqljsInitializer implements TestDbInitializer<SqljsConnectionOptions> {
  ...
  async init(
    ...
    (connectionOptions as Mutable<SqljsConnectionOptions>).location =
      this.dbFilePath;
  ...
  async populate(populateFn: () => Promise<void>): Promise<void> {
    if (!fs.existsSync(this.dbFilePath)) {
      ...
      (this.connectionOptions as Mutable<SqljsConnectionOptions>).autoSave =
        true;
      (this.connectionOptions as Mutable<SqljsConnectionOptions>).synchronize =
        true;
      await populateFn();
      await new Promise((resolve) =>
        setTimeout(resolve, this.postPopulateTimeoutMs),
      );
      (this.connectionOptions as Mutable<SqljsConnectionOptions>).autoSave =
        false;
      (this.connectionOptions as Mutable<SqljsConnectionOptions>).synchronize =
