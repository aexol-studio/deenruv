...
export class EntityRef {
  ...
  isTranslatable() {
    return this.classDeclaration
      .getImplements()
      .some((i) => i.getText() === "Translatable");
  }

  isTranslation() {
    return this.classDeclaration
      .getImplements()
      .some((i) => i.getText().includes("Translation<"));
  }

  hasCustomFields() {
    return this.classDeclaration
      .getImplements()
      .some((i) => i.getText() === "HasCustomFields");
  }
  ...
  getProps(): Array<{ name: string; type: Type; nullable: boolean }> {
    return this.classDeclaration.getProperties().map((prop) => {
      ...
      return {
        name,
        type: propType.getNonNullableType(),
