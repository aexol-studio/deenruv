...
export class PackageJson {
  ...
  async installPackages(requiredPackages: PackageToInstall[]) {
    ...
    const depsToInstall = packagesToInstall
      .filter(
        (p) =>
          !p.isDevDependency && packageJson.dependencies?.[p.pkg] === undefined,
      )
    ...
    const devDepsToInstall = packagesToInstall
      .filter(
        (p) =>
          p.isDevDependency &&
          packageJson.devDependencies?.[p.pkg] === undefined,
      )
      .map((p) => `${p.pkg}${p.version ? `@${p.version}` : ""}`);
  ...
  locatePackageJsonWithDeenruvDependency() {
    ...
    const rootPackageJsonPath = path.join(
      this.getPackageRootDir().getPath(),
      "package.json",
    );
