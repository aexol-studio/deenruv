...
export class DeenruvPluginRef {
  ...
  getPluginOptions():
    ...
    const optionsProviders = elements
      ...
      .filter((el) =>
        el
          .getProperty("useFactory")
          ?.getText()
          .includes(`${this.name}.options`),
      );

    if (!optionsProviders.length) {
      return;
    }
    const optionsSymbol = optionsProviders[0].getProperty(
      "provide",
    ...
    const constantDeclaration = initializer
      .getDefinitions()[0]
      ?.getDeclarationNode();
    if (
      !constantDeclaration ||
      !Node.isVariableDeclaration(constantDeclaration)
    ) {
      return;
    }
  ...
  addEntity(entityClassName: string) {
    ...
    if (entityProperty) {
      const entitiesArray = entityProperty.getFirstChildByKind(
  ...
  addProvider(providerClassName: string) {
    ...
    if (providerProperty) {
      const providersArray = providerProperty.getFirstChildByKind(
        SyntaxKind.ArrayLiteralExpression,
      );
      if (providersArray) {
  ...
  addAdminApiExtensions(extension: {
    ...
    if (
      ...
      if (resolversProp) {
        const resolversArray = resolversProp.getFirstChildByKind(
          SyntaxKind.ArrayLiteralExpression,
        );
        if (resolversArray) {
  ...
  getEntities(): EntityRef[] {
    ...
    const entitiesArray = entitiesProperty.getFirstChildByKind(
      SyntaxKind.ArrayLiteralExpression,
    );
    if (!entitiesArray) {
      return [];
    ...
    return entitySourceFiles
      .map((sourceFile) =>
        sourceFile
          .getClasses()
          .filter((c) => c.getExtends()?.getText() === "DeenruvEntity"),
      )
      .flat()
  ...
  hasUiExtensions(): boolean {
    return !!this.classDeclaration
      ...
      .find(
        (prop) =>
          prop.getType().getSymbol()?.getName() === AdminUiExtensionTypeName,
      );
  }
}
