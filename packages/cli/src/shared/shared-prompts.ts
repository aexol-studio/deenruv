...
export async function analyzeProject(options: {
  ...
  if (!providedDeenruvPlugin) {
    ...
    const { project: _project, tsConfigPath: _tsConfigPath } =
      await getTsMorphProject({}, tsConfigFile);
    project = _project;
...
export async function selectMultiplePluginClasses(
  ...
  return (targetPlugins as ClassDeclaration[]).map(
    (pc) => new DeenruvPluginRef(pc),
  );
}
