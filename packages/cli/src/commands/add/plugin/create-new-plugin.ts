...
export async function createNewPlugin(): Promise<CliCommandReturnVal> {
  ...
  const { plugin, modifiedSourceFiles } = await generatePlugin(
    project,
    options,
  );
...
export async function generatePlugin(
  ...
  pluginFile
    .getImportDeclaration("./constants.template")
    ?.setModuleSpecifier("./constants");
  pluginFile
    .getImportDeclaration("./types.template")
    ?.setModuleSpecifier("./types");
...
function findExistingPluginsDir(
  project: Project,
  ...
  if (pluginClasses.length === 1) {
    return {
      prefix: path.dirname(pluginClasses[0].getSourceFile().getDirectoryPath()),
      suffix: "",
    };
  }
  ...
  const rest = pluginDirs[0]
    .substring(suffixStartIndex)
    .replace(/^\//, "")
    .split("/");
...
function findCommonPath(paths: string[]): string {
  ...
  const minLength = Math.min(
    ...pathSegmentsList.map((segments) => segments.length),
  );
  ...
  for (let i = 0; i < minLength; i++) {
    ...
    const isCommon = pathSegmentsList.every(
      (segments) => segments[i] === currentSegment,
    );
    if (isCommon) {
