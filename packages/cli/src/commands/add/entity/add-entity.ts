...
async function addEntity(
  ...
  const customEntityName =
    options?.className ?? (await getCustomEntityName(cancelledMessage));
  ...
  const { entityClass, translationClass } = createEntity(
    deenruvPlugin,
    context,
  );
...
async function getFeatures(
  options?: Partial<AddEntityOptions>,
): Promise<AddEntityOptions["features"]> {
  if (options?.features) {
...
function createEntity(plugin: DeenruvPluginRef, options: AddEntityOptions) {
  ...
  const translationCustomFieldsClass = translationFile.getClass(
    "ScaffoldEntityCustomFieldsTranslation",
  );
  ...
  if (!translationCustomFieldsClass?.wasForgotten()) {
    translationCustomFieldsClass?.rename(
      `${options.className}CustomFieldsTranslation`,
    );
  }
...
function removeImplementsFromClass(
  ...
  const index =
    entityClass
      ?.getImplements()
      .findIndex((i) => i.getText() === implementsName) ?? -1;
  if (index > -1) {
