...
async function addService(
  ...
  const type =
    ...
    (await select({
      ...
      options: [
        ...
        {
          value: "entity",
          label: "Service to perform CRUD operations on an entity",
        },
      ],
      maxItems: 10,
    }));
  ...
  if (type === "entity") {
    ...
    try {
      ...
      if (e.message === Messages.NoEntitiesFound) {
        log.info(
          `No entities found in plugin ${deenruvPlugin.name}. Let's create one first.`,
        );
  ...
  if (options.type === "basic") {
    ...
    const serviceSourceFilePath = getServiceFilePath(
      deenruvPlugin,
      options.serviceName,
    );
    ...
    const templateTranslationEntityClass = serviceSourceFile.getClass(
    ...
    if (entityRef.isTranslatable()) {
      ...
      if (translationEntityClass && templateTranslationEntityClass) {
        templateTranslationEntityClass.rename(
          translationEntityClass?.getName() as string,
        );
    ...
    serviceClassDeclaration
      ...
      ?.addParameter({
        ...
        decorators: [
          {
            name: "Inject",
            arguments: [pluginOptions.constantDeclaration.getName()],
          },
        ],
      })
      .formatText();
...
function getServiceFilePath(plugin: DeenruvPluginRef, serviceName: string) {
  const serviceFileName = paramCase(serviceName).replace(
    /-service$/,
    ".service",
  );
  return path.join(
...
function customizeFindOneMethod(
  serviceClassDeclaration: ClassDeclaration,
  entityRef: EntityRef,
) {
...
function customizeFindAllMethod(
  serviceClassDeclaration: ClassDeclaration,
  entityRef: EntityRef,
) {
...
function customizeCreateMethod(
  serviceClassDeclaration: ClassDeclaration,
  entityRef: EntityRef,
) {
...
function customizeUpdateMethod(
  serviceClassDeclaration: ClassDeclaration,
  entityRef: EntityRef,
) {
...
function removedUnusedConstructorArgs(
  serviceClassDeclaration: ClassDeclaration,
  entityRef: EntityRef,
) {
  ...
  serviceClassDeclaration.getConstructors().forEach((constructor) => {
    constructor.getParameters().forEach((param) => {
      ...
      if (
        (paramName === "translatableSaver" || paramName === "translator") &&
        !isTranslatable
      ) {
        param.remove();
      }
