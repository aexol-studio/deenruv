...
async function addApiExtension(
  ...
  const plugin =
    providedDeenruvPlugin ?? (await selectPlugin(project, cancelledMessage));
  ...
  if (serviceEntityRef) {
    ...
    apiExtensions = createSimpleApiExtension(
      project,
      plugin,
      serviceRef,
      queryName,
  ...
  return {
    project,
    modifiedSourceFiles: [
      serviceRef.classDeclaration.getSourceFile(),
      ...modifiedSourceFiles,
    ],
    serviceRef,
  };
...
function createSimpleResolver(
  ...
  if (resolverClassDeclaration.getName() === "SimpleAdminResolver") {
    resolverClassDeclaration.rename(
      serviceRef.name.replace(/Service$/, "") +
        "AdminResolver" +
...
function createCrudResolver(
  ...
  if (serviceRef.features.findOne) {
    ...
    const serviceFindOneMethod =
      serviceRef.classDeclaration.getMethod("findOne");
      ...
      if (
        !serviceFindOneMethod
          ...
          .find(
            (p) =>
              p.getName() === "relations" &&
              p.getType().getText().includes("RelationPaths"),
          )
      ) {
        findOneMethod?.getParameters()[2].remove();
        findOneMethod?.setBodyText(
          `return this.${serviceRef.nameCamelCase}.findOne(ctx, args.id);`,
        );
      }
    }
  } else {
  ...
  if (serviceRef.features.findAll) {
    ...
    const serviceFindAllMethod =
      serviceRef.classDeclaration.getMethod("findAll");
  ...
  if (serviceRef.features.create) {
    resolverClassDeclaration
      ...
      ?.rename("create" + serviceEntityRef.name);
  } else {
  ...
  if (serviceRef.features.update) {
    resolverClassDeclaration
      ...
      ?.rename("update" + serviceEntityRef.name);
  } else {
  ...
  if (serviceRef.features.delete) {
    resolverClassDeclaration
      .getMethod("deleteEntity")
      ?.rename("delete" + serviceEntityRef.name);
  } else {
  ...
  resolverClassDeclaration
    ...
    .setType(serviceRef.name);
  resolverSourceFile
    .getClass("TemplateEntity")
    ?.rename(serviceEntityRef.name)
    .remove();
  resolverSourceFile.getClass("TemplateService")?.remove();
...
function createSimpleApiExtension(
  ...
  const adminApiExtensions =
    apiExtensionsFile.getVariableDeclaration("adminApiExtensions");
  const insertAtIndex =
    adminApiExtensions?.getParent().getParent().getChildIndex() ?? 2;
  const schemaVariableName = `${serviceRef.nameCamelCase.replace(/Service$/, "")}AdminApiExtensions`;
  const existingSchemaVariable =
    apiExtensionsFile.getVariableStatement(schemaVariableName);
  if (!existingSchemaVariable) {
    ...
    if (!taggedTemplateLiteral) {
      ...
      appendToGqlTemplateLiteral(
        existingSchemaVariable.getDeclarations()[0],
        (writer) => {
          writer.indent(() => {
            if (queryName) {
              writer.writeLine(`  extend type Query {`);
  ...
  addSchemaToApiExtensionsTemplateLiteral(
    adminApiExtensions,
    schemaVariableName,
  );
        ...
        initializer: (writer) => {
          ...
          if (entityRef) {
            writer.indent(() => {
              ...
              writer.writeLine(
                `  type ${entityRef.name}List implements PaginatedList {`,
              );
...
function getOrCreateApiExtensionsFile(
  project: Project,
  plugin: DeenruvPluginRef,
): SourceFile {
