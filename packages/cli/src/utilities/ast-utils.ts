...
export function addImportsToFile(
  ...
  const moduleSpecifier = getModuleSpecifierString(
    options.moduleSpecifier,
    sourceFile,
  );
  const existingDeclaration = sourceFile.getImportDeclaration(
    (declaration) =>
      declaration.getModuleSpecifier().getLiteralValue() === moduleSpecifier,
  );
...
function getModuleSpecifierString(
  moduleSpecifier: string | SourceFile,
  sourceFile: SourceFile,
): string {
...
export function getRelativeImportPath(locations: {
  ...
  const fromPath =
    locations.from instanceof SourceFile
      ? locations.from.getFilePath()
      : locations.from.getPath();
  const toPath =
    locations.to instanceof SourceFile
      ? locations.to.getFilePath()
      : locations.to.getPath();
  const fromDir = /\.[a-z]+$/.test(fromPath)
...
export function customizeCreateUpdateInputInterfaces(
  sourceFile: SourceFile,
  entityRef: EntityRef,
) {
