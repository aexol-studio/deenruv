...
This is the administration interface for Deenruv.
...
The Admin UI is structured as an Angular library conforming to the [ng-packagr format](https://github.com/ng-packagr/ng-packagr). This library is what is published to npm as `@deenruv/admin-ui`. The library consists
of a set of modules which are accessible from consuming applications as sub-packages:

-   `@deenruv/admin-ui/core`
-   `@deenruv/admin-ui/catalog`
-   `@deenruv/admin-ui/orders`
...
To verify localization changes add `<languageCode>.json` to `./src/lib/static/deenruv-ui-config.json` in the array `availableLanguages`. This will make the localization available in Admin UI development mode using `npm run start`
