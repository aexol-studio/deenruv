...
async function extractTranslations() {
  const locales = fs
    ...
    .map((file) => path.basename(file).replace(".json", ""));
  const report = {
  ...
  for (const locale of locales) {
    ...
    try {
      ...
      const { tokenCount, translatedCount, percentage } =
        getStatsForLocale(locale);
      console.log(
        `${locale}: ${translatedCount} of ${tokenCount} tokens translated (${percentage}%)`,
      );
      console.log("");
      report.translationStatus[locale] = {
        tokenCount,
        translatedCount,
        percentage,
      };
...
function runExtraction(locale) {
  ...
  return new Promise((resolve, reject) => {
    try {
      const child = spawn(`yarnpkg`, args, {
