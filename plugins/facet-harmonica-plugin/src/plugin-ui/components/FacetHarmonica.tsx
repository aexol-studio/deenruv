...
export const FacetHarmonica = () => {
  ...
  const handleFacetCheckboxChange = (
    facetValueId: string,
    checked: boolean,
  ) => {
    ...
    const facetValues = newFacetValues?.filter(
  ...
  const checkedFacetsIds = useMemo(
    () => entity?.facetValues.map((f) => f.id),
    [entity?.facetValues],
  );
  const { mainFacets, colorFacets, otherFacets } = useMemo(() => {
    const mainFacets = data?.facets?.items?.filter(
      (f) => f.customFields?.usedForProductCreations,
    );
    const colorFacets = data?.facets?.items?.filter(
      (f) => f.customFields?.colorsCollection,
    );
