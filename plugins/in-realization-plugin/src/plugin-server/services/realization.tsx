...
export function RealizationPDF({
  options,
  order,
  user,
}: Omit<PDFProps, "assets">) {
    ...
    >
      ...
      <View style={s.flex}>
        ...
        >
          ...
          <Text style={{ ...s.smText, marginTop: 4 }}>
            {city.toUpperCase()}
          </Text>
        ...
        <View style={{ ...s.borderL, width: "68%" }}>
          ...
          {(hasCustomOptions ? customOptions : orderLine.options).map(
            (option, i) => (
              <View
                key={"custom option key" + i}
                style={{ ...s.borderT, ...s.smText, ...s.line }}
              >
