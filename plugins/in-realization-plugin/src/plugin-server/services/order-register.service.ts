...
export class OrderRegisterService {
  ...
  private hydrateOrderLines = async (ctx: RequestContext, order: Order) => {
    ...
    const shippings = await this.orderService.getEligibleShippingMethods(
      ctx,
      order.id,
    );
    const shippingMethod = shippings.find(
      (s) => s.id === order.shippingLines[0].shippingMethodId,
    );
  ...
  async getRealization(ctx: RequestContext, orderID: string) {
    return await this.connection
      .getRepository(ctx, OrderRealizationEntity)
      .findOne({
        where: { orderID },
  ...
  async registerRealization(
    ctx: RequestContext,
    options: ModelTypes["OrderRealizationInput"],
  ) {
