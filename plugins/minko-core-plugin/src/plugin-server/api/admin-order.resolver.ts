...
export class AdminOrderResolver {
    ...
    async getRealization(@Ctx() ctx: RequestContext, @Parent() order: Order) {
        const realization = await this.orderRegisterService.getRealization(ctx, order.id as string);
        if (!realization) return null;
        return realization;
    }
    @ResolveField()
    async getProforma(@Ctx() ctx: RequestContext, @Parent() order: Order) {
        const proforma = await this.orderRegisterService.getProforma(ctx, order.id as string);
