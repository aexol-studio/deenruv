...
import { RealizationPDF } from "./react-pdf/realization";
import { PROFormaPDF } from "./react-pdf/proforma";
...
export class PDFService {
  ...
  private generatePROFormaDocument = async (
    ctx: RequestContext,
    props: PROFormaInputType,
  ) => {
    const {
      order: {
        code,
    ...
    const _products = await Promise.all(
      ...
      lines.map(async (line: any) => {
        return {
          ...
          discount:
            formatPrice(
              line.discounts?.reduce(
                (acc: number, d: any) => acc + -d.amountWithTax,
                0,
              ),
            ) ?? 0,
        ...
        : [
            {
              ...
              quantity: 1,
              vat: isNaN(shippingLines[0]?.taxRate)
                ? 0
                : shippingLines[0]?.taxRate,
              unitPrice: formatPrice(shippingWithTax),
              nettoPrice: formatPrice(
                shippingLines[0]?.price ?? shippingWithTax,
              ),
              discount: "-",
              ...
              vatPrice: formatPrice(
                shippingWithTax - shippingLines[0]?.price ?? 0,
              ),
  ...
  private generateRealizationDocument = async (
    ctx: RequestContext,
    props: PDFProps,
  ) => {
    ...
    const orderWithOptions = await Promise.all(
      order.lines.map(async (line) => {
        ...
        const image = additionalAsset?.preview
        ...
        results = await Promise.all(
          Object.entries(json).map(async ([code, value]) => {
            ...
            if (facetValue && facetCustomFields?.usedForColors) {
              ...
              result = {
                code:
                  this.translator.translate(facetValue.facet, ctx).name || code,
                ...
                asHandle: facetCustomFields?.usedForProductCreations
                  ? value
                  : null,
                cleanCode: code,
                cleanValue: value,
