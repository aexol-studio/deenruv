...
const timeToLive = IS_DEV
  ? 1000 * 60 * 5 + DATABASE_TIME_OFFSET
  : 1000 * 60 * 60 * OMNIBUS_TIME; // 30 days
...
export class ProductVariantCustomResolver {
  ...
  async discountedPrice(
    ...
    const isCurrentCustomerInActivePromotionGroup = activePromotions.reduce(
      (acc, promo) => {
        const isCustomerGroup = promo.conditions.reduce((acc, condition) => {
          if (
            condition.code === "customer_group" &&
            !!userID &&
            !!customerGroups
          ) {
    ...
    const price = activePromotions.reduce((mainAcc, promotion) => {
      ...
      if (shouldBeApplied) {
        ...
        mainAcc = promotion.actions.reduce((acc, action) => {
          ...
          if (
            action.code === "line_percentage_discount_full_price" &&
            mainAcc === basePrice
          ) {
            const [discount, minValue] = action.args;
            if (minValue && Number(minValue.value ?? 0) * 100 > mainAcc) {
          ...
          if (
            action.code === "line_fixed_discount_full_price" &&
            mainAcc === basePrice
          ) {
            const [discount, minValue] = action.args;
            if (minValue && Number(minValue.value ?? 0) * 100 > mainAcc) {
          ...
          if (action.code === "all_collections") {
            ...
            if (
              isProductSelectedInCollections &&
              !isCurrentCustomerInActivePromotionGroup
            ) {
              const discount = Math.floor(
                basePrice * (Number(action.args[1].value) / 100),
              );
              acc = acc - discount;
              value = discount;
              return acc;
        ...
        const translation = promotion.translations.find(
          (x) => x.languageCode === ctx.languageCode,
        );
...
export const lineItemFixedAmount = new PromotionItemAction({
  ...
  execute(ctx, item, args) {
    if (
      ...
      args[`💰 Dla Produktów z Minimalną Ceną Powyżej`] * 100
    ) {
      return 0;
    }
...
export const lineItemFixedAmountFullPrice = new PromotionItemAction({
  ...
  description: [
    ...
    },
    {
      languageCode: LanguageCode.pl,
      value:
        "Zniżka na wszystkie produkty (tylko w podstawowej cenie) o stałą kwotę",
    },
  ],
  args: {
    discount: {
...
export const lineItemPercentageFullPrice = new PromotionItemAction({
  ...
  description: [
    ...
    },
    {
      languageCode: LanguageCode.pl,
      value:
        "Zniżka na wszystkie produkty (tylko w podstawowej cenie) o {discount} %",
    },
  ],
  args: {
    discount: {
    ...
    if (args.collectionsID) {
      try {
        ...
        if (variantsIds.includes(item.productVariantId)) {
          const discount = Math.floor(
            item.unitPriceWithTax * (args["Zniżka"] / 100),
          );
          return -discount;
        }
  ...
  async discountedPrice(
    ...
    try {
      ...
      const activePromotions = await this.connection
        ...
        .andWhere(
          "(promotion.startsAt IS NULL OR promotion.startsAt < :date)",
          { date },
        )
        .orderBy("promotion.priorityScore", "ASC")
        .getMany();
      ...
      const basePrice =
        "min" in search.priceWithTax
          ? search.priceWithTax.min
          : search.priceWithTax.value;
      ...
      const price = activePromotions.reduce((mainAcc, promotion) => {
        ...
        const shouldBeApplied = promotion.conditions.reduce(
          (acc, condition) => {
            if (
              condition.code === "all_products" &&
              !isCurrentCustomerInActivePromotionGroup
        ...
        if (shouldBeApplied) {
          ...
          mainAcc = promotion.actions.reduce((acc, action) => {
            ...
            if (action.code === "line_percentage_discount") {
              ...
              acc =
                acc - Math.floor(basePrice * (Number(discount.value) / 100));
              value = Math.floor(basePrice * (Number(discount.value) / 100));
