...
const match = (facetValues?: FacetValueType[]) => {
  ...
  const facets = facetValues.reduce((acc, item) => {
    ...
    const newItem: FacetValueData = {
      ...
      imagePreview:
        (item.customFields?.image as unknown as { preview: string })?.preview ||
        "",
    };
    if (facet) {
      ...
      acc.push({
        ...
        usedForProductCreations:
          !!item.facet.customFields?.usedForProductCreations,
        facetValues: [newItem],
      });
    }
    return acc;
    ...
    <Dialog>
      ...
      <DialogContent className="w-[800px] max-w-[800px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {t("custom.editAttributesDescription")}
          </DialogDescription>
        </DialogHeader>
        <div className="flex h-full flex-col overflow-auto">
          <div className="flex items-center justify-between gap-4 pb-2 ">
            ...
            <Label>
              {t("custom.customAdditional", { value: custom.length })}
            </Label>
          </div>
          <div className="grow overflow-y-auto py-2  pr-4">
            <div className="flex flex-col gap-4">
              ...
              {facets.map((facet) => {
                const isCustom = !facet.facetValues.some(
                  (i) => i.code === defined[facet.code],
                );
                return (
