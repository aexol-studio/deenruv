...
import { AssetFragment } from "@deenruv/core/e2e/graphql/generated-e2e-admin-types";
...
describe("AssetServerPlugin", () => {
  ...
  const sourceFilePath = path.join(
    __dirname,
    TEST_ASSET_DIR,
    `source/b6/${IMAGE_BASENAME}.jpg`,
  );
  const previewFilePath = path.join(
    __dirname,
    TEST_ASSET_DIR,
    `preview/71/${IMAGE_BASENAME}__preview.jpg`,
  );
  ...
  it("names the Asset correctly", async () => {
    const filesToUpload = [
      path.join(__dirname, `fixtures/assets/${IMAGE_BASENAME}.jpg`),
    ];
    const { createAssets }: CreateAssetsMutation =
  ...
  it("can handle non-latin filenames", async () => {
    ...
    const filesToUpload = [
      path.join(__dirname, `fixtures/assets/${FILE_NAME_ZH}.jpg`),
    ];
    const { createAssets }: { createAssets: AssetFragment[] } =
      await adminClient.fileUploadMutation({
        mutation: CREATE_ASSETS,
        filePaths: filesToUpload,
        mapVariables: (filePaths) => ({
  ...
  describe("MIME type detection", () => {
    ...
    async function testMimeTypeOfAssetWithExt(
      ext: string,
    ...
    beforeAll(async () => {
      ...
      const filesToUpload = formats.map((ext) =>
        path.join(__dirname, `fixtures/assets/test.${ext}`),
      );
      const { createAssets }: CreateAssetsMutation =
        await adminClient.fileUploadMutation({
          mutation: CREATE_ASSETS,
  ...
  it("falls back to binary preview if image file cannot be processed", async () => {
    ...
    const { createAssets }: CreateAssets.Mutation =
      await adminClient.fileUploadMutation({
        mutation: CREATE_ASSETS,
        filePaths: filesToUpload,
        mapVariables: (filePaths) => ({
