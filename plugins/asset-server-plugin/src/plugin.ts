...
export class AssetServerPlugin implements NestModule, OnApplicationBootstrap {
  ...
  configure(consumer: MiddlewareConsumer) {
    ...
    registerPluginStartupMessage(
      "Asset server",
      AssetServerPlugin.options.route,
    );
  }
  ...
  private sendAsset() {
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        if (!mimeType) {
          mimeType =
            (await getFileType(file))?.mime || "application/octet-stream";
        }
    ...
    return async (
      ...
      if (err && (err.status === 404 || err.statusCode === 404)) {
        if (req.query) {
          ...
          Logger.debug(
            `Pre-cached Asset not found: ${decodedReqPath}`,
            loggerCtx,
          );
          let file: Buffer;
          try {
            file =
              await AssetServerPlugin.assetStorage.readFileToBuffer(
                decodedReqPath,
              );
          } catch (_err: any) {
