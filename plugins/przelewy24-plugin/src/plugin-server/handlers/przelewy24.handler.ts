...
export const przelewy24PaymentMethodHandler = new PaymentMethodHandler({
  ...
  async createPayment(ctx, order, amount, args, metadata) {
    const envs = {
      ...
      PRZELEWY24_URL:
        process.env.PRZELEWY24_URL || "https://sandbox.przelewy24.pl",
    };
    ...
    try {
      ...
      Logger.info(
        `Payment created for order ${order.id}`,
        "Przelewy24 Payment Handler",
      );
      ...
      if (blikCode) {
        ...
        if (verified) {
          ...
          Logger.error(
            `Blik payment failed for order ${order.id}`,
            "Przelewy24 Payment Handler",
          );
      ...
      const assigned = order.payments.find((p) => p.method === "przelewy24")
