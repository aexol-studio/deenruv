...
import { useCustomFields, useLazyQuery, useOrder } from '@deenruv/react-ui-devkit';
...
export const PaymentMethodInput = () => {
    ...
    const paymentURL = useMemo(() => {
        if (!value) return;
        const payment = order?.payments?.find(payment => payment.method === 'przelewy-24');
        if (!payment || !payment.metadata || payment.metadata.public) return null;
        return 'paymentUrl' in payment.metadata.public ? payment.metadata.public.paymentUrl : null;
    }, [order?.payments]);
    ...
    return (
        <div className="flex flex-col gap-2">
            <div style={{ width: '100%' }} className="flex flex-col gap-2">
                ...
                <div
                    style={{ width: '100%' }}
                    className="flex flex-col gap-2 border-[var(--color-card-border)] border-[1px] p-2"
                >
