...
export const molliePaymentHandler = new PaymentMethodHandler({
  ...
  args: {
    ...
    autoCapture: {
      type: "boolean",
      label: [
        { languageCode: LanguageCode.en, value: "Auto capture payments" },
      ],
      defaultValue: false,
      description: [
        {
    ...
    redirectUrl: {
      ...
      defaultValue: "",
      label: [
        { languageCode: LanguageCode.en, value: "Fallback redirect URL" },
      ],
      description: [
        {
          languageCode: LanguageCode.en,
          value:
            "Redirect URL to use when no URL is given by the storefront. Order code is appended to this URL",
        },
      ],
    },
  },
  ...
  createRefund: async (
    ...
    const molliePayment = molliePayments.find(
      (p) => p.status === PaymentStatus.paid,
    ); // Only one paid payment should be there
    if (!molliePayment) {
      throw Error(
