...
export class MollieService {
  ...
  async createPaymentIntent(
    ...
    if (!paymentMethod) {
      return new PaymentIntentError(
        `No paymentMethod found with code ${String(paymentMethodCode)}`,
      );
    }
    let redirectUrl = input.redirectUrl;
    if (!redirectUrl) {
      // Use fallback redirect if no redirectUrl is given
      let fallbackRedirect = paymentMethod.handler.args.find(
    ...
    const existingMollieOrderId = (order as OrderWithMollieReference)
    ...
    this.orderService
      ...
      .catch((e) => {
        Logger.error(
          `Failed to save Mollie order ID: ${(e as Error).message}`,
          loggerCtx,
        );
      });
    Logger.info(
  ...
  async handleMollieStatusUpdate(
    ...
    if (!paymentMethod) {
      ...
      return Logger.warn(
        `No paymentMethod found with id ${paymentMethodId}`,
        loggerCtx,
      );
    }
    const apiKey = paymentMethod.handler.args.find(
    ...
    const autoCapture =
      paymentMethod.handler.args.find((a) => a.name === "autoCapture")
        ?.value === "true";
    if (!apiKey) {
      throw Error(
        `No apiKey found for payment ${paymentMethod.id} for channel ${ctx.channel.token}`,
      );
    }
    ...
    let order = await this.orderService.findOneByCode(
      ctx,
      mollieOrder.orderNumber,
      ["payments"],
    );
    if (!order) {
      throw Error(
    ...
    if (
      ...
      mollieOrder.status === OrderStatus.authorized
    ) {
      ...
      if (autoCapture && mollieOrder.status === OrderStatus.authorized) {
        ...
        Logger.info(
          `Auto capturing payment for order ${order.code}`,
          loggerCtx,
        );
    ...
    if (
      ...
      mollieOrder.status === OrderStatus.completed
    ) {
  ...
  async settleExistingPayment(
    ...
    const payment = order.payments.find(
      (p) => p.transactionId === mollieOrderId,
    );
    if (!payment) {
      throw Error(
  ...
  async getEnabledPaymentMethods(
    ...
    if (!apiKey) {
      throw Error(
        `No apiKey configured for payment method ${paymentMethodCode}`,
      );
    }
  ...
  async getVariantsWithInsufficientStock(
    ctx: RequestContext,
    order: Order,
  ...
  async updateMollieOrder(
    ...
    const [order] = await Promise.all([
      this.updateMollieOrderData(
        mollieClient,
        existingMollieOrder,
        newMollieOrderInput,
      ),
      this.updateMollieOrderLines(
        mollieClient,
        existingMollieOrder,
        newMollieOrderInput.lines,
      ),
    ]);
    return order;
  }
  ...
  private async updateMollieOrderLines(
    ...
    return await mollieClient.manageOrderLines(
      existingMollieOrder.id,
      manageOrderLinesInput,
    );
  }
  ...
  private async canTransitionTo(
    ...
    const orderCopy = await assertFound(
  ...
  private async getOrder(
    ctx: RequestContext,
    orderId?: ID | null,
  ): Promise<Order | PaymentIntentError> {
    if (orderId) {
