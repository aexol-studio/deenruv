 ...
 */
export function createExtendedMollieClient(options: {
  apiKey: string;
}): ExtendedMollieClient {
  const client = createMollieClient(options) as ExtendedMollieClient;
  // Add our custom method
  client.manageOrderLines = async (
    orderId: string,
    input: ManageOrderLineInput,
  ): Promise<MollieOrder> => {
    ...
    const { status, data } = await instance.patch(
      `/v2/orders/${orderId}/lines`,
      input,
    );
