...
import { CurrencyCode, Customer, Order } from "@deenruv/core";
 ...
 */
export function toMollieAddress(
  address: OrderAddress,
  customer: Customer,
): MollieOrderAddress | undefined {
  if (
    address.city &&
    address.countryCode &&
    address.streetLine1 &&
 ...
 */
export function toMollieOrderLines(
  order: Order,
  alreadyPaid: number,
): CreateParameters["lines"] {
  // Add order lines
  const lines: CreateParameters["lines"] = order.lines.map((line) => ({
    name: line.productVariant.name,
    quantity: line.quantity,
    unitPrice: toAmount(
      line.proratedLinePriceWithTax / line.quantity,
      order.currencyCode,
    ), // totalAmount has to match unitPrice * quantity
  ...
  lines.push(
    ...order.surcharges.map((surcharge) => ({
      ...
      vatAmount: toAmount(
        surcharge.priceWithTax - surcharge.price,
        order.currencyCode,
      ),
    })),
  );
 ...
 * See https://github.com/aexol-studio/deenruv/issues/1939#issuecomment-1362962133 for more information on the rounding issue.
 */
export function calculateLineTaxAmount(
  taxRate: number,
  orderLinePriceWithTax: number,
): number {
...
export function getLocale(
  ...
  const channelLocale = allowedLocales.find(
    (locale) =>
      locale.toLowerCase().indexOf(channelLanguage.toLowerCase()) > -1,
  );
  if (channelLocale) {
