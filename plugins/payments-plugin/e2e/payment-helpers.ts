...
import { SimpleGraphQLClient, TestServer } from "@deenruv/testing";
...
export async function setShipping(
  ...
  const { setOrderShippingAddress: order } = await shopClient.query(
    SET_SHIPPING_ADDRESS,
    {
      input: {
        fullName: "name",
  ...
  const { eligibleShippingMethods } =
    await shopClient.query<GetShippingMethodsQuery>(
      GET_ELIGIBLE_SHIPPING_METHODS,
    );
...
export async function createFixedDiscountCoupon(
  ...
  if (createPromotion.__typename === "ErrorResult") {
    throw new Error(
      `Error creating coupon: ${(createPromotion as ErrorResult).errorCode}`,
    );
  }
}
/**
