import { EntityHydrator, mergeConfig } from "@deenruv/core";
import {
...
} from "@deenruv/core/e2e/graphql/shared-definitions";
import {
...
describe("Stripe payments", () => {
  ...
  it("Should add a Stripe paymentMethod", async () => {
    ...
    expect(createPaymentMethod.code).toBe(
      `stripe-payment-${E2E_DEFAULT_CHANNEL_TOKEN}`,
    );
  ...
  it("if no customer id exists, makes a call to create", async () => {
    ...
    const { createStripePaymentIntent } = await shopClient.query(
  ...
  it("Should not crash on signature validation failure", async () => {
    ...
    const result = await fetch(
      `http://localhost:${serverPort}/payments/stripe`,
      {
        method: "post",
        body: payloadString,
  ...
  it("Should validate the webhook's signature properly", async () => {
    ...
    const event = stripeWebhooks.constructEvent(
      payloadString,
      header,
      "test-signing-secret",
    );
  ...
  describe("currencies with no fractional units", () => {
    ...
    it("sends correct amount when creating payment intent", async () => {
      ...
      expect(createPaymentIntentPayload.amount).toBe(
        (activeOrder!.totalWithTax / 100).toString(),
      );
