...
} from "@deenruv/core/e2e/graphql/generated-e2e-admin-types";
import { SETTLE_PAYMENT } from "@deenruv/core/e2e/graphql/shared-definitions";
import {
...
describe("Mollie payments", () => {
  ...
  describe("Payment intent creation", () => {
    it("Should prepare an order", async () => {
      ...
      await server.app
        .get(OrderService)
        .addSurchargeToOrder(ctx, order.id.replace("T_", ""), {
    ...
    it("Should fail to get payment url when items are out of stock", async () => {
      let { updateProductVariants } = await adminClient.query(
      ...
      expect(result.message).toContain(
        "insufficient stock of Pinelab stickers",
      );
      // Set stock back to not tracking
      ({ updateProductVariants } = await adminClient.query(
        UPDATE_PRODUCT_VARIANTS,
        {
          input: {
            id: "T_5",
    ...
    it("Should get payment url without Mollie method", async () => {
      ...
      const { createMolliePaymentIntent } = await shopClient.query(
        CREATE_MOLLIE_PAYMENT_INTENT,
        {
          input: {
            paymentMethodCode: mockData.methodCode,
      ...
      expect(mollieRequest?.redirectUrl).toEqual(
        "given-storefront-redirect-url",
      );
    ...
    it("Should use fallback redirect appended with order code, when no redirect is given", async () => {
      ...
      expect(mollieRequest?.redirectUrl).toEqual(
        `${mockData.redirectUrl}/${order.code}`,
      );
    });
    ...
    it("Should recreate all order lines in Mollie", async () => {
      ...
      nock("https://api.mollie.com/")
        .patch(
          `/v2/orders/${mockData.mollieOrderResponse.id}/lines`,
          (body) => {
      ...
      const cancelledLines = molliePatchRequest.operations.filter(
        (o: any) => o.operation === "cancel",
      );
      expect(cancelledLines.length).toBe(3);
      // Should have added all 3 new order lines
      const addedLines = molliePatchRequest.operations.filter(
        (o: any) => o.operation === "add",
      );
    ...
    it("Should create intent as admin", async () => {
      ...
      expect(intent.url).toBe(
        mockData.mollieOrderResponse._links.checkout.href,
      );
    });
    ...
    it("Should get available paymentMethods", async () => {
      ...
      await shopClient.asUserWithCredentials(customers[0].emailAddress, "test");
      const { molliePaymentMethods } = await shopClient.query(
        GET_MOLLIE_PAYMENT_METHODS,
        {
          input: {
            paymentMethodCode: mockData.methodCode,
    ...
    it("Transitions to PaymentSettled for orders with a total of $0", async () => {
      ...
      await shopClient.query(APPLY_COUPON_CODE, {
        couponCode: "DISCOUNT_ORDER",
      });
      await shopClient.query(APPLY_COUPON_CODE, {
        couponCode: "FREE_SHIPPING",
      });
  ...
  describe("Handle standard payment methods", () => {
    it("Should transition to ArrangingPayment when partially paid", async () => {
      ...
      await fetch(
        `http://localhost:${serverPort}/payments/mollie/${E2E_DEFAULT_CHANNEL_TOKEN}/1`,
        {
          method: "post",
    ...
    it("Should have Mollie metadata on payment", async () => {
      ...
      expect(metadata.authorizedAt).toEqual(
        mockData.mollieOrderResponse.authorizedAt.toISOString(),
      );
      expect(metadata.paidAt).toEqual(
        mockData.mollieOrderResponse.paidAt.toISOString(),
      );
    });
