...
@DeenruvPlugin({
  ...
  providers: [
    ...
    SearchJobBufferService,
    {
      provide: ELASTIC_SEARCH_OPTIONS,
...
export class ElasticsearchPlugin implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    try {
      ...
      Logger.error(
        `Could not connect to Elasticsearch instance at "${nodeName}"`,
        loggerCtx,
      );
    ...
    Logger.info(
      `Successfully connected to Elasticsearch instance at "${nodeName}"`,
      loggerCtx,
    );
    ...
    this.eventBus.ofType(ProductEvent).subscribe((event) => {
      if (event.type === "deleted") {
        return this.elasticsearchIndexService.deleteProduct(
          event.ctx,
          event.product,
        );
      } else {
        return this.elasticsearchIndexService.updateProduct(
          event.ctx,
          event.product,
        );
      }
    ...
    this.eventBus.ofType(ProductVariantEvent).subscribe((event) => {
      if (event.type === "deleted") {
        return this.elasticsearchIndexService.deleteVariant(
          event.ctx,
          event.variants,
        );
      } else {
    ...
    this.eventBus.ofType(AssetEvent).subscribe((event) => {
      if (event.type === "updated") {
        return this.elasticsearchIndexService.updateAsset(
          event.ctx,
          event.asset,
        );
      }
      if (event.type === "deleted") {
        return this.elasticsearchIndexService.deleteAsset(
          event.ctx,
          event.asset,
        );
      }
    ...
    const collectionModification$ = this.eventBus.ofType(
      CollectionModificationEvent,
    );
    ...
    collectionModification$
      ...
      .subscribe((events) => {
        return this.elasticsearchIndexService.updateVariantsById(
          events.ctx,
          events.ids,
        );
      });
    ...
    this.eventBus
      ...
      .subscribe((event) => {
        ...
        if (
          defaultTaxZone &&
          idsAreEqual(defaultTaxZone.id, event.taxRate.zone.id)
        ) {
