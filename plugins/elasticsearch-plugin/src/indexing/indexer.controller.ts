...
export const defaultVariantRelations: Array<
  EntityRelationPaths<ProductVariant>
> = [
  "featuredAsset",
  "facetValues",
  "facetValues.facet",
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor(
    ...
    @Inject(ELASTIC_SEARCH_OPTIONS)
    private options: Required<ElasticsearchOptions>,
  ...
  async updateProduct({
    ctx: rawContext,
    productId,
  }: UpdateProductMessageData): Promise<boolean> {
  ...
  async updateVariants({
    ctx: rawContext,
    variantIds,
  }: UpdateVariantMessageData): Promise<boolean> {
    const ctx = MutableRequestContext.deserialize(rawContext);
  ...
  reindex({
    ctx: rawContext,
  }: ReindexMessageData): Observable<ReindexMessageResponse> {
    return asyncObservable(async (observer) => {
      return this.asyncQueue.push(async () => {
        const timeStart = Date.now();
        const ctx = MutableRequestContext.deserialize(rawContext);
        ...
        do {
          ...
          Logger.verbose(
            `Done ${finishedProductsCount} / ${totalProductIds} products`,
          );
  ...
  async updateAsset(data: UpdateAssetMessageData): Promise<boolean> {
    const result = await this.updateAssetFocalPointForIndex(
      VARIANT_INDEX_NAME,
      data.asset,
    );
  ...
  async deleteAsset(data: UpdateAssetMessageData): Promise<boolean> {
    const result = await this.deleteAssetForIndex(
      VARIANT_INDEX_NAME,
      data.asset,
    );
    await this.client.indices.refresh({
  ...
  private async updateAssetFocalPointForIndex(
    indexName: string,
    asset: Asset,
  ): Promise<boolean> {
    ...
    return this.updateAssetForIndex(
      ...
      {
        source:
          "ctx._source.productVariantPreviewFocalPoint = params.focalPoint",
        params,
      },
    );
  }

  private async deleteAssetForIndex(
    indexName: string,
    asset: Asset,
  ): Promise<boolean> {
    return this.updateAssetForIndex(
  ...
  private async updateAssetForIndex(
    ...
    for (const failure of result1.body.failures) {
      Logger.error(
        `${failure.cause.type as string}: ${failure.cause.reason as string}`,
        loggerCtx,
      );
    }
  ...
  private async updateProductsInternal(
  ...
  private async switchAlias(
    ...
    try {
      ...
      if (reindexVariantAliasExist.body) {
        ...
        const reindexVariantIndexName = Object.keys(
          reindexVariantAliasResult.body,
        )[0];
        await this.client.indices.delete({
          index: [reindexVariantIndexName],
        });
      ...
      for (const languageCode of uniqueLanguageVariants) {
        if (variantsInChannel.length) {
          for (const variant of variantsInChannel) {
            ...
            if (
              operations.length >= this.options.reindexBulkOperationSizeLimit
            ) {
  ...
  private async deleteProductOperations(
    ...
    const channels = await this.requestContextCache.get(
      ctx,
      "elastic-index-all-channels",
      () =>
  ...
  private async executeBulkOperations(
    ...
    const variantOperations: Array<
      BulkOperation | BulkOperationDoc<VariantIndexItem>
    > = [];
  ...
  private async runBulkOperationsOnIndex(
    indexName: string,
    operations: Array<
      BulkOperation | BulkOperationDoc<VariantIndexItem | ProductIndexItem>
    >,
  ) {
    ...
    try {
      ...
      Logger.error(
        `Error when attempting to run bulk operations [${JSON.stringify(e)}]`,
        loggerCtx,
      );
      Logger.error(
        "Error details: " + JSON.stringify(e.body?.error, null, 2),
        loggerCtx,
      );
    }
  }
  ...
  private async createVariantIndexItem(
    ...
    try {
      ...
      const collectionTranslations = v.collections.map((c) =>
      ...
      const item: VariantIndexItem = {
        ...
        productEnabled:
          variants.some((variant) => variant.enabled) && v.product.enabled,
        ...
        productCollectionSlugs: unique(
          productCollectionTranslations.map((c) => c.slug),
        ),
        productChannelIds: v.product.channels.map((c) => c.id),
        inStock:
          0 < (await this.productVariantService.getSaleableStockLevel(ctx, v)),
        productInStock: await this.getProductInStockValue(ctx, variants),
      };
      const variantCustomMappings = Object.entries(
        this.options.customProductVariantMappings,
      );
      for (const [name, def] of variantCustomMappings) {
        item[`variant-${name}`] = await def.valueFn(
          v,
          languageCode,
          this.injector,
          ctx,
      ...
      const productCustomMappings = Object.entries(
  ...
  private async getProductInStockValue(
    ctx: RequestContext,
    variants: ProductVariant[],
  ): Promise<boolean> {
  ...
  private async createSyntheticProductIndexItem(
    ...
    const item: VariantIndexItem = {
      ...
      productFacetIds:
        product.facetValues?.map((fv) => fv.facet.id.toString()) ?? [],
      productFacetValueIds:
        product.facetValues?.map((fv) => fv.id.toString()) ?? [],
  ...
  private getTranslation<T extends Translatable>(
    ...
    return (translatable.translations.find(
      (t) => t.languageCode === languageCode,
    ) ||
      translatable.translations.find(
        (t) => t.languageCode === this.configService.defaultLanguageCode,
      ) ||
  ...
  private getFacetValueIds(variants: ProductVariant[]): string[] {
    ...
    const productFacetValueIds =
      variants[0].product.facetValues.map(facetValueIds);
