...
import { DeepRequired, ID, Logger } from "@deenruv/core";
...
export async function createIndices(
  ...
  const createIndex = async (
    mappings: { [prop in keyof any]: any },
    index: string,
    alias: string,
  ) => {
    if (mapAlias) {
...
export async function deleteIndices(client: Client, prefix: string) {
  try {
    const index = await getIndexNameByAlias(
      client,
      prefix + VARIANT_INDEX_NAME,
    );
...
export async function deleteByChannel(
  ...
  try {
    ...
    Logger.verbose(
      `Deleted index "${index} for channel "${channelId}"`,
      loggerCtx,
    );
  } catch (e: any) {
    Logger.error(e, loggerCtx);
