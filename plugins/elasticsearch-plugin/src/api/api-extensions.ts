...
export function generateSchemaExtensions(
...
function generateCustomMappingTypes(
  options: ElasticsearchOptions,
): DocumentNode | undefined {
  const productMappings = Object.entries(
    options.customProductMappings || {},
  ).filter(([, value]) => value.public ?? true);
  const variantMappings = Object.entries(
    options.customProductVariantMappings || {},
  ).filter(([, value]) => value.public ?? true);
  const searchInputTypeExtensions = Object.entries(
    options.extendSearchInputType || {},
  );
  const scriptProductFields = Object.entries(
    options.searchConfig?.scriptFields || {},
  ).filter(([, scriptField]) => scriptField.context !== "variant");
  const scriptVariantFields = Object.entries(
    options.searchConfig?.scriptFields || {},
  ).filter(([, scriptField]) => scriptField.context !== "product");
  let sdl = "";
