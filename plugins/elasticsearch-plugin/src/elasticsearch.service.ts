...
import { SearchResultAsset } from "@deenruv/common/generated-types";
import {
  Collection,
  CollectionService,
  ConfigService,
...
export class ElasticsearchService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    private options: DeepRequired<ElasticsearchOptions>,
    ...
    await new Promise<void>(async (resolve, reject) => {
      ...
      while (attempts < connectionAttempts) {
        ...
        await new Promise((resolve1) =>
          setTimeout(resolve1, connectionAttemptInterval),
        );
      }
  ...
  async createIndicesIfNotExists() {
    ...
    const createIndex = async (indexName: string) => {
      ...
      if (!result.body) {
        Logger.verbose(
          `Index "${index}" does not exist. Creating...`,
          loggerCtx,
        );
        await createIndices(
          this.client,
        ...
        const existingIndexSettingsResult =
          await this.client.indices.getSettings({ index });
        const existingIndexSettings =
          existingIndexSettingsResult.body[
        ...
        const tempIndexSettings =
          tempIndexSettingsResult.body[tempIndex].settings.index;
        ...
        if (!equal(tempIndexSettings, existingIndexSettings))
          ...
          const existingIndexMappingsResult =
            await this.client.indices.getMapping({ index });
          ...
          const tempIndexMappings =
            tempIndexMappingsResult.body[tempIndex].mappings;
  ...
  async search(
    ...
    Omit<ElasticSearchResponse, "facetValues" | "collections" | "priceRange">
  > {
    const { indexPrefix } = this.options;
    ...
    if (groupByProduct) {
      try {
        const { body }: { body: SearchResponseBody<VariantIndexItem> } =
        ...
        return {
          items: body.hits.hits.map((hit) =>
            this.mapProductToSearchResult(hit),
          ),
          totalItems,
        };
      } catch (e: any) {
        if (
          ...
          e.meta.body.error.type === "search_phase_execution_exception"
        ) {
          ...
          Logger.verbose(
            JSON.stringify(e.meta.body.error.failed_shards || [], null, 2),
            loggerCtx,
          );
        } else {
  ...
  async facetValues(
    ...
    return facetValues.map((facetValue) => {
      const bucket = buckets.find(
        (b) => b.key.toString() === facetValue.id.toString(),
      );
      let count;
      if (groupByProduct) {
  ...
  async collections(
    ...
    return collections.map((collection) => {
      const bucket = buckets.find(
        (b) => b.key.toString() === collection.id.toString(),
      );
      let count;
      if (groupByProduct) {
        count = bucket ? bucket.total.value : 0;
  ...
  async getDistinctBucketsOfField(
    ...
    Array<{ key: string; doc_count: number; total: { value: number } }>
  > {
    const { indexPrefix } = this.options;
    const { groupByProduct } = input;
    const elasticSearchBody = buildElasticBody(
    ...
    try {
      const result = await this.client.search<
        SearchResponseBody<VariantIndexItem>
      >({
        index: indexPrefix + VARIANT_INDEX_NAME,
        body: elasticSearchBody,
      });
  ...
  private mapVariantToSearchResult(
    ...
    const { productAsset, productVariantAsset } =
      this.getSearchResultAssets(source);
    const result = {
      ...source,
      productAsset,
      productVariantAsset,
  ...
  private getSearchResultAssets(source: ProductIndexItem | VariantIndexItem): {
    ...
    const productAsset: SearchResultAsset | undefined = source.productAssetId
      ...
      : undefined;
    const productVariantAsset: SearchResultAsset | undefined =
      source.productVariantAssetId
        ? {
