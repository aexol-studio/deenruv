...
export function buildElasticBody(
  ...
  if (facetValueIds && facetValueIds.length) {
    ensureBoolFilterExists(query);
    const operator =
      facetValueOperator === LogicalOperator.AND ? "must" : "should";
  ...
  if (priceRange) {
    ensureBoolFilterExists(query);
    query.bool.filter = query.bool.filter.concat(
      createPriceFilters(priceRange, false),
    );
  }
  if (priceRangeWithTax) {
    ensureBoolFilterExists(query);
    query.bool.filter = query.bool.filter.concat(
      createPriceFilters(priceRangeWithTax, true),
    );
  }
  ...
  if (sort) {
    if (sort.name) {
      sortArray.push({
        "productName.keyword": {
          order: sort.name === SortOrder.ASC ? "asc" : "desc",
        },
      });
    }
    if (sort.price) {
      ...
      sortArray.push({
        [priceField]: { order: sort.price === SortOrder.ASC ? "asc" : "desc" },
      });
    }
  }
