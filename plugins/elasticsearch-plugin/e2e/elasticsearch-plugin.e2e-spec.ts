import { CurrencyCode, SortOrder } from "@deenruv/common/lib/generated-types";
...
describe("Elasticsearch plugin", () => {
            ...
            scriptFields: {
              answerMultiplied: {
                ...
                scriptFn: (input) => {
                  ...
                  return {
                    script: `doc['product-answer'].value * ${factor as string}`,
                  };
                },
              },
            },
  ...
  describe("shop api", () => {
    ...
    it("matches by facetValueId with AND operator", () =>
      testMatchFacetIdsAnd(shopClient));

    it("matches by facetValueId with OR operator", () =>
      testMatchFacetIdsOr(shopClient));

    it("matches by FacetValueFilters AND", () =>
      testMatchFacetValueFiltersAnd(shopClient));

    it("matches by FacetValueFilters OR", () =>
      testMatchFacetValueFiltersOr(shopClient));

    it("matches by FacetValueFilters OR and AND", () =>
      testMatchFacetValueFiltersOrWithAnd(shopClient));
    ...
    it("omits results for disabled ProductVariants", async () => {
      ...
      expect(
        result.search.items.map((i) => i.productVariantId).includes("T_3"),
      ).toBe(false);
    });
  ...
  describe("admin api", () => {
    ...
    describe("updating the index", () => {
      ...
      it("updates index when a Product is deleted", async () => {
        ...
        expect(search2.items.map((i) => i.productId).sort()).toEqual([
          "T_2",
          "T_3",
          "T_4",
          "T_6",
      ...
      it("does not include deleted ProductVariants in index", async () => {
        ...
        const variantToDelete = s1.items.find(
    ...
    describe("channel handling", () => {
      ...
      it("new channel is initially empty", async () => {
        ...
        const { search: searchUngrouped } = await doAdminSearchQuery(
          adminClient,
          {
            groupByProduct: false,
          },
      ...
      it("reindexes in channel", async () => {
        ...
        const { job } = await adminClient.query<
          GetJobInfoQuery,
          GetJobInfoQueryVariables
        >(GET_JOB_INFO, { id: reindex.id });
      ...
      it("adding product variant to channel", async () => {
        ...
        expect(searchGrouped.items.map((i) => i.productId).sort()).toEqual([
          "T_1",
          "T_3",
          "T_4",
        ]);
        ...
        expect(
          searchUngrouped.items.map((i) => i.productVariantId).sort(),
        ).toEqual(["T_1", "T_10", "T_15", "T_2", "T_3", "T_4"]);
      });
    ...
    describe("multiple language handling", () => {
      ...
      it("indexes product-level languages", async () => {
        const { search: search1 } = await searchInLanguage(
          LanguageCode.de,
          true,
        );
        ...
        const { search: search2 } = await searchInLanguage(
          LanguageCode.zh,
          true,
        );
      ...
      it("indexes product variant-level languages", async () => {
        ...
        expect(
          search1.items.length ? search1.items[0].productName : undefined,
        ).toBe("Laptop");
        expect(
          search1.items.length
            ? search1.items[0].productVariantName
            : undefined,
        ).toBe("laptop variant fr");
      });
    });
  });
  ...
  describe("customMappings", () => {
    ...
    // https://github.com/aexol-studio/deenruv/issues/1638
    it("hydrates variant custom field relation", async () => {
      ...
      expect(search.items.map((i: any) => i.customMappings)).toEqual([
        { materialCode: "electronics" },
      ]);
    });
