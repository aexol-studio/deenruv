...
describe("EmailPlugin", () => {
  ...
  describe("event filtering", () => {
    ...
    it("multiple filters", async () => {
      ...
      const ctxWithUser = RequestContext.deserialize({
        ...ctx,
        _session: { user: { id: 42 } },
      } as any);
  ...
  describe("templateVars", () => {
    ...
    it("loads globalTemplateVars async", async () => {
      ...
      await initPluginWithHandlers([handler], {
        globalTemplateVars: async (
          _ctxLocal: RequestContext,
          injector: Injector,
        ) => {
    ...
    it("interpolates from", async () => {
      ...
      eventBus.publish(new MockEvent(ctx, true));
      await pause();
      expect(onSend.mock.calls[0][0].from).toBe(
        '"test from baz" <noreply@test.com>',
      );
    });
    ...
    it("globalTemplateVars available in setTemplateVars method", async () => {
      const handler = new EmailEventListener("test")
        ...
        .setTemplateVars((event, globals) => ({
          testVar: (globals.globalVar as string) + " quux",
        }));
    ...
    it("formateDate", async () => {
      const handler = new EmailEventListener("test-helpers")
        ...
        .setTemplateVars((event) => ({
          myDate: new Date("2020-01-01T10:00:00.000Z"),
          myPrice: 0,
        }));
      ...
      eventBus.publish(new MockEvent(ctx, true));
      await pause();
      expect(onSend.mock.calls[0][0].body).toContain(
        "Date: Wed Jan 01 2020 10:00:00",
      );
    });
  ...
  describe("multiple configs", () => {
    ...
    it("additional LanguageCode", async () => {
      ...
      const ctxTa = RequestContext.deserialize({
        ...ctx,
        _languageCode: LanguageCode.ta,
      } as any);
      ...
      const ctxDe = RequestContext.deserialize({
        ...ctx,
    ...
    it("set LanguageCode", async () => {
      ...
      const ctxEn = RequestContext.deserialize({
        ...ctx,
        _languageCode: LanguageCode.en,
      } as any);
  ...
  describe("orderConfirmationHandler", () => {
    ...
    it("filters events with wrong order state", async () => {
      ...
      eventBus.publish(
        new OrderStateTransitionEvent("AddingItems", "Cancelled", ctx, order),
      );
      await pause();
      expect(onSend).not.toHaveBeenCalled();
    ...
    it("sets the Order Customer emailAddress as recipient", async () => {
      ...
      expect(onSend.mock.calls[0][0].recipient).toBe(
    ...
    it("sets the subject", async () => {
      ...
      expect(onSend.mock.calls[0][0].subject).toBe(
        `Order confirmation for #${order.code as string}`,
      );
    });
  });
  ...
  describe("error handling", () => {
    it("Logs an error if the template file is not found", async () => {
      ...
      eventBus.publish(new MockEvent(ctx, true));
      await pause();
      expect(testingLogger.errorSpy.mock.calls[0][0]).toContain(
  ...
  describe("Dynamic subject handling", () => {
    ...
    it("With callback function", async () => {
      ...
      await pause();
      expect(onSend.mock.calls[0][0].subject).toBe(
        "Hello from loaded data and foo",
      );
      expect(onSend.mock.calls[0][0].recipient).toBe("test@test.com");
      expect(onSend.mock.calls[0][0].from).toBe(
        '"test from" <noreply@test.com>',
...
class FakeCustomSender implements EmailSender {
  send: (
    email: EmailDetails<"unserialized">,
