...
> {
   ...
   */
  setRecipient(
    setRecipientFn: (event: Event) => string,
   ...
   */
  setTemplateVars(
    templateVarsFn: SetTemplateVarsFn<Event>,
  ): EmailEventHandler<T, Event> {
   ...
   */
  setSubject(
    defaultSubject: string | SetSubjectFn<Event>,
  ): EmailEventHandler<T, Event> {
  ...
  async handle(
    ...
    const optionalAddressFields =
      (await this.setOptionalAddressFieldsFn?.(event)) ?? {};
    return {
