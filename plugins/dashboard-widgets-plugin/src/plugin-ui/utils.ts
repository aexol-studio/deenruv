      ...
      switch (groupBy) {
        case "week": {
          ...
          item.additionalData?.forEach((data) => {
            const existingRecord = acc[week].additionalData?.find(
        ...
        case "month": {
          ...
          const month =
            language === "pl"
              ? monthsMap[monthDate as keyof typeof monthsMap]
              : monthDate;
          if (!acc[month])
          ...
          item.additionalData?.forEach((data) => {
            const existingRecord = acc[month].additionalData?.find(
              (item) => item.id === data.id,
            );
            if (existingRecord) {
        ...
        case "quarter": {
          ...
          item.additionalData?.forEach((data) => {
            const existingRecord = acc[quarter].additionalData?.find(
              (item) => item.id === data.id,
            );
            if (existingRecord) {
