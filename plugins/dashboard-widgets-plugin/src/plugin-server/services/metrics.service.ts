...
export class BetterMetricsService {
  ...
  async refreshViews(ctx: RequestContext) {
    await this.connection
      ...
      .query(
        `REFRESH MATERIALIZED VIEW CONCURRENTLY order_summary_view_entity`,
      );
    await this.connection
      .getRepository(ctx, OrderSummaryWithStateViewEntity)
      .query(
        `REFRESH MATERIALIZED VIEW CONCURRENTLY order_summary_with_state_view_entity`,
      );
    await this.connection
      .getRepository(ctx, TotalProductsViewEntity)
      .query(
        `REFRESH MATERIALIZED VIEW CONCURRENTLY total_products_view_entity`,
      );
    await this.connection
      .getRepository(ctx, TotalProductsWithStateViewEntity)
      .query(
        `REFRESH MATERIALIZED VIEW CONCURRENTLY total_products_with_state_view_entity`,
      );
  }
  ...
  async loadChartData(
    ...
    const shoulBeFromDefaultView = areArraysIdentical(
      DEFAULT_ORDER_STATES_ARRAY,
      orderStates,
    );
    ...
    if (metricType === ChartMetricType.OrderTotalProductsCount) {
      ...
      if (interval === MetricIntervalType.Hour) {
        viewQb.select(
          'extract(epoch from tpv.hour - :startDate)::integer / 3600 AS "intervalTick"',
        );
      } else {
        viewQb.select(
      ...
      const mappedResponse = Object.entries(reducedRes)?.map(
        ([key, r]: [key: string, r: any]) => ({
          intervalTick: key,
  ...
  async loadSummaryOrdersData(
    ...
    return {
      ...
      averageOrderValue: +(
        +(+viewResponse?.total / +viewResponse?.orderCount) || 0
      ).toFixed(2),
