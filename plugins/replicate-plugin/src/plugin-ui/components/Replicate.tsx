...
import type {
  ReplicateEntityListType,
  ReplicatePredictionListType,
} from "../graphql/selectors.js";
import {
  DatePickerWithRange,
  copyToClipboard,
  exportToCsv,
...
export const ReplicateInput = () => {
  ...
  const [totalItems, setTotalItems] =
    useState<ReplicateEntityListType["totalItems"]>(0);
  const [predictions, setPredictions] = useState<
    ReplicatePredictionListType["predictions"]
  >([]);
  const { register, handleSubmit } = methods;
  const [startOrderExportToReplicate] = useMutation(
    startOrderExportToReplicateMutation,
  );
    ...
    getReplicatePredictions({
      ...
      if (response?.getReplicatePredictions) {
        ...
        const startingPredictions = predictions.filter(
          (item) => item.status === PredictionStatus.starting,
        );
    ...
    const intervalId = setInterval(() => {
      ...
      getReplicatePredictions({
        ...
        if (response?.getReplicatePredictions) {
          ...
          const startingPrediction = predictions.find(
            (item) => item.status === PredictionStatus.starting,
          );
  ...
  function fetchPrediction(predictionID: string) {
    ...
    getPredictionItem({
      ...
      .then((response) => {
        if (response?.getPredictionItem) {
          if (
            response.getPredictionItem.status === PredictionStatus.succeeded
          ) {
            setPredictions(response.getPredictionItem.predictions);
            setLoading(false);
          } else if (
            response.getPredictionItem.status === PredictionStatus.failed
          ) {
            setPredictions([]);
            setLoading(false);
          } else if (
            response.getPredictionItem.status === PredictionStatus.starting
          ) {
  ...
  return (
    <FormProvider {...methods}>
      <div className="flex h-screen w-full gap-4 px-4 py-2 md:px-8 md:py-4">
        <div className="flex w-1/3 flex-col gap-4">
          <Card className="w-full">
            <CardHeader>
              <div className="flex w-full items-center justify-between">
                ...
                <Button onClick={handleSubmit((data) => submit(data))}>
                  {t("Run model")}
                </Button>
              </div>
            </CardHeader>
          ...
          <Card className="h-[calc(100vh-288px)] w-full">
            <CardHeader>
        ...
        <Card className="h-[calc(100vh-100px)] w-2/3">
          <CardHeader>
            <div className="flex w-full items-center justify-between">
              <div className="flex flex-col gap-2">
              ...
              {predictions?.length != 0 && (
                <div className="flex gap-1">
                      ...
                      copyToClipboard(
                        "[" +
                          predictions
                            .map(
                              (predictions) =>
                                predictions.customer?.emailAddress,
                            )
                            .join(", ") +
                          "]",
                      )
                  ...
                  <Button onClick={() => exportToCsv(predictions)}>
                    {t("Export to CSV")}
                  </Button>
                </div>
              )}
