...
import { Injector, Job } from "@deenruv/core";
...
describe("PubSubJobQueueStrategy", () => {
  ...
  beforeEach(async () => {
    ...
    const options = {
      ...
      queueNamePubSubPair: new Map([
        ["test-queue", ["test-topic", "test-subscription"]],
      ]),
    } as PubSubOptions;
  ...
  it("cannot publish to not configured queue", async () => {
    ...
    try {
      ...
      expect(err).toEqual(
        new Error("Topic name not set for queue: some-queue"),
      );
    }
  ...
  it("publishes new jobs to topic", async () => {
    ...
    expect(topic.publish).toHaveBeenCalledWith(
