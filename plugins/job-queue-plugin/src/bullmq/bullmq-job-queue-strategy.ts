...
export class BullMQJobQueueStrategy implements InspectableJobQueueStrategy {
  ...
  async init(injector: Injector): Promise<void> {
    ...
    this.queue = new Queue(QUEUE_NAME, {
      ...
      .on("error", (e: any) =>
        Logger.error(
          `BullMQ Queue error: ${JSON.stringify(e.message)}`,
          loggerCtx,
          e.stack,
        ),
      )
    ...
    this.workerProcessor = async (bullJob) => {
      ...
      if (processFn) {
        ...
        try {
          ...
          this.cancelRunningJob$
            ...
            .subscribe(() => {
              Logger.info(
                `Setting job ${job.id ?? ""} as cancelled`,
                loggerCtx,
              );
              job.cancel();
            });
          ...
          if (job.id) {
            await this.redisConnection.srem(
              this.CANCELLED_JOB_LIST_NAME,
              job.id?.toString(),
            );
          }
          completed$.next();
      ...
      throw new InternalServerError(
        `No processor defined for the queue "${queueName}"`,
      );
    };
  ...
  async add<Data extends JobData<Data> = object>(
    ...
    const retries =
      this.options.setRetries?.(job.queueName, job) ?? job.retries ?? 0;
  ...
  async findMany(options?: JobListOptions): Promise<PaginatedList<Job>> {
    ...
    return {
      items: await Promise.all(
        items.map((bullJob) => this.createDeenruvJob(bullJob)),
      ),
      totalItems,
    };
  }
  ...
  async findManyById(ids: ID[]): Promise<Job[]> {
    const bullJobs = await Promise.all(
      ids.map((id) => this.queue.getJob(id.toString())),
    );
    return Promise.all(
      bullJobs.filter(notNullOrUndefined).map((j) => this.createDeenruvJob(j)),
    );
  }
  ...
  async start<Data extends JobData<Data> = object>(
    ...
    if (!this.worker) {
      ...
      this.worker = new Worker(QUEUE_NAME, this.workerProcessor, options)
        ...
        .on("closing", (e) =>
          Logger.verbose(`BullMQ Worker closing: ${e}`, loggerCtx),
        )
        ...
        .on("completed", (job: Bull.Job) => {
          Logger.debug(
            `Job ${job?.id ?? "unknown id"} [${job.name}] completed`,
            loggerCtx,
          );
        });
  ...
  private async getState(bullJob: Bull.Job): Promise<JobState> {
    ...
    if (await bullJob.isActive()) {
      const isCancelled =
        jobId &&
        (await this.redisConnection.sismember(
          this.CANCELLED_JOB_LIST_NAME,
          jobId,
        ));
      if (isCancelled) {
