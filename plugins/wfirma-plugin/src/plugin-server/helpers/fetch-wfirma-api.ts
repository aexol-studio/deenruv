...
const API_RETURN_CODES = {
  ...
  "AUTH FAILED LIMIT WAIT 5 MINUTES":
    "Błąd autoryzacji. Przekroczono limit prób. Odczekaj 5 minut",
...
const buildAxiosHeaders = (
  authHeaders: WFirmaPluginConfig["authorization"],
) => {
  const headers: any = {
...
export const fetchWFirmaAPI =
  ...
  async <T extends keyof WFirmaTypes>(
    endpoint: T,
    data: WFirmaTypes[T]["input"],
    invoiceID?: string,
  ) => {
    ...
    try {
      ...
      Logger.info(
        `WFirma API response: ${JSON.stringify(response.data)}`,
        "fetchWFirmaAPI",
      );
