...
export class WFirmaService {
    ...
    async createInvoice(
        ...
        contractorID = orderData.contractor.email
            ? await this.findContractor(orderData.contractor.email, orderData.contractor.street ?? '')
            : null;
    ...
    private async getOrderDataForInvoice(ctx: RequestContext, orderID: string) {
        try {
            ...
            if (!order.shippingAddress || !order.shippingAddress.city || !order.shippingAddress.postalCode) {
            ...
            const contractor: Contractor = {
                name: companyName ? companyName : order.customer.firstName + ' ' + order.customer.lastName,
                street:
            ...
            const lineItemsDistributedDiscountArray = order.lines.map(line => {
                ...
                const price =
                    basePrice - distributedOderDiscount > 0 ? basePrice - distributedOderDiscount : 0;

                const excessDiscount =
                    basePrice - distributedOderDiscount > 0 ? 0 : distributedOderDiscount - basePrice;
            ...
            const invoicecontents: AddInvoiceInput['invoices'][number]['invoice']['invoicecontents'] =
                order.lines.map((line, index) => {
                    ...
                    if (totalExcessDiscount > 0) {
                        priceValue = totalExcessDiscount > priceValue ? 0 : priceValue - totalExcessDiscount;
                        totalExcessDiscount -= lineItemsDistributedDiscountArray[index].price;
                    }
