...
export const CopyOrderButton = () => {
  ...
  const NOT_ALLOWED =
    configs.get(PLUGIN_NAME)?.notAllowedStates || DEFAULT_NOT_ALLOWED;
  const copyOrder = async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : t("copy-order-error");
  ...
  return (
    <DropdownMenuItem asChild>
      <Button
        onClick={copyOrder}
        variant="ghost"
        className="w-full cursor-pointer justify-start px-4 py-2 text-blue-600 focus-visible:ring-transparent dark:focus-visible:ring-transparent"
      >
        {t("copy-order")}
      </Button>
    </DropdownMenuItem>
