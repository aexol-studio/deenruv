...
import { Collection, ID, Logger, Product, ProductVariant } from "@deenruv/core";
...
export class StellateService {
  ...
  async purgeProducts(products: Product[]) {
    Logger.verbose(
      `Purging cache: Product(${products.map((p) => p.id).join(", ")})`,
      loggerCtx,
    );
    await this.purge(
      "Product",
  ...
  async purgeSearchResults(items: Array<ProductVariant | Product>) {
    const productIds = items.map((item) =>
      item instanceof Product ? item.id : item.productId,
    );
    Logger.verbose(
      `Purging cache: SearchResult(${productIds.join(", ")})`,
      loggerCtx,
    );
  ...
  async purgeCollections(collections: Collection[]) {
    Logger.verbose(
      `Purging cache: Collection(${collections.map((c) => c.id).join(", ")})`,
      loggerCtx,
    );
    await this.purge(
      "Collection",
  ...
  async purgeSearchResponseCacheIdentifiers(collections: Collection[]) {
    ...
    if (slugs.length) {
      Logger.verbose(
        `Purging cache: SearchResponseCacheIdentifier(${slugs.join(", ")})`,
        loggerCtx,
      );
  ...
  purge(type: CachedType, keys?: ID[], keyName = "id") {
    const payload = {
      ...
      variables: {
        ...
        keyFields: keys
          ?.filter((id) => !!id)
          .map((id) => ({ name: keyName, value: id.toString() })),
      },
    };
    if (this.options.debugLogging === true) {
      ...
      if (5 < keyFieldsLength) {
        Logger.debug(
          `(A further ${keyFieldsLength - 5} keyFields truncated)`,
          loggerCtx,
        );
      }
    }
      ...
      return fetch(this.purgeApiUrl, {
        ...
        .then((json) => {
          if (json.data?._purgeType !== true) {
            ...
            Logger.error(
              `Purge failed: ${errors.join(", ") ?? JSON.stringify(json)}`,
              loggerCtx,
            );
          }
        })
