 ...
 * with the ui-devkit.
 * See https://github.com/deenruv-ecommerce/deenruv/issues/758 for more on this issue.
 */
async function checkAngularVersions() {
  ...
  for (const pkg of angularCompilerPackages) {
    const uiVersion =
      adminUiPackageJson.devDependencies[
        pkg as keyof typeof adminUiPackageJson.devDependencies
      ];
    const devkitVersion =
      uiDevkitPackageJson.dependencies[
        pkg as keyof typeof uiDevkitPackageJson.dependencies
      ];
