...
function generateGraphqlDocs(hugoOutputPath: string) {
  ...
  for (const type of sortedTypes) {
    ...
    if (isObjectType(type)) {
      if (type.name === "Query") {
        for (const field of Object.values(type.getFields()).sort(sortByName)) {
          ...
          queriesOutput += codeLine(`type ${identifier("Query")} &#123;`, [
            "top-level",
          ]);
        ...
        for (const field of Object.values(type.getFields()).sort(sortByName)) {
          ...
          mutationsOutput += codeLine(`type ${identifier("Mutation")} &#123;`, [
            "top-level",
          ]);
        ...
        objectTypesOutput += codeLine(`type ${identifier(type.name)} &#123;`, [
          "top-level",
        ]);
    ...
    if (isEnumType(type)) {
      ...
      enumsOutput += codeLine(`enum ${identifier(type.name)} &#123;`, [
        "top-level",
      ]);
      for (const value of type.getValues()) {
        enumsOutput += value.description
          ? renderDescription(value.description, "single")
          : "";
    ...
    if (isScalarType(type)) {
      ...
      objectTypesOutput += `<div class="graphql-code-block">\n`;
      objectTypesOutput += renderDescription(type, "multi", true);
      objectTypesOutput += codeLine(`scalar ${identifier(type.name)}`, [
        "top-level",
      ]);
      objectTypesOutput += "</div>\n";
    }
    ...
    if (isInputObjectType(type)) {
      ...
      inputTypesOutput += codeLine(`input ${identifier(type.name)} &#123;`, [
        "top-level",
      ]);
    ...
    if (isUnionType(type)) {
      ...
      objectTypesOutput += codeLine(`union ${identifier(type.name)} =`, [
        "top-level",
      ]);
  ...
  fs.writeFileSync(
    path.join(hugoOutputPath, FileName.QUERY + ".md"),
    queriesOutput,
  );
  fs.writeFileSync(
    path.join(hugoOutputPath, FileName.MUTATION + ".md"),
    mutationsOutput,
  );
  fs.writeFileSync(
    path.join(hugoOutputPath, FileName.OBJECT + ".md"),
    objectTypesOutput,
  );
  fs.writeFileSync(
    path.join(hugoOutputPath, FileName.INPUT + ".md"),
    inputTypesOutput,
  );
  fs.writeFileSync(
    path.join(hugoOutputPath, FileName.ENUM + ".md"),
    enumsOutput,
  );
...
function codeLine(
  content: string,
  extraClass?: ["top-level" | "comment"] | undefined,
): string {
...
function renderTypeAsLink(type: GraphQLType): string {
  ...
  return type
    .toString()
    .replace(innerType.name, `<a href="${url}">${innerType.name}</a>`);
}
