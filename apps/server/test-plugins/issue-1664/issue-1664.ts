...
export class Test1664Plugin implements OnApplicationBootstrap {
  ...
  async createDummyProfiles() {
    const profilesCount = await this.connection.rawConnection
    ...
    const asset = await this.connection.rawConnection
      ...
      .findOne({ where: { id: 1 } });
    if (asset) {
      const profileAsset = this.connection.rawConnection
        .getRepository(ProfileAsset)
        .save({
          asset,
          profile,
        });
  ...
  async createDummyOrder() {
    const orderCount = await this.connection.rawConnection
      .getRepository(Order)
      .count();
    if (0 < orderCount) {
      return;
    ...
    const defaultChannel = await this.connection
    ...
    if (!defaultChannel) {
      throw new Error(
        `Channel with code ${DEFAULT_CHANNEL_CODE} could not be found.`,
      );
    }
    ...
    await this.orderService.updateCustomFields(ctx, created.id, {
      productOwner,
    });
  }
}
