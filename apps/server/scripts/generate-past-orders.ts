...
async function generatePastOrders() {
  ...
  const { items: variants } = await productVariantService.findAll(ctxAdmin, {
    take: 500,
  });
  const { items: customers } = await customerService.findAll(
    ctxAdmin,
    { take: 500 },
    ["user"],
  );
  ...
  for (let i = DAYS_TO_COVER; i > 0; i--) {
    ...
    for (let j = 0; j < numberOfOrders; j++) {
      ...
      const eligibleShippingMethods =
        await orderService.getEligibleShippingMethods(ctx, order.id);
      ...
      const eligiblePaymentMethods =
        await orderService.getEligiblePaymentMethods(ctx, order.id);
      const paymentResult = await orderService.addPaymentToOrder(
        ctx,
        order.id,
        {
