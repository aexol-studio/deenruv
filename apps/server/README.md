...
This package is not published to npm. It is used in development of the Deenruv server and plugins.
...
Test data can be populated by running the `populate` script. This uses the same sample data as is used by the Deenruv CLI when running `init`, albeit with the additional step of populating some sample customer & address data too.
...
```
  "paths": {
      "@deenruv/admin-ui/*": ["../../admin-ui/package/*"]
  }
```

## Load testing

This package also contains scripts for load testing the Deenruv server. The load testing infrastructure and scripts are located in the [`./load-testing`](./load-testing) directory.
...
-   `deenruv-load-testing-1000`
-   `deenruv-load-testing-10000`
-   `deenruv-load-testing-100000`
