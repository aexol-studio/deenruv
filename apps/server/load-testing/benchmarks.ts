...
program
  ...
  .option(
    "--db <db>",
    "Select which database to test against",
    /^(mysql|postgres)$/,
    "mysql",
  )
...
async function createProducts(app: INestApplication) {
  ...
  const bar = new ProgressBar(
    "creating products [:bar] (:current/:total) :percent :etas",
    {
      complete: "=",
      incomplete: " ",
      total: PRODUCT_COUNT,
  ...
  for (let i = 0; i < PRODUCT_COUNT; i++) {
    const product: ParsedProductWithVariants = {
      ...
      variants: Array.from({ length: VARIANTS_PER_PRODUCT }).map(
        (value, index) => ({
          sku: `PROD-${i}-${index}`,
          facets: [],
          assetPaths: [],
...
async function createOrders(app: INestApplication) {
  ...
  const bar = new ProgressBar(
    "creating orders [:bar] (:current/:total) :percent :etas",
    {
      complete: "=",
      incomplete: " ",
      total: ORDER_COUNT,
