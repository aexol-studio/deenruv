...
if (require.main === module) {
  ...
  const init = spawn(
    "node",
    ["-r", "ts-node/register", "./init-load-test.ts", count.toString()],
    {
      cwd: __dirname,
      stdio: "inherit",
    },
...
async function closeAndExit(
  ...
  const summaryFile = path.join(
    __dirname,
    `results/load-test-${dateString}-${count}.json`,
  );
  ...
  for (const summary of summaries) {
    ...
    console.log(
      `Time series data written to ${path.relative(__dirname, timeSeriesFile)}`,
    );
  }
