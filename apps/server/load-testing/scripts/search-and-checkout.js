...
const setShippingAddressMutation = new ShopApiRequest(
  "shop/set-shipping-address.graphql",
);
const getShippingMethodsQuery = new ShopApiRequest(
  "shop/get-shipping-methods.graphql",
);
...
function addToCart(variantId) {
  ...
  check(result.data, {
    "Product added to cart": (r) =>
      !!r.addItemToOrder.lines.find(
        (l) => l.productVariant.id === variantId && l.quantity >= qty,
      ),
  });
}
...
function setShippingAddressAndCustomer() {
  ...
  check(result.data, {
    "Address set": (r) =>
      r.setOrderShippingAddress.shippingAddress.country === "United Kingdom",
  });
}
