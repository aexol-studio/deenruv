import { Badge, Card, CardTitle, Stack } from '@/components';
import { apiCall } from '@/graphql/client';
import { SummaryOrdersSelector } from '@/graphql/orders';
...
export const OrdersSummary: React.FC<LatestOrdersProps> = () => {
  ...
  const getOrders = async (range: { start: Date; end: Date }) => {
    const response = await apiCall()('query')({
      orders: [
        {
          options: {
            filter: { orderPlacedAt: { between: range } },
          },
        },
        {
          items: SummaryOrdersSelector,
          totalItems: true,
        },
      ],
    });
    ...
    setOrders({
      ...
      total: response.orders.items.map((i) => i.total).reduce((accumulator, total) => accumulator + total, 0),
  ...
  return (
    <Card className="relative p-6">
      <Stack className="items-center justify-between">
        ...
        <Stack className="items-center gap-6 lg:gap-12">
          <Stack className="items-center gap-2 lg:gap-4">
            <div className="mt-1">{t('totalOrdersCount')}</div>
            <h3 className="text-3xl">{orders?.totalCount}</h3>
          </Stack>
          <Stack className="items-center gap-4 lg:gap-6">
