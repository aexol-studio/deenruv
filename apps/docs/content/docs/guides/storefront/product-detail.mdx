...
description: Learn how to build a product detail page, fetch product data, display images, format prices, and add items to the order.
---
...
As explained in the [Money & Currency guide](/docs/guides/core-concepts/money), the prices are returned as integers in the
...
In the demo at the end of this guide, we'll use the [`formatCurrency` function](/docs/guides/core-concepts/money#displaying-monetary-values)
which makes use of the browser's `Intl` API to format the price according to the user's locale.

## Displaying images

If we are using the [`AssetServerPlugin`](/docs/reference/core-plugins/asset-server-plugin/) to serve our product images (as is the default), then we can take advantage
...
To add a particular product variant to the order, we need to call the [`addItemToOrder`](/docs/reference/graphql-api/shop/mutations#additemtoorder) mutation.
...
- Because the `addItemToOrder` mutation returns a union type, we need to use a [fragment](/docs/guides/getting-started/graphql-intro#fragments) to specify the fields we want to return.
  In this case we have defined a fragment called `UpdatedOrder` which contains the fields we are interested in.
- If any [expected errors](/docs/guides/developer-guide/error-handling) occur, the mutation will return an `ErrorResult` object. We'll be able to
