...
description: Learn how to manage the shopping cart including adding items, removing items, changing quantities, and applying coupon codes.
---
...
This fragment can then be used in subsequent queries and mutations. Let's start with a query to get the active order using the [`activeOrder` query](/docs/reference/graphql-api/shop/queries#activeorder):
...
To add an item to the active order, we use the [`addItemToOrder` mutation](/docs/reference/graphql-api/shop/mutations#additemtoorder), as we have seen in the [Product Detail Page guide](/docs/guides/storefront/product-detail).
...
See the [Configurable Products guide](/docs/guides/how-to/configurable-products) for more information.
</Callout>

## Remove an item

To remove an item from the active order, we use the [`removeOrderLine` mutation](/docs/reference/graphql-api/shop/mutations#removeorderline),
...
To change the quantity of an item in the active order, we use the [`adjustOrderLine` mutation](/docs/reference/graphql-api/shop/mutations#adjustorderline).
...
If you have defined any [Promotions](/docs/guides/core-concepts/promotions) which use coupon codes, you can apply the a coupon code to the active order
using the [`applyCouponCode` mutation](/docs/reference/graphql-api/shop/mutations#applycouponcode).
...
To remove a coupon code from the active order, we use the [`removeCouponCode` mutation](/docs/reference/graphql-api/shop/mutations#removecouponcode).
