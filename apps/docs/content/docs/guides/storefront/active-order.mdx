...
To add an item to the active order, we use the `addItemToOrder` mutation, as we have seen in the [Product Detail Page guide](/docs/guides/storefront/product-detail).
...
To remove an item from the active order, we use the `removeOrderLine` mutation,
