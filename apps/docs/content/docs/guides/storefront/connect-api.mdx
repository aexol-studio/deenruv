...
Deenruv supports two ways to manage user sessions: **cookies** and **bearer token**. The method you choose depends on your requirements, and is specified by the `authOptions.tokenMethod` property of the DeenruvConfig. By default, both are enabled on the server:
...
1. Certain mutations and queries initiate a session (e.g. logging in, adding an item to an order etc.). When this happens, the response will contain a HTTP header which by default is called `'deenruv-auth-token'`.
...
<Callout type="info">
The header name `deenruv-token` is the default, but can be changed using the `apiOptions.channelTokenKey` config option.
</Callout>
