...
description: 'Learn how to connect your storefront to the Deenruv Shop API using various GraphQL clients'
---
...
[`http://localhost:3000/shop-api`](http://localhost:3000/shop-api) when your Deenruv
...
Deenruv supports two ways to manage user sessions: **cookies** and **bearer token**. The method you choose depends on your requirements, and is specified by the [`authOptions.tokenMethod` property](/docs/reference/typescript-api/auth/auth-options/#tokenmethod) of the DeenruvConfig. By default, both are enabled on the server:
...
1. Certain mutations and queries initiate a session (e.g. logging in, adding an item to an order etc.). When this happens, the response will contain a HTTP header which [by default is called `'deenruv-auth-token'`](/docs/reference/typescript-api/auth/auth-options#authtokenheaderkey).
...
3. If the `'deenruv-auth-token'` header is present, read the value and store it because this is your bearer token.
...
export async function request(query: string, variables: any) {
    ...
    // Check the response headers to see if Deenruv has set the
    // auth token. The header key "deenruv-auth-token" may be set to
...
The duration of a session is determined by the [AuthOptions.sessionDuration](/docs/reference/typescript-api/auth/auth-options#sessionduration) config
...
If your project has multiple [channels](/docs/guides/core-concepts/channels/), you can specify the active channel by setting
the `deenruv-token` header on each request to match the `channelToken` for the desired channel.

Let's say you have a channel with the token `uk-channel` and you want to make a request to the Shop API to get the
products in that channel. You would set the `deenruv-token` header to `uk-channel`:
...
<Callout type="info">
The header name `deenruv-token` is the default, but can be changed using the [`apiOptions.channelTokenKey`](/docs/reference/typescript-api/configuration/api-options/#channeltokenkey) config option.
</Callout>

## Setting language

If you have translations of your products, collections, facets etc, you can specify the language for the request by setting the `languageCode` query string on the request. The value should be one of the ISO 639-1 codes defined by the [`LanguageCode` enum](/docs/reference/typescript-api/common/language-code/).
...
See the [GraphQL Code Generation guide](/docs/guides/storefront/codegen/) for more information.
...
const API_URL = 'https://readonlydemo.deenruv.io/shop-api';
...
const API_URL = `https://demo.deenruv.io/shop-api`;
...
const afterwareLink = new ApolloLink((operation, forward) => {
    return forward(operation).map(response => {
        ...
        if (authHeader) {
            // If the auth token has been returned by the Deenruv
...
const responseMiddleware: ResponseMiddleware = response => {
    if (!(response instanceof Error) && !response.errors) {
        ...
        if (authHeader) {
            // If the session token has been returned by the Deenruv
            // server, we store it in localStorage
            localStorage.setItem(AUTH_TOKEN_KEY, authHeader);
        }
    }
