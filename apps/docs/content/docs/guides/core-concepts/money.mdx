...
In the GraphQL APIs, we use a custom `Money` scalar type to represent
...
decorator, you should use Deenruv's `@Money()` decorator.
...
- The precision represented by the monetary value

For example, in addition to the `DefaultMoneyStrategy`, Deenruv
