...
description: 'Learn how Deenruv handles monetary values, currency formatting, and advanced money strategies'
---

import { Callout } from 'fumadocs-ui/components/callout';

In Deenruv, monetary values are stored as **integers** using the **minor unit** of the selected currency.
...
If you are building an Admin UI extension, you can use the built-in [`LocaleCurrencyPipe`](/docs/reference/admin-ui-api/pipes/locale-currency-pipe/):
...
Deenruv supports multiple currencies out-of-the-box. The available currencies must first be set at the Channel level
(see the [Channels, Currencies & Prices section](/docs/guides/core-concepts/channels/#channels-currencies--prices)), and then
a price may be set on a `ProductVariant` in each of the available currencies.

When using multiple currencies, the [ProductVariantPriceSelectionStrategy](/docs/reference/typescript-api/configuration/product-variant-price-selection-strategy/)
...
In the GraphQL APIs, we use a custom [`Money` scalar type](/docs/reference/graphql-api/admin/object-types/#money) to represent
...
If you are defining custom GraphQL types, or adding fields to existing types (see the [Extending the GraphQL API doc](/docs/guides/developer-guide/extend-graphql-api/)),
then you should also use the `Money` scalar for any monetary values.

## The `@Money()` decorator

When [defining new database entities](/docs/guides/developer-guide/database-entity), if you need to store a monetary value, then rather than using the TypeORM `@Column()`
decorator, you should use Deenruv's [`@Money()` decorator](/docs/reference/typescript-api/money/money-decorator).

Using this decorator allows Deenruv to correctly store the value in the database according to the configured `MoneyStrategy` (see below).
...
For advanced use-cases, it is possible to configure aspects of how Deenruv handles monetary values internally by defining
a custom [`MoneyStrategy`](/docs/reference/typescript-api/money/money-strategy/).
...
For example, in addition to the [`DefaultMoneyStrategy`](/docs/reference/typescript-api/money/default-money-strategy), Deenruv
also provides the [`BigIntMoneyStrategy`](/docs/reference/typescript-api/money/big-int-money-strategy) which stores monetary values
