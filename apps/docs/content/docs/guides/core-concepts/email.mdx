...
description: 'Learn how to configure email notifications and other notification methods in Deenruv'
---
...
Email is the most common way to notify customers of events, so a default Deenruv installation includes our [EmailPlugin](/docs/reference/core-plugins/email-plugin).
...
The plugin is configured with a list of [EmailEventHandlers](/docs/reference/core-plugins/email-plugin/email-event-handler) which are responsible for
...
and use it, see the [EmailPlugin API docs](/docs/reference/core-plugins/email-plugin).
</Callout>
...
All emails are triggered by a particular [Event](/docs/guides/developer-guide/events/) - in this case when the state of an
Order changes. The EmailPlugin ships with a set of [default email handlers](https://github.com/aexol-studio/deenruv/blob/master/packages/email-plugin/src/default-email-handlers.ts),
...
The full range of methods available when setting up an EmailEventHandler can be found in the [EmailEventHandler API docs](/docs/reference/core-plugins/email-plugin/email-event-handler).
