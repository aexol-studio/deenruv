---
title: 'Taxes'
description: 'Learn how to configure tax handling in Deenruv - tax categories, rates, zones, and calculation strategies'
---

import { Callout } from 'fumadocs-ui/components/callout';

E-commerce applications need to correctly handle taxes such as sales tax or value added tax (VAT). In Deenruv, tax handling consists of:
...
- **Tax rates** This is the tax rate applied to a specific tax category for a specific [Zone](/docs/reference/typescript-api/entities/zone/). E.g., the tax rate for "standard" goods in the UK Zone is 20%.
...
1. The price of the item, and whether that price is inclusive of tax, is determined according to the configured [OrderItemPriceCalculationStrategy](/docs/reference/typescript-api/orders/order-item-price-calculation-strategy/).
2. The active tax Zone is determined based on the configured [TaxZoneStrategy](/docs/reference/typescript-api/tax/tax-zone-strategy/).
...
4. The `TaxLineCalculationStrategy.calculate()` of the configured [TaxLineCalculationStrategy](/docs/reference/typescript-api/tax/tax-line-calculation-strategy/) is called, which will return one or more [TaxLines](/docs/reference/graphql-api/admin/object-types/#taxline).
5. The final `priceWithTax` of the order line is calculated based on all the above.

## Calculating tax on shipping

The taxes on shipping is calculated by the [ShippingCalculator](/docs/reference/typescript-api/shipping/shipping-calculator/) of the Order's selected [ShippingMethod](/docs/reference/typescript-api/entities/shipping-method/).
