...
description: 'Learn about the Deenruv service layer - business logic, database access, and using core services'
---
...
Services are generally scoped to a specific domain or entity. For instance, in the Deenruv core, there is a [`Product` entity](/docs/reference/typescript-api/entities/product),
and a corresponding [`ProductService`](/docs/reference/typescript-api/services/product-service) which contains all the methods for interacting with products.

Here's a simplified example of a `ProductService`, including an implementation of the `findOne()` method that was
used in the example in the [previous section](/docs/guides/developer-guide/the-api-layer/#resolvers):
...
- The `@Injectable()` decorator is a [NestJS](https://docs.nestjs.com/providers#services) decorator which allows the service to be injected into other services or resolvers.
...
All the internal Deenruv services can be used in your own plugins and scripts. They are listed in the [Services API reference](/docs/reference/typescript-api/services/) and
can be imported from the `@deenruv/core` package.
...
the [`TransactionalConnection` class](/docs/reference/typescript-api/data-access/transactional-connection/), which is a wrapper
...
a function which gets an entity passed to it by Deenruv. In this case, we can use the [`EntityHydrator`](/docs/reference/typescript-api/data-access/entity-hydrator/)
...
This allows you to specify which relations should be joined when the query is executed. For instance, in the [`ProductService`](/docs/reference/typescript-api/services/product-service)
