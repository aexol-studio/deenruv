...
description: 'Learn how to handle unexpected errors and expected ErrorResults in Deenruv GraphQL APIs'
---

import { Tabs, Tab } from 'fumadocs-ui/components/tabs';
import { Callout } from 'fumadocs-ui/components/callout';

Errors in Deenruv can be divided into two categories:
...
This type of error occurs when something goes unexpectedly wrong during the processing of a request. Examples include internal server errors, database connectivity issues, lacking permissions for a resource, etc. In short, these are errors that are _not supposed to happen_.
...
This type of error represents a well-defined result of (typically) a GraphQL mutation which is not considered "successful". For example, when using the `applyCouponCode` mutation, the code may be invalid, or it may have expired. These are examples of "expected" errors and are named in Deenruv "ErrorResults". These ErrorResults are encoded into the GraphQL schema itself.
...
<Tabs items={['Success case', 'Error case']}>
...
If you are writing a plugin which deals with internal Deenruv service methods that may return ErrorResults,
...
If we combine this approach with [GraphQL code generation](/docs/guides/storefront/codegen/), then TypeScript will even be able to
