---
title: 'The API Layer'
description: 'Learn about the Deenruv API layer - GraphQL queries, mutations, resolvers, middleware and decorators'
---

import { Callout } from 'fumadocs-ui/components/callout';
import { Tabs, Tab } from 'fumadocs-ui/components/tabs';

Deenruv is a headless platform, which means that all functionality is exposed via GraphQL APIs. The API can be thought of
...
"Middleware" is a term for a function which is executed before or after the main logic of a request. In Deenruv, middleware
is used to perform tasks such as authentication, logging, and error handling. There are several types of middleware:

### Express middleware

At the lowest level, Deenruv makes use of the popular Express server library. [Express middleware](https://expressjs.com/en/guide/using-middleware.html)
can be added to the sever via the [`apiOptions.middleware`](/docs/reference/typescript-api/configuration/api-options#middleware) config property. There are hundreds of tried-and-tested Express
...
Deenruv uses a number of these mechanisms internally to handle authentication, transaction management, error handling and
data transformation.
...
[Deenruv plugin](/docs/guides/developer-guide/plugins/):
...
Adding this plugin to your Deenruv config `plugins` array will now apply these middleware classes to all requests.
...
Apollo Server (the underlying GraphQL server library used by Deenruv) allows you to define
...
[`apiOptions.apolloServerPlugins`](/docs/reference/typescript-api/configuration/api-options#apolloserverplugins) config property.

## Resolvers

A "resolver" is a GraphQL concept, and refers to a function which is responsible for returning the data for a particular
field. In Deenruv, a resolver can also refer to a class which contains multiple resolver functions. For every query or
...
- The `@Ctx()` decorator injects the [`RequestContext` object](/docs/reference/typescript-api/request/request-context/), which contains information about the current request, such as the current user, the active channel, the active language, etc. The `RequestContext` is a key part of the Deenruv architecture, and is used throughout the application to provide context to the various services and plugins. In general, your resolver functions should always accept a `RequestContext` as the first argument, and pass it through to the services.
...
Following the pattern of NestJS, Deenruv makes use of decorators to control various aspects of the API. Here are the
...
The [`Allow` decorator](/docs/reference/typescript-api/request/allow-decorator) is exported by the `@deenruv/core` package. It is used to control access to queries and mutations. It takes a list
of [Permissions](/docs/reference/typescript-api/common/permission/) and if the current user does not have at least one of the
...
The [`Transaction` decorator](/docs/reference/typescript-api/request/transaction-decorator/) is exported by the `@deenruv/core` package. It is used to wrap a resolver function in a database transaction. It is
...
The [`Ctx` decorator](/docs/reference/typescript-api/request/ctx-decorator/) is exported by the `@deenruv/core` package. It is used to inject the
[`RequestContext` object](/docs/reference/typescript-api/request/request-context/) into a resolver function. The `RequestContext` contains information about the
current request, such as the current user, the active channel, the active language, etc. The `RequestContext` is a key part
of the Deenruv architecture, and is used throughout the application to provide context to the various services and plugins.
...
Although Deenruv is primarily a GraphQL-based API, it is possible to add REST endpoints to the API. This is useful if
...
guide](/docs/guides/developer-guide/rest-endpoint/).
