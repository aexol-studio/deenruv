...
email sending, and search. For documentation on these, see the [Core Plugins reference](/docs/reference/core-plugins/).
...
object on the type [`VendurePluginMetadata`](/docs/reference/typescript-api/plugin/deenruv-plugin-metadata/).
...
you can inject the [ProcessContext provider](/docs/reference/typescript-api/common/process-context/).
</Callout>
...
<Callout type="info">
For a complete working example of a Deenruv plugin, see the [real-world-deenruv Reviews plugin](https://github.com/aexol-studio/real-world-deenruv/tree/master/src/plugins/reviews)
...
- The `WishlistItem` entity extends the [`VendureEntity` class](/docs/reference/typescript-api/entities/deenruv-entity/). This is a base class which provides the `id`, `createdAt` and `updatedAt` fields, and all custom entities should extend it.
...
- The arguments passed to the constructor will be injected by the NestJS DI system. The `connection` argument is a [TransactionalConnection](/docs/reference/typescript-api/data-access/transactional-connection/) instance, which is used to access and manipulate data in the database. The [`ProductVariantService`](/docs/reference/typescript-api/services/product-variant-service/) argument is a built-in Deenruv service which contains methods relating to ProductVariants.
- The [`RequestContext`](/docs/reference/typescript-api/request/request-context/) object is usually the first argument to any service method, and contains information and context about the current request as well as any open database transactions. It should always be passed to the methods of the `TransactionalConnection`.
...
Since Deenruv v2.0.0, it is possible for a plugin to specify which versions of Deenruv core it is compatible with. This is especially important if the plugin is intended to be made publicly available via npm or another package registry.
...
The value of this property is a [semver range](https://docs.npmjs.com/about-semantic-versioning) which specifies the range of compatible versions. In this case, we are saying that this plugin is compatible with any version of Deenruv core which is `>= 2.0.0 < 3.0.0`.
