...
The worker is started by calling the [`bootstrapWorker()`](/docs/reference/typescript-api/worker/bootstrap-worker/) function with the same
...
configured [`JobQueueStrategy`](/docs/reference/typescript-api/job-queue/job-queue-strategy/), but by default
...
[`JobQueueOptions.activeQueues`](/docs/reference/typescript-api/job-queue/job-queue-options#activequeues) configuration, it is even possible to have particular workers dedicated
...
In this case you can inject the [`ProcessContext`](/docs/reference/typescript-api/common/process-context/) provider and query it like this:
...
The actual queue part is defined by the configured [`JobQueueStrategy`](/docs/reference/typescript-api/job-queue/job-queue-strategy/).

If no strategy is defined, Deenruv uses an [in-memory store](/docs/reference/typescript-api/job-queue/in-memory-job-queue-strategy/)
...
A better alternative is to use the [DefaultJobQueuePlugin](/docs/reference/typescript-api/job-queue/default-job-queue-plugin/)
(which will be used in a standard `@deenruv/create` installation), which configures Deenruv to use the [SqlJobQueueStrategy](/docs/reference/typescript-api/job-queue/sql-job-queue-strategy).
...
It is common for larger Deenruv projects to define multiple custom job queues, When using the [DefaultJobQueuePlugin](/docs/reference/typescript-api/job-queue/default-job-queue-plugin/)
...
In this case it is recommended to try the [BullMQJobQueuePlugin](/docs/reference/core-plugins/job-queue-plugin/bull-mqjob-queue-plugin/),
...
<Callout type="info">
A real example of this can be seen in the [EmailPlugin source](https://github.com/aexol-studio/deenruv/blob/master/packages/email-plugin/src/plugin.ts)
</Callout>
...
The [`JobQueueService`](/docs/reference/typescript-api/job-queue/job-queue-service/) creates and manages job queues. The queue is created when the
...
what context you are in. This can be done with the [ProcessContext](/docs/reference/typescript-api/common/process-context/) provider.
</Callout>

Finally, the `ProductVideoPlugin` brings it all together, extending the GraphQL API, defining the required CustomField to store the transcoded video URL, and registering our service and resolver. The [PluginCommonModule](/docs/reference/typescript-api/plugin/plugin-common-module/) is imported as it exports the `JobQueueService`.
...
It is common to need to pass the [RequestContext object](/docs/reference/typescript-api/request/request-context) to the `process` function of a job, since `ctx` is required by many Deenruv
...
method](/docs/reference/typescript-api/request/request-context/#serialize), and then deserialize it in the `process` function using the static `deserialize` method:
