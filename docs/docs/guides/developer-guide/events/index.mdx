...
Deenruv emits events which can be subscribed to by plugins. These events are published by the [EventBus](/reference/typescript-api/events/event-bus/) and
...
import { EventBus, ProductEvent, PluginCommonModule, VendurePlugin } from '@deenruv/core';
...
You can create your own custom events by extending the [`VendureEvent`](/reference/typescript-api/events/deenruv-event) class. For example, to create a custom event which is triggered when a customer submits a review, you could do this:

```ts title="src/plugins/reviews/events/review-submitted.event.ts"
import { ID, RequestContext, VendureEvent } from '@deenruv/core';
