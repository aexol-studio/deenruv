...
You can create your own custom events by extending the [`DeenruvEvent`](/reference/typescript-api/events/deenruv-event) class. For example, to create a custom event which is triggered when a customer submits a review, you could do this:

```ts title="src/plugins/reviews/events/review-submitted.event.ts"
import { type ID, RequestContext, DeenruvEvent } from '@deenruv/core';
...
import { type ID, RequestContext, VendureEntityEvent } from '@deenruv/core';
