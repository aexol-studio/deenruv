...
If no strategy is defined, Deenruv uses an [in-memory store](/reference/typescript-api/job-queue/in-memory-job-queue-strategy/)
...
(which will be used in a standard `@deenruv/create` installation), which configures Deenruv to use the [SqlJobQueueStrategy](/reference/typescript-api/job-queue/sql-job-queue-strategy).
...
It is common for larger Deenruv projects to define multiple custom job queues, When using the [DefaultJobQueuePlugin](/reference/typescript-api/job-queue/default-job-queue-plugin/)
...
It is common to need to pass the [RequestContext object](/reference/typescript-api/request/request-context) to the `process` function of a job, since `ctx` is required by many Deenruv
...
import { type ID, Product, TransactionalConnection } from '@deenruv/core';
