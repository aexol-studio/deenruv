...
If you have hundreds, thousands or more products, inputting all the data by hand via the Admin UI can be too inefficient. To solve this, Deenruv supports bulk-importing product and other data.

Data import is also useful for setting up test or demo environments, and is also used by the `@deenruv/testing` package for end-to-end tests.

## Product Import Format

Deenruv uses a flat **.csv** format for importing product data. The format encodes data about:
...
-   `assets`: One or more asset file names separated by the pipe (`|`) character. The files can be located on the local file system, in which case the path is interpreted as being relative to the [`importAssetsDir`](/reference/typescript-api/import-export/import-export-options/#importassetsdir) as defined in the DeenruvConfig. Files can also be urls which will be fetched from a remote http/https url. If you need more control over how assets are imported, you can implement a custom [AssetImportStrategy](/reference/typescript-api/import-export/asset-import-strategy#assetimportstrategy). The first asset will be set as the featuredAsset.
...
:::info
For a real example, see the [products.csv file used to populate the Deenruv demo data](https://github.com/aexol-studio/deenruv/blob/master/packages/core/mock-data/data-sources/products.csv)
:::
...
```ts
import { InitialData, LanguageCode } from '@deenruv/core';
...
The `@deenruv/core` package exposes a [`populate()` function](/reference/typescript-api/import-export/populate/) which can be used along with the data formats described above to populate your Deenruv server:

```ts title="src/my-populate-script.ts"
import { bootstrap, DefaultJobQueuePlugin } from '@deenruv/core';
...
When installing with @deenruv/create, you have the option of populating test data (products, payment methods, countries, zones, tax rates etc).

This guide illustrates how to populate that test data again on an existing Deenruv installation, without needing to re-install from scratch.

1. `npm install --save-dev @deenruv/create`. This installs the "create" package, which contains the test data we will need.
...
import { populate } from '@deenruv/core/cli';
import { bootstrap, DeenruvConfig } from '@deenruv/core';
import { config } from './deenruv-config';
        ...
        bootstrap({
            ...
            importExportOptions: {
                importAssetsDir: path.join(
                    require.resolve('@deenruv/create/assets/products.csv'),
                    '../images',
                ),
            },
            dbConnectionOptions: { ...config.dbConnectionOptions, synchronize: true },
        }),
    require('@deenruv/create/assets/initial-data.json'),
    require.resolve('@deenruv/create/assets/products.csv'),
)
    .then(app => app.close())
    .catch(err => {
...
Running this script will populate the database with the test data like when you first installed Deenruv.
...
If you are migrating from another platform, you can create a custom import script to import your data into Deenruv.
...
1. Export the data to a file, and then transform this into the Deenruv CSV format for import as above.
2. Write a script which import the data via the other platform's API, and then import this data into Deenruv using the services described above, or any other of the Deenruv core services.
...
As an illustrative example, let's imagine we are migrating away from an imaginary commerce platform, "OldCommerce", and we want to import our data into Deenruv.
...
async function importData() {
    ...
    // Let's grab a reference to each of the Deenruv services we'll need.
