...
:::cli
Use `npx deenruv add` to easily add a new entity to a plugin.
:::
...
import { DeepPartial } from '@deenruv/common/shared-types';
...
:::note
Once you have added a new entity to your plugin, and the plugin has been added to your DeenruvConfig plugins array, you must create a [database migration](/guides/developer-guide/migrations/) to create the new table in the database.
:::
...
import { RequestContext, Product, TransactionalConnection } from '@deenruv/core';
...
There is also another Deenruv-specific decorator for representing monetary values specifically:
...
From Deenruv v2.2, it is possible to add support for [custom fields](/guides/developer-guide/custom-fields/) to your custom entities. This
...
Now you'll be able to add custom fields to the `ProductReview` entity via the DeenruvConfig:
