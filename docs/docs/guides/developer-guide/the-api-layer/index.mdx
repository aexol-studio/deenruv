...
[Deenruv plugin](/guides/developer-guide/plugins/):
...
-   The `@Ctx()` decorator injects the [`RequestContext` object](/reference/typescript-api/request/request-context/), which contains information about the
    ...
    of the Deenruv architecture, and is used throughout the application to provide context to the various services and plugins. In general, your
...
The [`Allow` decorator](/reference/typescript-api/request/allow-decorator) is exported by the `@deenruv/core` package. It is used to control access to queries and mutations. It takes a list
...
The [`Transaction` decorator](/reference/typescript-api/request/transaction-decorator/) is exported by the `@deenruv/core` package. It is used to wrap a resolver function in a database transaction. It is
...
The [`Ctx` decorator](/reference/typescript-api/request/ctx-decorator/) is exported by the `@deenruv/core` package. It is used to inject the
