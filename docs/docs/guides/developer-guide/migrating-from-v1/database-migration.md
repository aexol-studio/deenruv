...
Deenruv v2 introduces a number of breaking changes to the database schema, some of which require quite complex migrations in order to preserve existing data. To make this process as smooth as possible, we have created a migration tool which will handle the hard parts for you!
...
1. Make sure all your Deenruv packages to the latest v2 versions.
2. Use your package manager to install the [v2 migration tool](https://github.com/deenruv-ecommerce/v2-migration-tool): `npm install @deenruv/migrate-v2`
...
3. Add the `MigrationV2Plugin` to your plugins array:
    ...
    ```ts
    import { MigrationV2Plugin } from '@deenruv/migrate-v2';

    //...
    const config: DeenruvConfig = {
        //..
        plugins: [MigrationV2Plugin],
    };
    ```
