...
Services are generally scoped to a specific domain or entity. For instance, in the Deenruv core, there is a [`Product` entity](/reference/typescript-api/entities/product),
...
All the internal Deenruv services can be used in your own plugins and scripts. They are listed in the [Services API reference](/reference/typescript-api/services/) and
...
import { Injectable } from '@nestjs/common';
import { type ID, RequestContext, TransactionalConnection } from '@deenruv/core';
import { IsNull } from 'typeorm';
...
a function which gets an entity passed to it by Deenruv. In this case, we can use the [`EntityHydrator`](/reference/typescript-api/data-access/entity-hydrator/)
