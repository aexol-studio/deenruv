...
import { DataService, SharedModule } from '@deenruv/admin-ui/core';
...
:::note
We also need to define an `NgModule` for this component. This is because we will be lazy-loading the component at run-time, and the NgModule is required for us to use shared providers (e.g. `DataService`) and any shared components, directives or pipes defined in the `@deenruv/admin-ui/core` package.
:::
...
import { registerDashboardWidget } from '@deenruv/admin-ui/core';
...
```ts title="src/plugins/reviews/ui/providers.ts"
import { registerDashboardWidget, setDashboardWidgetLayout } from '@deenruv/admin-ui/core';
...
```ts title="src/plugins/reviews/ui/providers.ts"
import { registerDashboardWidget } from '@deenruv/admin-ui/core';
import { OrderSummaryWidgetComponent } from '@deenruv/admin-ui/dashboard';
