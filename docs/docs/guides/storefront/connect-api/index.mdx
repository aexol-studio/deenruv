...
Deenruv supports two ways to manage user sessions: **cookies** and **bearer token**. The method you choose depends on your requirements, and is specified by the [`authOptions.tokenMethod` property](/reference/typescript-api/auth/auth-options/#tokenmethod) of the DeenruvConfig. By default, both are enabled on the server:
...
1. Certain mutations and queries initiate a session (e.g. logging in, adding an item to an order etc.). When this happens, the response will contain a HTTP header which [by default is called `'deenruv-auth-token'`](/reference/typescript-api/auth/auth-options#authtokenheaderkey).
...
:::info
The header name `deenruv-token` is the default, but can be changed using the [`apiOptions.channelTokenKey`](/reference/typescript-api/configuration/api-options/#channeltokenkey) config option.
:::
...
<Stackblitz id="deenruv-docs-fetch-react" />
...
<Stackblitz id="deenruv-docs-apollo-client" />
...
<Stackblitz id="deenruv-docs-tanstack-query" />
