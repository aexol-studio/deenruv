...
[Render](https://render.com/) is a managed hosting platform which allows you to deploy and scale your Deenruv server and infrastructure with ease.
...
For this guide you'll need to have your Deenruv project in a git repo on either GitHub or GitLab.

:::info
If you'd like to quickly get started with a ready-made Deenruv project which includes sample data, you can use our
[Deenruv one-click-deploy repo](https://github.com/deenruv-ecommerce/one-click-deploy), which means you won't have
...
Render defines the port via the `PORT` environment variable and [defaults to `10000`](https://docs.render.com/web-services#host-and-port-configuration), so make sure your Deenruv Config uses this variable:
...
to connect your account and grant access to the repository containing your Deenruv project.
...
```
https://github.com/deenruv-ecommerce/one-click-deploy
```
...
-   **Runtime**: If you have a Dockerfile then it should be auto-detected. If not you should select "Node" and enter the appropriate build and start commands. For a
    typical Deenruv project these would be:
...
Name the group "deenruv configuration" and add the following variables. The database variables can be found by navigating
...
Next, click the "deenruv-server" service and go to the "Environment" tab to link the environment group to the service:
...
Finally, click the "Environment" tab and link the "deenruv configuration" environment group to the worker.

## Test your Deenruv server

Navigate back to the dashboard, click the "deenruv-server" service, and you should see a link to the temporary domain:
