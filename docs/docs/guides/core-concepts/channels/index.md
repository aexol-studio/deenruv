...
Channels are a feature of Deenruv which allows multiple sales channels to be represented in a single Deenruv instance. A Channel allows you to:
...
-   **Multi-tenancy**: Each channel can be configured with its own set of products, shipping methods, payment methods, etc. This
    allows you to run multiple shops from a single Deenruv server.
...
-   **Region-specific stores**: Each channel can be configured with its own set of languages, currencies, tax rates, etc. This
    allows you to run multiple stores for different regions from a single Deenruv server.
...
Every Deenruv server always has a **default Channel**, which contains _all_ entities. Subsequent channels can then contain a subset of channel-aware entities.
...
To specify which channel to use when making an API call, set the `'deenruv-token'` header to match the token of the desired Channel.
...
Then we can make a GraphQL API call to the UK Channel by setting the `'deenruv-token'` header to `'uk-channel'`:
...
:::note
This is an example using Apollo Client in React. The same principle applies to any GraphQL client library - set the `'deenruv-token'` header to the token of the desired Channel.
:::
