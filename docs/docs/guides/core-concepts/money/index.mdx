...
decorator, you should use Deenruv's [`@Money()` decorator](/reference/typescript-api/money/money-decorator).
...
import { VendureEntity, Order, EntityId, Money, CurrencyCode, ID } from '@deenruv/core';
...
For example, in addition to the [`DefaultMoneyStrategy`](/reference/typescript-api/money/default-money-strategy), Deenruv
