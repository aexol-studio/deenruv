...
Shipping in Deenruv is handled by [ShippingMethods](/reference/typescript-api/entities/shipping-method/).
...
export const maxWeightChecker = new ShippingEligibilityChecker({
    ...
    description: [{ languageCode: LanguageCode.en, value: 'Max Weight Checker' }],
    args: {
        maxWeight: {
            type: 'int',
...
Custom checkers are then passed into the DeenruvConfig [ShippingOptions](/reference/typescript-api/shipping/shipping-options/#shippingeligibilitycheckers) to make them available when setting up new ShippingMethods:

```ts title="src/deenruv-config.ts"
import { defaultShippingEligibilityChecker, DeenruvConfig } from '@deenruv/core';
import { maxWeightChecker } from './shipping-methods/max-weight-checker';

export const config: DeenruvConfig = {
    // ...
    shippingOptions: {
        shippingEligibilityCheckers: [defaultShippingEligibilityChecker, maxWeightChecker],
    },
};
```
...
import { LanguageCode, ShippingCalculator } from '@deenruv/core';
...
Custom calculators are then passed into the DeenruvConfig [ShippingOptions](/reference/typescript-api/shipping/shipping-options/#shippingcalculators) to make them available when setting up new ShippingMethods:

```ts
import { defaultShippingCalculator, DeenruvConfig } from '@deenruv/core';
...
By default, Deenruv uses a manual fulfillment handler, which requires the Administrator to manually enter the method and tracking code of the Fulfillment.
...
These states cover the typical workflow for fulfilling orders. However, it is possible to customize the fulfillment workflow by defining a [FulfillmentProcess](/reference/typescript-api/fulfillment/fulfillment-process) and passing it to your DeenruvConfig:

```ts title="src/deenruv-config.ts"
import { FulfillmentProcess, DeenruvConfig } from '@deenruv/core';
