...
<GenerationInfo sourceFile="packages/core/src/config/merge-config.ts" sourceLine="30" packageName="@deenruv/core" />
...
```ts title="Signature"
function mergeConfig<T extends DeenruvConfig>(target: T, source: PartialVendureConfig, depth:  = 0): T
```
