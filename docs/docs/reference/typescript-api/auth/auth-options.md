...
<GenerationInfo sourceFile="packages/core/src/config/deenruv-config.ts" sourceLine="330" packageName="@deenruv/core" />
...
<MemberInfo kind="property" type={`string`} default={`'deenruv-auth-token'`} />
...
If set to "true", when registering via the `registerCustomerAccount` mutation, one should _not_ set the
