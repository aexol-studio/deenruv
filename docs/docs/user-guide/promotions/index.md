...
A condition defines the criteria that must be met for the Promotion to be activated. Deenruv comes with some simple conditions provided which enable things like:
...
Deenruv allows completely custom conditions to be defined by your developers, implementing the specific logic needed by your business.
...
If all the defined conditions pass (or if the specified coupon code is used), then the actions are performed on the order. Deenruv comes with some commonly-used actions which allow promotions like:
